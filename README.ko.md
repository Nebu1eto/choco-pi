...
언어를 설정하면 "사용자 언어에 맞춘다"는 기본 규칙보다 우선합니다. 다만 이 블록은 프로젝트 정책이 아니라 사용자 설정이므로 두 가지가 더 우선합니다. 첫째, 메시지에서 특정 산출물의 언어를 명시하면 그 산출물에는 요청이 우선합니다. 일본어 문서를 요청하면 문서는 일본어로 작성하고 대화는 설정한 언어를 유지합니다. 둘째, 산출물의 언어를 고정하는 경로 범위 프로젝트 지침이 우선합니다. 예를 들어 프롬프트와 스킬, 문서를 영어로 요구하는 저장소가 그렇습니다. 스타일 설정에도 같은 우선순위를 적용합니다.

코드, 식별자, 파일 경로는 영향을 받지 않습니다. 커밋 메시지는 이 설정이 아니라 저장소가 결정합니다. 프로젝트 정책을 먼저 따르고, 없으면 해당 저장소에 남긴 최근 커밋의 언어를 따르며, 둘 다 없으면 영어로 씁니다. 따라서 커밋 이력이 한국어인 저장소는 응답 언어와 무관하게 한국어 커밋 메시지를 유지합니다.
