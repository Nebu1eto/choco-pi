...
- Resolve the child model and reasoning effort in this order: explicit user selection, project policy, the selected role file's `default_model`/`default_thinking`, then parent/runtime default. Read that frontmatter and pass both values on every spawn; role defaults are recommendations, not locks, and the orchestrator may override them for the task. Never silently replace an explicit selection.
