...
When choco-pi-lsp is active, run `lsp_diagnostics` on changed supported source files before broader build or test gates, then query `lens_diagnostics` with `mode=all` before completion. Use `mode=full` only when the requirement needs a project-wide verdict. Treat `partial`, `stale`, `unconfirmed`, `cold`, or `unavailable` results as incomplete evidence and use the documented fallback or report the limitation.
