...
- MCP servers expose no direct tools; only the `mcp` gateway is registered. Before the first call to a server in a session, discover its tools with `tool_search` or `mcp({ search })`, then call each tool by the exact name printed in that output. Never infer a tool name from the provider's own API, and never treat the `server` argument as a substitute for the registered name prefix. A cached entry is a candidate, not evidence that the server is connected or authenticated.
