...
- Spawn sub-agents in the background by default, overriding the `Agent` tool's own foreground default. A foreground child holds the main conversation until it finishes, so the user cannot steer the main agent while it runs. Keep a spawn in the foreground only when it is short and the next step genuinely cannot proceed without its result.
