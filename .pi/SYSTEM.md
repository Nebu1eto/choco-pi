...
- Never pass a `tool` name to `mcp` that you have not seen printed in this session. Run `tool_search` or `mcp({ search })` first, even when you already believe you know the name, then copy the name exactly as printed and resolve unfamiliar enum or object parameters with `mcp({ describe })` before supplying values. A cached entry is a candidate, not evidence that the server is connected or authenticated.
