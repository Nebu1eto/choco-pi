...
async function ancestorPids(): Promise<number[]> {
  ...
  for (let depth = 0; depth < MAX_ANCESTOR_DEPTH && current > 1; depth += 1) {
    pids.push(current);
    try {
      const result = await execFileAsync("ps", ["-o", "ppid=", "-p", String(current)], {
        encoding: "utf8",
      });
...
): Promise<{ owner: string; source: OwnerSource }> {
...
const baseDir = path.resolve(
  process.env.CHOCO_PI_LEASE_DIR || path.join(tmpdir(), "choco-pi-checkout-mutation-leases"),
);
...
if (action === "acquire") {
  ...
  if (await exists(leaseDir)) {
    ...
    if (metadata?.owner === owner && metadata.checkout === checkout) {
      console.log(
        JSON.stringify({ status: "held", checkout, leaseDir, owner, ownerSource }, null, 2),
      );
      process.exit(0);
    }
  ...
  try {
    ...
    console.log(
      JSON.stringify(
        { status: "acquired", checkout, leaseDir, owner, ownerSource, reclaimedFrom },
        null,
        2,
      ),
    );
...
await rmdir(leaseDir);
console.log(
  JSON.stringify({ status: "released", checkout, leaseDir, owner, ownerSource }, null, 2),
);
