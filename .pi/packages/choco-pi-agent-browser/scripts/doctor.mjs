...
const EXTENSION_ENTRYPOINTS = Object.freeze(["extensions/agent-browser/index.ts"]);
...
function entrySource(entry) {
  if (Guard.IsString(entry)) return entry;
  if (Guard.IsObject(entry)) return entry.source ?? entry.path ?? entry.package;
  return undefined;
}
...
async function checkPiVersion({ runPi }) {
  try {
    ...
    const code = Guard.IsObject(error) ? error.code : undefined;
    return {
      status: "warn",
...
async function checkAgentBrowserVersion({ runAgentBrowser }) {
  try {
    ...
    return {
      status: "pass",
      title: `agent-browser version matches baseline: ${version}`,
      lines: [],
    };
  } catch (error) {
    const code = Guard.IsObject(error) ? error.code : undefined;
