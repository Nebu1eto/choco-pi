...
} from "../extensions/agent-browser/lib/navigation-policy.ts";

test("parseAllowedDomainsPolicyFromArgs supports separated equals comma and whitespace values", () => {
  assert.deepEqual(
    parseAllowedDomainsPolicyFromArgs([
      "--allowed-domains",
      "example.com,docs.example.org other.test",
    ])?.allowedDomains,
    ["example.com", "docs.example.org", "other.test"],
  );
  assert.deepEqual(
    parseAllowedDomainsPolicyFromArgs(["--allowed-domains=https://Example.COM:443/path"])
      ?.allowedDomains,
    ["example.com"],
  );
...
test("getAllowedDomainsViolation reports http and https outside-domain final URLs only", () => {
  ...
  assert.equal(
    getAllowedDomainsViolation({ policy, url: "https://docs.example.com/page" }),
    undefined,
  );
  ...
  assert.match(
    violation?.summary ?? "",
    /--allowed-domains example\.com does not allow www\.iana\.org/,
  );
