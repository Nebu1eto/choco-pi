...
// SAFETY: The preceding fixed VM bootstrap defines `${stateName}.deliver` as a frozen single-string callback.
...
interface ScriptErrorDescription {
	message: string;
	name: string;
}

interface ScriptStartMessage {
	code: string;
	type: "start";
}
...
function describeError(cause: unknown, fallback: string): ScriptErrorDescription {
	...
	return {
		message: hasRuntimeType(cause.message, "string") ? cause.message.replace(/[\r\n]+/g, " ").slice(0, 400) : fallback,
...
function handleLine(line: string): void {
	...
	if (!started) {
		if (!isScriptStartMessage(message)) {
	...
	if (!isScriptResponseMessage(message)) {
