...
import { canonicalizeAgentBrowserNamespace, getAgentBrowserSessionIdentityKey } from "./argv-grammar.ts";
...
function parseOwner(content: string): PolicyLockOwner | undefined {
	...
	try {
		...
		return parsed.version === 3
			...
			&& hasRuntimeType(parsed.startIdentity, "string") && parsed.startIdentity.length > 0
			&& hasRuntimeType(parsed.token, "string") && parsed.token.length > 0
