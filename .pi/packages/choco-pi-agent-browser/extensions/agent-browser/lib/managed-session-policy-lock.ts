...
import { processStartIdentitiesMatch, readProcessStartIdentity } from "./process-identity.ts";
...
function getCoordinationDirectory(platform: NodeJS.Platform = process.platform): string {
  if (platform !== "win32") {
    const uid = hasRuntimeType(process.getuid, "function") ? process.getuid() : undefined;
...
async function ensureCoordinationDirectory(
  ...
  try {
    try {
      ...
      // SAFETY: mkdir reports filesystem failures as Node ErrnoException values.
...
export function getLegacyManagedSessionPolicyLockPath(
  sessionName: string,
  namespace?: string,
): string {
  return join(
    getCoordinationDirectory(),
    `.pi-agent-browser-policy-${getPolicyLockDigest(sessionName, namespace)}.lock-v2`,
  );
}
...
function parseOwner(content: string): PolicyLockOwner | undefined {
  ...
  try {
    // SAFETY: every PolicyLockOwner member read below is validated before the candidate is returned.
    const parsed = JSON.parse(content) as Partial<PolicyLockOwner>;
    // SAFETY: the complete conjunction validates every required PolicyLockOwner member.
    return parsed.version === 3 &&
      Number.isSafeInteger(parsed.pid) &&
      (parsed.pid ?? 0) > 0 &&
      hasRuntimeType(parsed.startIdentity, "string") &&
      parsed.startIdentity.length > 0 &&
      hasRuntimeType(parsed.token, "string") &&
...
function parseLegacyBridgeOwner(
  content: string,
): PolicyLockOwner | LegacyPolicyLockOwner | undefined {
  if (Buffer.byteLength(content) > POLICY_LOCK_MAX_BYTES) return undefined;
  try {
    // SAFETY: every lock-owner member read below is validated before the candidate is returned.
    const parsed = JSON.parse(content) as Partial<PolicyLockOwner | LegacyPolicyLockOwner>;
    // SAFETY: the complete conjunction validates every required current or legacy owner member.
...
function parseTicket(content: string, token: string): number | undefined {
  if (Buffer.byteLength(content) > POLICY_LOCK_MAX_BYTES) return undefined;
  try {
    // SAFETY: every PolicyLockTicket member read below is validated before the candidate is returned.
...
async function readClaim(path: string): Promise<PolicyLockClaim | undefined> {
  try {
    ...
    try {
      ...
      // SAFETY: lstat reports filesystem failures as Node ErrnoException values.
...
async function readLegacyBridgeOwner(
  path: string,
): Promise<PolicyLockOwner | LegacyPolicyLockOwner | undefined> {
  try {
    const directory = await lstat(path);
    const ownerPath = join(path, LOCK_OWNER_FILE);
    const ownerEntry = await lstat(ownerPath);
...
async function ownerAlive(
  ...
  try {
    ...
    // SAFETY: process.kill reports OS process lookup failures as Node ErrnoException values.
...
async function removeClaimOwnedBy(path: string, token: string): Promise<boolean> {
  ...
  try {
    ...
    // SAFETY: rename reports filesystem failures as Node ErrnoException values.
    return (error as NodeJS.ErrnoException).code === "ENOENT";
  }
...
async function removeLegacyBridgeOwnedBy(path: string, token: string): Promise<boolean> {
  ...
  const movedPath = join(
    dirname(path),
    `.pi-agent-browser-policy-bridge-remove-${token}-${randomUUID()}`,
  );
  try {
    await rename(path, movedPath);
  } catch (error) {
...
async function acquireLegacyPolicyBridge(options: {
  ...
  try {
    ...
    await writeFile(join(candidatePath, LOCK_OWNER_FILE), JSON.stringify(options.owner), {
      encoding: "utf8",
      flag: "wx",
      mode: 0o600,
    });
    while (!options.signal?.aborted) {
      ...
      try {
        ...
        return installed?.version === 3 && installed.token === options.owner.token
          ? {
              release: async () => {
                await removeLegacyBridgeOwnedBy(path, options.owner.token);
              },
            }
          : undefined;
      } catch (error) {
