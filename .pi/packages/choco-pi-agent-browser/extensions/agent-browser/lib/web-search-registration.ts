...
export function createDeferredAgentBrowserWebSearchTool(
	...
	const getRuntimeTool = (): Promise<RuntimeWebSearchTool> => {
		runtimeToolPromise ??= import("./web-search.ts").then(({ createAgentBrowserWebSearchTool }) => createAgentBrowserWebSearchTool(configState, options));
	...
	return {
		...
		async execute(toolCallId: string, params: AgentBrowserWebSearchParamsInput, signal?: AbortSignal, onUpdate?: AgentToolUpdateCallback<WebSearchToolDetails>, ctx?: ExtensionContext) {
