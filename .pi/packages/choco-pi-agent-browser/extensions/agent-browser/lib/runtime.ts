...
import { isWrapperManagedSessionName, redactManagedSessionRestoreKeys } from "./managed-session-capabilities.ts";
import { isCloseAllCommand, isCloseCommand, isOpenNavigationCommand } from "./command-taxonomy.ts";
import {
	hasLaunchScopedFlagToken,
	LAUNCH_SCOPED_FLAG_DEFINITIONS,
	LAUNCH_SCOPED_FLAG_LABEL,
} from "./launch-scoped-flags.ts";
import {
	MANAGED_SESSION_NAME_PREFIX,
	type ManagedSessionRestoreIdentity,
} from "./managed-session-restore.ts";

export type { CommandInfo } from "./argv-descriptor.ts";
export { extractCommandTokens, extractUpstreamCommandTokens, findCommandStartIndex, parseArgvDescriptor, parseCommandInfo, parseWaitCommandTokens } from "./argv-descriptor.ts";
...
import { TARGET_AGENT_BROWSER_VERSION } from "./upstream-version.ts";
...
interface HeadlessCompatUserAgents {
	[platform: string]: string | undefined;
}

const DEFAULT_HEADLESS_COMPAT_USER_AGENT_BY_PLATFORM: HeadlessCompatUserAgents = {
...
function isStringArray<Value>(value: Value): value is Value & string[] {
	return Array.isArray(value) && value.every((item) => hasRuntimeType(item, "string"));
}
...
function redactEmbeddedStructuredText(text: string): string {
	...
	while (cursor < text.length) {
		...
		try {
			const parsed: RuntimeValue = JSON.parse(candidate);
			const redacted = hasRuntimeType(parsed, "string") ? redactSensitiveText(parsed) : JSON.stringify(redactSensitiveValue(parsed));
			const original = hasRuntimeType(parsed, "string") ? parsed : JSON.stringify(parsed);
...
export function redactSensitiveValue<Value>(value: Value): RuntimeValue {
	...
	if (!isRecord(value)) {
		// SAFETY: RuntimeValue enumerates every JavaScript primitive and object category returned unchanged here.
...
export function resolveManagedSessionState(options: {
	...
	if (!managedSessionName) {
		return priorNamespace
			? { active: priorActive, namespace: priorNamespace, sessionName: priorSessionName }
			: { active: priorActive, sessionName: priorSessionName };
	}
	if (isCloseCommand(command) && managedSessionName === priorSessionName) {
		...
		const active = succeeded ? false : priorActive;
		const namespace = succeeded ? undefined : priorNamespace;
		return namespace
			? { active, namespace, sessionName: priorSessionName }
			: { active, sessionName: priorSessionName };
	}
	if (!succeeded) {
		return priorNamespace
			? { active: priorActive, namespace: priorNamespace, sessionName: priorSessionName }
	...
	const replacedSessionName = priorActive && priorSessionName !== managedSessionName ? priorSessionName : undefined;
	return managedSessionNamespace
		? { active: true, namespace: managedSessionNamespace, replacedSessionName, sessionName: managedSessionName }
		: { active: true, replacedSessionName, sessionName: managedSessionName };
}
...
function getRestorableManagedSessionName<Value>(value: Value, fallbackSessionName: string): string | undefined {
	return hasRuntimeType(value, "string") && isRestorableManagedSessionName(value, fallbackSessionName) ? value : undefined;
}

function getElectronCleanupClosedManagedSessions(details: RuntimeRecord<RuntimeValue>, fallbackSessionName: string): Array<{ namespace?: string; sessionName: string }> {
	...
	for (const result of results) {
		...
		const fallbackNamespace = hasRuntimeType(record?.namespace, "string")
			? record.namespace
		...
		for (const step of result.steps) {
			...
			const namespace = hasRuntimeType(step.namespace, "string") ? step.namespace : fallbackNamespace;
...
export function restoreManagedSessionStateFromBranch(
	...
	for (const entry of branch) {
		...
		const namespace = canonicalizeAgentBrowserNamespace(hasRuntimeType(details.namespace, "string") ? details.namespace : undefined);
		...
		const command = hasRuntimeType(details.command, "string") ? details.command : parseCommandInfo(args).command;
		...
		if (details.managedSessionRestoreDisabled === true && hasRuntimeType(sessionName, "string")) {
	...
	const managedSessionRestoreDisabledIdentities = [...restoreDisabledIdentities.values()];
	return closedSessionName
		? { ...restoredState, closedSessionName, freshSessionOrdinal, managedSessionRestoreDisabledIdentities }
		: { ...restoredState, freshSessionOrdinal, managedSessionRestoreDisabledIdentities };
}
...
function extractTabSelection(tab: { index?: number; label?: string; tabId?: string }): Pick<OpenResultTabCorrection, "selectedTab" | "selectionKind"> | undefined {
	...
	if (hasRuntimeType(tab.index, "number") && Number.isInteger(tab.index) && tab.index >= 0) {
...
export function chooseOpenResultTabCorrection(options: {
	...
	const normalizedTargetUrl =
		hasRuntimeType(options.targetUrl, "string") ? normalizeComparableUrl(options.targetUrl) : undefined;
	...
	const tabsWithIndices = options.tabs.map((tab, index) => ({
		...tab,
		index: hasRuntimeType(tab.index, "number") ? tab.index : index,
	...
	const activeTab =
		...
		(hasRuntimeType(options.activeTabIndex, "number") ? tabsWithIndices.find((tab) => tab.index === options.activeTabIndex) : undefined);
	...
	const trimmedTargetTitle = hasRuntimeType(options.targetTitle, "string") ? options.targetTitle.trim() : "";
	const titledMatch =
		trimmedTargetTitle.length === 0
			? undefined
			: matchingTabs.find((tab) => hasRuntimeType(tab.title, "string") && tab.title.trim() === trimmedTargetTitle);
