...
import {
  ...
  redactManagedSessionRestoreKeys,
...
function redactEmbeddedStructuredText(text: string): string {
  ...
  while (cursor < text.length) {
    ...
    try {
      ...
      const redacted = hasRuntimeType(parsed, "string")
    ...
    redactEnvSecretAssignments(
      redactStandaloneBasicCredential(
        redactBearerCredentials(
          redactLooseUrlUserinfo(redactLooseUrlMatches(redactManagedSessionRestoreKeys(text))),
        )
...
function getUnsupportedLeadingIdentityAssignment(
  args: string[],
...
function getRestorableManagedSessionName<Value>(
  value: Value,
  fallbackSessionName: string,
): string | undefined {
  return hasRuntimeType(value, "string") &&
    isRestorableManagedSessionName(value, fallbackSessionName)
    ? value
    : undefined;
}
...
function getElectronCleanupClosedManagedSessions(
  ...
  for (const result of results) {
    ...
    const fallbackNamespace = hasRuntimeType(record?.namespace, "string")
      ? record.namespace
      : hasRuntimeType(details.namespace, "string")
        ? details.namespace
        : undefined;
...
export function chooseOpenResultTabCorrection(options: {
  ...
  const normalizedTargetUrl = hasRuntimeType(options.targetUrl, "string")
    ? normalizeComparableUrl(options.targetUrl)
    : undefined;
  ...
  const activeTab =
    ...
    (hasRuntimeType(options.activeTabIndex, "number")
  ...
  const trimmedTargetTitle = hasRuntimeType(options.targetTitle, "string")
  ...
  const titledMatch =
    trimmedTargetTitle.length === 0
      ? undefined
      : matchingTabs.find(
          (tab) => hasRuntimeType(tab.title, "string") && tab.title.trim() === trimmedTargetTitle,
        );
