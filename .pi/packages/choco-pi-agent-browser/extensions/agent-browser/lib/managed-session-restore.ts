...
import { extractUpstreamCommandTokens, parseCommandInfo } from "./argv-descriptor.ts";
import {
...
} from "./managed-session-storage.ts";
import { parseUserBatchStdin } from "./orchestration/batch-stdin.ts";
import { getAgentBrowserProcessEnvironment } from "./process-environment.ts";
import { writeSecureTempFile } from "./temp.ts";

export { createManagedSessionRestoreKey, ensureManagedSessionRestoreStorageIsSecure, getManagedSessionRestoreScope } from "./managed-session-storage.ts";
export { pruneOwnedManagedSessionRestoreSnapshots } from "./managed-session-snapshots.ts";
...
export class ManagedSessionRestoreState {
	...
	getDaemonRestoreKey(sessionName: string | undefined, namespace?: string): string | null | undefined {
		return hasRuntimeType(sessionName, "string") ? this.#daemonRestoreKeys.get(getAgentBrowserSessionIdentityKey(sessionName, namespace)) : undefined;
	}

	hasDaemonRestoreKey(sessionName: string | undefined, namespace?: string): boolean {
		return hasRuntimeType(sessionName, "string") && this.#daemonRestoreKeys.has(getAgentBrowserSessionIdentityKey(sessionName, namespace));
	}
	...
	isDisabled(sessionName: string | undefined, namespace?: string): boolean {
		return hasRuntimeType(sessionName, "string") && this.#disabled.has(getAgentBrowserSessionIdentityKey(sessionName, namespace));
	}
...
export function getOwnedManagedSessionCompatibilityEnv(options: ManagedSessionRestoreEnvOptions): NodeJS.ProcessEnv {
	...
	if (ownedContext.compatibilityUserAgent) env.AGENT_BROWSER_USER_AGENT = ownedContext.compatibilityUserAgent;
	if (ownedContext.headedManagedAutosaveInterval !== undefined && !explicitIntervalMatches) {
