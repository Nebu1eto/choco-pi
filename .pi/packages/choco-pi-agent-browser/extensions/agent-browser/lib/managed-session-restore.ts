...
const MANAGED_SESSION_RESTORE_EMPTY_CONFIG_NAME =
  ".pi-agent-browser-managed-restore-config-v1.json";
...
export class ManagedSessionRestoreState {
  ...
  getDaemonRestoreKey(
    ...
    return hasRuntimeType(sessionName, "string")
  ...
  hasDaemonRestoreKey(sessionName: string | undefined, namespace?: string): boolean {
    return (
      hasRuntimeType(sessionName, "string") &&
...
export function getOwnedManagedSessionCompatibilityEnv(
  ...
  if (ownedContext.compatibilityUserAgent)
...
export function shouldOmitOwnedManagedSessionRestoreEnv(
  options: ManagedSessionRestoreEnvOptions,
): boolean {
...
export function canonicalizeOwnedManagedSessionCloseArgs(
  ...
  const namespace =
    canonicalizeAgentBrowserNamespace(policy.ownedContext?.namespace ?? policy.namespace) ?? "";
...
async function ensureManagedSessionRestoreEmptyConfig(
  ...
  for (let attempt = 0; attempt < 2; attempt += 1) {
    try {
      ...
      if (
        ...
        !entry.isFile() ||
        (platform !== "win32" && (entry.mode & 0o777) !== 0o400)
      )
