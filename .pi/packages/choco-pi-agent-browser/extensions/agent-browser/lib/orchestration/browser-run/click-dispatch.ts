...
import { withOptionalSessionArgs, type AgentBrowserNextAction } from "../../results/next-actions.ts";
import type { SessionRefSnapshot } from "../../session-page-state.ts";
import { runSessionCommandData } from "./session-state.ts";
import type { ClickDispatchDiagnostic, ClickDispatchProbe, ClickDispatchProbeTarget } from "./types.ts";
...
function getClickDispatchProbeTarget(commandTokens: string[], refSnapshot?: SessionRefSnapshot): ClickDispatchProbeTarget | undefined {
	...
	if (refId) {
		...
		const target: Extract<ClickDispatchProbeTarget, { kind: "accessible" }> = { kind: "accessible", name: ref.name, refId, role: ref.role };
		if (duplicateIndex !== undefined) target.duplicateIndex = duplicateIndex;
		return target;
	}
...
function getEvalResultRecord(data: Awaited<ReturnType<typeof runSessionCommandData>>): RuntimeRecord<RuntimeValue> | undefined {
...
export function buildClickDispatchNextActions(options: { commandTokens: string[]; diagnostic?: ClickDispatchDiagnostic; sessionName?: string }): AgentBrowserNextAction[] {
	...
	if (options.diagnostic?.scrollContainer) {
		actions.push({
			...
			params: { args: withOptionalSessionArgs(options.sessionName, ["scrollintoview", retryArgs[1]].filter((item): item is string => item !== undefined)) },
...
function getClickDispatchScrollContainerDiagnostic(result: RuntimeRecord<RuntimeValue>): ClickDispatchDiagnostic["scrollContainer"] {
	...
	const targetOutsideViewport = hasRuntimeType(target?.targetOutsideViewport, "boolean") ? target.targetOutsideViewport : undefined;
	const targetOutsideContainer = hasRuntimeType(scrollContainer?.targetOutsideContainer, "boolean") ? scrollContainer.targetOutsideContainer : undefined;
	...
	const selector = hasRuntimeType(scrollContainer?.selector, "string") ? redactSensitiveText(scrollContainer.selector) : undefined;
...
export async function collectClickDispatchDiagnostic(options: { cwd: string; namespace?: string; probe?: ClickDispatchProbe; sessionName?: string; signal?: AbortSignal }): Promise<ClickDispatchDiagnostic | undefined> {
	...
	const status = hasRuntimeType(result.status, "string") ? result.status : undefined;
	if (status !== "no-native-event-observed") return undefined;
	const nativeEventCount = hasRuntimeType(result.nativeEventCount, "number") ? result.nativeEventCount : 0;
	...
	const diagnostic: ClickDispatchDiagnostic = {
		nativeEventCount,
	...
	if (scrollContainer) diagnostic.scrollContainer = scrollContainer;
	return diagnostic;
}
