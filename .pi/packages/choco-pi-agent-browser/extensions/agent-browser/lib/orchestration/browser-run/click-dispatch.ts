...
function getClickDispatchProbeTarget(
  ...
  if (refId) {
    ...
    const target: Extract<ClickDispatchProbeTarget, { kind: "accessible" }> = {
      kind: "accessible",
      name: ref.name,
      refId,
      role: ref.role,
...
function getEvalResultRecord(
  data: Awaited<ReturnType<typeof runSessionCommandData>>,
    ...
    actions.push({
      ...
      params: {
        args: withOptionalSessionArgs(
          options.sessionName,
          ["scrollintoview", retryArgs[1]].filter((item): item is string => item !== undefined),
        ),
      },
...
function getClickDispatchScrollContainerDiagnostic(
  result: RuntimeRecord<RuntimeValue>,
  ...
  const scrollContainer = isRecord(target?.nearestScrollContainer)
    ...
    : undefined;
  const targetOutsideViewport = hasRuntimeType(target?.targetOutsideViewport, "boolean")
    ? target.targetOutsideViewport
    : undefined;
  const targetOutsideContainer = hasRuntimeType(scrollContainer?.targetOutsideContainer, "boolean")
  ...
  const selector = hasRuntimeType(scrollContainer?.selector, "string")
...
export async function collectClickDispatchDiagnostic(options: {
  ...
  const nativeEventCount = hasRuntimeType(result.nativeEventCount, "number")
    ? result.nativeEventCount
    : 0;
