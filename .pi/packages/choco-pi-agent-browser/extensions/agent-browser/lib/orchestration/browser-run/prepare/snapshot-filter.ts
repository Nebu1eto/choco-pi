...
import { buildSnapshotPresentation } from "../../../results/snapshot.ts";
import { extractRefSnapshotFromData, type SessionRefSnapshot } from "../../../session-page-state.ts";
import type { CompatibilityWorkaround } from "../../../runtime.ts";
import { collectScrollPositionSnapshot } from "../diagnostics.ts";
...
import type { AgentBrowserToolResult, BrowserRunOptions } from "../types.ts";
...
type SnapshotCommandData = Awaited<ReturnType<typeof runSessionCommandData>>;

interface FilteredSnapshotData {
...
function filterSnapshotData(data: SnapshotCommandData, request: SnapshotFilterRequest): FilteredSnapshotData | undefined {
	if (!isRecord(data)) return undefined;
	const refs = isRecord(data.refs) ? data.refs : {};
	const snapshot = hasRuntimeType(data.snapshot, "string") ? data.snapshot : "";
	...
	for (const [refId, refValue] of Object.entries(refs)) {
		...
		const role = hasRuntimeType(refValue.role, "string") ? refValue.role.toLowerCase() : "";
		const name = hasRuntimeType(refValue.name, "string") ? refValue.name : "";
...
export async function trySnapshotFilter(options: {
	...
	const snapshotText = isRecord(snapshotData) && hasRuntimeType(snapshotData.snapshot, "string") ? snapshotData.snapshot : undefined;
	const snapshotLineCount = snapshotText?.split(/\r?\n/).filter((line) => line.length > 0).length ?? 0;
	const filtered: FilteredSnapshotData | undefined = request.role || request.search
		? filterSnapshotData(snapshotData, request)
		: isRecord(snapshotData)
			? { data: snapshotData, matchedRefs: isRecord(snapshotData.refs) ? Object.keys(snapshotData.refs).length : 0, totalLines: snapshotLineCount, totalRefs: isRecord(snapshotData.refs) ? Object.keys(snapshotData.refs).length : 0, visibleLines: snapshotLineCount }
			: undefined;
