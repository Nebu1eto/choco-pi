...
function filterSnapshotData(
  data: SnapshotCommandData,
  request: SnapshotFilterRequest,
): FilteredSnapshotData | undefined {
...
export async function trySnapshotFilter(options: {
  ...
  const snapshotText =
    isRecord(snapshotData) && hasRuntimeType(snapshotData.snapshot, "string")
      ? snapshotData.snapshot
      : undefined;
  const snapshotLineCount =
    snapshotText?.split(/\r?\n/).filter((line) => line.length > 0).length ?? 0;
  const filtered: FilteredSnapshotData | undefined =
  ...
  const prefix = [summary, diffText, viewportText]
