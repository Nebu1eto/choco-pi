...
function extractCurrentUrl(data: SessionCommandData): string | undefined {
  ...
  for (const candidate of candidates)
    if (hasRuntimeType(candidate, "string") && candidate.length > 0) return candidate;
  return undefined;
}
...
): FilteredNetworkRequestsData | undefined {
  if (!isRecord(data)) return undefined;
...
function formatNetworkRequestRow(row: RuntimeValue): string {
  ...
  const method = hasRuntimeType(row.method, "string")
    ? row.method
    : hasRuntimeType(row.requestMethod, "string")
      ? row.requestMethod
      : "?";
  const id = hasRuntimeType(row.id, "string")
    ? ` id=${row.id}`
    : hasRuntimeType(row.requestId, "string")
      ? ` id=${row.requestId}`
      : "";
