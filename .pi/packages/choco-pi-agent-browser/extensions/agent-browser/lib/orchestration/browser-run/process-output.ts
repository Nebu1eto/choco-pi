...
import { getAgentBrowserSessionIdentityKey, isAgentBrowserSessionIdentityKeyInNamespace } from "../../argv-grammar.ts";
import { batchHasSuccessfulCloseAll, getSuccessfulBatchCloseLifecycle } from "../../batch-lifecycle.ts";
import { isCloseAllCommand, isCloseCommand, isNavigationObservableCommandName, isOpenNavigationCommand, isRecordPageTransitionCommand, isUnverifiedPageTransitionCommand } from "../../command-taxonomy.ts";
import { OPEN_RESULT_TAB_CORRECTION_FLAGS } from "../../launch-scoped-flags.ts";
import { cleanupElectronLaunchResources, inspectElectronLaunchStatus, type ElectronCleanupResult } from "../../electron/cleanup.ts";
import type { ElectronLaunchRecord } from "../../electron/launch.ts";
import { getAllowedDomainsViolation, parseAllowedDomainsPolicyFromArgs } from "../../navigation-policy.ts";
import { getManagedSessionResultingPageState, getObservedBrowserPageValidationError, managedSessionCommandRequiresLivePageVerification } from "../../managed-session-state-policy.ts";
import { analyzeNetworkSourceLookupResults, analyzeSourceLookupResults, redactNetworkSourceLookupAnalysis } from "../../input-modes/lookups.ts";
import { analyzeQaPresetResults, analyzeQaPresetTimeout, buildQaCompactFailureText, buildQaCompactPassText, extractQaPageContext } from "../../input-modes/job.ts";
import { applyNetworkRouteRecords, buildNetworkRouteDiagnostics } from "../../results/network-routes.ts";
import { buildToolPresentation } from "../../results/presentation.ts";
import { getAgentBrowserErrorText, parseAgentBrowserEnvelope } from "../../results/envelope.ts";
import { type AgentBrowserEnvelope } from "../../results/contracts.ts";
import type { NetworkRouteRecord } from "../../results/contracts.ts";
import { omitUpstreamLifecycle } from "../../results/presentation/common.ts";
import { getClipboardWritePayloadCandidates, redactClipboardPermissionEcho, redactClipboardPermissionErrorValue } from "../../results/presentation/errors.ts";
import { shouldCaptureSemanticActionNavigationSummary } from "../../results/presentation/semantic-action.ts";
import {
...
import { createFreshSessionName, extractUpstreamCommandTokens, resolveManagedSessionState } from "../../runtime.ts";
import { getUpstreamEffectiveBatchSteps } from "../batch-stdin.ts";
import { closeManagedSession, inspectManagedSessionDaemon } from "./managed-session-daemon-policy.ts";
import {
...
import { repairScreenshotData } from "./prepare.ts";
import { getPersistentSessionArtifactStore } from "./session-state.ts";
import {
...
function getEnvelopeErrorString(envelope: AgentBrowserEnvelope | undefined): string | undefined {
	...
	if (hasRuntimeType(envelope.error, "string")) return envelope.error;
	if (isRecord(envelope.error) && hasRuntimeType(envelope.error.message, "string")) return envelope.error.message;
...
function batchStartedManagedBrowser(data: AgentBrowserEnvelope["data"]): boolean {
	if (!Array.isArray(data)) return false;
	return data.some((entry) => {
		if (!isRecord(entry) || entry.success !== true || !Array.isArray(entry.command)) return false;
		const command = hasRuntimeType(entry.command[0], "string") ? entry.command[0] : undefined;
...
function applyBatchNetworkRouteState(options: { data: AgentBrowserEnvelope["data"]; routesBySession: Map<string, NetworkRouteRecord[]>; sessionName: string | undefined; succeeded: boolean }): Map<string, NetworkRouteRecord[]> {
	...
	for (const item of options.data) {
		if (!isRecord(item) || !Array.isArray(item.command) || !item.command.every((token: RuntimeValue): token is string => hasRuntimeType(token, "string"))) continue;
...
export async function processBrowserOutput(input: ProcessBrowserOutputInput): Promise<BrowserProcessOutputResult> {
	...
	try {
		...
		let navigationSummary: Awaited<ReturnType<typeof collectNavigationSummary>> | undefined;
		...
		const rawCloseStatePath = isCloseCommand(prepared.executionPlan.commandInfo.command)
			...
			&& hasRuntimeType(presentationEnvelope.data.statePath, "string")
		...
		if (presentationEnvelope && prepared.exactSensitiveValues.length > 0) {
			// SAFETY: Recursive exact-value redaction preserves every envelope field and changes string leaf contents only.
			presentationEnvelope = redactExactSensitiveValue(presentationEnvelope, prepared.exactSensitiveValues) as AgentBrowserEnvelope;
		}
		...
		if (prepared.executionPlan.commandInfo.command === "batch" && Array.isArray(presentationEnvelope?.data)) {
			for (const [index, row] of presentationEnvelope.data.entries()) {
				...
				const rowCommand = Array.isArray(row.command) && row.command.every((token: RuntimeValue): token is string => hasRuntimeType(token, "string"))
					? row.command
		...
		const dataClicked = hasRuntimeType(presentationDataRecord?.clicked, "string") ? presentationDataRecord.clicked : undefined;
		const cssClickWithoutHref = prepared.executionPlan.commandInfo.command === "click" && dataClicked !== undefined && !dataClicked.startsWith("@") && !dataClicked.startsWith("ref=") && !hasRuntimeType(presentationDataRecord?.href, "string");
		...
		const qaAttachedDiagnosticsText = prepared.compiledQaPreset?.checks.attached && prepared.compiledQaPreset.checks.diagnosticsResetAtStart === false && (prepared.compiledQaPreset.checks.checkNetwork || prepared.compiledQaPreset.checks.checkConsole || prepared.compiledQaPreset.checks.checkErrors)
			...
			: undefined;
		const qaAttachedBannerText = [qaAttachedTargetText, qaAttachedDiagnosticsText].filter((part): part is string => part !== undefined && part.length > 0).join("\n");
		...
		let resultWithCloseAll = result;
		if (closeAllApplied) {
			const details = isRecord(result.details) ? { ...result.details } : {};
			details.closeAllApplied = true;
			resultWithCloseAll = { ...result, details };
		}
