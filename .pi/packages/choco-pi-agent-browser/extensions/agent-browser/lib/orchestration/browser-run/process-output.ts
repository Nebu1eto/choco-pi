...
import {
  ...
  managedSessionCommandRequiresLivePageVerification,
...
} from "../../input-modes/job.ts";
import {
  applyNetworkRouteRecords,
  buildNetworkRouteDiagnostics,
} from "../../results/network-routes.ts";
...
} from "../../results/presentation/errors.ts";
...
} from "./managed-session-daemon-policy.ts";
import {
...
function getEnvelopeErrorString(envelope: AgentBrowserEnvelope | undefined): string | undefined {
  ...
  if (isRecord(envelope.error) && hasRuntimeType(envelope.error.message, "string"))
...
function applyBatchNetworkRouteState(options: {
  data: AgentBrowserEnvelope["data"];
  routesBySession: Map<string, NetworkRouteRecord[]>;
  sessionName: string | undefined;
  succeeded: boolean;
}): Map<string, NetworkRouteRecord[]> {
  ...
  for (const item of options.data) {
    if (
      ...
      !item.command.every((token: RuntimeValue): token is string => hasRuntimeType(token, "string"))
    )
      continue;
...
export async function processBrowserOutput(
  ...
  try {
    ...
    if (
      ...
      for (const [index, row] of presentationEnvelope.data.entries()) {
        ...
        const rowCommand =
          ...
          row.command.every((token: RuntimeValue): token is string =>
            hasRuntimeType(token, "string"),
          )
            ? row.command
    ...
    const presentationDataRecord = isRecord(presentationEnvelope?.data)
      ? presentationEnvelope.data
      : undefined;
    const dataClicked = hasRuntimeType(presentationDataRecord?.clicked, "string")
      ? presentationDataRecord.clicked
      : undefined;
    const cssClickWithoutHref =
      prepared.executionPlan.commandInfo.command === "click" &&
      dataClicked !== undefined &&
      !dataClicked.startsWith("@") &&
      !dataClicked.startsWith("ref=") &&
      !hasRuntimeType(presentationDataRecord?.href, "string");
    ...
    const shouldCaptureAllowedDomainNavigationSummary =
      sessionAllowedDomainsPolicy !== undefined &&
      !cssClickWithoutHref &&
      (prepared.executionPlan.commandInfo.command === "batch" ||
        isNavigationObservableCommandName(prepared.executionPlan.commandInfo.command));
    if (
      succeeded &&
      !navigationSummary &&
      (shouldCaptureNavigationSummary(
    ...
    const observedSessionTabTarget =
      ...
      (succeeded && trustsReportedPageTarget
        ...
        : undefined);
    const observedPageValidationError = succeeded
      ? getObservedBrowserPageValidationError(
          prepared.executionPlan.effectiveArgs,
          observedSessionTabTarget?.url,
          cwd,
        )
      : undefined;
    ...
    const safeObservedSessionTabTarget = observedPageValidationError
      ? normalizeSessionTabTarget({ url: observedSessionTabTarget?.url })
    ...
    const allowedDomainsViolation = succeeded
      ? getAllowedDomainsViolation({
          policy: sessionAllowedDomainsPolicy,
          url:
            currentSessionTabTarget?.url ?? observedSessionTabTarget?.url ?? navigationSummary?.url,
        })
      : undefined;
    if (allowedDomainsViolation) {
      ...
      presentationEnvelope = {
        ...presentationEnvelope,
        error: allowedDomainsViolation.summary,
        success: false,
      };
    }
    ...
    if (sessionStateKey) {
      ...
      if (sessionClosed) {
        ...
        const plannedBatchRecordSwap =
          !Array.isArray(presentationEnvelope?.data) &&
          batchCommandSteps.some((step) => isRecordPageTransitionCommand(step));
        const pageTransitionInvalidation =
          processResult.agentBrowserStarted &&
          (isRecordPageTransitionCommand(prepared.commandTokens) || plannedBatchRecordSwap)
            ? buildPageTransitionRefSnapshotInvalidation()
    ...
    const policyBlockedFreshManagedSession =
    ...
    const managedTransitionSucceeded =
      ...
      nestedBatchClosed ||
      nestedBatchRemainsActive ||
      policyBlockedFreshManagedSession ||
