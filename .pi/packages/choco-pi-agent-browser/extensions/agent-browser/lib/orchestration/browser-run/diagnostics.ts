...
} from "../../results/selector-recovery.ts";
import {
...
import {
  ...
  isFileUrl,
} from "../../managed-session-state-policy.ts";
import {
...
function extractScrollPositionSnapshot(
  data: BrowserDiagnosticData,
): ScrollPositionSnapshot | undefined {
  ...
  const scrollHeight = hasRuntimeType(result.scrollHeight, "number")
    ? result.scrollHeight
    : undefined;
  ...
  const containers = Array.isArray(result.containers)
    ? result.containers.flatMap((entry, index): ScrollPositionSnapshot["containers"] => {
        ...
        const scrollLeft = hasRuntimeType(entry.scrollLeft, "number")
          ? entry.scrollLeft
          : undefined;
    ...
    : [];
  return {
    containerCount: hasRuntimeType(result.containerCount, "number")
...
function extractComboboxFocusDiagnostic(
  data: BrowserDiagnosticData,
): ComboboxFocusDiagnostic | undefined {
  const result = isRecord(data) && isRecord(data.result) ? data.result : data;
  if (!isRecord(result) || result.comboboxLike !== true || !isRecord(result.activeElement))
    return undefined;
  const visibleListboxCount = hasRuntimeType(result.visibleListboxCount, "number")
    ? result.visibleListboxCount
    : 0;
  const visibleOptionCount = hasRuntimeType(result.visibleOptionCount, "number")
    ? result.visibleOptionCount
    : 0;
  const expanded = hasRuntimeType(result.activeElement.expanded, "string")
    ? result.activeElement.expanded
    : undefined;
  if (
  ...
  return {
    activeElement: {
      expanded,
      hasPopup: hasRuntimeType(result.activeElement.hasPopup, "string")
        ? result.activeElement.hasPopup
        : undefined,
      name: hasRuntimeType(result.activeElement.name, "string")
        ? redactSensitiveText(result.activeElement.name)
        : undefined,
      role: hasRuntimeType(result.activeElement.role, "string")
        ? result.activeElement.role
        : undefined,
      tagName: hasRuntimeType(result.activeElement.tagName, "string")
        ? result.activeElement.tagName
        : undefined,
    },
    message:
...
function getOverlayBlockerCandidates(
  snapshotData: BrowserDiagnosticData,
): OverlayBlockerCandidate[] {
  ...
  const hasOverlayContext = Object.values(refs).some(
    (entry) =>
      isRecord(entry) &&
      OVERLAY_CONTEXT_ROLES.has(
        (hasRuntimeType(entry.role, "string") ? entry.role : "").toLowerCase(),
      ),
  );
...
export async function collectOverlayBlockerDiagnostic(options: {
  ...
  if (
    ...
    !hasRuntimeType(options.data.clicked, "string")
  )
    return undefined;
...
function parseSelectorTextVisibilityCandidates(
  ...
  const candidates = value.flatMap(
    (entry): NonNullable<SelectorTextVisibilityDiagnostic["visibleCandidates"]> => {
      if (
        !isRecord(entry) ||
        !hasRuntimeType(entry.index, "number") ||
        !hasRuntimeType(entry.tagName, "string")
      )
        return [];
      const role =
        hasRuntimeType(entry.role, "string") && entry.role.length > 0 ? entry.role : undefined;
      const textPreview =
        hasRuntimeType(entry.textPreview, "string") && entry.textPreview.length > 0
          ? redactSensitiveText(entry.textPreview)
          : undefined;
      const candidate: NonNullable<SelectorTextVisibilityDiagnostic["visibleCandidates"]>[number] =
        { index: entry.index, tagName: entry.tagName };
      if (role) candidate.role = role;
...
function parseSelectorTextVisibilityProbe(
  ...
  const visibleCount = hasRuntimeType(parsed.visibleCount, "number")
    ? parsed.visibleCount
    : undefined;
  if (matchCount === undefined || visibleCount === undefined) return undefined;
  return {
    firstMatchVisible: hasRuntimeType(parsed.firstMatchVisible, "boolean")
      ? parsed.firstMatchVisible
      : undefined,
    firstVisibleTextPreview:
      hasRuntimeType(parsed.firstVisibleTextPreview, "string") &&
...
function getSuccessfulGetTextSelectors(options: {
  ...
  return options.commandInfo.command === "get" && options.commandInfo.subcommand === "text"
    ? [options.commandTokens[2]].filter(
        (selector): selector is string => selector !== undefined && selector.length > 0,
      )
...
async function collectManagedSessionCommandData(options: {
  ...
  signal?: AbortSignal;
  timeoutMs?: number;
}): Promise<{ data?: BrowserDiagnosticData; error?: string }> {
  try {
...
export async function validateQaAttachedPrecondition(options: {
  ...
  if (!url) {
    return {
      error:
        "qa.attached requires an attached session with a readable http(s) page URL. Run tab list, select a stable tab, then snapshot -i before retrying.",
      nextActions: buildQaAttachedRecoveryNextActions(options.sessionName),
    };
  }
...
export async function collectTimeoutPartialProgress(options: {
  ...
  const recoveredUrl =
    extractStringResultField(urlData, "result") ?? extractStringResultField(urlData, "url");
  const titleData =
    recoveredUrl && !isFileUrl(recoveredUrl)
      ? await runSessionCommandData({
          args: ["get", "title"],
          cwd: options.cwd,
          namespace: options.namespace,
...
export function formatTimeoutPartialProgressText(progress: TimeoutPartialProgress): string {
  ...
  for (const artifact of progress.artifacts)
    lines.push(
      `Artifact from step ${artifact.stepIndex}: ${redactSensitivePathSegmentsForDiagnostic(artifact.path)} (${artifact.exists ? `exists${artifact.sizeBytes !== undefined ? `, ${artifact.sizeBytes} bytes` : ""}` : "missing"})`,
    );
  return lines.join("\n");
}
