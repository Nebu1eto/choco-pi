...
import { isCloseCommand, isOpenNavigationCommand } from "../../command-taxonomy.ts";
import type { ElectronLaunchRecord } from "../../electron/launch.ts";
import { boundElectronProbeString } from "../../electron/cdp.ts";
import { executableExistsOnPath } from "../../executable-path.ts";
import type { AgentBrowserSourceLookupAnalysis, CompiledAgentBrowserJob, CompiledAgentBrowserSemanticAction } from "../../input-modes/types.ts";
import { isHttpOrHttpsUrl } from "../../input-modes/job.ts";
import type { AgentBrowserNextAction } from "../../results/contracts.ts";
import { formatSessionArtifactRetentionSummary } from "../../results/artifact-manifest.ts";
import { buildNextToolAction, withOptionalSessionArgs } from "../../results/next-actions.ts";
import { buildVisibleRefFallbackDiagnosticFromSnapshot, getVisibleRefFallbackTarget, type VisibleRefFallbackDiagnostic } from "../../results/selector-recovery.ts";
import { extractRefSnapshotFromData, isAboutBlankUrl, normalizeComparableUrl, type SessionRefSnapshot, type SessionTabTarget } from "../../session-page-state.ts";
import { extractUpstreamCommandTokens, parseWaitCommandTokens, redactInvocationArgs, redactSensitiveText, type CommandInfo } from "../../runtime.ts";
import { hasRuntimeType, isRecord, type RuntimeRecord, type RuntimeValue } from "../../parsing.ts";
import { getManagedSessionStateAccessValidationError, isFileUrl } from "../../managed-session-state-policy.ts";
import {
...
import { parseValidBatchStepEntries } from "../batch-stdin.ts";
import { getScreenshotPathTokenIndex } from "./artifact-paths.ts";
import type {
...
import type { SessionArtifactManifest } from "../../results/contracts.ts";

const ELECTRON_FILL_VERIFICATION_TIMEOUT_MS = 2_000;
type BrowserDiagnosticData = Awaited<ReturnType<typeof runSessionCommandData>>;
...
interface TimeoutProgressStepsResult {
...
function extractScrollPositionSnapshot(data: BrowserDiagnosticData): ScrollPositionSnapshot | undefined {
	const result = isRecord(data) && isRecord(data.result) ? data.result : data;
	if (!isRecord(result)) return undefined;
	const scrollX = hasRuntimeType(result.scrollX, "number") ? result.scrollX : undefined;
	const scrollY = hasRuntimeType(result.scrollY, "number") ? result.scrollY : undefined;
	const innerHeight = hasRuntimeType(result.innerHeight, "number") ? result.innerHeight : undefined;
	const innerWidth = hasRuntimeType(result.innerWidth, "number") ? result.innerWidth : undefined;
	const scrollHeight = hasRuntimeType(result.scrollHeight, "number") ? result.scrollHeight : undefined;
	const scrollWidth = hasRuntimeType(result.scrollWidth, "number") ? result.scrollWidth : undefined;
	...
	const containers = Array.isArray(result.containers)
		? result.containers.flatMap((entry, index): ScrollPositionSnapshot["containers"] => {
			...
			const rawId = hasRuntimeType(entry.id, "string") ? entry.id : undefined;
			const id = rawId && /^\d+:[a-z][a-z0-9-]*(?:\[role=[a-z-]+\])?$/i.test(rawId) ? rawId : `sample-${index}`;
			const scrollTop = hasRuntimeType(entry.scrollTop, "number") ? entry.scrollTop : undefined;
			const scrollLeft = hasRuntimeType(entry.scrollLeft, "number") ? entry.scrollLeft : undefined;
			return scrollTop !== undefined && scrollLeft !== undefined ? [{ id, scrollLeft, scrollTop }] : [];
		})
		: [];
	return { containerCount: hasRuntimeType(result.containerCount, "number") ? result.containerCount : containers.length, containers, innerHeight, innerWidth, scrollHeight, scrollWidth, scrollX, scrollY };
}
...
function extractComboboxFocusDiagnostic(data: BrowserDiagnosticData): ComboboxFocusDiagnostic | undefined {
	const result = isRecord(data) && isRecord(data.result) ? data.result : data;
	if (!isRecord(result) || result.comboboxLike !== true || !isRecord(result.activeElement)) return undefined;
	const visibleListboxCount = hasRuntimeType(result.visibleListboxCount, "number") ? result.visibleListboxCount : 0;
	const visibleOptionCount = hasRuntimeType(result.visibleOptionCount, "number") ? result.visibleOptionCount : 0;
	const expanded = hasRuntimeType(result.activeElement.expanded, "string") ? result.activeElement.expanded : undefined;
	if ((expanded !== "false" && expanded !== "true") || visibleListboxCount > 0 || visibleOptionCount > 0) return undefined;
	return {
		activeElement: {
			expanded,
			hasPopup: hasRuntimeType(result.activeElement.hasPopup, "string") ? result.activeElement.hasPopup : undefined,
			name: hasRuntimeType(result.activeElement.name, "string") ? redactSensitiveText(result.activeElement.name) : undefined,
			role: hasRuntimeType(result.activeElement.role, "string") ? result.activeElement.role : undefined,
			tagName: hasRuntimeType(result.activeElement.tagName, "string") ? result.activeElement.tagName : undefined,
		},
...
function getSnapshotRefRecord(data: BrowserDiagnosticData): RuntimeRecord<RuntimeValue> | undefined {
...
function getOverlayBlockerCandidates(snapshotData: BrowserDiagnosticData): OverlayBlockerCandidate[] {
	const refs = getSnapshotRefRecord(snapshotData);
	if (!refs) return [];
	const hasOverlayContext = Object.values(refs).some((entry) => isRecord(entry) && OVERLAY_CONTEXT_ROLES.has((hasRuntimeType(entry.role, "string") ? entry.role : "").toLowerCase()));
	...
	for (const [ref, entry] of Object.entries(refs)) {
		...
		const role = hasRuntimeType(entry.role, "string") ? entry.role : undefined;
		const name = hasRuntimeType(entry.name, "string") ? entry.name : undefined;
...
export function collectSnapshotOverlayBlockerDiagnostic(data: BrowserDiagnosticData): OverlayBlockerDiagnostic | undefined {
...
export async function collectOverlayBlockerDiagnostic(options: { command?: string; cwd: string; data: BrowserDiagnosticData; namespace?: string; navigationSummary?: NavigationSummary; priorTarget?: SessionTabTarget; sessionName?: string; signal?: AbortSignal }): Promise<OverlayBlockerDiagnostic | undefined> {
	if (options.command !== "click" || !isRecord(options.data) || !hasRuntimeType(options.data.clicked, "string")) return undefined;
...
function parseSelectorTextVisibilityCandidates(value: RuntimeValue): SelectorTextVisibilityDiagnostic["visibleCandidates"] {
	if (!Array.isArray(value)) return undefined;
	const candidates = value.flatMap((entry): NonNullable<SelectorTextVisibilityDiagnostic["visibleCandidates"]> => {
		if (!isRecord(entry) || !hasRuntimeType(entry.index, "number") || !hasRuntimeType(entry.tagName, "string")) return [];
		const role = hasRuntimeType(entry.role, "string") && entry.role.length > 0 ? entry.role : undefined;
		const textPreview = hasRuntimeType(entry.textPreview, "string") && entry.textPreview.length > 0 ? redactSensitiveText(entry.textPreview) : undefined;
		const candidate: NonNullable<SelectorTextVisibilityDiagnostic["visibleCandidates"]>[number] = { index: entry.index, tagName: entry.tagName };
		if (role) candidate.role = role;
		if (textPreview) candidate.textPreview = textPreview;
		return [candidate];
	});
	return candidates.length > 0 ? candidates : undefined;
}

function parseSelectorTextVisibilityProbe(data: BrowserDiagnosticData, selector: string): Omit<SelectorTextVisibilityDiagnostic, "summary"> | undefined {
	...
	if (!isRecord(parsed) || hasRuntimeType(parsed.error, "string")) return undefined;
	const matchCount = hasRuntimeType(parsed.matchCount, "number") ? parsed.matchCount : undefined;
	const visibleCount = hasRuntimeType(parsed.visibleCount, "number") ? parsed.visibleCount : undefined;
	if (matchCount === undefined || visibleCount === undefined) return undefined;
	return {
		firstMatchVisible: hasRuntimeType(parsed.firstMatchVisible, "boolean") ? parsed.firstMatchVisible : undefined,
		firstVisibleTextPreview: hasRuntimeType(parsed.firstVisibleTextPreview, "string") && parsed.firstVisibleTextPreview.length > 0 ? redactSensitiveText(parsed.firstVisibleTextPreview) : undefined,
		matchCount,
		selector,
...
function getBatchGetTextSelectors(data: BrowserDiagnosticData): string[] {
...
function getSuccessfulGetTextSelectors(options: { commandInfo: CommandInfo; commandTokens: string[]; data: BrowserDiagnosticData }): string[] {
	return options.commandInfo.command === "get" && options.commandInfo.subcommand === "text"
		? [options.commandTokens[2]].filter((selector): selector is string => selector !== undefined && selector.length > 0)
		: options.commandInfo.command === "batch" ? getBatchGetTextSelectors(options.data) : [];
}

export async function collectSelectorTextVisibilityDiagnostics(options: { commandInfo: CommandInfo; commandTokens: string[]; cwd: string; data: BrowserDiagnosticData; namespace?: string; sessionName?: string; signal?: AbortSignal }): Promise<SelectorTextVisibilityDiagnostic[]> {
...
export function collectElectronBroadGetTextScopeDiagnostics(options: { commandInfo: CommandInfo; commandTokens: string[]; currentTarget?: SessionTabTarget; data: BrowserDiagnosticData; electronLaunchRecords: Map<string, ElectronLaunchRecord>; namespace?: string; priorTarget?: SessionTabTarget; sessionName?: string }): ElectronBroadGetTextScopeDiagnostic[] {
...
function isPlainEmptyObject(value: RuntimeValue): boolean {
	if (!isRecord(value) || Array.isArray(value)) return false;
	const prototype = Object.getPrototypeOf(value);
	return (prototype === Object.prototype || prototype === null) && Object.keys(value).length === 0;
}

export function getEvalStdinHint(options: { command?: string; data: BrowserDiagnosticData; stdin?: string }) {
...
export function getEvalResultWarning(options: { command?: string; data: BrowserDiagnosticData; navigationSummary?: { url?: string }; pageUrl?: string; stdin?: string }) {
...
async function collectManagedSessionCommandData(options: { allowManagedSessionTarget?: boolean; args: string[]; cwd: string; namespace?: string; sessionName: string; signal?: AbortSignal; timeoutMs?: number }): Promise<{ data?: BrowserDiagnosticData; error?: string }> {
...
export async function collectElectronManagedSessionTarget(options: { allowManagedSessionTarget?: boolean; cwd: string; namespace?: string; sessionName?: string; signal?: AbortSignal; timeoutMs?: number }): Promise<ElectronManagedSessionTarget | undefined> {
	...
	const target: ElectronManagedSessionTarget = { sessionName: options.sessionName, title, url };
	if (titleResult.error) target.error = titleResult.error;
	return target;
}
...
export function formatQaAttachedTargetText(target: QaAttachedTarget | undefined): string | undefined {
	...
	return ["QA attached target:", target.sessionName, target.title, target.url].filter((part): part is string => part !== undefined && part.length > 0).join(" — ");
}
...
function getTopLevelFillInvocation(commandTokens: string[]): FillInvocation | undefined {
	...
	const invocation: FillInvocation = { expected, selector };
	if (refId) invocation.refId = refId;
	return invocation;
}
...
function extractFillVerificationValue(data: BrowserDiagnosticData): string | undefined {
	if (hasRuntimeType(data, "string")) return data;
	if (!isRecord(data)) return undefined;
	if (hasRuntimeType(data.value, "string")) return data.value;
	if (hasRuntimeType(data.result, "string")) return data.result;
	return undefined;
}
...
export async function collectFillVerificationDiagnostic(options: { commandTokens: string[]; cwd: string; forceValueVerification?: boolean; namespace?: string; refSnapshot?: SessionRefSnapshot; sessionName?: string; signal?: AbortSignal }): Promise<FillVerificationDiagnostic | undefined> {
	...
	let valueData: BrowserDiagnosticData;
...
export async function collectElectronHandoff(options: { cwd: string; handoff: "connect" | "snapshot" | "tabs"; namespace?: string; sessionName?: string; signal?: AbortSignal }): Promise<ElectronHandoffSummary> {
	...
	const handoff: ElectronHandoffSummary = { handoff: "snapshot", refSnapshot, snapshot, tabs };
	if (snapshotRetryCount > 0) handoff.snapshotRetryCount = snapshotRetryCount;
	return handoff;
}
...
export function formatTimeoutPartialProgressText(progress: TimeoutPartialProgress): string {
	...
	for (const artifact of progress.artifacts) lines.push(`Artifact from step ${artifact.stepIndex}: ${redactSensitivePathSegmentsForDiagnostic(artifact.path)} (${artifact.exists ? `exists${artifact.sizeBytes !== undefined ? `, ${artifact.sizeBytes} bytes` : ""}` : "missing"})`);
	return lines.join("\n");
}
