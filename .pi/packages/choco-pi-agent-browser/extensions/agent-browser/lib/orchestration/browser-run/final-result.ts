...
export function redactToolDetails<Details extends object>(
  details: Details,
  sensitiveValues: string[],
): Details {
...
export function buildJsonVisibleContent(options: {
  ...
  if (
    ...
    hasRuntimeType(presentation.data.wsUrl, "string")
  )
...
export function buildRedactedPresentationContent(options: {
  ...
  const redactedContent: NonNullable<AgentBrowserToolResult["content"]> =
    contentWithSessionWarnings.map((item) => {
...
function formatFailureNextActionsText(
  ...
  const lines = nextActions.slice(0, 6).map((action) => {
    ...
    if (params && action.params?.stdin !== undefined)
      params.stdin = "[omitted; use details.nextActions]";
