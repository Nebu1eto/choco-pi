...
import { formatSessionArtifactRetentionSummary, mergeSessionArtifactManifest } from "../../../results/artifact-manifest.ts";
import type { SessionArtifactManifest, SessionArtifactManifestEntry } from "../../../results/contracts.ts";
...
export async function tryDirectAnchorDownload(options: {
	...
	try {
		...
		if (!isRecord(probe) || probe.status !== "fetched-anchor" || !hasRuntimeType(probe.href, "string") || !hasRuntimeType(probe.pageUrl, "string") || !hasRuntimeType(probe.bodyBase64, "string")) return undefined;
		const href = new URL(probe.href);
		const pageUrl = new URL(probe.pageUrl);
		const responseUrl = hasRuntimeType(probe.responseUrl, "string") ? new URL(probe.responseUrl) : href;
		...
		if (hasRuntimeType(probe.sizeBytes, "number") && probe.sizeBytes !== body.byteLength) return undefined;
		const absolutePath = resolve(options.cwd, request.path);
		await mkdir(dirname(absolutePath), { recursive: true });
		await writeFile(absolutePath, body);
		const fileStat = await stat(absolutePath);
		const mediaType = hasRuntimeType(probe.contentType, "string") && probe.contentType.length > 0 ? probe.contentType : undefined;
