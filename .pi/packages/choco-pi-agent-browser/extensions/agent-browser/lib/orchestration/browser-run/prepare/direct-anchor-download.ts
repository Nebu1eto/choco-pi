...
export async function tryDirectAnchorDownload(options: {
  ...
  try {
    ...
    if (
      ...
      !hasRuntimeType(probe.href, "string") ||
      !hasRuntimeType(probe.pageUrl, "string") ||
      !hasRuntimeType(probe.bodyBase64, "string")
    )
      return undefined;
    const href = new URL(probe.href);
    const pageUrl = new URL(probe.pageUrl);
    const responseUrl = hasRuntimeType(probe.responseUrl, "string")
      ? new URL(probe.responseUrl)
      : href;
    if (
    ...
    if (hasRuntimeType(probe.sizeBytes, "number") && probe.sizeBytes !== body.byteLength)
      return undefined;
    ...
    const mediaType =
      hasRuntimeType(probe.contentType, "string") && probe.contentType.length > 0
        ? probe.contentType
        : undefined;
