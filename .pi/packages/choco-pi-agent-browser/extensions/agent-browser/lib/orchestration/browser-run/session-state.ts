...
import { getAgentBrowserSessionIdentityKey, VALUE_FLAGS } from "../../argv-grammar.ts";
import type { PersistentSessionArtifactStore } from "../../temp.ts";
import type { ElectronLaunchStatus } from "../../electron/cleanup.ts";
import type { ElectronCdpTarget, ElectronLaunchRecord } from "../../electron/launch.ts";
import { runAgentBrowserProcess } from "../../process.ts";
import { buildAgentBrowserNextActions } from "../../results/action-recommendations.ts";
import { parseAgentBrowserEnvelope } from "../../results/envelope.ts";
import { type AgentBrowserBatchResult, type AgentBrowserEnvelope, type AgentBrowserNextAction } from "../../results/contracts.ts";
import { buildNextToolAction, withOptionalNamespaceArgs, withOptionalSessionArgs } from "../../results/next-actions.ts";
import {
...
import { chooseOpenResultTabCorrection, type OpenResultTabCorrection } from "../../runtime.ts";
import { hasRuntimeType, isRecord, type RuntimeRecord, type RuntimeValue } from "../../parsing.ts";
import { getUpstreamEffectiveBatchSteps, parseUserBatchStdin } from "../batch-stdin.ts";
...
type BrowserSessionData = AgentBrowserEnvelope["data"];

interface ElectronIdentifiers {
	appName: string;
	launchId: string;
	sessionName?: string;
}

interface MappedSessionTab {
	active?: boolean;
	index?: number;
	label?: string;
	tabId?: string;
...
interface PinnedSessionBatchEnvelopeResult {
	envelope?: AgentBrowserEnvelope;
	navigationSummary?: NavigationSummary;
	parseError?: string;
}

interface SessionDetailFields {
...
function isAgentBrowserBatchResult(value: RuntimeValue): value is AgentBrowserBatchResult {
...
export function buildSessionDetailFields(
	...
	if (namespace !== undefined) fields.namespace = namespace;
	if (sessionName) {
		fields.sessionName = sessionName;
		fields.usedImplicitSession = usedImplicitSession;
		if (managedSessionRestoreDisabled) fields.managedSessionRestoreDisabled = true;
	}
	return fields;
}
...
export function buildManagedSessionOutcome(options: {
	...
	const outcome: ManagedSessionOutcome = {
		activeAfter,
		activeBefore,
	...
	if (currentSessionNamespace) outcome.currentSessionNamespace = currentSessionNamespace;
	return outcome;
}
...
export function extractStringResultField(data: BrowserSessionData, fieldName: "result" | "title" | "url" | "value"): string | undefined {
...
export function extractNavigationSummaryFromData(data: BrowserSessionData): NavigationSummary | undefined {
...
export function shouldCaptureNavigationSummary(command: string | undefined, data: BrowserSessionData): boolean {
	if (command === "eval") return true;
	return (
		isNavigationObservableCommandName(command) &&
		(!isRecord(data) || (!hasRuntimeType(data.title, "string") && !hasRuntimeType(data.url, "string")))
	);
}

export function mergeNavigationSummaryIntoData(data: BrowserSessionData, navigationSummary: NavigationSummary): BrowserSessionData {
	if (isRecord(data)) {
...
function extractBatchResultCommand(item: RuntimeRecord<RuntimeValue>): string[] {
	return Array.isArray(item.command) ? item.command.filter((token): token is string => hasRuntimeType(token, "string")) : [];
}
...
export function buildPinnedBatchPlan(options: {
	...
	const tabSelectionStep: BatchCommandStep = ["tab", options.selectedTab];
	const [commandToken, ...commandArgs] = options.commandTokens;
	const commandStep: BatchCommandStep = [commandToken, ...commandArgs];
...
function getTabSelection(tab: { index?: number; label?: string; tabId?: string }): Pick<OpenResultTabCorrection, "selectedTab" | "selectionKind"> | undefined {
	if (tab.tabId !== undefined && tab.tabId.trim().length > 0) return { selectedTab: tab.tabId.trim(), selectionKind: "tabId" };
	if (tab.label !== undefined && tab.label.trim().length > 0) return { selectedTab: tab.label.trim(), selectionKind: "label" };
	return tab.index !== undefined ? { selectedTab: String(tab.index), selectionKind: "index" } : undefined;
}
...
function selectAnySessionTargetTab(options: {
	...
	const targetUrl = options.target.url !== undefined ? normalizeComparableUrl(options.target.url) : undefined;
	...
	const correction: OpenResultTabCorrection = { ...selection, targetUrl };
	if (targetTitle) correction.targetTitle = targetTitle;
	return correction;
}
...
export function unwrapPinnedSessionBatchEnvelope(options: {
...
}): PinnedSessionBatchEnvelopeResult {
	if (!options.envelope) {
		return {};
	}
	...
	const steps = options.envelope.data.filter(isAgentBrowserBatchResult);
...
export async function runSessionCommandData(options: {
	...
	const processArgs = ["--json"];
	if (namespace !== undefined || pinNamespace) processArgs.push("--namespace", namespace ?? "");
	processArgs.push("--session", sessionName, ...args);
...
export async function collectOpenResultTabCorrection(options: {
	...
	const tabs = tabData.tabs.filter(isRecord).map((tab, index) => ({
		...
		label: hasRuntimeType(tab.label, "string") ? tab.label : undefined,
		tabId: hasRuntimeType(tab.tabId, "string") ? tab.tabId : undefined,
		title: hasRuntimeType(tab.title, "string") ? tab.title : undefined,
		url: hasRuntimeType(tab.url, "string") ? tab.url : undefined,
	}));
	return chooseOpenResultTabCorrection({ tabs, targetTitle, targetUrl });
}

function mapTabData(tabData: BrowserSessionData): MappedSessionTab[] | undefined {
...
export function electronTargetLabel(target: ElectronCdpTarget | undefined): string {
	...
	return [target.title, target.url, target.id].find((value) => value !== undefined && value.trim().length > 0) ?? "unknown target";
}
...
export function buildElectronIdentifiers(record: ElectronLaunchRecord): ElectronIdentifiers {
...
export function getPersistentSessionArtifactStore(ctx: BrowserRunContext): PersistentSessionArtifactStore | undefined {
	const sessionDir = hasRuntimeType(ctx.sessionManager.getSessionDir, "function") ? ctx.sessionManager.getSessionDir() : undefined;
