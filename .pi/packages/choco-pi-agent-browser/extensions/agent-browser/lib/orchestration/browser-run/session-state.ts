...
export function extractNavigationSummaryFromData(
  data: BrowserSessionData,
...
export function shouldCaptureNavigationSummary(
  ...
  return (
    ...
    (!isRecord(data) ||
      (!hasRuntimeType(data.title, "string") && !hasRuntimeType(data.url, "string")))
  );
}
...
function getTabSelection(tab: {
  ...
  if (tab.tabId !== undefined && tab.tabId.trim().length > 0)
    return { selectedTab: tab.tabId.trim(), selectionKind: "tabId" };
  if (tab.label !== undefined && tab.label.trim().length > 0)
    return { selectedTab: tab.label.trim(), selectionKind: "label" };
  return tab.index !== undefined
...
function selectAnySessionTargetTab(options: {
  ...
  const targetUrl =
    options.target.url !== undefined ? normalizeComparableUrl(options.target.url) : undefined;
...
export function getPersistentSessionArtifactStore(
  ...
  const sessionDir = hasRuntimeType(ctx.sessionManager.getSessionDir, "function")
