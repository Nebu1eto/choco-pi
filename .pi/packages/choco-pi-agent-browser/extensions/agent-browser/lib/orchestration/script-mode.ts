...
function isFailureCategory(
  value: RuntimeValue,
): value is NonNullable<AgentBrowserScriptBrowserEnvelope["failureCategory"]> {
  return (
    value === "aborted" ||
    value === "artifact-missing" ||
    value === "cleanup-failed" ||
    value === "confirmation-required" ||
    value === "download-not-verified" ||
...
function isSuccessCategory(
  value: RuntimeValue,
): value is NonNullable<AgentBrowserScriptBrowserEnvelope["successCategory"]> {
  return (
    value === "artifact-pending" ||
    value === "artifact-saved" ||
    value === "artifact-unverified" ||
    value === "completed" ||
    value === "inspection"
  );
}
...
): Promise<ScriptPayload | undefined> {
  const details = isRecord(result.details) ? result.details : undefined;
  const path = hasRuntimeType(details?.fullOutputPath, "string")
...
function getScriptCompatibleNextActions(
  ...
  const actions = value.flatMap((action): AgentBrowserNextAction[] => {
    ...
    if (
      ...
      action.params.args.some((token: RuntimeValue) => !hasRuntimeType(token, "string"))
    )
      return [];
    if (action.params.stdin !== undefined && !hasRuntimeType(action.params.stdin, "string"))
      return [];
...
export async function buildScriptBrowserEnvelope(
  ...
  const failureCategory =
    resultCategory === "failure" && isFailureCategory(details?.failureCategory)
      ? details.failureCategory
      : undefined;
  const successCategory =
    resultCategory === "success" && isSuccessCategory(details?.successCategory)
  ...
  let boundedEnvelopeDetails: NonNullable<AgentBrowserScriptBrowserEnvelope["details"]> = {
    resultCategory,
  };
  if (isRecord(envelopeDetails)) {
    ...
    boundedEnvelopeDetails = envelopeDetails as NonNullable<
      AgentBrowserScriptBrowserEnvelope["details"]
    >;
  }
...
function getLatestScriptResultDetail(
  results: AgentBrowserToolResult[],
  key: string,
): AgentBrowserToolResult["details"] {
