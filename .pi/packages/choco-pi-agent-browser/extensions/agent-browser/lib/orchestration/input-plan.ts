...
import { validateToolArgs, redactInvocationArgs, redactSensitiveText } from "../runtime.ts";
...
import { compileAgentBrowserNetworkSourceLookup, compileAgentBrowserSourceLookup, redactNetworkSourceLookupArgs, redactNetworkSourceLookupUrl } from "../input-modes/lookups.ts";
import { compileAgentBrowserSemanticAction } from "../input-modes/semantic-action.ts";
import { AGENT_BROWSER_SCRIPT_MAX_TIMEOUT_MS, compileAgentBrowserScript, type CompiledAgentBrowserScript } from "../input-modes/script.ts";
import type { AgentBrowserToolResult } from "./browser-run/types.ts";
import { type CompiledAgentBrowserElectron, type CompiledAgentBrowserJob, type CompiledAgentBrowserNetworkSourceLookup, type CompiledAgentBrowserQaPreset, type CompiledAgentBrowserSemanticAction, type CompiledAgentBrowserSourceLookup } from "../input-modes/types.ts";
interface ExplicitEvalStdin {
	args: string[];
	stdin?: string;
}
...
function normalizeExplicitEvalStdinArgs(args: string[], stdin: string | undefined): ExplicitEvalStdin {
	if (stdin !== undefined) {
		return { args, stdin };
	}
		...
		? compiledGeneratedBatch
			...
			: compiledElectron
				...
				: compiledScript
					...
					: undefined
		: undefined;
	const outputPathError = params.outputPath !== undefined && (!hasRuntimeType(params.outputPath, "string") || params.outputPath.trim().length === 0)
		? "outputPath must be a non-empty string when provided."
		: undefined;
	const timeoutMsError = params.timeoutMs !== undefined && (!hasRuntimeType(params.timeoutMs, "number") || !Number.isSafeInteger(params.timeoutMs) || params.timeoutMs <= 0)
...
export function buildValidationFailureResult(input: ResolvedAgentBrowserInvalidInput): AgentBrowserToolResult {
