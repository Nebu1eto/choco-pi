...
import { canonicalizeAgentBrowserNamespace } from "./argv-grammar.ts";

export { isManagedSessionRestoreKey } from "./managed-session-capabilities.ts";
...
export function ensureOwnerOnlyDirectory(path: string, platform: NodeJS.Platform = process.platform): boolean {
	try {
		try {
			...
			// SAFETY: mkdirSync reports filesystem failures as Node ErrnoException values.
...
function resolveGitCheckout(cwd: string, platform: NodeJS.Platform): { gitDirectory: string; worktreeDirectory: string } | undefined {
	...
	while (true) {
		...
		try {
			...
			// SAFETY: realpathSync reports filesystem failures as Node ErrnoException values.
...
function readProjectGenerationMarker(path: string, platform: NodeJS.Platform): string | undefined {
	try {
		...
		return isRecord(parsed) && parsed.version === 1 && hasRuntimeType(parsed.id, "string") && /^[a-f\d]{8}(?:-[a-f\d]{4}){3}-[a-f\d]{12}$/i.test(parsed.id) ? parsed.id : undefined;
	} catch {
		return undefined;
	}
}
		...
		if (!marker) {
			...
			if (platform === "android") {
				...
				try { writeFileSync(markerPath, content, { encoding: "utf8", flag: "wx", mode: 0o600 }); } catch (error) {
					// SAFETY: writeFileSync reports filesystem failures as Node ErrnoException values.
				...
				try {
					...
					try { linkSync(candidatePath, markerPath); } catch (error) {
						// SAFETY: linkSync reports filesystem failures as Node ErrnoException values.
						if ((error as NodeJS.ErrnoException).code !== "EEXIST") return undefined;
					}
				} finally {
...
function hasValidEncryptionKey(parentEnv: NodeJS.ProcessEnv): boolean {
	...
	return hasRuntimeType(value, "string") && /^[a-f\d]{64}$/i.test(value);
}
