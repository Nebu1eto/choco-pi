...
import { compileAgentBrowserElectron } from "./input-modes/electron.ts";
import { compileAgentBrowserJob, compileAgentBrowserQaPreset } from "./input-modes/job.ts";
import { compileAgentBrowserNetworkSourceLookup, compileAgentBrowserSourceLookup } from "./input-modes/lookups.ts";
import { compileAgentBrowserSemanticAction } from "./input-modes/semantic-action.ts";
import { hasRuntimeType, isRecord } from "./parsing.ts";
import { redactInvocationArgs } from "./runtime.ts";

const TUI_INVOCATION_PREVIEW_MAX_CHARS = 160;
const TUI_COLLAPSED_OUTPUT_MAX_LINES = 12;
const ANSI_CONTROL_SEQUENCE_PATTERN = new RegExp(String.raw`\x1B(?:\][^\x07\x1B\r\n\u2028\u2029]*(?:\x07|\x1B\\)|\[[0-?]*[ -/]*[@-~]|P[^\x1B\r\n\u2028\u2029]*(?:\x1B\\)|_[^\x1B\r\n\u2028\u2029]*(?:\x1B\\)|\^[^\x1B\r\n\u2028\u2029]*(?:\x1B\\)|[@-Z\\-_])`, "g");
const JSON_TOKEN_PATTERN = /"(?:\\.|[^"\\])*"(?=\s*:)|"(?:\\.|[^"\\])*"|-?\d+(?:\.\d+)?(?:[eE][+-]?\d+)?|true|false|null|[{}[\],:]/g;
const UNSAFE_DISPLAY_CONTROL_PATTERN = new RegExp(String.raw`[\x00-\x08\x0B\x0C\x0E-\x1F\x7F\x80-\x9F]`, "g");
...
interface StructuredModeInvocation {
	mode?: string;
	rawArgs: string[];
	scriptSource?: string;
}

function getStructuredModeInvocation(input: StructuredModeInput): StructuredModeInvocation {
	if (hasRuntimeType(input.script, "string")) return { mode: "script", rawArgs: [], scriptSource: input.script };
	if (Array.isArray(input.args)) return { rawArgs: input.args.filter((value): value is string => hasRuntimeType(value, "string")) };
...
export function formatAgentBrowserRenderCall<Args>(args: Args, theme: Theme, expanded = false): string {
	const input: StructuredModeInput = isRecord(args) ? args : {};
	...
	if (hasRuntimeType(input.stdin, "string")) {
...
export function formatAgentBrowserRenderResult(
	...
	if (outputLines.length === 0) {
		...
		const rawSummary = hasRuntimeType(details?.summary, "string") ? details.summary : isError ? "agent-browser failed" : "Done";
...
function formatModelVisibleFailureCategoryNotice<Details>(details: Details): string | undefined {
	if (!isRecord(details) || details.resultCategory !== "failure") return undefined;
	const failureCategory = hasRuntimeType(details.failureCategory, "string") && details.failureCategory.length > 0
...
function agentBrowserToolResultHasParseableJsonContent(content: AgentBrowserToolContent): boolean {
	return content.some((item) => {
		if (item.type !== "text" || !hasRuntimeType(item.text, "string")) return false;
...
function appendModelVisibleFailureCategoryNotice(content: AgentBrowserToolContent, notice: string): AgentBrowserToolContent | undefined {
	...
	const textIndex = content.findIndex((item) => item.type === "text" && hasRuntimeType(item.text, "string"));
	if (textIndex === -1) return [noticeContent, ...content];
	const textItem = content[textIndex];
	if (textItem.type !== "text" || !hasRuntimeType(textItem.text, "string") || textItem.text.includes(notice)) return undefined;
...
export function buildAgentBrowserToolResultPatch(event: ToolResultEvent): AgentBrowserToolResultPatch | undefined {
	...
	if (content && shouldMarkError) return { content, isError: true };
