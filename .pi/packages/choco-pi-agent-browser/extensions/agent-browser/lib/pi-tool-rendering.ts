...
function getStructuredModeInvocation(input: StructuredModeInput): StructuredModeInvocation {
  if (hasRuntimeType(input.script, "string"))
    return { mode: "script", rawArgs: [], scriptSource: input.script };
  if (Array.isArray(input.args))
    return {
      rawArgs: input.args.filter((value): value is string => hasRuntimeType(value, "string")),
    };
...
export function formatAgentBrowserRenderCall<Args>(
  args: Args,
  theme: Theme,
  expanded = false,
): string {
...
export function formatAgentBrowserRenderResult(
  ...
  if (outputLines.length === 0) {
    ...
    const rawSummary = hasRuntimeType(details?.summary, "string")
      ? details.summary
      : isError
...
function formatModelVisibleFailureCategoryNotice<Details>(details: Details): string | undefined {
  ...
  const failureCategory =
    hasRuntimeType(details.failureCategory, "string") && details.failureCategory.length > 0
...
function appendModelVisibleFailureCategoryNotice(
  ...
  const textIndex = content.findIndex(
    (item) => item.type === "text" && hasRuntimeType(item.text, "string"),
  );
  if (textIndex === -1) return [noticeContent, ...content];
  const textItem = content[textIndex];
  if (
    textItem.type !== "text" ||
    !hasRuntimeType(textItem.text, "string") ||
