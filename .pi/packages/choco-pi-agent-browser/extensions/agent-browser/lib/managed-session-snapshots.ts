import { hasRuntimeType, type RuntimeValue } from "./parsing.ts";
...
function pathExistsOrIsUnreadable(path: string): boolean {
	try {
		...
		// SAFETY: lstatSync reports filesystem failures as Node ErrnoException values.
...
function ensureManifestLineage(directory: string, lineage: string, platform: NodeJS.Platform): boolean {
	...
	try {
		...
		// SAFETY: openSync reports filesystem failures as Node ErrnoException values.
...
function writeRecord(directory: string, snapshotPath: string, platform: NodeJS.Platform): boolean {
	...
	try {
		...
		// SAFETY: snapshot file operations report filesystem failures as Node ErrnoException values.
...
function readRecord(options: {
	...
	try {
		...
		const parsed: RuntimeValue = JSON.parse(readFileSync(options.path, "utf8"));
		if (!hasRuntimeType(parsed, "string") || !isAbsolute(parsed)) return undefined;
