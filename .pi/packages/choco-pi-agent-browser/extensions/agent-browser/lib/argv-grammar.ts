...
export function isNonFlagToken(token: string | undefined): token is string {
  return hasRuntimeType(token, "string") && !token.startsWith("-");
}
...
export interface UpstreamGlobalFlagProjection {
  indices: number[];
  tokens: string[];
}

export function projectUpstreamGlobalFlags(args: readonly string[]): UpstreamGlobalFlagProjection {
...
export function stripSessionlessGlobalFlags(commandTokens: readonly string[]): string[] {
