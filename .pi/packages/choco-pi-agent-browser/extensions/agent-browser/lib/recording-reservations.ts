...
function parseReservationTransition<Data>(data: Data): RecordingReservationTransition | undefined {
  ...
  if (
    !hasRuntimeType(data.absolutePath, "string") ||
    !hasRuntimeType(data.cwd, "string") ||
...
export function restoreRecordingReservationStateFromBranch(
  ...
  for (const entry of branch) {
    ...
    const details =
      message?.toolName === "agent_browser" && isRecord(message.details)
        ? message.details
        : undefined;
    if (!details) continue;
    if (isSessionArtifactManifest(details.artifactManifest)) {
      const artifacts: FileArtifactMetadata[] = [...details.artifactManifest.entries]
        ...
        .filter(
          (manifestEntry) =>
            manifestEntry.command === "record" &&
            manifestEntry.kind === "video" &&
            manifestEntry.session,
        )
        .map((manifestEntry) => ({
          ...
          status: isPendingRecordingCommand(
            manifestEntry.command,
            manifestEntry.subcommand,
            "video",
          )
            ? "pending"
            : "saved",
    ...
    const directCloseSucceeded =
      messageSucceeded &&
      isCloseCommand(hasRuntimeType(details.command, "string") ? details.command : undefined);
    const batchRecordingClosed =
      getSuccessfulBatchCloseLifecycle(details.batchSteps)?.recordingClosedAfterBatch === true;
    if (
      (directCloseSucceeded || batchRecordingClosed) &&
      hasRuntimeType(details.sessionName, "string")
    ) {
