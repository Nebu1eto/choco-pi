...
import { isPendingRecordingArtifact, isPendingRecordingCommand, isSessionArtifactManifest } from "./results/artifact-manifest.ts";
import type { FileArtifactMetadata } from "./results/contracts.ts";
...
function parseReservationTransition<Data>(data: Data): RecordingReservationTransition | undefined {
	...
	if (!hasRuntimeType(data.sessionName, "string") || data.sessionName.length === 0) return undefined;
	if (data.namespace !== undefined && !hasRuntimeType(data.namespace, "string")) return undefined;
	...
	if (!hasRuntimeType(data.absolutePath, "string") || !hasRuntimeType(data.cwd, "string") || !hasRuntimeType(data.path, "string")) return undefined;
	return {
		reservation: {
...
export function restoreRecordingReservationStateFromBranch(branch: unknown[]): RecordingReservationBranchState {
	...
	for (const entry of branch) {
		...
		const messageSucceeded = hasRuntimeType(message?.isError, "boolean")
			? !message.isError
			: !hasRuntimeType(details.exitCode, "number") || details.exitCode === 0;
		...
		const directCloseSucceeded = messageSucceeded && isCloseCommand(hasRuntimeType(details.command, "string") ? details.command : undefined);
		const batchRecordingClosed = getSuccessfulBatchCloseLifecycle(details.batchSteps)?.recordingClosedAfterBatch === true;
		if ((directCloseSucceeded || batchRecordingClosed) && hasRuntimeType(details.sessionName, "string")) {
