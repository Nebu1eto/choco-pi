...
import { getEditableRefEvidence } from "./editable-ref-evidence.ts";
import { type AgentBrowserNextAction, withOptionalSessionArgs } from "./next-actions.ts";
import {
  getAgentBrowserRichInputRecoveryNextActionId,
  getAgentBrowserRichInputRecoveryNextActionIds,
} from "./recovery-actions.ts";
import {
  getSnapshotLineTextByRef,
  getSnapshotRefRecord,
  getSnapshotRefRole,
} from "./snapshot-refs.ts";
import { compareRefIds } from "./text.ts";
...
const SELECTOR_RECOVERY_ACTION_NAMES = new Set<string>([
  "check",
  "click",
  "fill",
  "select",
...
const RICH_INPUT_RECOVERY_HINT =
  "After the editable ref is focused, use keyboard inserttext or keyboard type with the intended text in a separate call, and do not press Enter or otherwise submit unless the user flow explicitly calls for it.";
...
function getFindNameFlagValue(args: string[], startIndex: number): string | undefined {
  ...
  return hasRuntimeType(name, "string") && name.length > 0 ? name : undefined;
}
...
function getVisibleRefFallbackCandidates<Value>(
  ...
  const candidates = Object.entries(refs).flatMap(([ref, entry]): VisibleRefFallbackCandidate[] => {
      ...
      target.action === "fill"
        ...
        : target.action === "select" && target.optionValues && target.optionValues.length > 0
          ...
          : target.action === "select"
            ? undefined
            : [target.action, `@${ref}`];
    const reason = `Current snapshot shows ${role} ${JSON.stringify(name)} at @${ref}, matching the failed ${target.action} locator exactly.`;
    if (directRefArgs && editableEvidence !== undefined) {
      return [
        {
    ...
    if (directRefArgs) {
      return [{ action: target.action, args: directRefArgs, name, reason, ref: `@${ref}`, role }];
    }
    if (editableEvidence !== undefined) {
      return [{ action: target.action, name, reason, ref: `@${ref}`, role, editableEvidence }];
    }
    return [{ action: target.action, name, reason, ref: `@${ref}`, role }];
  });
  candidates.sort(
    (left, right) =>
