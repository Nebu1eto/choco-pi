...
const SELECTOR_RECOVERY_ACTION_NAMES = new Set<string>(["check", "click", "fill", "select", "uncheck"]);
...
function getVisibleRefFallbackCandidates<Value>(target: VisibleRefFallbackTarget, snapshotData: Value): VisibleRefFallbackCandidate[] {
	...
	const candidates = Object.entries(refs).flatMap(([ref, entry]): VisibleRefFallbackCandidate[] => {
		...
		if (directRefArgs && editableEvidence !== undefined) {
			return [{ action: target.action, args: directRefArgs, name, reason, ref: `@${ref}`, role, editableEvidence }];
		}
		if (directRefArgs) {
