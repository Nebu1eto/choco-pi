...
import type { AgentBrowserBatchResult, AgentBrowserEnvelope } from "./contracts.ts";
import { stringifyUnknown } from "./text.ts";

function hasStructuredBatchStepFailure<Value>(data: Value): data is Value & AgentBrowserBatchResult[] {
	return Array.isArray(data) && data.some((item) => isRecord(item) && item.success === false);
}

function isAgentBrowserEnvelope<Value>(value: Value): value is Value & AgentBrowserEnvelope {
	return isRecord(value) && hasRuntimeType(value.success, "boolean");
}
...
function extractEnvelopeErrorText<Value>(error: Value): string | undefined {
	if (hasRuntimeType(error, "string")) {
		return error.trim() || undefined;
	}
	if (hasRuntimeType(error, "number") || hasRuntimeType(error, "boolean")) {
		return String(error);
	}
...
export async function parseAgentBrowserEnvelope(options: string | { stdout: string; stdoutPath?: string }): Promise<{
	...
	let stdout: string;
	try {
		stdout = hasRuntimeType(options, "string") ? options : await readEnvelopeSource(options);
	} catch (error) {
	...
	try {
		...
		if (!isAgentBrowserEnvelope(parsed)) {
