...
import type { FileArtifactKind, FileArtifactMetadata, SessionArtifactManifest, SessionArtifactManifestEntry } from "./contracts.ts";
...
function isManifestEntry<Value>(value: Value): value is Value & SessionArtifactManifestEntry {
	if (!isRecord(value)) return false;
	if (!hasRuntimeType(value.path, "string") || value.path.trim().length === 0) return false;
	if (!hasRuntimeType(value.createdAtMs, "number") || !Number.isFinite(value.createdAtMs)) return false;
	...
	if (!hasRuntimeType(value.kind, "string") || value.kind.trim().length === 0) return false;
	for (const key of ["absolutePath", "command", "cwd", "extension", "mediaType", "namespace", "requestedPath", "session", "subcommand"] as const) {
		if (value[key] !== undefined && !hasRuntimeType(value[key], "string")) return false;
	}
	if (value.evictedAtMs !== undefined && (!hasRuntimeType(value.evictedAtMs, "number") || !Number.isFinite(value.evictedAtMs))) return false;
	if (value.exists !== undefined && !hasRuntimeType(value.exists, "boolean")) return false;
	if (value.sizeBytes !== undefined && (!hasRuntimeType(value.sizeBytes, "number") || !Number.isFinite(value.sizeBytes) || value.sizeBytes < 0)) return false;
	return true;
}

export function isSessionArtifactManifest<Value>(value: Value): value is Value & SessionArtifactManifest {
	if (!isRecord(value)) return false;
	...
	if (!hasRuntimeType(value.updatedAtMs, "number") || !Number.isFinite(value.updatedAtMs)) return false;
	if (!hasRuntimeType(value.maxEntries, "number") || !Number.isSafeInteger(value.maxEntries) || value.maxEntries <= 0) return false;
	if (!hasRuntimeType(value.liveCount, "number") || !Number.isSafeInteger(value.liveCount) || value.liveCount < 0) return false;
	if (!hasRuntimeType(value.evictedCount, "number") || !Number.isSafeInteger(value.evictedCount) || value.evictedCount < 0) return false;
	return true;
}
