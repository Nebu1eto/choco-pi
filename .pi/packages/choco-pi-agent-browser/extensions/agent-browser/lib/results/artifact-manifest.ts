...
function isManifestEntry<Value>(value: Value): value is Value & SessionArtifactManifestEntry {
  ...
  if (!hasRuntimeType(value.createdAtMs, "number") || !Number.isFinite(value.createdAtMs))
    return false;
  ...
  if (
    ...
    (!hasRuntimeType(value.evictedAtMs, "number") || !Number.isFinite(value.evictedAtMs))
  )
    return false;
  if (value.exists !== undefined && !hasRuntimeType(value.exists, "boolean")) return false;
  if (
    value.sizeBytes !== undefined &&
    (!hasRuntimeType(value.sizeBytes, "number") ||
...
export function isSessionArtifactManifest<Value>(
  value: Value,
): value is Value & SessionArtifactManifest {
  if (!isRecord(value)) return false;
  ...
  if (!hasRuntimeType(value.updatedAtMs, "number") || !Number.isFinite(value.updatedAtMs))
    return false;
  if (
    !hasRuntimeType(value.maxEntries, "number") ||
    !Number.isSafeInteger(value.maxEntries) ||
    value.maxEntries <= 0
  )
    return false;
  if (
    !hasRuntimeType(value.liveCount, "number") ||
    !Number.isSafeInteger(value.liveCount) ||
    value.liveCount < 0
  )
    return false;
  if (
    !hasRuntimeType(value.evictedCount, "number") ||
