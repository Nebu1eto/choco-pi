...
import { isHighValueControlEntry, selectHighValueControlEntries } from "./snapshot-high-value-controls.ts";
import {
	...
	isNoiseName,
	parseSnapshotLines,
} from "./snapshot-segments.ts";
import { applySnapshotArtifactManifest, writeSnapshotSpillFile, type SnapshotSpillWriteResult } from "./snapshot-spill.ts";
import {
	enrichSnapshotRefEntries,
	getSnapshotRefEntries,
	type SnapshotRefEntry,
} from "./snapshot-refs.ts";
import { compareRefIds, countLines, truncateText } from "./text.ts";
...
function getSnapshotOrigin<Value>(data: Record<string, Value>): string {
	return hasRuntimeType(data.origin, "string") ? data.origin : "(unknown origin)";
}

function getSnapshotRoleCounts(refEntries: SnapshotRefEntry[]): SnapshotRoleCounts {
	const counts: SnapshotRoleCounts = {};
...
function formatRoleCounts(roleCounts: SnapshotRoleCounts): string | undefined {
...
function shouldCompactSnapshot<Value>(rawText: string, data: Record<string, Value>): boolean {
...
export async function buildSnapshotPresentation<Value>(
