...
export function getStringRecordField<Value>(value: Record<string, Value>, key: string): string | undefined {
...
function isFailedNetworkRequest<Value>(request: Record<string, Value>): boolean {
	return (hasRuntimeType(request.status, "number") && request.status >= 400) || request.failed === true || hasRuntimeType(request.error, "string");
}
...
function isBenignAssetFailure<Value>(request: Record<string, Value>, url: string | undefined, resourceType: string | undefined): boolean {
	const path = getNetworkRequestUrlPath(url);
	if (!path) return false;
	const normalizedResourceType = resourceType?.toLowerCase();
	return /(?:^|\/)(?:favicon(?:[-.\w]*)?\.(?:ico|png|svg)|apple-touch-icon(?:[-.\w]*)?\.png)$/i.test(path)
		&& (request.status === 404 || request.failed === true || hasRuntimeType(request.error, "string"))
...
export function classifyNetworkRequestFailure<Value>(request: Record<string, Value>): NetworkFailureClassification | undefined {
