...
import { getStringField, redactModelFacingText, stringifyModelFacing } from "./common.ts";

function formatSkillsListText<Value>(skills: Value[]): string {
...
function getSkillContent<Value>(data: Value): string | undefined {
	if (hasRuntimeType(data, "string")) return data;
	if (isRecord(data) && hasRuntimeType(data.content, "string")) return data.content;
	if (!Array.isArray(data)) return undefined;
	const content = data.flatMap((item) => (isRecord(item) && hasRuntimeType(item.content, "string") ? [item.content] : []));
...
export function formatSkillsText<Value>(commandInfo: CommandInfo, data: Value): string | undefined {
	if (commandInfo.command !== "skills") return undefined;
	if (commandInfo.subcommand === "path") return hasRuntimeType(data, "string") ? redactModelFacingText(data) : undefined;
	...
	if (hasRuntimeType(data, "string")) return redactModelFacingText(formatNativeSkillContent(data));
	return undefined;
}
