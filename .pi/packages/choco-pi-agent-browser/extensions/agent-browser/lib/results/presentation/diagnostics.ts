...
import { isSensitiveFieldName, redactSensitiveText, redactSensitiveValue, type CommandInfo } from "../../runtime.ts";
import type { AgentBrowserNextAction, AgentBrowserResultRecord, AgentBrowserResultValue, NetworkRouteDiagnostic } from "../contracts.ts";
import { classifyNetworkRequestFailure, isApiLikeNetworkRequest, isNetworkArtifactNoiseRequest, summarizeNetworkFailures } from "../network.ts";
import { withOptionalSessionArgs } from "../next-actions.ts";
...
export function getTabSummary<Value>(data: Record<string, Value>): string | undefined {
	...
	const lines = tabs.map((tab, index) => {
		...
		const title = hasRuntimeType(tab.title, "string") ? tab.title : "(untitled)";
		const url = hasRuntimeType(tab.url, "string") ? tab.url : "(no url)";
		const label = hasRuntimeType(tab.label, "string") && tab.label.trim().length > 0 ? tab.label.trim() : undefined;
		const tabSelector =
			hasRuntimeType(tab.tabId, "string") && tab.tabId.trim().length > 0
				? tab.tabId.trim()
				: label
					? label
					: hasRuntimeType(tab.index, "number")
						? String(tab.index)
						: String(index);
		const labelText = label && label !== tabSelector ? ` label=${redactModelFacingText(label)}` : "";
		const targetId = hasRuntimeType(tab.targetId, "string") && tab.targetId.trim().length > 0 ? tab.targetId.trim() : undefined;
...
export function getStreamSummary<Value>(data: Record<string, Value>): string | undefined {
	if (data.alreadyEnabled === true) {
		const lines = ["Stream already enabled (idempotent no-op)."];
		if (hasRuntimeType(data.port, "number")) {
	...
	if (!hasRuntimeType(data.enabled, "boolean") || !hasRuntimeType(data.connected, "boolean")) {
		return undefined;
	}
...
export function enrichStreamStatusData<Value>(commandInfo: CommandInfo, data: Value) {
	if (commandInfo.command !== "stream" || commandInfo.subcommand !== "status" || !isRecord(data) || !hasRuntimeType(data.port, "number")) {
		return data;
	}
	return {
		...data,
...
export function formatDiagnosticSummary<Value>(commandInfo: CommandInfo, data: Record<string, Value>): string | undefined {
	...
	if (commandInfo.command === "dialog") {
		const open = hasRuntimeType(data.open, "boolean") ? data.open : undefined;
	...
	if (commandInfo.command === "stream") {
		if (commandInfo.subcommand === "enable") {
			...
			const port = hasRuntimeType(data.port, "number") ? ` on port ${data.port}` : "";
	...
	if (commandInfo.command === "dashboard") {
		if (hasRuntimeType(data.port, "number")) return `Dashboard running on port ${data.port}`;
...
function formatSessionText<Value>(data: Record<string, Value>): string | undefined {
	const sessions = getArrayField(data, "sessions");
	if (sessions) {
		...
		return visibleSessions
			.map((item, index) => {
				...
				const tabCount = hasRuntimeType(item.tabCount, "number") ? `${item.tabCount} tab${item.tabCount === 1 ? "" : "s"}` : undefined;
				const metadata = [
...
export function formatProfilesText<Value>(profiles: Value[], label: string): string {
...
function formatAuthShowText<Value>(data: Record<string, Value>): string | undefined {
	const lines = AUTH_SHOW_SAFE_FIELDS.flatMap((key) => {
		const value = data[key];
		return hasRuntimeType(value, "string") && value.trim().length > 0 ? [`${key}: ${redactModelFacingText(value.trim())}`] : [];
	});
	return lines.length > 0 ? lines.join("\n") : undefined;
}

function getPreviewCandidate<Value>(item: Record<string, Value>, keys: readonly string[]): Value | undefined {
...
function formatNetworkPreviewValue<Value>(value: Value, maxChars: number): string | undefined {
	if (value === undefined || value === null) return undefined;
	const previewValue = hasRuntimeType(value, "string") ? parseJsonPreviewString(value) : value;
	const redacted = redactSensitiveValue(previewValue);
	const raw = hasRuntimeType(redacted, "string") ? redacted : stringifyUnknown(redacted);
	const normalized = raw.replace(/\s+/g, " ").trim();
	if (normalized.length === 0) return undefined;
	return truncateText(redactSensitiveText(normalized), maxChars);
}

function appendNetworkPreview<Value>(lines: string[], label: string, value: Value, maxChars: number): void {
	const preview = formatNetworkPreviewValue(value, maxChars);
	if (!preview) return;
	lines.push(`   ${label}: ${preview}`);
}

function formatNetworkRequestLine<Value>(item: Record<string, Value>, index: number): string[] {
	const method = getStringField(item, "method") ?? "GET";
	const status = hasRuntimeType(item.status, "number") ? String(item.status) : "pending";
...
function formatNetworkRequestsText<Value>(data: Record<string, Value>, commandInfo: CommandInfo): string | undefined {
	const requests = getArrayField(data, "requests");
...
function formatNetworkRequestText<Value>(data: Record<string, Value>): string | undefined {
...
interface NetworkRequestActionCandidate<Value> {
	filter?: string;
...
function getNetworkRequestId<Value>(item: Record<string, Value>): string | undefined {
...
function getNetworkRequestPathFilter<Value>(item: Record<string, Value>): string | undefined {
	const url = getStringField(item, "url");
...
function getNetworkRequestActionCandidate<Value>(item: Record<string, Value>): NetworkRequestActionCandidate<Value> | undefined {
	...
	const kind: NetworkRequestActionCandidate<Value>["kind"] = classification?.impact === "actionable"
		? "actionable"
...
function chooseNetworkRequestActionCandidate<Value>(candidates: NetworkRequestActionCandidate<Value>[]): NetworkRequestActionCandidate<Value> | undefined {
...
function formatNetworkRequestActionDescriptor<Value>(candidate: NetworkRequestActionCandidate<Value>): string {
	const method = getStringField(candidate.item, "method") ?? "GET";
	const status = hasRuntimeType(candidate.item.status, "number") ? String(candidate.item.status) : "pending";
	const target = candidate.filter ? ` ${candidate.filter}` : "";
	return `${status} ${method}${target} [${candidate.requestId}]`;
}

function getNetworkRequestDetailActionId<Value>(candidate: NetworkRequestActionCandidate<Value>): string {
...
export function buildNetworkRequestsNextActions<Value>(data: Value, sessionName: string | undefined, routeDiagnostics?: NetworkRouteDiagnostic[]): AgentBrowserNextAction[] | undefined {
	...
	if (selected.kind === "actionable") {
		const networkSourceLookup: NonNullable<AgentBrowserNextAction["params"]>["networkSourceLookup"] = { requestId: selected.requestId };
		if (sessionName) networkSourceLookup.session = sessionName;
		actions.push({
...
export function buildStreamNextActions<Value>(commandInfo: CommandInfo, data: Value, sessionName: string | undefined): AgentBrowserNextAction[] | undefined {
...
function formatConsoleText<Value>(data: Record<string, Value>, commandInfo: CommandInfo): string | undefined {
	const messages = getArrayField(data, "messages");
...
function formatA11yText<Value>(data: Record<string, Value>): string | undefined {
	...
	const violationCount = hasRuntimeType(counts?.violations, "number") ? counts.violations : violations.length;
	const incompleteCount = hasRuntimeType(counts?.incomplete, "number") ? counts.incomplete : incomplete.length;
	const passCount = hasRuntimeType(counts?.passes, "number") ? counts.passes : undefined;
	const inapplicableCount = hasRuntimeType(counts?.inapplicable, "number") ? counts.inapplicable : undefined;
	...
	const preview = violations.slice(0, previewLimit).map((item, index) => {
		...
		const nodeCount = hasRuntimeType(item.nodeCount, "number") ? item.nodeCount : getArrayField(item, "nodes")?.length;
		const nodePart = hasRuntimeType(nodeCount, "number") ? `, ${nodeCount} node${nodeCount === 1 ? "" : "s"}` : "";
...
function formatErrorsText<Value>(data: Record<string, Value>, commandInfo: CommandInfo): string | undefined {
	const errors = getArrayField(data, "errors");
	...
	const shown = errors.slice(0, DIAGNOSTIC_LOG_PREVIEW_LIMIT).map((item, index) => {
		...
		const location = [
			...
			hasRuntimeType(item.line, "number") ? `line ${item.line}` : undefined,
			hasRuntimeType(item.column, "number") ? `column ${item.column}` : undefined,
		]
			.filter(Boolean)
...
function formatDashboardText<Value>(data: Record<string, Value>): string | undefined {
	const lines: string[] = [];
	if (hasRuntimeType(data.port, "number")) lines.push(`Port: ${data.port}`);
	if (hasRuntimeType(data.pid, "number")) lines.push(`PID: ${data.pid}`);
	if (hasRuntimeType(data.stopped, "boolean")) lines.push(`Stopped: ${data.stopped}`);
	const reason = getStringField(data, "reason");
	if (reason) lines.push(`Reason: ${redactModelFacingText(reason)}`);
	return lines.length > 0 ? lines.join("\n") : undefined;
}

function formatChatText<Value>(data: Record<string, Value>): string | undefined {
...
function formatDoctorText<Value>(data: Record<string, Value>): string | undefined {
	const lines: string[] = [];
	const status = getStringField(data, "status") ?? getStringField(data, "result");
	if (status) lines.push(`Status: ${redactModelFacingText(status)}`);
	const summary = isRecord(data.summary) ? data.summary : undefined;
	if (summary) {
		const parts = ["pass", "warn", "fail"].flatMap((key) => hasRuntimeType(summary[key], "number") ? [`${key}:${summary[key]}`] : []);
...
function formatCookieRecordText<Value>(item: Record<string, Value>, fallbackName: string): string {
...
function formatCookiesText<Value>(data: Record<string, Value>): string | undefined {
	const cookies = getArrayField(data, "cookies");
	if (cookies) {
...
function valueContainsStorageSecret<Value>(value: Value): boolean {
...
function shouldRevealStorageValue<Value>(key: string | undefined, value: Value): boolean {
	...
	if (hasRuntimeType(value, "string")) return value.length <= STORAGE_VALUE_PREVIEW_MAX_CHARS;
	return value === null || hasRuntimeType(value, "number") || hasRuntimeType(value, "boolean");
}

function formatStorageValue<Value>(key: string | undefined, value: Value): string {
	if (!shouldRevealStorageValue(key, value)) return "[REDACTED]";
	if (hasRuntimeType(value, "string")) return redactModelFacingText(value);
	return stringifyModelFacing(value);
}

function isAgentBrowserResultRecord(value: AgentBrowserResultValue): value is AgentBrowserResultRecord {
	return hasRuntimeType(value, "object") && value !== null && !Array.isArray(value);
}

function redactStorageEntryValue(item: AgentBrowserResultRecord): AgentBrowserResultRecord {
	if (!Object.hasOwn(item, "value")) return redactStructuredPresentationRecord(item);
	const key = getStringField(item, "key") ?? getStringField(item, "name");
	const value = item.value;
	if (shouldRevealStorageValue(key, value)) return redactStructuredPresentationRecord(item);
	return {
		...redactStructuredPresentationRecord({ ...item, value: undefined }),
		value: "[REDACTED]",
		valueRedacted: true,
...
function redactStorageData(value: AgentBrowserResultValue): AgentBrowserResultValue {
	if (Array.isArray(value)) return value.map((item) => redactStorageData(item));
	if (!isAgentBrowserResultRecord(value)) return redactStructuredPresentationValue(value);
	const entries: AgentBrowserResultRecord = Object.fromEntries(Object.entries(value).map(([key, entryValue]) => {
		if ((key === "entries" || key === "items") && Array.isArray(entryValue)) return [key, entryValue.map((item) => isAgentBrowserResultRecord(item) ? redactStorageEntryValue(item) : redactStructuredPresentationValue(item))];
		if (key === "value") {
...
function formatStorageText<Value>(data: Record<string, Value>): string | undefined {
...
function formatDialogText<Value>(data: Record<string, Value>): string | undefined {
	const lines: string[] = [];
	if (hasRuntimeType(data.open, "boolean")) lines.push(data.open ? "Dialog open." : "No dialog open.");
...
function formatFrameText<Value>(data: Record<string, Value>): string | undefined {
...
function formatStateText<Value>(data: Record<string, Value>, subcommand?: string): string | undefined {
	if (subcommand === "show") {
		...
		if (hasRuntimeType(data.encrypted, "boolean")) lines.push(`Encrypted: ${data.encrypted ? "yes" : "no"}`);
		if (hasRuntimeType(data.size, "number")) lines.push(`Size: ${data.size} bytes`);
		return lines.join("\n");
	}
	const states = getArrayField(data, "states") ?? getArrayField(data, "files");
	if (states) {
...
function redactStructuredPresentationRecord(value: AgentBrowserResultRecord): AgentBrowserResultRecord {
...
function redactStructuredPresentationValue(value: AgentBrowserResultValue): AgentBrowserResultValue {
	if (hasRuntimeType(value, "string")) return redactModelFacingTextIfSensitive(value);
	if (Array.isArray(value)) return value.map((item) => redactStructuredPresentationValue(item));
	if (!isAgentBrowserResultRecord(value)) return value;
	return redactStructuredPresentationRecord(value);
}

function redactStatefulValues(value: AgentBrowserResultValue, sensitiveKeys: Set<string>): AgentBrowserResultValue {
...
export function redactPresentationData<Value>(commandInfo: CommandInfo, data: Value): AgentBrowserResultValue {
	// SAFETY: presentation data is decoded from the agent-browser CLI JSON envelope before command-specific redaction.
	const parsedData = data as AgentBrowserResultValue;
	if (commandInfo.command === "cookies") return redactStatefulValues(parsedData, new Set(["value"]));
	if (commandInfo.command === "storage") return redactStorageData(parsedData);
	if (commandInfo.command === "session" && commandInfo.subcommand === "list") {
		// SAFETY: filtering session rows preserves the parsed JSON value domain.
		const filteredData = filterManagedSessionListRows(parsedData) as AgentBrowserResultValue;
		return redactStructuredPresentationValue(filteredData);
	}
	if (commandInfo.command === "state" && commandInfo.subcommand === "list") {
		// SAFETY: filtering state rows preserves the parsed JSON value domain.
		const filteredData = filterManagedStateListRows(parsedData) as AgentBrowserResultValue;
		return redactStructuredPresentationValue(filteredData);
	}
	if (commandInfo.command === "state" && commandInfo.subcommand === "show") return redactStatefulValues(parsedData, new Set(["value"]));
	return redactStructuredPresentationValue(parsedData);
}

export function formatDiagnosticText<Value>(commandInfo: CommandInfo, data: Record<string, Value>): string | undefined {
