...
import type {
  ...
  AgentBrowserResultRecord,
  AgentBrowserResultValue,
...
export function getTabSummary<Value>(data: Record<string, Value>): string | undefined {
  ...
  const lines = tabs.map((tab, index) => {
    ...
    const label =
      hasRuntimeType(tab.label, "string") && tab.label.trim().length > 0
        ? tab.label.trim()
        : undefined;
    const tabSelector =
    ...
    const targetId =
      hasRuntimeType(tab.targetId, "string") && tab.targetId.trim().length > 0
        ? tab.targetId.trim()
        : undefined;
    const targetText =
...
export function enrichStreamStatusData<Value>(commandInfo: CommandInfo, data: Value) {
  if (
    ...
    !isRecord(data) ||
    !hasRuntimeType(data.port, "number")
  ) {
    return data;
  }
  return {
...
export function formatDiagnosticSummary<Value>(
  ...
  if (commandInfo.command === "state") {
    ...
    if (states) {
      const visibleStates =
        commandInfo.subcommand === "list" ? filterCallerOwnedStateListItems(states) : states;
  ...
  if (sessions) {
    ...
    return visibleSessions
      .map((item, index) => {
        ...
        const tabCount = hasRuntimeType(item.tabCount, "number")
...
function getPreviewCandidate<Value>(
...
function appendNetworkPreview<Value>(
  lines: string[],
  label: string,
  value: Value,
  maxChars: number,
...
function formatNetworkRequestsText<Value>(
  data: Record<string, Value>,
  commandInfo: CommandInfo,
): string | undefined {
  const requests = getArrayField(data, "requests");
...
function getNetworkRequestActionCandidate<Value>(
  item: Record<string, Value>,
): NetworkRequestActionCandidate<Value> | undefined {
  ...
  const kind: NetworkRequestActionCandidate<Value>["kind"] =
...
function chooseNetworkRequestActionCandidate<Value>(
  candidates: NetworkRequestActionCandidate<Value>[],
): NetworkRequestActionCandidate<Value> | undefined {
  return (
...
function formatNetworkRequestActionDescriptor<Value>(
  candidate: NetworkRequestActionCandidate<Value>,
): string {
  const method = getStringField(candidate.item, "method") ?? "GET";
  const status = hasRuntimeType(candidate.item.status, "number")
    ? String(candidate.item.status)
    : "pending";
  const target = candidate.filter ? ` ${candidate.filter}` : "";
  return `${status} ${method}${target} [${candidate.requestId}]`;
}

function getNetworkRequestDetailActionId<Value>(
  candidate: NetworkRequestActionCandidate<Value>,
): string {
...
export function buildNetworkRequestsNextActions<Value>(
  data: Value,
  sessionName: string | undefined,
  ...
  if (selected.kind === "actionable") {
    const networkSourceLookup: NonNullable<
      AgentBrowserNextAction["params"]
    >["networkSourceLookup"] = { requestId: selected.requestId };
...
export function buildStreamNextActions<Value>(
  commandInfo: CommandInfo,
  data: Value,
  sessionName: string | undefined,
): AgentBrowserNextAction[] | undefined {
...
function formatConsoleText<Value>(
  data: Record<string, Value>,
  commandInfo: CommandInfo,
): string | undefined {
  const messages = getArrayField(data, "messages");
...
function formatA11yText<Value>(data: Record<string, Value>): string | undefined {
  ...
  const violationCount = hasRuntimeType(counts?.violations, "number")
    ? counts.violations
    : violations.length;
  const incompleteCount = hasRuntimeType(counts?.incomplete, "number")
    ? counts.incomplete
    : incomplete.length;
  const passCount = hasRuntimeType(counts?.passes, "number") ? counts.passes : undefined;
  const inapplicableCount = hasRuntimeType(counts?.inapplicable, "number")
    ? counts.inapplicable
    : undefined;
  const countParts = [
  ...
  const preview = violations.slice(0, previewLimit).map((item, index) => {
    ...
    const nodeCount = hasRuntimeType(item.nodeCount, "number")
      ? item.nodeCount
      : getArrayField(item, "nodes")?.length;
    const nodePart = hasRuntimeType(nodeCount, "number")
...
function formatErrorsText<Value>(
  data: Record<string, Value>,
  commandInfo: CommandInfo,
): string | undefined {
  const errors = getArrayField(data, "errors");
...
function formatDoctorText<Value>(data: Record<string, Value>): string | undefined {
  ...
  if (summary) {
    const parts = ["pass", "warn", "fail"].flatMap((key) =>
      hasRuntimeType(summary[key], "number") ? [`${key}:${summary[key]}`] : [],
    );
...
function isAgentBrowserResultRecord(
  value: AgentBrowserResultValue,
): value is AgentBrowserResultRecord {
...
function redactStorageData(value: AgentBrowserResultValue): AgentBrowserResultValue {
  ...
  const entries: AgentBrowserResultRecord = Object.fromEntries(
    Object.entries(value).map(([key, entryValue]) => {
      if ((key === "entries" || key === "items") && Array.isArray(entryValue))
        return [
          key,
          entryValue.map((item) =>
            isAgentBrowserResultRecord(item)
...
function formatDialogText<Value>(data: Record<string, Value>): string | undefined {
  const lines: string[] = [];
  if (hasRuntimeType(data.open, "boolean"))
...
function formatStateText<Value>(
  ...
  if (subcommand === "show") {
    ...
    if (hasRuntimeType(data.encrypted, "boolean"))
...
function redactStructuredPresentationRecord(
  value: AgentBrowserResultRecord,
): AgentBrowserResultRecord {
...
function redactStructuredPresentationValue(
  value: AgentBrowserResultValue,
): AgentBrowserResultValue {
...
export function redactPresentationData<Value>(
  commandInfo: CommandInfo,
  data: Value,
): AgentBrowserResultValue {
  ...
  if (commandInfo.command === "cookies")
    return redactStatefulValues(parsedData, new Set(["value"]));
...
export function formatDiagnosticText<Value>(
  commandInfo: CommandInfo,
  data: Record<string, Value>,
): string | undefined {
