import { containsManagedSessionRestoreKey, isWrapperManagedSessionName } from "../../managed-session-capabilities.ts";
...
function isWrapperManagedSessionListItem<Value>(item: Value): boolean {
	...
	return ["name", "session", "id"].some((key) => isWrapperManagedSessionName(hasRuntimeType(item[key], "string") ? item[key] : undefined));
}
