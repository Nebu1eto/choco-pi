...
export function isAgentBrowserBatchResultArray<Value>(
  value: Value,
): value is Value & AgentBrowserBatchResult[] {
...
function redactParsedExactValues(
  value: AgentBrowserResultValue,
  ...
  if (Array.isArray(value))
    return value.map((item) => redactParsedExactValues(item, sensitiveValues));
  if (!isAgentBrowserResultRecord(value)) return value;
  const redactedEntries = Object.fromEntries(
    Object.entries(value).map(([key, entryValue]) => [
      key,
      redactParsedExactValues(entryValue, sensitiveValues),
    ]),
  );
...
function redactExactValues<Value>(
  value: Value,
  sensitiveValues: string[],
): Value | AgentBrowserResultValue {
...
function getTypedTextLength(command: string[] | undefined): number | undefined {
  return command?.[0] === "keyboard" &&
    command[1] === "type" &&
    hasRuntimeType(command[2], "string")
    ? Array.from(command[2]).length
    : undefined;
}

function getWaitDelayMs(command: string[] | undefined): string | undefined {
  return command?.[0] === "wait" && hasRuntimeType(command[1], "string") && /^\d+$/.test(command[1])
    ? command[1]
    : undefined;
}
...
function formatTypedSequenceSummary(
  ...
  if (
    ...
    hasRuntimeType(pressDetails.command[1], "string")
  ) {
...
function extractBatchStepLifecycle<Value>(
  result: Value,
): BatchStepPresentationDetails["lifecycle"] {
  if (
    !isRecord(result) ||
  ...
  return hasRuntimeType(browserLaunched, "boolean")
