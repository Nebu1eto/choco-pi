import { isCloseCommand } from "../../command-taxonomy.ts";
import { hasRuntimeType, isRecord } from "../../parsing.ts";
import { getAgentBrowserSessionIdentityKey } from "../../argv-grammar.ts";
import { extractUpstreamCommandTokens, parseCommandInfo, redactInvocationArgs, redactSensitiveText, redactSensitiveValue, type CommandInfo } from "../../runtime.ts";
import type { PersistentSessionArtifactStore } from "../../temp.ts";
import { buildAgentBrowserNextActions } from "../action-recommendations.ts";
import { formatSessionArtifactRetentionSummary, isPendingRecordingArtifact } from "../artifact-manifest.ts";
import { classifyAgentBrowserFailureCategory } from "../categories.ts";
...
import { applyNetworkRouteRecords, buildNetworkRouteDiagnostics } from "../network-routes.ts";
import { appendUniqueAgentBrowserNextActions, applyNamespaceToNextActions, withOptionalSessionArgs } from "../next-actions.ts";
import { stringifyModelFacing } from "./common.ts";
import { buildArtifactVerificationSummary, classifyPresentationSuccessCategory, manifestHasNewNoticeWorthyEntries, type ArtifactRequestContext } from "./artifacts.ts";
import { formatBatchStepCommand, getPresentationImages, getPresentationPaths, getPresentationText, isStringArray } from "./content.ts";
import { buildPageChangeSummary } from "./navigation.ts";
import { appendSelectorRecoveryHint, getClipboardWritePayloadCandidates, redactClipboardPermissionErrorValue } from "./errors.ts";
...
export function isAgentBrowserBatchResultArray<Value>(value: Value): value is Value & AgentBrowserBatchResult[] {
...
function formatBatchStepError<Value>(error: Value): string {
...
function redactParsedExactValues(value: AgentBrowserResultValue, sensitiveValues: string[]): AgentBrowserResultValue {
	if (sensitiveValues.length === 0) {
		// SAFETY: the runtime redactor recursively preserves the JSON-compatible value domain supplied here.
		return redactSensitiveValue(value) as AgentBrowserResultValue;
	}
	...
	if (Array.isArray(value)) return value.map((item) => redactParsedExactValues(item, sensitiveValues));
	if (!isAgentBrowserResultRecord(value)) return value;
	const redactedEntries = Object.fromEntries(Object.entries(value).map(([key, entryValue]) => [key, redactParsedExactValues(entryValue, sensitiveValues)]));
	// SAFETY: the runtime redactor recursively preserves the JSON-compatible record assembled above.
	return redactSensitiveValue(redactedEntries) as AgentBrowserResultValue;
}

function redactExactValues<Value>(value: Value, sensitiveValues: string[]): Value | AgentBrowserResultValue {
	// SAFETY: batch result and error values are decoded from the agent-browser CLI JSON envelope.
	const parsedValue = value as AgentBrowserResultValue;
	return redactParsedExactValues(parsedValue, sensitiveValues);
}

function getTypedTextLength(command: string[] | undefined): number | undefined {
	return command?.[0] === "keyboard" && command[1] === "type" && hasRuntimeType(command[2], "string")
		? Array.from(command[2]).length
		: undefined;
}

function getWaitDelayMs(command: string[] | undefined): string | undefined {
	return command?.[0] === "wait" && hasRuntimeType(command[1], "string") && /^\d+$/.test(command[1]) ? command[1] : undefined;
}
...
function formatTypedSequenceSummary(steps: Array<{ details: BatchStepPresentationDetails; presentation: ToolPresentation }>, startIndex: number): { nextIndex: number; text: string } | undefined {
	...
	if (firstDetails.command?.[0] === "focus" && hasRuntimeType(firstDetails.command[1], "string")) {
	...
	if (pressDetails?.success && pressDetails.command?.[0] === "press" && hasRuntimeType(pressDetails.command[1], "string")) {
...
function extractBatchStepLifecycle<Value>(result: Value): BatchStepPresentationDetails["lifecycle"] {
	...
	return hasRuntimeType(browserLaunched, "boolean") ? { effectiveLaunch: { browserLaunched } } : undefined;
}
