...
import { hasRuntimeType, isRecord, parsePositiveInteger } from "../../parsing.ts";
...
import { classifyAgentBrowserSuccessCategory } from "../categories.ts";
import type {
...
const IMAGE_EXTENSION_TO_MIME_TYPE: MediaTypeByExtension = {
	".gif": "image/gif",
	".jpeg": "image/jpeg",
	".jpg": "image/jpeg",
	".png": "image/png",
...
function extractPathStrings<Value>(data: Value): string[] {
	...
	for (const key of PATH_FIELD_CANDIDATES) {
		...
		if (hasRuntimeType(value, "string") && value.trim().length > 0 && !isNonFileArtifactPathCandidate(value)) {
			paths.push(value);
		}
		if (Array.isArray(value)) {
			for (const item of value) {
				if (hasRuntimeType(item, "string") && item.trim().length > 0 && !isNonFileArtifactPathCandidate(item)) {
					paths.push(item);
				}
			}
		}
...
export function formatArtifactMetadataLines(artifacts: FileArtifactMetadata[]): string[] {
	return artifacts.map((artifact, index) => {
		...
		return [
			...
			hasRuntimeType(artifact.sizeBytes, "number") ? `Size: ${formatByteCount(artifact.sizeBytes)}` : undefined,
			hasRuntimeType(artifact.sizeBytes, "number") ? `Size bytes: ${artifact.sizeBytes}` : undefined,
...
function extractSavedFilePath<Value>(data: Record<string, Value>): string | undefined {
	return hasRuntimeType(data.path, "string") && data.path.trim().length > 0 ? data.path : undefined;
}

export function getSavedFileDetails<Value>(commandInfo: CommandInfo, data: Record<string, Value>): SavedFilePresentationDetails | undefined {
	...
	if (Object.keys(metadata).length > 0) {
		// SAFETY: this is the upstream saved-file JSON payload with only the validated path field removed; known metadata fields follow the CLI contract and extension fields remain intact at runtime.
		details.metadata = metadata as SavedFileMetadata;
	}
...
export function extractImagePath<Value>(commandInfo: CommandInfo, cwd: string, data: Value): string | undefined {
	...
	if (!isRecord(data) || !hasRuntimeType(data.path, "string")) {
		return undefined;
	}
