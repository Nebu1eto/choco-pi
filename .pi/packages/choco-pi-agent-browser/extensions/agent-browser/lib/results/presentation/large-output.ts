...
import { buildEvictedSessionArtifactEntries } from "../artifact-manifest.ts";
import type { ArtifactStorageScope, SessionArtifactManifest, SessionArtifactManifestEntry, ToolPresentation } from "../contracts.ts";
import { countLines, truncateText } from "../text.ts";
import { applyArtifactManifest } from "./artifacts.ts";
import { getPresentationText } from "./content.ts";
import { redactModelFacingText, stringifyModelFacing } from "./common.ts";
...
interface LargeOutputPreview {
	omittedLineCount: number;
	previewText: string;
}

function buildLargeOutputPreview(text: string): LargeOutputPreview {
...
function buildLargeOutputFailureContext(presentation: ToolPresentation): string[] {
	...
	const stepPaths = [failedStep.fullOutputPath, ...(failedStep.fullOutputPaths ?? [])].filter((path, index, paths): path is string => path !== undefined && path.length > 0 && paths.indexOf(path) === index);
...
async function writeLargeOutputSpillFile(options: {
	...
	const payload =
		hasRuntimeType(options.data, "string")
			? redactModelFacingText(options.data)
			: hasRuntimeType(options.data, "number") || hasRuntimeType(options.data, "boolean")
				? String(options.data)
				: options.data === undefined
					? redactModelFacingText(options.text)
					: stringifyModelFacing(options.data);
	const isStructuredPayload = !hasRuntimeType(options.data, "string") && !hasRuntimeType(options.data, "number") && !hasRuntimeType(options.data, "boolean");
	const fileOptions = {
		content: payload,
