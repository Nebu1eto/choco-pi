...
function buildLargeOutputFailureContext(presentation: ToolPresentation): string[] {
  ...
  const stepPaths = [failedStep.fullOutputPath, ...(failedStep.fullOutputPaths ?? [])].filter(
    (path, index, paths): path is string =>
      path !== undefined && path.length > 0 && paths.indexOf(path) === index,
  );
...
async function writeLargeOutputSpillFile(options: {
  ...
  const payload = hasRuntimeType(options.data, "string")
  ...
  const isStructuredPayload =
    ...
    !hasRuntimeType(options.data, "number") &&
    !hasRuntimeType(options.data, "boolean");
  const fileOptions = {
    content: payload,
