import { isNavigationObservableCommandName, isPageChangeSummaryCommand } from "../../command-taxonomy.ts";
...
function isCompletePageChangeSummary(value: PageChangeSummaryDraft): value is AgentBrowserPageChangeSummary {
...
function getScalarExtractionResult<Value>(commandInfo: CommandInfo, data: Record<string, Value>): string | undefined {
...
export function formatExtractionSummary<Value>(commandInfo: CommandInfo, data: Record<string, Value>): string | undefined {
...
export function formatExtractionText<Value>(commandInfo: CommandInfo, data: Record<string, Value>): string | undefined {
...
function isNavigationSummary<Value>(value: Value): value is Value & NavigationSummary {
	return isRecord(value) && (hasRuntimeType(value.title, "string") || hasRuntimeType(value.url, "string"));
}

export function getNavigationSummary<Value>(data: Record<string, Value>): NavigationSummary | undefined {
	const candidate = data[NAVIGATION_SUMMARY_FIELD];
	return isNavigationSummary(candidate) ? candidate : undefined;
}

function getTopLevelNavigationSummary<Value>(data: Record<string, Value>): NavigationSummary | undefined {
...
function getNormalizedNavigationSummary(summary: NavigationSummary | undefined): NavigationSummary | undefined {
	const title = summary?.title && summary.title.trim().length > 0 ? summary.title.trim() : undefined;
...
export function formatNavigationActionResult<Value>(data: Record<string, Value>): string | undefined {
	const actionData = omitUpstreamLifecycle(stripNavigationSummary(data));
	const lines: string[] = [];
	if (hasRuntimeType(actionData.clicked, "string") || hasRuntimeType(actionData.clicked, "boolean")) {
