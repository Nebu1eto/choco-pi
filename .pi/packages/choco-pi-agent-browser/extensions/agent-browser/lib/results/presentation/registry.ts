...
function formatVitalsText<Value>(data: Record<string, Value>): string {
  const url =
    hasRuntimeType(data.url, "string") && data.url.trim().length > 0
...
const COMMAND_PRESENTERS: CommandPresenters = {
  ...Object.fromEntries(
    Object.keys(SIMPLE_ACTION_RESULTS).map((command) => [
      command,
      buildSimpleActionPresenter(command),
    ]),
  ),
  close: {
  ...
  read: {
    ...
    text: (_commandInfo, data) =>
      isRecord(data) && hasRuntimeType(data.content, "string")
        ? redactModelFacingText(data.content)
        : undefined,
  },
  screenshot: {
    summary: (_commandInfo, data) =>
      isRecord(data) && hasRuntimeType(data.path, "string")
