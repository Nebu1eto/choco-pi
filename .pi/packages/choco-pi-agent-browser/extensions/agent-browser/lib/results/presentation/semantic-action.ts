...
import { getPageSummary, redactModelFacingText } from "./common.ts";
...
export function shouldCaptureSemanticActionNavigationSummary<Value>(
	compiled: CompiledAgentBrowserSemanticAction | undefined,
	data: Value,
): boolean {
	...
	return !isRecord(data) || (!hasRuntimeType(data.title, "string") && !hasRuntimeType(data.url, "string"));
}

export function formatSemanticActionPresentationText<Value>(
...
export function formatSemanticActionPresentationSummary<Value>(
	compiled: CompiledAgentBrowserSemanticAction,
	data: Record<string, Value>,
): string | undefined {
	const presentationCommand = resolveSemanticPresentationCommand(compiled);
