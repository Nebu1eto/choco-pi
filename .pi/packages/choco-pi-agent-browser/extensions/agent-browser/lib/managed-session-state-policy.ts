...
import { getUpstreamEffectiveBatchSteps, type BatchCommandStep, parseBatchCommandArgument, parseUserBatchStdin } from "./orchestration/batch-stdin.ts";
...
import { extractManagedSessionRestoreKeys, isWrapperManagedSessionName } from "./managed-session-capabilities.ts";
...
function getBrowserFileOperands(args: string[], env: NodeJS.ProcessEnv): string[] {
	...
	return values.filter((value): value is string => hasRuntimeType(value, "string") && value.length > 0);
}

function rawBrowserArgsEnableFileAccess(args: string[], env: NodeJS.ProcessEnv): boolean {
	return getRawBrowserArgsValues(args, env).some((value) => hasRuntimeType(value, "string") && /(?:^|[,\p{White_Space}])--(?:allow-file-access(?:-from-files)?|disable-web-security)(?=$|[=,\p{White_Space}])/iu.test(value));
}
...
export function getManagedSessionResultingPageState(options: {
	...
	const descriptor = parseArgvDescriptor(options.args);
	let state: ResultingPageState = { currentPageUrl: options.currentPageUrl, pageUrlUnknown: options.pageUrlUnknown ?? false };
