...
import { getScreenshotPathTokenIndex } from "./orchestration/browser-run/artifact-paths.ts";
import {
  getUpstreamEffectiveBatchSteps,
  type BatchCommandStep,
  parseBatchCommandArgument,
  parseUserBatchStdin,
} from "./orchestration/batch-stdin.ts";
import { isUnverifiedPageTransitionCommand } from "./command-taxonomy.ts";
import {
...
import {
  extractManagedSessionRestoreKeys,
  isWrapperManagedSessionName,
} from "./managed-session-capabilities.ts";
import { agentBrowserExplicitConfigIsPresent } from "./managed-session-restore.ts";
import { hasManagedSessionRestoreProjectIdentity } from "./managed-session-storage.ts";
import { getAgentBrowserProcessEnvironment } from "./process-environment.ts";

const BLOCKED_GLOBAL_STATE_MESSAGE =
  "This operation could read or modify wrapper-owned browser state outside the current checkout. Use a caller-owned state name or path instead.";
const BLOCKED_MANAGED_BROWSER_FILE_MESSAGE =
...
const BLOCKED_MANAGED_SESSION_MESSAGE =
  "This session name is reserved for a browser managed by this extension instance. Use the current managed session or a caller-owned session name instead.";
const FILE_ACCESS_FLAG_MESSAGE =
...
const UNVERIFIED_PAGE_MESSAGE =
  "The active page became unverified after a tab, attachment, history, script, or state-load transition. Run get url or navigate explicitly to a safe URL before page-content inspection.";
...
const UNSAFE_BATCH_ARGUMENT_MESSAGE =
  "Batch command arguments could not be safely inspected. Use batch stdin JSON command arrays instead.";
const NESTED_BATCH_ARGUMENT_MESSAGE =
  "Nested batch commands are blocked by the wrapper's page-state safety policy. Flatten the batch steps instead.";
const NON_BAIL_BATCH_NAVIGATION_MESSAGE =
  "Batches that navigate before page-content access must use exact batch --bail so a failed navigation cannot expose the prior page.";
...
const FILE_PATH_LIST_ENV_VARIABLES = [
...
function hasAgentBrowserPathComponent(value: string): boolean {
  return decodeUrlComponent(value)
    ...
    .split("/")
    .some((component) => {
      const windowsCanonicalComponent = component
        .replace(/^[a-z]:/i, "")
        .split(":", 1)[0]
        ?.replace(/[. ]+$/g, "");
...
export function isProtectedAgentBrowserFileTarget(value: string | undefined, cwd: string): boolean {
  ...
  const pathValue =
    filePath ?? (isAbsolute(value) || windowsAbsolutePath ? value : resolve(cwd, value));
...
export function getAgentBrowserStoragePathValidationError(
  value: string | undefined,
  cwd: string,
): string | undefined {
  return isProtectedAgentBrowserFileTarget(value, cwd)
    ? BLOCKED_MANAGED_BROWSER_FILE_MESSAGE
    : undefined;
}

export function getObservedBrowserPageValidationError(
  args: string[],
  ...
  return explicitTarget !== undefined &&
    isFileUrl(explicitTarget) &&
    !isProtectedAgentBrowserFileTarget(explicitTarget, cwd)
    ? undefined
    : BLOCKED_MANAGED_BROWSER_FILE_MESSAGE;
}
...
function getBrowserContentUrlOperands(args: string[]): string[] {
  ...
  if (
    command === "record" &&
    ["start", "restart"].includes(descriptor.commandInfo.subcommand ?? "")
  )
...
function getRawBrowserArgsValues(
  args: string[],
...
function getRawBrowserFileOperands(args: string[], env: NodeJS.ProcessEnv): string[] {
  return getRawBrowserArgsValues(args, env).flatMap(
    (value) =>
      value?.split(/[,\r\n]/).flatMap((rawToken) => {
...
function getBrowserFileEnvOperands(env: NodeJS.ProcessEnv): Array<string | undefined> {
  return [
    ...FILE_PATH_ENV_VARIABLES.map((name) =>
      env[name] === undefined ? undefined : trimUnicodeWhitespace(env[name]),
    ),
    ...FILE_PATH_LIST_ENV_VARIABLES.flatMap(
...
function getBrowserFileOperands(args: string[], env: NodeJS.ProcessEnv): string[] {
  ...
  if (command === "wait")
    values.push(
      ...getFlagValues(descriptor.upstreamCommandTokens, "--download", true),
      ...getFlagValues(descriptor.upstreamCommandTokens, "-d", true),
    );
  ...
  if (command === "diff")
    values.push(
      ...getFlagValues(descriptor.upstreamCommandTokens, "--baseline", true),
      ...getFlagValues(descriptor.upstreamCommandTokens, "--output", true),
      ...getFlagValues(descriptor.upstreamCommandTokens, "-o", true),
    );
  if (command === "record" && ["start", "restart"].includes(subcommand ?? ""))
    values.push(...positionals.slice(1));
  return values.filter(
    (value): value is string => hasRuntimeType(value, "string") && value.length > 0,
  );
}

function rawBrowserArgsEnableFileAccess(args: string[], env: NodeJS.ProcessEnv): boolean {
  return getRawBrowserArgsValues(args, env).some(
    (value) =>
      hasRuntimeType(value, "string") &&
      /(?:^|[,\p{White_Space}])--(?:allow-file-access(?:-from-files)?|disable-web-security)(?=$|[=,\p{White_Space}])/iu.test(
        value,
      ),
  );
}
...
function getManagedBrowserFileAccessValidationError(options: {
  ...
  const safelyInspectsPageTarget =
    (command === "tab" && subcommand === "list") || (command === "get" && subcommand === "url");
  const safelySelectsUnverifiedTab =
    command === "tab" && subcommand !== undefined && !["close", "list", "new"].includes(subcommand);
  const safelyTransitionsUnverifiedPage =
    options.allowUnverifiedPageTransitions === true &&
    command !== "eval" &&
    isUnverifiedPageTransitionCommand(command, subcommand);
  const safelyLeavesPage =
    explicitTarget !== undefined &&
    !isFileUrl(explicitTarget) &&
    !isProtectedAgentBrowserFileTarget(explicitTarget, options.cwd);
  if (
    fileAccessFlagIsEnabled(options.args, options.env) ||
    rawBrowserArgsEnableFileAccess(options.args, options.env)
  )
    return FILE_ACCESS_FLAG_MESSAGE;
  if (getBrowserContentUrlOperands(options.args).some(isFileUrl))
    return BLOCKED_MANAGED_BROWSER_FILE_MESSAGE;
  if (
    getBrowserFileOperands(options.args, options.env).some((value) =>
      isProtectedAgentBrowserFileTarget(value, options.cwd),
    )
  )
    return BLOCKED_MANAGED_BROWSER_FILE_MESSAGE;
  if (
    ...
    /(?:\.agent-browser|%2eagent-browser)/i.test(options.stdin)
  )
    return BLOCKED_MANAGED_BROWSER_FILE_MESSAGE;
  if (
    options.pageUrlUnknown &&
    !closesPage &&
    !safelyInspectsPageTarget &&
    !safelySelectsUnverifiedTab &&
    !safelyTransitionsUnverifiedPage &&
    !safelyLeavesPage &&
    !(options.trustedBatchTabSelection && command === "tab")
  )
    return UNVERIFIED_PAGE_MESSAGE;
  if (
    isProtectedAgentBrowserFileTarget(options.currentPageUrl, options.cwd) &&
    !closesPage &&
    !safelyInspectsPageTarget &&
    !safelyLeavesPage
  )
    return BLOCKED_MANAGED_BROWSER_FILE_MESSAGE;
  if (
    isFileUrl(options.currentPageUrl) &&
    !closesPage &&
    !safelyInspectsPageTarget &&
    !safelyLeavesPage
  )
...
interface BatchCommandStepsResult {
  error?: string;
  steps: BatchCommandStep[];
}

function getBatchCommandSteps(args: string[], stdin?: string): BatchCommandStepsResult {
  const descriptor = parseArgvDescriptor(args);
...
export function invocationMayNavigateToLocalFile(args: string[], stdin?: string): boolean {
  ...
  const descriptor = parseArgvDescriptor(args);
  return getUpstreamEffectiveBatchSteps(descriptor.upstreamCommandTokens, stdin).some((step) =>
    isFileUrl(getResultingExplicitNavigationTarget(step)),
  );
}
...
export function managedSessionCommandRequiresLivePageVerification(
  args: string[],
  stdin?: string,
): boolean {
  const descriptor = parseArgvDescriptor(args);
  ...
  return (
    ...
    batch.steps.some((step) => managedSessionCommandRequiresLivePageVerification(step))
  );
}

interface ResultingPageState {
  currentPageUrl?: string;
  pageUrlUnknown: boolean;
}

export interface ManagedSessionResultingPageState extends ResultingPageState {
  pageTargetMayHaveChanged: boolean;
}
...
}): ManagedSessionResultingPageState {
  const descriptor = parseArgvDescriptor(options.args);
  let state: ResultingPageState = {
...
export function getCallerOwnedSessionLivePageVerificationRequirement(options: {
  ...
  if (
    descriptor.commandInfo.command !== "eval" &&
    isUnverifiedPageTransitionCommand(
      descriptor.commandInfo.command,
      descriptor.commandInfo.subcommand,
    )
...
export function getManagedSessionTargetAccessValidationError(
  args: string[],
...
function getReferencedValues(args: string[], cwd: string, env: NodeJS.ProcessEnv): string[] {
  ...
  ].filter((value): value is string => hasRuntimeType(value, "string") && value.length > 0);
  if (
    descriptor.commandInfo.command === "state" &&
    ["clear", "load", "rename", "save", "show"].includes(descriptor.commandInfo.subcommand ?? "")
  ) {
...
export function getManagedSessionStateAccessValidationError(options: {
  ...
  if (
    !options.trustedPinnedEmptyConfig &&
    needsManagedSession(descriptor) &&
    agentBrowserExplicitConfigIsPresent(effectiveEnv, options.args)
  )
  ...
  if (command === "batch") {
    ...
    if (batch.error) {
      if (
        batch.error === BLOCKED_MANAGED_BROWSER_FILE_MESSAGE ||
        batch.error === NESTED_BATCH_ARGUMENT_MESSAGE ||
        batch.error.startsWith("agent_browser batch stdin")
      )
        return batch.error;
    ...
    for (let index = 0; index < batch.steps.length; index += 1) {
      ...
      for (const state of possibleStates) {
        ...
        if (!error) continue;
        if (
          state.retainedAfterFailedNavigation &&
          [BLOCKED_MANAGED_BROWSER_FILE_MESSAGE, UNVERIFIED_PAGE_MESSAGE].includes(error)
        ) {
      ...
      if (directError)
        return directError === UNVERIFIED_PAGE_MESSAGE
      ...
      for (const state of possibleStates) {
        ...
        nextStates.push({
          ...successState,
          retainedAfterFailedNavigation: mayChangePageTarget
            ? false
            : state.retainedAfterFailedNavigation,
        });
  ...
  if (command === "state" && subcommand === "clear") {
    const target = descriptor.upstreamCommandTokens
      .slice(2)
      .find((token) => !token.startsWith("-"));
    if (
      !target ||
      descriptor.upstreamCommandTokens.includes("--all") ||
      descriptor.upstreamCommandTokens.includes("-a") ||
      isWrapperManagedSessionName(target)
    ) {
  ...
  return referencedKeys.every((key) => key === currentKey)
    ? undefined
    : BLOCKED_GLOBAL_STATE_MESSAGE;
}
