import type { ArgvDescriptor } from "./argv-descriptor.ts";
import { hasOnlyBooleanFlags, hasOnlyOptionFlags, isNonFlagToken, stripSessionlessGlobalFlags } from "./argv-grammar.ts";
...
function isSessionlessCommand(commandTokens: readonly string[]): boolean {
	const normalizedTokens = stripSessionlessGlobalFlags(commandTokens);
