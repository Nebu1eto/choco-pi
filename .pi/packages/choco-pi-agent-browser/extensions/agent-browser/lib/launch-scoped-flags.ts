import { findCommandStartIndex } from "./argv-descriptor.ts";
import { isBooleanFlagEnabled, optionalGlobalValueFlagConsumesNext } from "./argv-grammar.ts";
...
export function hasLaunchScopedFlagToken(args: string[], flag: string): boolean {
  ...
  return args.some((token, index) => {
    ...
    if (
      flag === "--restore" &&
      token === "--restore" &&
      optionalGlobalValueFlagConsumesNext(flag, args[index + 1])
    )
      return true;
    if (
      flag === "--state" &&
