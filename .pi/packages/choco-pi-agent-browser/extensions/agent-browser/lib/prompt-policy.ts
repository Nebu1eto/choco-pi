...
function getMessageText<Content>(content: Content): string {
	if (hasRuntimeType(content, "string")) return content;
	if (!Array.isArray(content)) return "";

	return content
		.map((item) => {
			if (!hasRuntimeType(item, "object") || item === null) return "";
			return item.type === "text" && hasRuntimeType(item.text, "string") ? item.text : "";
		})
...
export function getLatestUserPrompt(branch: unknown[]): string {
	for (let index = branch.length - 1; index >= 0; index -= 1) {
		...
		if (!hasRuntimeType(entry, "object") || entry === null || !("type" in entry) || entry.type !== "message") {
			continue;
		}
		const message = "message" in entry ? entry.message : undefined;
		if (!hasRuntimeType(message, "object") || message === null || !("role" in message) || message.role !== "user") {
			continue;
		}
