...
function isTempRootOwnershipRecord<Value>(value: Value): value is Value & TempRootOwnershipRecord {
  ...
  if (
    value.kind !== TEMP_ROOT_MARKER_KIND ||
    (value.version !== TEMP_ROOT_LEGACY_MARKER_VERSION &&
      value.version !== TEMP_ROOT_MARKER_VERSION)
  )
    return false;
  ...
  if (value.ownerPid !== undefined) {
    if (
      !hasRuntimeType(value.ownerPid, "number") ||
  ...
  if (value.ownerProcessStartIdentity !== undefined) {
    if (
      !hasRuntimeType(value.ownerProcessStartIdentity, "string") ||
  ...
  if (value.ownerUid !== undefined) {
    if (
      !hasRuntimeType(value.ownerUid, "number") ||
