...
import { hasRuntimeType, isRecord, parsePositiveInteger, type RuntimeValue } from "./parsing.ts";
...
function getCurrentProcessUid(): number | undefined {
	return hasRuntimeType(process.getuid, "function") ? process.getuid() : undefined;
}

function isPositiveFiniteNumber<Value>(value: Value): value is Value & number {
	return hasRuntimeType(value, "number") && Number.isFinite(value) && value > 0;
}

function isProtectedTempChildName<Value>(value: Value): value is Value & string {
	if (!hasRuntimeType(value, "string")) return false;
	if (value === "" || value === "." || value === ".." || value === TEMP_ROOT_MARKER_FILE_NAME) return false;
	if (value.includes("/") || value.includes("\\")) return false;
	return basename(value) === value;
}

function isTempRootOwnershipRecord<Value>(value: Value): value is Value & TempRootOwnershipRecord {
	if (!isRecord(value)) return false;
	if (value.kind !== TEMP_ROOT_MARKER_KIND || (value.version !== TEMP_ROOT_LEGACY_MARKER_VERSION && value.version !== TEMP_ROOT_MARKER_VERSION)) return false;
	...
	if (value.ownerPid !== undefined) {
		if (!hasRuntimeType(value.ownerPid, "number") || !Number.isSafeInteger(value.ownerPid) || value.ownerPid <= 0) return false;
	}
	if (value.ownerProcessStartIdentity !== undefined) {
		if (!hasRuntimeType(value.ownerProcessStartIdentity, "string") || value.ownerProcessStartIdentity.trim() === "") return false;
	}
	if (value.ownerUid !== undefined) {
		if (!hasRuntimeType(value.ownerUid, "number") || !Number.isSafeInteger(value.ownerUid) || value.ownerUid < 0) return false;
	}
...
function getTempArtifactByteLength(content: string | Uint8Array): number {
	return hasRuntimeType(content, "string") ? Buffer.byteLength(content) : content.byteLength;
}
...
async function readTempRootOwnershipMarker(tempRoot: string): Promise<TempRootOwnershipRecord | undefined> {
	try {
		...
		const parsed: RuntimeValue = JSON.parse(markerText);
