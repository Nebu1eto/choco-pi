...
function getRowBrowserLaunched(row: RuntimeRecord<RuntimeValue>): boolean | undefined {
  ...
  const effectiveLaunch = isRecord(lifecycle?.effectiveLaunch)
    ...
    : undefined;
  return hasRuntimeType(effectiveLaunch?.browserLaunched, "boolean")
    ? effectiveLaunch.browserLaunched
    : undefined;
}

export function batchHasSuccessfulCloseAll<Data>(
  data: Data,
  ...
  return data.some((row, index) => {
    ...
    const rowCommand =
      Array.isArray(row.command) && row.command.every(isString)
        ? row.command
