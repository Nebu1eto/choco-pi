...
import { createSecureTempDirectory } from "../temp.ts";

export type { ElectronCdpTarget, ElectronCdpVersion } from "./cdp.ts";
...
function normalizeTimeoutMs(timeoutMs: number | undefined): number {
  if (!hasRuntimeType(timeoutMs, "number") || !Number.isSafeInteger(timeoutMs) || timeoutMs <= 0)
...
function appIdentifiers(app: ElectronAppDiscovery): string[] {
  return [app.name, app.bundleId, app.desktopId, app.appPath, app.executablePath].filter(
    (value): value is string => hasRuntimeType(value, "string") && value.trim().length > 0,
  );
}
...
async function readDevToolsActivePort(
  ...
  try {
    ...
    const validPort = Number.isSafeInteger(port) && port > 0 && port <= 65_535;
    const result: ElectronDevToolsActivePortRead = {
      found: true,
      path,
      port: validPort ? port : undefined,
    };
    if (!validPort) result.error = "DevToolsActivePort did not contain a valid TCP port.";
    return result;
  } catch (error) {
