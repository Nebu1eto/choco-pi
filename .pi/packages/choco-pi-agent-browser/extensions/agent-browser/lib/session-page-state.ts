...
import { isCloseAllCommand, isCloseCommand, isReadOnlyDiagnosticSessionTargetCommand, isRecordPageTransitionCommand, isUnverifiedPageTransitionCommand } from "./command-taxonomy.ts";
...
function extractStringResultField<Data>(data: Data, fieldName: "result" | "title" | "url" | "value"): string | undefined {
...
export function extractSessionTabTargetFromCommandData<Data>(commandTokens: string[], data: Data): SessionTabTarget | undefined {
...
export function extractSessionTabTargetFromBatchResults<Data>(data: Data): SessionTabTarget | undefined {
...
function getRestoredSessionTabTarget(details: RuntimeRecord<RuntimeValue>, command: string | undefined, subcommand: string | undefined): SessionTabTarget | undefined {
	if (isReadOnlyDiagnosticSessionTargetCommand(command, subcommand)) {
		return undefined;
	}
	const storedTarget = isRecord(details.sessionTabTarget)
		? normalizeSessionTabTarget({
			title: hasRuntimeType(details.sessionTabTarget.title, "string") ? details.sessionTabTarget.title : undefined,
			url: hasRuntimeType(details.sessionTabTarget.url, "string") ? details.sessionTabTarget.url : undefined,
		  })
		: undefined;
	if (command !== "batch") {
		return storedTarget;
...
function buildSessionRefDetails(name: string, role: string, isContentEditable?: boolean, isEditable?: boolean): SessionRefDetails {
	if (isContentEditable !== undefined && isEditable !== undefined) return { isContentEditable, isEditable, name, role };
...
function extractRefSnapshotRefs<Data>(data: Data): SessionRefMap | undefined {
	...
	const snapshotLines = hasRuntimeType(data.snapshot, "string") ? parseSnapshotLines(data.snapshot) : [];
	...
	const refs = Object.fromEntries(entries.flatMap((entry) => {
		...
		return [[entry.id, buildSessionRefDetails(entry.name, entry.role, isContentEditable === true ? true : undefined, entry.isEditable)] as const];
	}));
...
function getBatchResultFailureText(item: RuntimeRecord<RuntimeValue>): string | undefined {
	...
	const parts = [item.error, result?.error, hasRuntimeType(item.result, "string") ? item.result : undefined]
...
export function extractLatestRefSnapshotStateFromBatchResults<Data>(data: Data): BatchRefSnapshotState | undefined {
...
function getRestoredRefSnapshotInvalidation(details: RuntimeRecord<RuntimeValue>, command: string | undefined): SessionRefSnapshotInvalidation | undefined {
	...
	if (invalidation?.reason === "page-transition") return buildPageTransitionRefSnapshotInvalidation(hasRuntimeType(invalidation.summary, "string") ? invalidation.summary : undefined);
...
function getRestoredRefSnapshot(details: RuntimeRecord<RuntimeValue>): SessionRefSnapshot | undefined {
	...
	const refIds = refSnapshot.refIds.filter((refId): refId is string => hasRuntimeType(refId, "string") && /^e\d+$/.test(refId));
	...
	const refEntries = refRecord
		? Object.fromEntries(refIds.flatMap((refId) => {
			...
			if (!isRecord(entry) || !hasRuntimeType(entry.name, "string") || !hasRuntimeType(entry.role, "string")) return [];
			const isContentEditable = hasRuntimeType(entry.isContentEditable, "boolean") ? entry.isContentEditable : undefined;
			const isEditable = hasRuntimeType(entry.isEditable, "boolean") ? entry.isEditable : undefined;
			return [[refId, buildSessionRefDetails(entry.name, entry.role, isContentEditable, isEditable)] as const];
		}))
		: undefined;
	const target = isRecord(refSnapshot.target)
		? normalizeSessionTabTarget({
			title: hasRuntimeType(refSnapshot.target.title, "string") ? refSnapshot.target.title : undefined,
...
export class SessionPageState {
	...
	static fromBranch(branch: unknown[]): SessionPageState {
		...
		for (const entry of branch) {
			...
			const command = hasRuntimeType(details.command, "string") ? details.command : commandTokens[0];
			const subcommand = hasRuntimeType(details.subcommand, "string") ? details.subcommand : commandTokens[1];
