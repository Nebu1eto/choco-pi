...
async function readConfigLayer(path: string, scope: ConfigLayer["scope"], errors: string[], warnings: string[]): Promise<ConfigLayer | undefined> {
	...
	// SAFETY: config-policy validates and returns only ConfigLayer fields; @ts-nocheck hides that converted module's declaration precision.
	return parseAgentBrowserConfigLayer(raw, path, scope, errors, warnings) as ConfigLayer | undefined;
}
