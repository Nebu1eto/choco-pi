...
function compileJobTypeSteps(step: InputRecord): CompileJobTypeResult {
  ...
  if (delayMs === undefined) {
    compiledSteps.push({
      action: "type",
      args: isString(selector) ? ["type", selector, typedText] : ["keyboard", "type", typedText],
    });
  } else {
...
export function compileAgentBrowserJob<Input>(
  input: Input,
): CompilationResult<CompiledAgentBrowserJob> {
  ...
  for (const [index, rawStep] of rawSteps.entries()) {
    ...
    steps.push(
      { action: jobAction, args: compiledStep.args, generatedFrom: compiledStep.generatedFrom },
...
export function buildQaCompactPassText(options: {
  ...
  const pageParts = [options.page?.title, options.page?.url].filter(
    (part): part is string => isString(part) && part.length > 0,
  );
...
function subtractQaBaselineErrors<ErrorValue>(
  errors: ErrorValue[],
  baselineErrors: ErrorValue[],
): QaBaselineComparison<ErrorValue> {
...
export function analyzeQaPresetResults<Data>(
  data: Data,
  compiled?: CompiledAgentBrowserQaPreset,
): AgentBrowserQaPresetAnalysis | undefined {
  ...
  const baselineErrorResult = isInputRecord(baselineErrorItem?.result)
  ...
  for (const [index, item] of items.entries()) {
    ...
    if (commandName === "console" && Array.isArray(result?.messages)) {
      const errorCount = result.messages.filter(
        (message) =>
          isInputRecord(message) && /error/i.test(String(message.type ?? message.level ?? "")),
      ).length;
...
export function compileAgentBrowserQaPreset<Input>(
  input: Input,
): CompilationResult<CompiledAgentBrowserQaPreset> {
  if (!isInputRecord(input)) {
  ...
  const loadState = isOneOf(rawLoadState, AGENT_BROWSER_QA_LOAD_STATES)
    ? rawLoadState
    : "domcontentloaded";
