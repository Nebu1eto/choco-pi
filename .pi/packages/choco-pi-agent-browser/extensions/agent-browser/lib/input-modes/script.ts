...
function resolveScriptWorkerPath(): string {
  ...
  if (!existsSync(workerPath))
    throw new Error("TypeScript script worker is missing; reinstall choco-pi-agent-browser.");
  return workerPath;
}
...
type ScriptChildMessage =
  ...
  | {
      error?: { message?: InputRecord[string]; name?: InputRecord[string] };
      hasValue?: boolean;
      type: "complete";
      value?: unknown;
    };
...
export function compileAgentBrowserScript<Input>(
  input: Input,
): CompilationResult<CompiledAgentBrowserScript> {
...
export function validateAgentBrowserScriptBrowserParams<Input>(
  input: Input,
): AgentBrowserScriptBrowserParamsValidation {
  ...
  if (
    ...
    (!isNumber(input.timeoutMs) || !Number.isSafeInteger(input.timeoutMs) || input.timeoutMs <= 0)
  ) {
...
function describeScriptError(
  error: { message?: InputRecord[string]; name?: InputRecord[string] } | undefined,
): string {
...
function isScriptChildMessage<Value>(value: Value): value is Value & ScriptChildMessage {
  ...
  if (value.type === "call")
    return isNumber(value.id) && Number.isSafeInteger(value.id) && value.id > 0;
