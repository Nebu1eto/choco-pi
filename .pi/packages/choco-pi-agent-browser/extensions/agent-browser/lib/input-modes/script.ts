...
import { getFlagName } from "../argv-grammar.ts";
import { needsManagedSession } from "../command-policy.ts";
import { isCloseCommand } from "../command-taxonomy.ts";
import { LAUNCH_SCOPED_FLAGS, MANAGED_RESTORE_INCOMPATIBLE_FLAGS } from "../launch-scoped-flags.ts";
import {
...
function resolveScriptWorkerPath(): string {
	const workerPath = join(findPackageRoot(dirname(fileURLToPath(import.meta.url))), "extensions", "agent-browser", "script-worker.ts");
	if (!existsSync(workerPath)) throw new Error("TypeScript script worker is missing; reinstall choco-pi-agent-browser.");
	return workerPath;
}
...
export interface AgentBrowserScriptBrowserParamsValidation {
	error?: string;
...
type ScriptChildMessage =
	...
	| { error?: { message?: InputRecord[string]; name?: InputRecord[string] }; hasValue?: boolean; type: "complete"; value?: unknown };

type ScriptParentMessage =
	| { code: string; type: "start" }
	| { envelope: AgentBrowserScriptBrowserEnvelope; id: number; type: "response" };

export function compileAgentBrowserScript<Input>(input: Input): CompilationResult<CompiledAgentBrowserScript> {
	if (!isString(input)) return { error: "script must be a string." };
...
export function isAgentBrowserScriptSessionName<Value>(value: Value): value is Value & string {
	return isString(value) && SCRIPT_SESSION_NAME_PATTERN.test(value);
}
...
export function validateAgentBrowserScriptBrowserParams<Input>(input: Input): AgentBrowserScriptBrowserParamsValidation {
	if (!isInputRecord(input)) return { error: "script browser(params) requires an object." };
	...
	if (!isStringArray(input.args) || input.args.length === 0) {
		return { error: "script browser(params).args must be a non-empty string array." };
	}
	if (input.stdin !== undefined && !isString(input.stdin)) {
		return { error: "script browser(params).stdin must be a string when provided." };
	}
	if (input.timeoutMs !== undefined && (!isNumber(input.timeoutMs) || !Number.isSafeInteger(input.timeoutMs) || input.timeoutMs <= 0)) {
	...
	const params: AgentBrowserScriptBrowserParams = {
		args: input.args,
		stdin: isString(input.stdin) ? input.stdin : undefined,
...
function normalizeBrowserEnvelope(value: AgentBrowserScriptBrowserEnvelope): AgentBrowserScriptBrowserEnvelope {
	...
	return {
		...
		details: isInputRecord(value.details) ? value.details : undefined,
...
function describeScriptError(error: { message?: InputRecord[string]; name?: InputRecord[string] } | undefined): string {
	const name = isString(error?.name) && error.name.length > 0 ? error.name.slice(0, 80) : "Error";
	const message = isString(error?.message) && error.message.length > 0
		? error.message.replace(/[\r\n]+/g, " ").slice(0, 400)
		: "Script execution failed.";
	return `${name}: ${message}`;
}

function isScriptChildMessage<Value>(value: Value): value is Value & ScriptChildMessage {
	if (!isInputRecord(value) || !isString(value.type)) return false;
	if (value.type === "ready") return true;
	if (value.type === "call") return isNumber(value.id) && Number.isSafeInteger(value.id) && value.id > 0;
...
function serializeFinalOutput<Value>(value: Value): string | undefined {
...
export async function runAgentBrowserScript(options: RunAgentBrowserScriptOptions): Promise<AgentBrowserScriptRunResult> {
	...
	child.stdout.on("data", (chunk: Buffer) => {
		...
		for (;;) {
			...
			cumulativeBytes += bytes;
			try {
				const parsed: InputRecord[string] = JSON.parse(lineBuffer.toString("utf8"));
