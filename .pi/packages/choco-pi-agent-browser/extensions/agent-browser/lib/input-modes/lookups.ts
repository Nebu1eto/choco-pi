...
import { redactInvocationArgs, redactSensitiveText } from "../runtime.ts";
import {
	...
	isNumber,
	isString,
	validateLookupMaxWorkspaceFiles,
} from "./shared.ts";
import {
...
export function compileAgentBrowserSourceLookup<Input>(input: Input): CompilationResult<CompiledAgentBrowserSourceLookup> {
	...
	if (reactFiberId !== undefined && !isNonEmptyString(reactFiberId)) {
		return { error: "sourceLookup.reactFiberId must be a non-empty string when provided." };
	}
	if (componentName !== undefined && !isNonEmptyString(componentName)) {
	...
	if (input.includeDomHints !== undefined && input.includeDomHints !== true && input.includeDomHints !== false) {
	...
	if (rawMaxWorkspaceFiles !== undefined && (!isNumber(rawMaxWorkspaceFiles) || !Number.isInteger(rawMaxWorkspaceFiles) || rawMaxWorkspaceFiles <= 0)) {
		return { error: "sourceLookup.maxWorkspaceFiles must be a positive integer when provided." };
	}
	if (isNumber(rawMaxWorkspaceFiles) && rawMaxWorkspaceFiles > SOURCE_LOOKUP_MAX_WORKSPACE_FILES) {
	...
	const maxWorkspaceFiles = isNumber(rawMaxWorkspaceFiles) ? rawMaxWorkspaceFiles : SOURCE_LOOKUP_DEFAULT_MAX_WORKSPACE_FILES;
...
function extractStringField(value: InputRecord, names: string[]): string | undefined {
...
function extractNumberField(value: InputRecord, names: string[]): number | undefined {
	for (const name of names) {
		const field = value[name];
		if (isNumber(field) && Number.isFinite(field)) return field;
		if (isString(field) && /^\d+$/.test(field)) return Number(field);
	}
	return undefined;
}
...
function collectSourceCandidatesFromValue<Value>(value: Value, source: "react-inspect" | "dom-attribute", candidates: AgentBrowserSourceLookupCandidate[], evidence: string[], depth = 0): void {
...
function collectDomSourceCandidates<Html>(html: Html, candidates: AgentBrowserSourceLookupCandidate[]): void {
...
export async function analyzeSourceLookupResults<Data>(
	data: Data,
	...
	for (const item of items) {
		...
		const result = isInputRecord(item.result) && "data" in item.result ? item.result.data : item.result;
...
export function compileAgentBrowserNetworkSourceLookup<Input>(input: Input): CompilationResult<CompiledAgentBrowserNetworkSourceLookup> {
	if (!isInputRecord(input)) return { error: "networkSourceLookup must be an object." };
	const filter = input.filter;
	const requestId = input.requestId;
	const namespace = input.namespace;
	const session = input.session;
	const url = input.url;
	if (filter !== undefined && !isNonEmptyString(filter)) return { error: "networkSourceLookup.filter must be a non-empty string when provided." };
	if (requestId !== undefined && !isNonEmptyString(requestId)) return { error: "networkSourceLookup.requestId must be a non-empty string when provided." };
	if (namespace !== undefined && (!isString(namespace) || (namespace !== "" && namespace.trim().length === 0))) return { error: "networkSourceLookup.namespace must be a non-empty string or the empty default namespace when provided." };
	if (session !== undefined && !isNonEmptyString(session)) return { error: "networkSourceLookup.session must be a non-empty string when provided." };
	if (url !== undefined && !isNonEmptyString(url)) return { error: "networkSourceLookup.url must be a non-empty string when provided." };
	...
	const effectiveFilter = isString(filter) ? filter : isString(url) ? url : undefined;
	if (effectiveFilter) {
		steps.push({ action: "network", args: ["network", "requests", "--filter", effectiveFilter] });
	}
	const args: string[] = [];
	if (isString(namespace)) args.push("--namespace", namespace);
	if (isString(session)) args.push("--session", session);
	args.push("batch");
	return { compiled: { args, query: { filter: isString(filter) ? filter : undefined, maxWorkspaceFiles: maxWorkspaceFiles.value ?? SOURCE_LOOKUP_DEFAULT_MAX_WORKSPACE_FILES, namespace: isString(namespace) ? namespace : undefined, requestId: isString(requestId) ? requestId : undefined, session: isString(session) ? session : undefined, url: isString(url) ? url : undefined }, stdin: JSON.stringify(steps.map((step) => step.args)), steps } };
}

function getResultPayload(item: InputRecord): InputValue {
	return isInputRecord(item.result) && "data" in item.result ? item.result.data : item.result;
}
...
function isFailedNetworkRecord(request: InputRecord): boolean {
	const status = isNumber(request.status) ? request.status : undefined;
	const error = isString(request.error) ? request.error : undefined;
	return request.failed === true || error !== undefined || (status !== undefined && status >= 400);
}

function getFailedNetworkRequests<Data>(data: Data, queryText?: string): AgentBrowserNetworkSourceLookupRequest[] {
	...
	for (const item of getBatchResultItems(data)) {
		...
		const requests = isInputRecord(payload) && Array.isArray(payload.requests) ? payload.requests : Array.isArray(payload) ? payload : isInputRecord(payload) ? [payload] : [];
		for (const request of requests) {
			if (!isInputRecord(request)) continue;
			const url = isString(request.url) ? request.url : undefined;
			if (!networkRequestMatchesQuery(url, queryText) || !isFailedNetworkRecord(request)) continue;
			failed.push({
				error: isString(request.error) ? request.error : undefined,
				method: isString(request.method) ? request.method : undefined,
				requestId: isString(request.id) ? request.id : isString(request.requestId) ? request.requestId : undefined,
				status: isNumber(request.status) ? request.status : undefined,
				url,
			});
		}
	}
...
function collectInitiatorCandidates<Data>(data: Data, failedRequests: AgentBrowserNetworkSourceLookupRequest[], candidates: AgentBrowserNetworkSourceLookupCandidate[]): void {
	...
	for (const item of getBatchResultItems(data)) {
		const payload = getResultPayload(item);
		const requestValues = isInputRecord(payload) && Array.isArray(payload.requests) ? payload.requests : [payload];
		for (const value of requestValues) {
			if (!isInputRecord(value)) continue;
			const requestUrl = isString(value.url) ? value.url : undefined;
			const requestId = isString(value.id) ? value.id : isString(value.requestId) ? value.requestId : undefined;
...
async function collectWorkspaceRequestCandidates(query: CompiledAgentBrowserNetworkSourceLookup["query"], failedRequests: AgentBrowserNetworkSourceLookupRequest[], cwd: string, candidates: AgentBrowserNetworkSourceLookupCandidate[], limitations: string[]): Promise<void> {
	const needles = [...new Set([query.url, query.filter, ...failedRequests.map((request) => request.url)].filter((value): value is string => isString(value) && value.length > 0).flatMap((value) => {
		try {
...
export function redactNetworkSourceLookupSurface<Value>(value: Value): Value | InputValue {
	if (isString(value)) return redactNetworkSourceLookupUrl(value) ?? value;
	if (Array.isArray(value)) return value.map((item) => redactNetworkSourceLookupSurface(item));
	if (!isInputRecord(value)) return value;
...
export async function analyzeNetworkSourceLookupResults<Data>(data: Data, compiled: CompiledAgentBrowserNetworkSourceLookup, cwd: string): Promise<AgentBrowserNetworkSourceLookupAnalysis> {
	const limitations = [
