...
export function compileAgentBrowserSourceLookup<Input>(
  input: Input,
): CompilationResult<CompiledAgentBrowserSourceLookup> {
  ...
  if (
    input.includeDomHints !== undefined &&
    input.includeDomHints !== true &&
    input.includeDomHints !== false
  ) {
  ...
  const maxWorkspaceFiles = isNumber(rawMaxWorkspaceFiles)
    ? rawMaxWorkspaceFiles
    : SOURCE_LOOKUP_DEFAULT_MAX_WORKSPACE_FILES;
...
function collectSourceCandidatesFromValue<Value>(
  value: Value,
...
function collectDomSourceCandidates<Html>(
  html: Html,
  candidates: AgentBrowserSourceLookupCandidate[],
): void {
...
export async function analyzeSourceLookupResults<Data>(
  ...
  for (const item of items) {
    ...
    const result =
      isInputRecord(item.result) && "data" in item.result ? item.result.data : item.result;
...
export function compileAgentBrowserNetworkSourceLookup<Input>(
  input: Input,
): CompilationResult<CompiledAgentBrowserNetworkSourceLookup> {
  ...
  const url = input.url;
  if (filter !== undefined && !isNonEmptyString(filter))
  ...
  if (
    namespace !== undefined &&
    (!isString(namespace) || (namespace !== "" && namespace.trim().length === 0))
  )
    return {
      error:
  ...
  return {
    compiled: {
      args,
      query: {
        filter: isString(filter) ? filter : undefined,
        maxWorkspaceFiles: maxWorkspaceFiles.value ?? SOURCE_LOOKUP_DEFAULT_MAX_WORKSPACE_FILES,
        namespace: isString(namespace) ? namespace : undefined,
        requestId: isString(requestId) ? requestId : undefined,
        session: isString(session) ? session : undefined,
...
function getFailedNetworkRequests<Data>(
  data: Data,
  queryText?: string,
  ...
  for (const item of getBatchResultItems(data)) {
    ...
    const requests =
      isInputRecord(payload) && Array.isArray(payload.requests)
        ? payload.requests
        : Array.isArray(payload)
          ? payload
          : isInputRecord(payload)
            ? [payload]
            : [];
...
function collectInitiatorCandidates<Data>(
  data: Data,
...
export async function analyzeNetworkSourceLookupResults<Data>(
  data: Data,
