...
export function buildAgentBrowserSpawnCommand(args: string[], platform: NodeJS.Platform = processPlatform): AgentBrowserSpawnCommand {
	if (platform !== "win32") {
...
export function getAgentBrowserSocketDir(
	platform: NodeJS.Platform = processPlatform,
	uid: number | undefined = hasRuntimeType(process.getuid, "function") ? process.getuid() : undefined,
...
export async function getAgentBrowserSocketDirValidationError(
	...
	try {
		...
		try {
			...
			if (code !== "EEXIST") return `the directory could not be created (${code ?? "unknown error"})`;
		}
