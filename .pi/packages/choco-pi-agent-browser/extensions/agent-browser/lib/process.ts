...
import { getImplicitSessionIdleTimeoutMs, isPlainTextInspectionArgs } from "./runtime.ts";
import { getAgentBrowserProcessEnvironment } from "./process-environment.ts";
import { openSecureTempFile, writeSecureTempChunk } from "./temp.ts";
...
export function reorderWindowsLeadingGlobalArgs(args: string[]): string[] {
  ...
  for (let index = 0; index < args.length; index += 1) {
    ...
    if (GLOBAL_VALUE_FLAGS.some((valueFlag) => valueFlag === flag)) {
      const value = args[index + 1];
...
export function pinAgentBrowserFileAccessDisabled(
  args: string[],
...
export interface AgentBrowserSpawnCommand {
  args: string[];
  command: string;
}
...
export function getAgentBrowserSocketDir(
  platform: NodeJS.Platform = processPlatform,
  uid: number | undefined = hasRuntimeType(process.getuid, "function")
    ? process.getuid()
    : undefined,
  ...
  return `${prefix}${!termuxAppRoot && hasRuntimeType(uid, "number") ? `-${uid}` : ""}`;
}
...
async function socketDirEntriesAreOwned(
  ...
  for (const name of await readdir(socketDir)) {
    ...
    try {
      ...
      // SAFETY: realpath rejects filesystem failures as Node ErrnoException values.
...
export async function getAgentBrowserSocketDirValidationError(
  ...
  if (!hasRuntimeType(uid, "number")) return "POSIX ownership metadata is unavailable";
  try {
    ...
    try {
      ...
      // SAFETY: mkdir rejects filesystem failures as Node ErrnoException values.
      const code = (error as NodeJS.ErrnoException).code;
      if (code !== "EEXIST")
        return `the directory could not be created (${code ?? "unknown error"})`;
    }
    ...
    // SAFETY: lstat rejects filesystem failures as Node ErrnoException values.
...
function getManagedPreSpawnPolicyError(
  ...
  const managedSessionTargetError = getManagedSessionTargetAccessValidationError(
    ...
    allowManagedSessionTarget ||
      ...
      isOwnedManagedSessionTarget(options.args),
    policyEnv,
  );
...
export async function runAgentBrowserProcess(options: {
  ...
  if (
    invocationMayNavigateToLocalFile(args, stdin) &&
    (!ownedManagedSession || preserveAttachedBrowserSession)
  ) {
    return {
      aborted: false,
      agentBrowserStarted: false,
  ...
  const ownedManagedSessionClose = shouldOmitOwnedManagedSessionRestoreEnv(
    managedSessionRestoreOptions,
  );
  const browserConfigPinRequired =
    !isPlainTextInspectionArgs(args) && needsManagedSession(parseArgvDescriptor(args));
  const managedSessionRestoreConfigEnv = await getManagedSessionRestoreConfigEnv(
    managedSessionRestoreEnv,
    ownedManagedSessionClose || browserConfigPinRequired,
  );
  if (managedSessionRestoreConfigEnv === undefined) {
    return {
      ...
      spawnError: new Error(
        "Browser-backed agent-browser commands require a protected empty config, but secure temp storage was unavailable.",
      ),
      stderr: "",
      stdout: "",
      timedOut: false,
  ...
  return await new Promise<ProcessRunResult>((resolve) => {
    ...
    const spawnPolicyError = getManagedPreSpawnPolicyError(
      ...
      managedStatePageUrlUnknown,
      trustedFirstBatchTabSelection,
      managedSessionRestoreConfigEnv.AGENT_BROWSER_CONFIG !== undefined,
    );
    if (spawnPolicyError) {
      resolve({
        aborted: false,
    ...
    const recordStdinError = (cause: unknown) => {
      const stdinError = cause instanceof Error ? cause : new Error(String(cause));
      // SAFETY: Node stream errors extend Error with an optional errno code.
