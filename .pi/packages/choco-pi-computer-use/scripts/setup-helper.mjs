...
const localSigningLockPath = path.join(os.tmpdir(), `pi-computer-use-local-signing-${process.getuid !== undefined ? process.getuid() : "user"}.lock`);

const args = new Set(process.argv.slice(2));
const allowBuildFallback = args.has("--allow-build") || process.env.PI_COMPUTER_USE_ALLOW_BUILD === "1";
