...
import { isNumber, type JsonField } from "./json.ts";
...
function storeOutput(value: string): StoredOutput {
	...
	while (outputBytes > OUTPUT_STORE_MAX_BYTES && outputs.size > 1) {
		const oldestRef: string | undefined = outputs.keys().next().value;
		if (!oldestRef) break;
...
export function boundToolError(tool: string, cause: unknown): Error {
	const message = cause instanceof Error ? cause.message : String(cause);
	const bytes = new TextEncoder().encode(message).byteLength;
	const lines = message.split("\n").length;
	if (bytes <= MODEL_TEXT_MAX_BYTES && lines <= MODEL_TEXT_MAX_LINES) return cause instanceof Error ? cause : new Error(message);
...
export function readStoredOutput(ref: string, offsetValue: JsonField): { text: string; offset: number; limit: number; totalBytes: number; hasMore: boolean; complete: boolean } | undefined {
	const entry = outputs.get(ref);
	if (!entry) return undefined;
	let offset = Math.min(entry.storedBytes, Math.max(0, Math.trunc(isNumber(offsetValue) && Number.isFinite(offsetValue) ? offsetValue : 0)));
