...
function appendBrowserJavaScriptAppleEventsHint(error: Error): Error {
  ...
  if (
    !/not allowed to send javascript commands|executing javascript through applescript is turned off|allow javascript from apple events|enable javascript from apple events/i.test(
      error.message,
    ) ||
...
export function isChromeFamilyApp(appName: string, bundleId?: string): boolean {
  return (
    CHROME_FAMILY_BUNDLE_IDS.has(bundleId ?? "") ||
    CHROME_FAMILY_APP_NAMES.has(normalizeText(appName))
  );
}

function scriptForBrowserOpenLocation(
...
export async function openBrowserLocationWithAppleScript(
  ...
  try {
    ...
    throw appendBrowserJavaScriptAppleEventsHint(
