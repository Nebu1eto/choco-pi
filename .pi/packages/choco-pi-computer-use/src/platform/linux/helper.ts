...
export const LINUX_HELPER_PATH =
  process.env.PI_COMPUTER_USE_LINUX_HELPER_PATH ||
...
export class LinuxHelperClient {
  ...
  private buffer = "";
  private pending = new Map<string, Pending<JsonValue>>();
  ...
  async ensureInstalled(signal?: AbortSignal): Promise<void> {
    ...
    if (!this.installPromise) {
      const installPromise = (async () => {
        await runProcess(
          process.execPath,
          [this.setupHelperScript, "--platform", "linux", "--runtime"],
  ...
  private async startProcess(): Promise<ChildProcessWithoutNullStreams> {
    ...
    child.on("exit", (code, signalName) => {
      ...
      this.rejectPending(
        new Error(
          `Linux helper exited${signalName ? ` on ${signalName}` : ` with code ${code ?? "unknown"}`}.`,
        ),
      );
    });
  ...
  private onStdout(chunk: string): void {
    ...
    for (;;) {
      ...
      if (parsed.protocolVersion !== LINUX_HELPER_PROTOCOL_VERSION) {
        pending.reject(
          new Error(
            `Linux helper protocol mismatch: expected ${LINUX_HELPER_PROTOCOL_VERSION}, got ${parsed.protocolVersion ?? "unknown"}. Restart Pi to use the installed helper.`,
          ),
        );
        ...
        const error: Error & { code?: string } = new Error(
          parsed.error?.message ?? "Linux helper command failed.",
        );
  ...
  async command<T = JsonValue, TArgs extends object = object>(
    ...
    return await new Promise<T>((resolve, reject) => {
      ...
      this.pending.set(id, {
        resolve: (value) => {
          options?.signal?.removeEventListener("abort", onAbort);
          // SAFETY: The selected T is the response type for this command in the native helper protocol.
          resolve(value as T);
        },
        reject: (error) => {
          options?.signal?.removeEventListener("abort", onAbort);
      ...
      child.stdin.write(
        `${JSON.stringify({ protocolVersion: LINUX_HELPER_PROTOCOL_VERSION, id, cmd, args })}\n`,
        (error) => {
          if (!error) return;
          options?.signal?.removeEventListener("abort", onAbort);
          this.pending.delete(id);
