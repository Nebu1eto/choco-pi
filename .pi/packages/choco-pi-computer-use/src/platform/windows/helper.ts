...
export class WindowsHelperClient {
	...
	private onStdout(chunk: string): void {
		...
		for (;;) {
			...
			if (parsed.protocolVersion !== WINDOWS_HELPER_PROTOCOL_VERSION) {
				...
				const error: Error & { code?: string } = new Error(parsed.error?.message ?? "Windows helper command failed.");
	...
	async command<T = JsonValue, TArgs extends object = object>(cmd: string, args: TArgs, options?: { timeoutMs?: number; signal?: AbortSignal }): Promise<T> {
