...
function parseFramePoints(raw: JsonField): FramePoints {
	...
	const frame = isJsonObject(record.framePoints) ? record.framePoints : isJsonObject(record.bounds) ? record.bounds : {};
	return {
...
async function ensureReady(_ctx: ExtensionContext, state: PlatformReadyState, signal?: AbortSignal): Promise<PlatformReadyState> {
	await windowsHelper.ensureInstalled(signal);
	const diagnostics = await windowsHelper.command<PlatformDiagnostics>("diagnostics", {}, { signal, timeoutMs: 5_000 });
