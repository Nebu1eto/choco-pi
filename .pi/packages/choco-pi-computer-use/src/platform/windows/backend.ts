...
function classifyBrowser(
  appName: string,
): false | "chrome" | "edge" | "brave" | "firefox" | "vivaldi" | "opera" {
...
function parseFramePoints(raw: JsonField): FramePoints {
  ...
  const frame = isJsonObject(record.framePoints)
    ? record.framePoints
    : isJsonObject(record.bounds)
      ? record.bounds
      : {};
  return {
...
function parseRootKind(raw: JsonField): PlatformRootKind {
  return raw === "menu" || raw === "dialog" || raw === "popover" || raw === "window"
    ? raw
    : "window";
}
...
function parseRoots(result: JsonValue): PlatformRoot[] {
  ...
  return array.map((value, index): PlatformRoot => {
    ...
    return {
      ...
      windowId:
        isNumber(raw.windowId) && Number.isFinite(raw.windowId)
          ? Math.trunc(raw.windowId)
          : undefined,
...
async function ensureReady(
  ...
  const diagnostics = await windowsHelper.command<PlatformDiagnostics>(
    "diagnostics",
    {},
  ...
  if (diagnostics?.protocolVersion !== WINDOWS_HELPER_PROTOCOL_VERSION) {
    throw new Error(
      `Windows helper protocol mismatch: expected ${WINDOWS_HELPER_PROTOCOL_VERSION}, got ${diagnostics?.protocolVersion ?? "unknown"}. Restart Pi to use the installed helper.`,
    );
  }
...
export const windowsBackend: ComputerUsePlatformBackend = {
  ...
  async focusWindow(
    ...
    signal?: AbortSignal,
  ): Promise<PlatformFocusWindowResult> {
    return await windowsHelper.command<PlatformFocusWindowResult>(
      "focusWindow",
      { ...target },
      { signal },
    );
  ...
  async actBatch(
    ...
    return await windowsHelper.command<HelperActResult>(
      "actBatch",
      { actions: requests.map(helperAction) },
      options,
    );
  },
  async readText(
  ...
  isChromeFamilyApp(appName: string): boolean {
    return (
      classifyBrowser(appName) === "chrome" ||
      classifyBrowser(appName) === "edge" ||
      classifyBrowser(appName) === "brave"
    );
  },
