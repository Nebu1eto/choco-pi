...
function structuralToken(node: OutlineNode): string {
  return [node.role, node.subrole, node.identifier, node.title, node.description]
...
export function stabilizeRefs(base: Outline | undefined, next: Outline): Outline {
  ...
  const byWireRef = new Map(
    base.nodes.filter((node) => node.wireRef).map((node) => [node.wireRef!, node.ref]),
  );
  ...
  for (const node of next.nodes) {
    ...
    const stable =
      wireStable ?? (structuralMatches.length === 1 ? structuralMatches[0].ref : undefined);
...
type ComparableNode = Omit<SerializedOutlineNode, "children">;
type ChangedNodeFields = Partial<ComparableNode>;

function comparable(node: OutlineNode): ComparableNode {
  const { children: _children, ...fields } = serializeOutlineNode(node);
  return {
    ...fields,
    rect: fields.rect
      ? {
          x: Math.round(fields.rect.x),
          y: Math.round(fields.rect.y),
          w: Math.round(fields.rect.w),
          h: Math.round(fields.rect.h),
        }
      : undefined,
    text: fields.text.map((item) => ({
      string: item.string,
...
function setChangedField<Key extends keyof ComparableNode>(
  fields: ChangedNodeFields,
  key: Key,
  value: ComparableNode[Key],
): void {
  fields[key] = value;
}

function changedFields(base: OutlineNode, next: OutlineNode): ChangedNodeFields {
  const before = comparable(base);
  const after = comparable(next);
  const fields: ChangedNodeFields = {};
  // SAFETY: Object.keys(after) can only return own keys declared by ComparableNode.
  const keys = Object.keys(after) as Array<keyof ComparableNode>;
  for (const key of keys) {
    if (key === "ref" || key === "wireRef") continue;
    if (JSON.stringify(before[key]) !== JSON.stringify(after[key]))
      setChangedField(fields, key, after[key]);
  }
  return fields;
}
...
export function changesBetween(base: Outline, next: Outline): OutlineDiff {
  if (base.root.role !== next.root.role || base.root.subrole !== next.root.subrole) {
    return {
      changes: [],
      changedNodeCount: next.nodes.length,
  ...
  for (const node of next.nodes) {
    ...
    if (!previous)
      changes.push({
        type: "added",
        ref: node.ref,
        parent: node.parent?.ref,
        node: { ...serializeOutlineNode(node), children: [] },
      });
    else {
  ...
  const identityConfidence =
    next.nodes.length === 0
      ? 1
      : next.nodes.filter((node) => before.has(node.ref)).length / next.nodes.length;
  ...
  return changes
    .map((change) => {
      ...
      const fields = Object.entries(change.fields)
        .filter(([key]) => !["rect", "text", "actions"].includes(key))
        .slice(0, 6)
        .map(([key, value]) => `${key}=${JSON.stringify(value)}`)
        .join(", ");
      const supplemental = [
        change.fields.text
          ? `text=${JSON.stringify(change.fields.text.map((item) => item.string))}`
          : undefined,
