...
function setChangedField<Key extends keyof ComparableNode>(fields: ChangedNodeFields, key: Key, value: ComparableNode[Key]): void {
	fields[key] = value;
}
...
function changedFields(base: OutlineNode, next: OutlineNode): ChangedNodeFields {
	...
	for (const key of keys) {
		...
		if (JSON.stringify(before[key]) !== JSON.stringify(after[key])) setChangedField(fields, key, after[key]);
	}
	return fields;
}
