...
export class CdpTab {
	...
	async keypress(keys: string[]): Promise<void> {
		const modifierBits: ModifierBits = { alt: 1, option: 1, control: 2, ctrl: 2, meta: 4, command: 4, cmd: 4, shift: 8 };
	...
	private async withBackendNode(backendNodeId: number, functionDeclaration: string, args: JsonValue[] = []): Promise<void> {
		const resolved = await this.send("DOM.resolveNode", { backendNodeId });
		const objectId = resolved?.object?.objectId;
		if (!isString(objectId)) throw new Error(`CDP could not resolve backend node ${backendNodeId}.`);
...
function cdpSnapshotOutline(snapshotId: string, nodes: CdpAxNode[]): CdpSnapshotOutline {
	...
	const build = (raw: CdpAxNode, seen: Set<string>): CdpRawOutlineNode | undefined => {
		...
		const backendNodeId = isNumber(raw.backendDOMNodeId) && Number.isFinite(raw.backendDOMNodeId) ? Math.trunc(raw.backendDOMNodeId) : undefined;
	...
	const children = roots
		...
		.filter((child): child is CdpRawOutlineNode => child !== undefined);
	const rawOutline: CdpRawOutlineNode = children.length === 1 ? children[0] : {
