...
export type OutlineChange =
  ...
  | {
      ...
      ref: string;
      path: string[];
      fields: Partial<Omit<SerializedOutlineNode, "children">>;
    }
...
export type SerializedOutlineNode = Omit<OutlineNode, "parent" | "children"> & {
...
function toString(value: JsonField): string {
  return isString(value) ? value : "";
}

function toNumber(value: JsonField, fallback = 0): number {
...
function parseRect(raw: JsonField): OutlineRect {
  const rect = isJsonObject(raw) ? raw : {};
  return {
    x: toNumber(rect.x),
    y: toNumber(rect.y),
...
function parseText(raw: JsonField): OutlineText[] {
...
function parseNode(raw: JsonField, parent?: OutlineNode): OutlineNode {
  ...
  const node: OutlineNode = {
    ...
    actions: Array.isArray(record.actions) ? record.actions.filter(isString) : [],
    ...
    scrollExtent: isJsonObject(record.scrollExtent)
      ? {
          seen: Math.max(0, Math.trunc(toNumber(record.scrollExtent.seen))),
          total: Math.max(0, Math.trunc(toNumber(record.scrollExtent.total))),
        }
      : undefined,
  ...
  node.children = (Array.isArray(record.children) ? record.children : []).map((child) =>
    parseNode(child, node),
  );
  return node;
}

export function parseLookResponse(raw: JsonValue): LookResponse {
  const record = isJsonObject(raw) ? raw : {};
  const image = isJsonObject(record.image) ? record.image : undefined;
  const window = isJsonObject(record.window) ? record.window : {};
  const metadata = isJsonObject(window.metadata) ? window.metadata : undefined;
  const outline = buildOutline(toString(record.lookId), parseNode(record.outline));
  const readText = isJsonObject(record.readText) ? record.readText : undefined;
  const requestedReadText =
    readText?.requested === "auto" ||
    readText?.requested === "always" ||
    readText?.requested === "never"
      ? readText.requested
      : undefined;
  const look: LookResponse = {
    ...
    timings: isJsonObject(record.timings)
      ? Object.fromEntries(
          Object.entries(record.timings).map(([key, value]) => [key, toNumber(value)]),
        )
      : {},
    readText: readText
      ? { requested: requestedReadText, executed: toBoolean(readText.executed) }
      : undefined,
    parsedOutline: outline,
  };
...
function countDescendants(node: OutlineNode): DescendantCounts {
...
function foldedSummary(node: OutlineNode): string {
  ...
  const scroll = node.scrollExtent
    ? ` [scrollable ${node.scrollExtent.seen}/${node.scrollExtent.total}]`
    : "";
...
export function foldToBudget(
  ...
  if (truncated) {
    ...
    lines.push(
      `… render budget reached: ${remaining} more nodes not shown; use search_ui or expand_ui(@eN)`,
    );
  }
  return {
    text: lines.join("\n"),
...
export function rankedTextMatch(
  values: string[],
  text: string,
): { reason: "exact" | "prefix" | "substring" | "fuzzy"; score: number } | undefined {
  const query = text.toLowerCase().replace(/\s+/g, " ").trim().slice(0, 256);
  if (!query) return undefined;
  const candidates = values
    .map((value) => value.toLowerCase().replace(/\s+/g, " ").trim().slice(0, 512))
    .filter(Boolean);
  if (candidates.some((value) => value === query)) return { reason: "exact", score: 1 };
  if (
    candidates.some(
      (value) =>
        value.startsWith(query) || value.split(/\W+/).some((token) => token.startsWith(query)),
    )
  )
  ...
  for (const value of candidates) {
    for (const candidate of [value, ...value.split(/\W+/)]) {
      ...
      score = Math.max(
        score,
        1 - damerauLevenshtein(query, candidate) / Math.max(query.length, candidate.length),
      );
    }
  }
...
export interface RankedOutlineMatches {
  matches: OutlineSearchMatch[];
  totalMatches: number;
}

export function searchOutlineRanked(
  outline: Outline,
  text?: string,
  role?: string,
  capability?: string,
  limit = 12,
): RankedOutlineMatches {
    ...
    const match = query
      ? rankedTextMatch(
          [
            ...
            node.description,
            node.value,
            ...node.text.map((item) => item.string),
          ],
          query,
        )
      : undefined;
  ...
  const sorted = [
    ...strong.sort(
      (a, b) =>
        rank[a.matchReason!] - rank[b.matchReason!] || b.score! - a.score! || a.order - b.order,
    ),
  ];
  const useFuzzy = sorted.length < limit;
  if (useFuzzy) sorted.push(...fuzzy.sort((a, b) => b.score! - a.score! || a.order - b.order));
  return {
    matches: sorted.slice(0, limit).map(({ order: _order, ...match }) => match),
    totalMatches: strong.length + (useFuzzy ? fuzzy.length : 0),
  };
}
...
export function serializeOutlineSearchMatch(
  match: OutlineSearchMatch,
): SerializedOutlineSearchMatch {
...
function copyNodeFields(target: OutlineNode, source: OutlineNode, preserveWireRef = false): void {
  ...
  target.text = source.text.map((text) => ({
    ...text,
    rect: text.rect ? { ...text.rect } : undefined,
  }));
}

function preserveUnreused(
  node: OutlineNode,
  parent: OutlineNode,
  used: Set<OutlineNode>,
...
function cloneForGraft(
  source: OutlineNode,
  parent: OutlineNode,
  nextRef: () => string,
  reusableByWireRef: Map<string, OutlineNode>,
  used: Set<OutlineNode>,
): OutlineNode {
  ...
  node.parent = parent;
  const graftedChildren = source.children.map((child) =>
    cloneForGraft(child, node, nextRef, reusableByWireRef, used),
  );
...
export function graftScopedOutline(
  outline: Outline,
  targetRef: string,
  scoped: Outline,
): OutlineNode {
  ...
  if (!target)
    throw new Error(
      `Cannot graft scoped outline: target ${targetRef} is not in the current outline.`,
    );
  ...
  const graftedChildren = scoped.root.children.map((child) =>
    cloneForGraft(child, target, nextRef, reusableByWireRef, used),
  );
  const preservedChildren = oldChildren
