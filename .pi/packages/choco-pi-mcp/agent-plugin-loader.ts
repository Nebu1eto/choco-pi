...
function loadAgentPluginMcpConfig(path: string, cwd: string): McpConfig | null {
  ...
  if (!statSync(mcpPath).isFile()) {
    console.warn(
      `Agent Plugin ${manifest.name} has invalid MCP config: mcp.json is not a regular file`,
    );
    return { mcpServers: {} };
  }
  ...
  try {
    ...
    console.warn(
      `Agent Plugin ${manifest.name} has invalid MCP config: failed to parse mcp.json`,
      error,
    );
    return { mcpServers: {} };
  }
...
function translateAgentPluginMcpConfig<BoundaryValue>(
  ...
  for (const key of Object.keys(mcpConfig)) {
    if (!MCP_CONFIG_FIELDS.has(key)) {
      console.warn(
        `Agent Plugin ${manifest.name} has invalid MCP config: unknown top-level field ${key}`,
      );
      return { mcpServers: {} };
    }
  }
  if (mcpConfig.$schema !== MCP_SCHEMA) {
    console.warn(
      `Agent Plugin ${manifest.name} has invalid MCP config: unsupported mcp.json $schema`,
    );
    return { mcpServers: {} };
  }
  if (
    ...
    console.warn(
      `Agent Plugin ${manifest.name} has invalid MCP config: mcpServers must be an object`,
    );
    return { mcpServers: {} };
  }
...
function translateAgentPluginServer<BoundaryValue>(
  ...
  console.warn(
    `Agent Plugin ${manifest.name} skips invalid MCP server ${serverName}: unsupported type`,
  );
  return null;
}
...
function formatAgentPluginServerName(pluginName: string, serverName: string): string {
  const pluginPart =
    pluginName.replace(/[^A-Za-z0-9_-]+/g, "_").replace(/^[_-]+|[_-]+$/g, "") || "plugin";
  const serverPart =
    serverName.replace(/[^A-Za-z0-9_-]+/g, "_").replace(/^[_-]+|[_-]+$/g, "") || "server";
...
function translateHeaders<BoundaryValue>(
  ...
  for (const [key, entry] of Object.entries(value)) {
    ...
    if (seen.has(normalized)) {
      console.warn(
        `Agent Plugin ${manifest.name} skips invalid MCP server ${serverName}: duplicate header ${key}`,
      );
      return null;
    }
    seen.add(normalized);
