...
export function buildComponentImplementationHints(
  ...
  if (!codeConnect?.matches.length && options.includeCodeConnect)
    nextSteps.push(
  ...
  return {
    ...
    tokenDependencies: (Array.isArray(resolvedTokens)
      ? resolvedTokens.map(asFigmaRecord)
      : []
    ).slice(0, 40),
...
function inferProps(summary: FigmaNodeSummary): FigmaRecord[] {
  ...
  if (hasRole(summary, "button"))
    props.push({ name: "onClick", type: "() => void", source: "button-like layer" });
  if (hasRole(summary, "form-control"))
    props.push({ name: "value", type: "string", source: "form-control-like layer" });
  return props;
}
...
function collectVariants(summary: FigmaNodeSummary): FigmaRecord[] {
  ...
  for (const node of flatten(summary)) {
    ...
    for (const [name, raw] of Object.entries(properties))
      out.push({ nodeId: node.id, nodeName: node.name, name, ...asFigmaRecord(raw) });
  }
...
function hasRole(summary: FigmaNodeSummary, role: string): boolean {
  return flatten(summary).some(
    (node) => node.roleGuess === role || String(node.name).toLowerCase().includes(role),
  );
}
