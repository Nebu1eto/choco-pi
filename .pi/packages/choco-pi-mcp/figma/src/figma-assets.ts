...
export function collectAssetCandidates(
  node: FigmaValue,
  options: { assetTypes?: FigmaAssetType[]; includeHidden?: boolean; maxAssets?: number } = {},
): FigmaAssetCollectionResult {
  const assetTypes = new Set(options.assetTypes?.length ? options.assetTypes : DEFAULT_ASSET_TYPES);
  ...
  function push(candidate: FigmaAssetCandidate): void {
    if (assets.length >= maxAssets) {
      if (!truncatedReasons.some((reason) => reason.includes("maxAssets")))
        truncatedReasons.push(`Reached maxAssets ${maxAssets}; additional assets were omitted.`);
      return;
    }
  ...
  walk(node, options.includeHidden ?? false, (record, path, level) => {
    ...
    if (assetTypes.has("svgIcons") && id && isIconCandidate(record)) {
      push({
        ...
        nodeType: type,
        nodePath: path,
        suggestedName: `${safeFilename(name || id)}.svg`,
      });
    }
    if (
      assetTypes.has("nodeRenders") &&
      id &&
      (level === 0 || /asset|illustration|image|card|avatar|logo/i.test(name))
    ) {
      push({
        kind: "nodeRender",
        nodeId: id,
      ...
      fills.forEach((fill, index) => {
        ...
        if (imageRef)
          push({
            ...
            imageRef,
            fillIndex: index,
            suggestedName: `${safeFilename(name || imageRef)}.${formatFromScaleMode(fillRecord.scaleMode)}`,
          });
      });
    }
  });
  ...
  return {
    ...
    metadata: {
      ...
      truncatedReasons,
      nextSteps: truncatedReasons.length
        ? ["Raise maxAssets or extract assets from a narrower child node."]
        : [],
    },
  };
}
...
export function formatFromPath(path: string): FigmaAssetManifestEntry["format"] {
  const extension = extname(path || basename(path))
    .toLowerCase()
    .replace(/^\./, "");
...
function formatFromScaleMode(_value: FigmaValue): "png" {
  return "png";
}

function isIconCandidate(record: FigmaRecord): boolean {
  const type = String(record.type ?? "UNKNOWN");
  const name = String(record.name ?? "");
  const box = asFigmaRecord(record.absoluteBoundingBox);
  const width = numberValue(box.width) ?? 0;
  const height = numberValue(box.height) ?? 0;
  return (
    VECTOR_TYPES.has(type) ||
    /\b(icon|logo|glyph)\b/i.test(name) ||
    (width > 0 && height > 0 && width <= 64 && height <= 64 && !Array.isArray(record.children))
  );
}
...
function walk(
  ...
  visit: (record: FigmaRecord, path: string, level: number) => void,
  path = "",
  level = 0,
): void {
  const record = asFigmaRecord(node);
