import type { FigmaNodeSummary } from "./figma-summarizer.ts";
...
export function buildCssLayoutHints(node: FigmaValue): FigmaRecord {
  const record = asRecord(node);
  const mode = stringValue(record.layoutMode);
  const css: FigmaRecord = {};
  const notes: string[] = [];
  ...
  return (
    compactObject({
      ...
      figma: compactObject({ layoutMode: mode, layoutWrap: record.layoutWrap }),
      css,
      sizing,
      notes,
    }) ?? {}
...
export function buildResponsiveHints(node: FigmaValue): FigmaRecord[] {
  const hints: FigmaRecord[] = [];
  walk(node, (record, path) => {
    const recommendations: string[] = [];
    const constraints = asRecord(record.constraints);
    if (constraints.horizontal === "SCALE" || constraints.horizontal === "LEFT_RIGHT")
      recommendations.push("Use width: 100% or stretch within the parent container.");
    if (constraints.vertical === "SCALE" || constraints.vertical === "TOP_BOTTOM")
      recommendations.push("Let height stretch or derive it from content plus min-height.");
    if (record.layoutGrow === 1) recommendations.push("Use flex: 1 for fill-container sizing.");
    if (record.layoutAlign === "STRETCH" || record.layoutSizingHorizontal === "FILL")
      recommendations.push("Use align-self: stretch or width: 100%.");
    if (record.layoutSizingHorizontal === "HUG" || record.layoutSizingVertical === "HUG")
      recommendations.push(
        "Prefer intrinsic/content sizing; avoid hard-coded dimensions unless necessary.",
      );
    if (record.layoutSizingHorizontal === "FIXED")
      recommendations.push("Fixed width from Figma may need max-width or responsive breakpoints.");
    if (record.layoutWrap === "WRAP")
      recommendations.push("Enable flex-wrap and test narrow container widths.");
    if (numberValue(record.minWidth) !== undefined)
      recommendations.push(`Respect min-width: ${px(numberValue(record.minWidth))}.`);
    if (numberValue(record.maxWidth) !== undefined)
      recommendations.push(`Respect max-width: ${px(numberValue(record.maxWidth))}.`);
    if (recommendations.length) {
      hints.push({
        ...
        type: record.type,
        path,
        constraints: compactObject(constraints),
...
export function buildAccessibilityHints(summary: FigmaNodeSummary): FigmaRecord[] {
  const hints: FigmaRecord[] = [];
...
export function buildDesignTokenHints(node: FigmaValue, tokenMap?: FigmaTokenMap): FigmaRecord {
  const resolved: FigmaRecord[] = [];
...
export function buildFrameworkHints(
  ...
  return compactObject({
    ...
    snippet: options.includeCodeSnippets
      ? snippetFor(summary, framework, styling, componentName, className)
      : undefined,
  });
}
...
function walk(
  node: FigmaValue,
  visit: (record: FigmaRecord, path: string) => void,
  path = "",
): void {
  const record = asRecord(node);
...
function collectVariableBindings(value: FigmaValue): Array<{ property: string; id: string }> {
  const out: Array<{ property: string; id: string }> = [];
  function visit(raw: FigmaValue, property: string): void {
    if (Array.isArray(raw)) raw.forEach((item, index) => visit(item, `${property}[${index}]`));
    else {
      const record = asRecord(raw);
      if (isStringValue(record.id)) out.push({ property, id: record.id });
      for (const [key, child] of Object.entries(record))
        if (key !== "id" && key !== "type") visit(child, property ? `${property}.${key}` : key);
    }
  }
  visit(value, "");
  return out;
...
function compactLayoutGrids(value: FigmaValue): FigmaRecord[] {
...
function paddingValue(record: FigmaRecord): string | undefined {
...
function mapPrimary(value: FigmaValue): string | undefined {
...
function mapCounter(value: FigmaValue): string | undefined {
  return alignmentValue(
    {
      MIN: "flex-start",
      CENTER: "center",
...
function alignmentValue(mappings: Record<string, string>, key: string): string | undefined {
  return mappings[key];
}
...
function compactObject(value: FigmaRecord): FigmaRecord | undefined {
  ...
  for (const [key, raw] of Object.entries(value)) {
    ...
    if (isFigmaRecord(raw) && Object.keys(raw).length === 0) continue;
    out[key] = raw;
  }
