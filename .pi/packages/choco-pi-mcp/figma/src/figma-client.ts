...
} from "./figma-search.ts";
import { buildFigmaTokenMap } from "./figma-tokens.ts";
import { asFigmaRecord, type FigmaRecord, type FigmaValue } from "./figma-values.ts";
...
export class FigmaClient {
  ...
  getFile(fileKey: string, depth?: number): Promise<FigmaValue> {
    ...
    return this.cached(`file:${fileKey}:${depth ?? "all"}`, () =>
  ...
  getNodes(
    ...
    return this.cached(`nodes:${fileKey}:${ids}:${depth ?? "all"}`, () =>
      this.get(`/v1/files/${fileKey}/nodes?ids=${encodeURIComponent(ids)}${depthQuery}`),
    );
  }
  ...
  async getNodeSummary(
    ...
    return summarizeNode(
      await this.getSingleNodeDocument(fileKey, nodeId, options.depth ?? 2),
      options,
    );
  }
  ...
  async getImplementationContext(
    ...
    try {
      ...
      return getImplementationContext(document, {
        ...options,
  ...
  async extractAssets(
    ...
    const svgCandidates = collected.assets.filter(
      (asset): asset is FigmaAssetCandidate & { nodeId: string } =>
        asset.kind === "svgIcon" && Boolean(asset.nodeId),
    );
    if (svgCandidates.length) {
      const rendered = await this.renderNodes(
        fileKey,
        svgCandidates.map((asset) => asset.nodeId),
        ...
        manifest.push(
          file
            ? await manifestEntryFromFile(
                candidate,
                file.path,
                rendered.images[candidate.nodeId],
                "svg",
              )
            : manifestEntryFromUrl(candidate, rendered.images[candidate.nodeId], "svg"),
        );
      }
    }
    ...
    const renderCandidates = collected.assets.filter(
      (asset): asset is FigmaAssetCandidate & { nodeId: string } =>
        asset.kind === "nodeRender" && Boolean(asset.nodeId),
    );
    if (renderCandidates.length) {
      const rendered = await this.renderNodes(
        fileKey,
        renderCandidates.map((asset) => asset.nodeId),
      ...
      for (const candidate of renderCandidates) {
        ...
        manifest.push(
          file
            ...
            : manifestEntryFromUrl(candidate, rendered.images[candidate.nodeId], "png"),
        );
      }
    }

    const imageFillCandidates = collected.assets.filter(
      (asset): asset is FigmaAssetCandidate & { imageRef: string } =>
        asset.kind === "imageFill" && Boolean(asset.imageRef),
    );
    if (imageFillCandidates.length) {
      ...
      for (const candidate of imageFillCandidates) {
        ...
        const bytes = await this.http.download(url);
        const extension = url.includes(".webp")
          ? "webp"
        ...
        const outputPath = resolve(
          outputDir,
          `${safeFilename(candidate.suggestedName.replace(/\.[^.]+$/, ""))}-${safeFilename(candidate.imageRef).slice(0, 12)}.${extension}`,
        );
        await writeFile(outputPath, Buffer.from(bytes));
    ...
    const unresolvedFills = manifest.filter(
      (entry) => entry.kind === "imageFill" && !entry.url,
    ).length;
    return {
  ...
  async getComponentImplementationHints(
    ...
    const implementation = await this.getImplementationContext(fileKey, nodeId, {
      ...options,
      includeCodeSnippets: options.includeSnippet ?? options.includeCodeSnippets,
    });
    const codeConnect =
    ...
    return buildComponentImplementationHints(
      ...
      implementation,
      {
        ...
        includeSnippet: options.includeSnippet ?? options.includeCodeSnippets,
  ...
  private async getSingleNodeDocument(
    ...
    if (!document)
      throw new Error(`Figma node ${normalizedNodeId} was not found in file ${fileKey}.`);
    return document;
  }
...
function collectTopLevelStructure(value: FigmaValue): Array<{
  id: FigmaValue;
  name: FigmaValue;
  type: FigmaValue;
  children?: Array<{ id: FigmaValue; name: FigmaValue; type: FigmaValue }>;
}> {
  ...
  return getChildren(document).map((page) => {
    ...
    return {
      ...
      children: getChildren(pageRecord)
        .slice(0, 100)
        .map((child) => {
          const childRecord = asRecord(child);
    ...
    for (const child of pageChildren) {
      ...
      if (childRecord.id === nodeId) {
        return {
          ...
          siblingNames: pageChildren
            .filter((sibling) => asRecord(sibling).id !== nodeId)
            .slice(0, 100)
            .map((sibling) => asRecord(sibling).name),
        };
      }
    }
  }
