...
import { readAuthToken } from "../common/auth.ts";
import { createHttpClient, type HttpClient } from "../common/http-client.ts";
import { createRateLimiter, type RateLimiter } from "../common/rate-limiter.ts";
import { figmaCache } from "./figma-cache.ts";
import {
	collectAssetCandidates,
	manifestEntryFromFile,
	manifestEntryFromUrl,
	safeFilename,
	type FigmaAssetCandidate,
	type FigmaAssetManifestEntry,
	type FigmaAssetType,
	type FigmaExtractAssetsResult,
} from "./figma-assets.ts";
import { findCodeConnectMapping, type CodeConnectScanResult } from "./code-connect.ts";
import { buildComponentImplementationHints, type FigmaComponentImplementationHints } from "./figma-component-hints.ts";
import {
	explainNode,
	extractVisibleText,
...
} from "./figma-summarizer.ts";
import { findNodesByName, findNodesByText, type FigmaFindNodesOptions, type FigmaNodeSearchResult } from "./figma-search.ts";
import { buildFigmaTokenMap } from "./figma-tokens.ts";
import {
	asFigmaRecord,
	type FigmaRecord,
	type FigmaValue,
...
export interface RenderNodesResult extends FigmaRecord {
	images: Record<string, string | null>;
	savedFiles: Array<{ nodeId: string; path: string }>;
}

interface RenderImagesResponse extends FigmaRecord {
	images?: Record<string, string | null>;
	err?: string;
}

interface ImageFillsResponse extends FigmaRecord {
...
const DEFAULT_ASSET_TYPES = ["svgIcons", "nodeRenders", "imageFills"] as const;
...
export class FigmaClient {
	...
	getFile(fileKey: string, depth?: number): Promise<FigmaValue> {
		const query = depth ? `?depth=${depth}` : "";
		return this.cached(`file:${fileKey}:${depth ?? "all"}`, () => this.get(`/v1/files/${fileKey}${query}`));
	}

	getNodes(fileKey: string, nodeIds: readonly string[], options: FigmaGetNodesOptions = {}): Promise<FigmaValue> {
	...
	async getNodeMetadata(fileKey: string, nodeIds: readonly string[]): Promise<FigmaValue> {
	...
	async renderNodes(fileKey: string, nodeIds: readonly string[], options: RenderNodesOptions): Promise<RenderNodesResult> {
		...
		const response = await this.get<RenderImagesResponse>(
	...
	getImageFills(fileKey: string): Promise<Record<string, string>> {
		return this.cached(`imageFills:${fileKey}`, async () => {
			const response = await this.get<ImageFillsResponse>(`/v1/files/${fileKey}/images`);
	...
	async extractAssets(fileKey: string, nodeId: string, options: ExtractAssetsOptions): Promise<FigmaExtractAssetsResult> {
		...
		const assetTypes = options.assetTypes?.length ? options.assetTypes : [...DEFAULT_ASSET_TYPES];
		...
		const svgCandidates = collected.assets.filter((asset): asset is FigmaAssetCandidate & { nodeId: string } => asset.kind === "svgIcon" && Boolean(asset.nodeId));
		if (svgCandidates.length) {
			const rendered = await this.renderNodes(fileKey, svgCandidates.map((asset) => asset.nodeId), { cwd: options.cwd, outputDir, format: "svg", download: true });
			for (const candidate of svgCandidates) {
				const file = rendered.savedFiles.find((saved) => saved.nodeId === candidate.nodeId);
				manifest.push(file ? await manifestEntryFromFile(candidate, file.path, rendered.images[candidate.nodeId], "svg") : manifestEntryFromUrl(candidate, rendered.images[candidate.nodeId], "svg"));
			}
		}

		const renderCandidates = collected.assets.filter((asset): asset is FigmaAssetCandidate & { nodeId: string } => asset.kind === "nodeRender" && Boolean(asset.nodeId));
		if (renderCandidates.length) {
			const rendered = await this.renderNodes(fileKey, renderCandidates.map((asset) => asset.nodeId), { cwd: options.cwd, outputDir, format: "png", download: true });
			for (const candidate of renderCandidates) {
				const file = rendered.savedFiles.find((saved) => saved.nodeId === candidate.nodeId);
				manifest.push(file ? await manifestEntryFromFile(candidate, file.path, rendered.images[candidate.nodeId], "png") : manifestEntryFromUrl(candidate, rendered.images[candidate.nodeId], "png"));
			}
		}

		const imageFillCandidates = collected.assets.filter((asset): asset is FigmaAssetCandidate & { imageRef: string } => asset.kind === "imageFill" && Boolean(asset.imageRef));
		if (imageFillCandidates.length) {
			const images = await this.getImageFills(fileKey);
			for (const candidate of imageFillCandidates) {
				const url = images[candidate.imageRef];
				if (!url) {
				...
				const outputPath = resolve(outputDir, `${safeFilename(candidate.suggestedName.replace(/\.[^.]+$/, ""))}-${safeFilename(candidate.imageRef).slice(0, 12)}.${extension}`);
				await writeFile(outputPath, Buffer.from(bytes));
	...
	private get<T = FigmaValue>(path: string): Promise<T> {
		return this.limiter.schedule(() => this.http.get<T>(path));
	}

	private async getTargetDesignContext(fileKey: string, nodeId: string): Promise<FigmaValue> {
	...
	private async getSingleNodeDocument(fileKey: string, nodeId: string, depth: number): Promise<FigmaValue> {
		const normalizedNodeId = normalizeNodeId(nodeId);
	...
	private async getSearchRoot(fileKey: string, params: FigmaFindNodesOptions & { nodeId?: string }): Promise<FigmaValue> {
	...
	private cached<T>(key: string, load: () => Promise<T>): Promise<T> {
		// SAFETY: each cache key is private to one load function, so a stored value has the same T produced by that function.
...
function getChildren(value: FigmaValue): FigmaValue[] {
...
function collectTopLevelStructure(value: FigmaValue): Array<{ id: FigmaValue; name: FigmaValue; type: FigmaValue; children?: Array<{ id: FigmaValue; name: FigmaValue; type: FigmaValue }> }> {
...
function findShallowLocation(value: FigmaValue, nodeId: string): FigmaValue {
