...
export function normalizeSummarizerOptions(
...
export function explainNode(
  ...
  if (summary.visibleText?.length) {
    ...
    if (summary.visibleText.length > 40)
      lines.push(`- …${summary.visibleText.length - 40} more text nodes omitted`);
  }
  if (summary.children?.length) {
    ...
    if (summary.children.length > 20)
      lines.push(`${summary.children.length - 20} additional sections omitted.`);
  }
  ...
  if (options.assets?.length) {
    ...
    for (const asset of options.assets)
      lines.push(`- ${asset.nodeId}: ${asset.path ?? asset.url ?? "not available"}`);
  }
...
export function getImplementationContext(
  ...
  return {
    ...
    designTokens:
      options.resolveTokens === false ? undefined : buildDesignTokenHints(node, options.tokenMap),
...
function summarizeNodeInternal(
  ...
  const shouldCollapseInstance =
    type === "INSTANCE" && !state.options.includeComponentInternals && !isRoot;
  ...
  if (shouldCollapseInstance) {
    ...
    if (instanceText.length)
      summary.text = uniqueStrings([...(summary.text ?? []), ...instanceText]).slice(0, 20);
    if (children.length)
...
function collectControls(summary: FigmaNodeSummary): CollectedControls {
  ...
  for (const node of flattenHierarchy(summary)) {
    ...
    if (
      ...
      /\b(button|continue|back|cancel|save|submit|done|next|previous|go back)\b/i.test(searchable)
    ) {
  ...
  if (type === "INSTANCE" && !options.includeComponentInternals) {
    for (const child of getChildren(record)) {
      ...
      if (childRecord.type === "TEXT")
        visit(childRecord, `${nextPath} > ${String(childRecord.name ?? "Text")}`);
    }
    return;
  }
...
function buildMetadata(state: SummaryState, summary?: FigmaNodeSummary): FigmaSummaryMetadata {
  ...
  if (
    (summary?.children?.length ?? 0) >= state.options.maxChildren ||
    truncatedReasons.some((reason) => reason.includes("Capped children"))
  ) {
    nextSteps.add("Inspect a specific child node by ID with figma_get_node_summary.");
  }
  if (
    truncatedReasons.some((reason) => reason.includes("depth limit")) &&
  ...
  const paints = value
    ...
    .map((paint) => {
      ...
      return compactObject({
        ...
        hex:
          record.type === "SOLID"
            ? colorToHex(asRecord(record.color), numberValue(record.opacity) ?? 1)
            : undefined,
        rgba:
          record.type === "SOLID"
            ? colorToRgba(asRecord(record.color), numberValue(record.opacity) ?? 1)
            : undefined,
        opacity: record.opacity,
...
function compactComponentProperties(value: FigmaValue): FigmaRecord | undefined {
  ...
  for (const [key, raw] of Object.entries(properties).slice(0, 30)) {
    const record = asRecord(raw);
    out[key] = compactObject({
      type: record.type,
      value: record.value,
      preferredValues: record.preferredValues,
    });
  }
  return Object.keys(out).length ? out : undefined;
}
