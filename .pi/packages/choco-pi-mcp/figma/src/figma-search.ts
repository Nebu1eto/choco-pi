import { asFigmaRecord, isStringValue, type FigmaRecord, type FigmaValue } from "./figma-values.ts";
...
function findNodes(
  ...
  function visit(
    ...
    if (type === "INSTANCE" && !options.includeComponentInternals && level > 0) {
      ...
      for (const child of getChildren(record)) {
        const childRecord = asFigmaRecord(child);
  ...
  if (skippedVectors)
    truncatedReasons.push(
      `Skipped ${skippedVectors} vector/icon node(s). Set includeVectors=true to include them.`,
    );
  if (skippedInstances)
    truncatedReasons.push(
      `Collapsed ${skippedInstances} component instance subtree/subtrees. Set includeComponentInternals=true for internals.`,
    );
  ...
  if (truncatedReasons.some((reason) => reason.includes("maxResults")))
    nextSteps.add("Raise maxResults or narrow the search with nodeId/depth.");
  if (
    truncatedReasons.some((reason) => reason.includes("depth limit")) &&
  ...
  return {
    ...
    metadata: {
      truncated:
        truncatedReasons.some((reason) => !reason.startsWith("Skipped")) ||
...
function compactParent(record: FigmaRecord, path: string): FigmaNodeSearchMatch["parent"] {
  return {
    id: stringValue(record.id),
