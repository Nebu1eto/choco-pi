...
export function buildFigmaTokenMap(stylesResponse: FigmaValue, variablesResponse: FigmaValue): FigmaTokenMap {
	...
	for (const [collectionId, raw] of Object.entries(rawCollections)) {
		...
		collections[collectionId] = {
			...
			modes: Array.isArray(record.modes) ? record.modes.map((mode) => ({ modeId: String(asFigmaRecord(mode).modeId ?? asFigmaRecord(mode).mode_id ?? ""), name: String(asFigmaRecord(mode).name ?? "Mode") })) : undefined,
		};
	}
