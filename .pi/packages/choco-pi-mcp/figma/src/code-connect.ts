...
export async function findCodeConnectMapping(
    ...
    for (const entry of await readdir(dir, { withFileTypes: true })) {
      if (entry.isDirectory()) {
        if (
          ...
          (entry.name.startsWith(".") && !entry.name.startsWith(".figma"))
        )
          continue;
  ...
  if (truncatedReasons.length)
    nextSteps.push("Narrow rootDir or raise maxMatches/maxFiles if you expect more mappings.");
  return {
    rootDir,
    matches,
    metadata: {
...
function classify(
  ...
  if (
    (/figma\.config\.|figma\.config\.(js|ts|mjs|cjs)$/.test(line) ||
      /figma\.config\./.test(lowerPath) ||
      /\.figma\./.test(lowerPath)) &&
    line.trim()
  )
    kinds.push("figma-config");
  if (line.includes(fileKey)) kinds.push("figma-file-reference");
  if (nodeId && (line.includes(nodeId) || (urlNodeId && line.includes(urlNodeId))))
...
function resolveRoot(cwd: string, rootDir?: string): string {
  ...
  if (resolved !== cwdResolved && !resolved.startsWith(`${cwdResolved}/`))
    throw new Error("rootDir must be inside the current working directory.");
  return resolved;
}
