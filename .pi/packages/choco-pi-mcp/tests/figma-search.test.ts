...
import { findNodesByName, findNodesByText } from "../figma/src/figma-search.ts";
...
test("findNodesByName supports partial, exact, and case-sensitive matching", async () => {
  ...
  assert.deepEqual(
    findNodesByName(node, { query: "button" }).matches.map((match) => match.name),
    ["Save button", "Button label"],
  );
  assert.equal(findNodesByName(node, { query: "save button", exact: true }).matches.length, 1);
  assert.equal(
    findNodesByName(node, { query: "save button", exact: true, caseSensitive: true }).matches
      .length,
    0,
  );
});
...
test("findNodesByName enforces depth and result caps", async () => {
  ...
  assert.ok(
    depthLimited.metadata.truncatedReasons.some((reason) => reason.includes("depth limit")),
  );
