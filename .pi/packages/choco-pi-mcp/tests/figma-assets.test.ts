...
import {
  ...
  safeFilename,
  sha256,
} from "../figma/src/figma-assets.ts";
...
test("collectAssetCandidates detects icons, node renders, image fills, and paths", () => {
  const result = collectAssetCandidates(assetTree, {
    assetTypes: ["svgIcons", "nodeRenders", "imageFills"],
  });
  assert.ok(
    result.assets.some(
      (asset) => asset.kind === "svgIcon" && asset.nodePath === "Hero Card > Close icon",
    ),
  );
  assert.ok(
    result.assets.some((asset) => asset.kind === "nodeRender" && asset.nodePath === "Hero Card"),
  );
  assert.ok(
    result.assets.some((asset) => asset.kind === "imageFill" && asset.imageRef === "abc123"),
  );
  assert.equal(
    result.assets.some((asset) => asset.nodeName === "Hidden asset"),
    false,
  );
});
