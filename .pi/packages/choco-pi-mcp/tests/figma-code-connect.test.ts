...
import { findCodeConnectMapping } from "../figma/src/code-connect.ts";
...
test("findCodeConnectMapping discovers figma.connect, URLs, and node IDs", async () => {
  ...
  await writeFile(
    join(root, "Button.figma.tsx"),
    "figma.connect(Button, 'https://www.figma.com/design/FILE123/Name?node-id=1-2')\n",
  );
  await writeFile(join(root, "README.md"), "component key COMPONENT123\n");
  const result = await findCodeConnectMapping({
    cwd: root,
    fileKey: "FILE123",
    nodeId: "1:2",
    componentKey: "COMPONENT123",
  });
...
test("findCodeConnectMapping rejects rootDir outside cwd", async () => {
  ...
  await assert.rejects(
    () => findCodeConnectMapping({ cwd: root, rootDir: "/", fileKey: "FILE123" }),
    /rootDir/,
  );
});
