...
import {
  ...
  summarizeNode,
} from "../figma/src/figma-summarizer.ts";
...
async function fixture(name: string): Promise<FigmaValue> {
  const path = join(import.meta.dirname, "..", "figma", "fixtures", name);
  return parseFigmaJson(await readFile(path, "utf8"));
}
...
test("summarizeNode excludes hidden nodes and vector internals by default", async () => {
  ...
  assert.equal(
    summary.children?.some((child) => child.name === "Hidden text"),
    false,
  );
  assert.equal(
    summary.children?.some((child) => child.name === "Search icon"),
    false,
  );
});
...
test("component instances collapse structural internals while retaining useful text", async () => {
  ...
  assert.ok(
    summary.metadata?.truncatedReasons.some((reason) =>
      reason.includes("Collapsed component instance"),
    ),
  );
});
...
test("extractVisibleText returns capped text metadata", async () => {
  ...
  assert.ok(
    result.metadata.nextSteps.includes(
...
test("getImplementationContext deterministically extracts typography colors spacing and controls", async () => {
  ...
  assert.ok(
    context.typography.some((entry) => entry.fontFamily === "Inter" && entry.fontSize === 18),
  );
...
test("depth and children caps produce truncation metadata", async () => {
  ...
  assert.ok(
    childLimited.metadata?.truncatedReasons.some((reason) => reason.includes("Capped children")),
  );
});
