...
test("buildResponsiveHints recommends fill, hug, fixed, and wrap behavior", async () => {
  ...
  assert.ok(
    hints.some(
      (hint) =>
        String(hint.name) === "Header Row" &&
        Array.isArray(hint.recommendations) &&
        hint.recommendations.some((rec) => String(rec).includes("width: 100%")),
    ),
  );
  assert.ok(
    hints.some(
      (hint) =>
        String(hint.name) === "Dashboard Card" &&
        Array.isArray(hint.recommendations) &&
        hint.recommendations.some((rec) => String(rec).includes("Fixed width")),
    ),
  );
});
...
test("implementation context includes layout, responsive, accessibility, tokens, and snippets", async () => {
  ...
  const context = getImplementationContext(node, {
    ...
    tokenMap: {
      styles: {
        "S:primary-fill": { name: "Color/Primary", type: "FILL" },
        "S:text-button": { name: "Typography/Button", type: "TEXT" },
      },
      variables: {
        "VariableID:color-primary": { name: "color.primary" },
        "VariableID:text-on-primary": { name: "color.onPrimary" },
        "VariableID:radius-md": { name: "radius.md" },
      },
      collections: {},
      warnings: [],
    },
