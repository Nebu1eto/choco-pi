...
export class GroupJoinManager {
  ...
  // choco-pi fork: constructor parameter properties are desugared to explicit
  // fields so every source file stays erasable-syntax-only and loads under
  // Node's type stripping (`node --test`, plain `node src/index.ts`) as well as
  // under pi's jiti loader. Assignment order and visibility are unchanged.
  private deliverCb: DeliveryCallback;
  private groupTimeout: number;

  constructor(deliverCb: DeliveryCallback, groupTimeout: number = DEFAULT_TIMEOUT) {
