...
import { createLensDiagnosticsTool } from "./tools/diagnostics-report-registration.ts";
...
function activateExtension(hostPi: ExtensionAPI) {
  ...
  type LspStatusTheme = {
    fg: (color: "accent" | "success" | "error" | "warning" | "dim", text: string) => string;
  };
  ...
  function getLensFlag(name: string, editedFilePath?: string): boolean | string | undefined {
  ...
  type LensWidgetFactory = (tui: LensWidgetTui, theme: LensWidgetTheme) => LensWidgetComponent;
   ...
   */
  function mountLensWidget(ui: LensWidgetUi | undefined, mode: ExtensionRunMode): boolean {
  ...
  pi.registerCommand("lens-toggle", {
    description: "Toggle choco-pi-lsp on/off for the current session. Usage: /lens-toggle",
  ...
  // choco-pi fork: human-facing LSP usage toggle. Wires into the SAME
  // machinery as the upstream `--no-lsp` flag (`lsp.enabled` config key):
  // a runtime override makes every `getLensFlag("no-lsp")` consumer — the
  // dispatch pipeline, runtime hooks, lsp_diagnostics/lsp_navigation tools,
  // and the footer status — see the change immediately, and the persisted
    ...
    handler: async (args, ctx) => {
      ...
      const repaintLspStatus = () => {
        try {
          ...
          if (ui?.setStatus && ui.theme) updateLspStatus(ui.setStatus, ui.theme);
        } catch (err) {
          dbg(`/lsp status repaint skipped: ${err}`);
        }
      };
      switch (subcommand) {
        case "on": {
          setRuntimeLensFlagOverride("no-lsp", false);
          const persisted = persistPiLensGlobalConfigKey("lsp.enabled", true);
          repaintLspStatus();
          notifyUi(
            ctx,
            persisted
        ...
        case "off": {
          setRuntimeLensFlagOverride("no-lsp", true);
          const persisted = persistPiLensGlobalConfigKey("lsp.enabled", false);
          // Actually release the running language servers, not just gate new
          // touches — same teardown the idle timer uses.
          resetLSPService({ fast: true, reason: "user_lsp_off" });
          repaintLspStatus();
          notifyUi(
            ctx,
            persisted
        ...
        case undefined:
        case "status": {
          const disabled = getLensFlag("no-lsp") === true;
          let detail: string;
          if (disabled) {
            detail = "language servers are not started and LSP tools report disabled";
          } else {
            const { activeIds, failedIds } = selectLspStatus(
              getLSPService().getAliveServerIds(),
              getFailedLspServerIds(),
            ...
            detail =
              activeIds.length > 0
                ...
                : "LSP Inactive (no server running for this session yet)";
            if (failedIds.length > 0) {
              detail += ` · LSP Failed: ${failedIds.join(", ")}`;
            }
          }
          notifyUi(
            ctx,
            `LSP usage: ${disabled ? "off" : "on"} — ${detail}. Use /lsp on|off to change.`,
            "info",
          );
          return;
        }
        default:
          notifyUi(ctx, "Usage: /lsp on|off|status", "warning");
      }
    },
  });
  ...
  pi.registerCommand("lens-widget-toggle", {
    ...
    handler: async (_args, ctx) => {
      ...
      const changed = nextVisible ? mountLensWidget(ctx.ui, mode) : unmountLensWidget(ctx.ui);
      if (!changed) {
        notifyUi(ctx, "choco-pi-lsp widget is not supported by this pi version.", "warning");
        return;
      }
  ...
  pi.registerCommand("lens-health", {
    ...
    handler: async (_args, ctx) => {
      ...
      const dispatchIntegration = loadedDispatchIntegration ?? (await loadDispatchIntegration());
    ...
    handler: async (_args, ctx) => {
      try {
        ...
        const degradationText = degradations.length
          ...
          : "";
        notifyUi(ctx, `${renderLatencyPerformanceReport(report)}${degradationText}`, "info");
      } catch (err) {
        notifyUi(
          ctx,
  ...
  pi.registerCommand("lens-tools", {
    ...
    handler: async (_args, ctx) => {
      ...
      if (bySource["choco-pi-lsp-auto"].length > 0) {
        lines.push("", `🤖 Auto-installed (${bySource["choco-pi-lsp-auto"].length}):`);
  ...
  pi.registerCommand("lens-allow-edit", {
    ...
    handler: async (args, ctx) => {
      ...
      notifyUi(ctx, `Read guard override armed for next edit: ${targetPath}`, "info");
    },
  });
  ...
  const alwaysActiveTools = [
    ...
    createLspDiagnosticsTool(
      ...
      // choco-pi fork: the /lsp runtime toggle (and --no-lsp / lsp.enabled)
      // makes the tool report disabled instead of spawning servers.
      (name: string) => getLensFlag(name),
    ),
  ...
  (pi as any).on("tool_result", async (event: any, ctx: any) => {
    ...
    try {
      ...
      for (const [index, dispatchEvent] of dispatchEvents.entries()) {
        ...
        if (dispatchEvent.toolName === "edit" || dispatchEvent.toolName === "write") {
          logLatency({
            type: "phase",
        ...
        if (index === 0) {
          ...
          nestedContent.push(...result.content.slice(dispatchEvent.content.length));
  ...
  pi.on("turn_end", async (_event: any, ctx) => {
    ...
    try {
      ...
      if (shouldLogLoopBlock(loopMaxMs)) {
        ...
        const isNewSessionWorst = shouldLogWorstBlock(loopMaxMs, lastLoggedLoopWorstMs);
      ...
      await flushDebouncedToolResults();
      const { testRunnerClient } = await loadBootstrapClients();
      await handleTurnEnd({
        ctxCwd: ctx.cwd,
        getFlag: (name: string) => getLensFlag(name),
        dbg,
  ...
  const isPlainUserPrompt = (msg: { role: string; content: unknown }): boolean => {
