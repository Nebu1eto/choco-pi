...
the first diagnostics/navigation request, except that an explicit non-empty
`warmFiles` project setting remains its own prewarm opt-in. TypeScript automatic
type acquisition is also off by default to prevent implicit npm network activity;
opt in with `typeAcquisition.enabled=true` or
`CHOCO_PI_LSP_TYPE_ACQUISITION=1`. The asynchronous TypeScript spawn path reads
that global setting without blocking and memoizes it by config path for the
process lifetime.
