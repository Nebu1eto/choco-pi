...
async function restoreQuarantinedLock(lockPath: string, quarantined: string): Promise<void> {
  try {
...
async function releaseQuarantineLock(lockPath: string, token: string): Promise<void> {
...
function quarantineOwnerIsStale(owner: QuarantineLockOwner, staleMs: number): boolean {
  return (
...
async function reclaimQuarantineLock(lockPath: string, staleMs: number): Promise<boolean> {
...
export async function acquireQuarantinePidFileLock(
  lockPath: string,
  options: QuarantinePidFileLockOptions &
    ({ onContention?: "throw" } | { onContention: "skip-log"; logContention: () => void }),
): Promise<(() => Promise<void>) | null> {
