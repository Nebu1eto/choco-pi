...
import type { OpengrepFinding, OpengrepResult } from "../../opengrep-client.ts";
import type { ProjectDiagnostic, ProjectDiagnosticSeverity } from "../types.ts";
...
export function opengrepFindingToProjectDiagnostic(
  ...
  return {
    filePath: path.isAbsolute(finding.path) ? finding.path : path.resolve(cwd, finding.path),
