...
export interface TreeSitterParseCacheStats extends TreeCacheStats, TreeSitterParserCounters {}
...
export class TreeSitterClient {
  ...
  private queryBatchCacheCap(): number {
    const value = Number.parseInt(
      process.env.CHOCO_PI_LSP_TREE_SITTER_QUERY_BATCH_CACHE_CAP ?? "",
      10,
    );
    return Number.isSafeInteger(value) && value > 0
      ? value
  ...
  private parseCacheMeasurement = new AsyncLocalStorage<ParseCacheMeasurement>();
  ...
  private recordParserCounter(key: keyof TreeSitterParserCounters, amount = 1): void {
   ...
   */
  private reportPoisonedGrammarFile(candidate: string, grammarFile: string): void {
  ...
  private findGrammarsDir(): string {
    ...
    if (grammarsDir && fs.existsSync(path.join(grammarsDir, "tree-sitter-typescript.wasm"))) {
      return grammarsDir;
    }
  ...
  private async fetchGrammar(grammarFile: string): Promise<boolean> {
    ...
    if (ok) {
      ...
      this.recordGrammarFailure(
        grammarFile,
        reason ?? "The package manager skipped install scripts and the runtime download failed.",
        retryable,
      );
    }
    return ok;
   ...
   */
  private recordGrammarFailure(grammarFile: string, detail: string, retryable: boolean): void {
    ...
    const retryDelayMs = retryable ? transientRetryDelayMs(attempts, "probe-timeout") : undefined;
    this.grammarRetryAtMs.set(
      grammarFile,
      retryDelayMs === undefined ? Number.POSITIVE_INFINITY : Date.now() + retryDelayMs,
    );
  ...
  private async compileQueryBatch(
    ...
    const identity = crypto.createHash("sha256").update(JSON.stringify(queryDefs)).digest("hex");
  ...
  private containsYieldInFunctionBody(node: TreeSitterNode, root: TreeSitterNode = node): boolean {
  ...
  private isLikelySqlAlchemyReceiver(text: string): boolean {
    ...
    return new Set(["session", "db_session", "async_session", "sync_session"]).has(
      tail.toLowerCase(),
    );
  }
   ...
   */
  private bodyHasLoopExit(node: TreeSitterNode, insideNestedLoop: boolean): boolean {
   ...
   */
  private resolvesToFileLiteralConst(name: string, rootNode: TreeSitterNode): boolean {
   ...
   */
  private resolveFileConstValueNode(name: string, rootNode: TreeSitterNode): TreeSitterNode | null {
   ...
   */
  private isFixedUrlBaseExpr(base: TreeSitterNode, rootNode: TreeSitterNode): boolean {
   ...
   */
  private isFixedBaseIdentifier(ident: TreeSitterNode, rootNode: TreeSitterNode): boolean {
   ...
   */
  private isShadowedByEnclosingParam(node: TreeSitterNode, name: string): boolean {
  ...
  private patternBindsName(pattern: TreeSitterNode, name: string): boolean {
    ...
    while (stack.length > 0) {
      ...
      if (!n) continue;
      if (
        (n.type === "identifier" || n.type === "shorthand_property_identifier_pattern") &&
        n.text === name
      ) {
        return true;
      }
   ...
   */
  private resolvesToFixedNewUrlConst(name: string, rootNode: TreeSitterNode): boolean {
    ...
    const isUrlCtor =
      ctorName === "URL" ||
      (ctor?.type === "identifier" && this.importedAs(ctorName, rootNode) === "URL");
  ...
  private applyPostFilter(
    ...
    switch (postFilter) {
      ...
      case "unsafe_regex_dynamic_identifier": {
        ...
        try {
          ...
          const hasEscapeSignal = (text: string): boolean =>
            /\b(?:escape|Escape)\w*\s*\(/.test(text) || /\.\s*replace\s*\(\s*\//.test(text);

          // Track the last write to the identifier before the use site.
          let lastWriteRow = -1;
          let lastWriteSafe = false;
          const consider = (row: number, valueText: string | undefined): void => {
            if (row >= useRow || row < lastWriteRow) return;
            lastWriteRow = row;
            lastWriteSafe = valueText !== undefined && hasEscapeSignal(valueText);
          };
      ...
      case "is_generator_with_valued_return": {
        ...
        const functionNode =
          captures.FUNCTION ??
          (returnNode ? this.navigator.findParent(returnNode, ["function_definition"]) : undefined);
      ...
      case "status_204_with_value_return": {
        ...
        while (queue.length > 0) {
          ...
          if (node.type === "return_statement") {
            ...
            const hasValue = node.children.some((c: any) => c.isNamed && c.type !== "comment");
      ...
      case "python_empty_except": {
        ...
        return !bodyNode.children.some(
          (c: any) => c.isNamed && c.type !== "pass_statement" && c.type !== "comment",
        );
      }
      ...
      case "torchscript_super_call": {
        ...
        const isTorchScriptDecorated = (node: TreeSitterNode | null | undefined): boolean => {
          const decorated = node?.parent;
          if (!decorated || decorated.type !== "decorated_definition") return false;
          // biome-ignore lint/suspicious/noExplicitAny: tree-sitter node
          return (decorated.children ?? []).some(
            (c: any) =>
              c.type === "decorator" && /^@(torch\.jit\.script|jit\.script)$/.test(c.text ?? ""),
          );
        };
        const methodNode = this.navigator.findParent(call, ["function_definition"]);
        if (!methodNode) return false;
        const classNode = this.navigator.findParent(methodNode, ["class_definition"]);
      ...
      case "ruby_empty_rescue": {
        ...
        // biome-ignore lint/suspicious/noExplicitAny: tree-sitter node
        return !bodyNode.children.some(
          (c: any) => c.isNamed && !["comment", "nil", "nil_literal"].includes(c.type),
        );
      }
      ...
      case "ts_ssrf_sink": {
        ...
        const allowedFns = new Set(["fetch", "request", "get", "post", "put", "patch", "delete"]);
        ...
        return new Set(["axios", "http", "https", "got", "request", "superagent", "undici"]).has(
          obj,
        );
      }
      ...
      case "incomplete_assertion": {
        ...
        while (current && current.type === "member_expression") {
          const propNode = current.children?.find((c: any) => c.type === "property_identifier");
          ...
          if (parent.type === "expression_statement" || parent.type === "return_statement") {
            if (lastPropertyName && CHAI_PROPERTY_ASSERTIONS.has(lastPropertyName)) return false;
            return true;
          }
      ...
      case "c_memset_sensitive_arg": {
        ...
        const argList = callNode.children?.find((c: any) => c.type === "argument_list");
