...
function updateGraphBuildInfo(graph: ReviewGraph, patch: Partial<GraphBuildInfo>): GraphBuildInfo {
...
export function _setReviewGraphBuildGateForTests(gate: (() => Promise<void>) | undefined): void {
 ...
 */
async function sourceSignatureMapAsync(files: string[]): Promise<Map<string, string>> {
...
export function getReviewGraphMaxFiles(cwd?: string): number {
  const override = Number.parseInt(process.env.CHOCO_PI_LSP_REVIEW_GRAPH_MAX_FILES ?? "", 10);
...
function getReviewGraphMaxFileBytes(): number {
  const override = Number.parseInt(process.env.CHOCO_PI_LSP_REVIEW_GRAPH_MAX_FILE_BYTES ?? "", 10);
...
export function getLastReviewGraphBuildAttempt(cwd: string): ReviewGraphBuildAttempt | undefined {
...
function recordBuildAttempt(
  ...
  if (prior?.buildId !== undefined && (buildId === undefined || buildId < prior.buildId)) {
    return;
  }
...
function recordPersistFailure(
  ...
  if (belongsToCurrentBuild && (prior === undefined || prior.outcome === "succeeded")) {
    recordBuildAttempt(
      cwd,
      "succeeded",
...
function isReviewGraphSizeNearMiss(sourceFileCount: number, maxFileCount: number): boolean {
  return (
...
export function _setReviewGraphEntryBudgetForTests(maxScanEntries?: number): void {
  _reviewGraphEntryBudgetForTests = maxScanEntries;
}

let _reviewGraphEntryCounterForTests: (() => void) | undefined;
export function _setReviewGraphEntryCounterForTests(counter?: () => void): void {
  _reviewGraphEntryCounterForTests = counter;
}

export async function getGraphSourceFiles(cwd: string): Promise<GraphSourceFilesResult> {
  ...
  const { files: collected, entryBudgetExceeded } = await collectProjectSourceFilesWithBudgetAsync(
    cwd,
    {
...
const _workerRequests = new Map<number, { key: string; pending: PendingPersist }>();
...
function persistedData(pending: PendingPersist): PersistedGraphData {
  return {
    ...
    fileHashes: pending.fileHashes ? Array.from(pending.fileHashes.entries()) : undefined,
...
function countRetainedSourceFiles(graph: ReviewGraph, sourceFilePaths?: Iterable<string>): number {
 ...
 */
function loadReviewGraphCheckpoint(cwd: string): LoadedReviewGraphCheckpoint | null {
  const checkpointPath = reviewGraphCheckpointPath(cwd);
  let data: ReviewGraphCheckpointData;
  try {
...
export async function waitForReviewGraphPersistsForTests(): Promise<void> {
  for (
    let attempts = 0;
    attempts < 200 && (_workerRequests.size > 0 || _checkpointWorkerRequests.size > 0);
    attempts++
  ) {
...
export function getReviewGraphWorkerFallbackReasonForTests(): string | undefined {
...
function addJsTsFile(
  ...
  for (const fn of functions) {
    ...
    for (const callee of fn.outgoingCalls) {
      ...
      const importHintFile = !callee.includes(".") ? importedNameToFile.get(callee) : undefined;
      addEdge(graph, {
        from: symbolId,
        to: targetId,
        kind: "calls",
...
async function extractTreeSitterSymbols(
  ...
  const extracted = await treeSitterClient.withParsedTree(filePath, languageId, content, (tree) =>
    extractor.extract(tree, filePath, content),
  );
...
export async function captureReviewGraphStructuralIr(
  ...
  if (kind === "jsts") {
    ...
    return {
      ...
      structural: {
        ...
        reexports: facts.getFileFact<ReExportEntry[]>(filePath, "file.reexports") ?? [],
        functionSummaries:
  ...
  const result = await client.withParsedTree(filePath, languageId, content, (tree) =>
    extractor.extract(tree, filePath, content),
  );
...
function addTreeSitterFile(
  ...
  for (const imp of extracted.imports) {
    ...
    // ignored-file node.
    const resolved = resolveImportToFiles(cwd, filePath, languageId, imp.source).filter(
      (target) => !ignoredIds?.has(target),
    );
    if (resolved.length > 0) {
      for (const target of resolved) {
        const toNode = ensureFileNode(
          ...
          target,
          cwd,
          mapKindToTreeSitterLanguage(detectFileKind(target), target) ?? languageId,
        );
        addEdge(graph, {
          from: fileNodeId,
          to: toNode,
...
export function addLspFallbackSymbols(
  ...
  const visit = (items: LSPSymbol[], parentId: string, ancestry: string[]): void => {
...
function resolveCxxInclude(cwd: string, filePath: string, source: string): string | undefined {
  const candidates = [
...
function removeFileOwnedGraphData(graph: ReviewGraph, filePath: string): ReviewGraphEdge[] {
  const normalized = normalizeMapKey(filePath);
  const fileNodeId = `file:${normalized}`;
...
async function addFileToGraph(
  ...
  if (kind === "jsts") {
    ...
    const dispatchOwnsContent =
      facts.getFileFact<string>(file, "file.content") !== undefined && contentOverride == null;
    try {
  ...
  const irExtracted =
    sharedIr?.kind === "tree-sitter" && sharedIr.languageId === languageId
      ...
      : undefined;
  const extracted =
    irExtracted ?? (await extractTreeSitterSymbols(file, languageId, contentOverride));
...
function restoreValidIncomingEdges(graph: ReviewGraph, edges: ReviewGraphEdge[]): void {
...
export function getGraphImportChanges(graph: ReviewGraph): GraphImportDelta | undefined {
...
type SeqFastpathResult = { graph: ReviewGraph } | { fallback: SeqFastpathFallback };
...
async function trySeqFastpath(
  ...
  const changedSet = new Set(
    seqHint.getFilesChangedSince(cached.builtAtProjectSeq).map((file) => normalizeMapKey(file)),
  );
...
async function _doBuildGraph(
  ...
  let memCached = sourceFilesTruncated ? undefined : _workspaceGraphCache.get(normalizedCwd);
  ...
  const testStopAfter = Number(process.env.CHOCO_PI_LSP_GRAPH_CHECKPOINT_TEST_STOP_AFTER);
    ...
    for (const file of filesToExtract) {
      ...
      if (Number.isFinite(testStopAfter) && testStopAfter > 0 && extractedCount >= testStopAfter) {
        writeReviewGraphCheckpointSync(
          cwd,
          buildReviewGraphCheckpointData(cwd, graph, fileHashes, filesToBuild.length, ignoredIds),
          {
            nodes: graph.nodes.size,
            edges: graph.edges.length,
...
export function extractSymbolsAndRefsFromGraph(graph: ReviewGraph): {
  ...
  for (const edge of graph.edges) {
    ...
    const resolution =
      edge.resolution ??
      (targetNode && !targetNode.metadata?.unresolvedName ? "exact" : "unresolved");
