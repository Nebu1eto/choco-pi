...
interface LspServiceLike {
  ...
  implementation: (f: string, line: number, character: number) => Promise<LSPLocation[]>;
}
...
function lspLocationsToUsedBy(locs: LSPLocation[], cap: number): ModuleSymbolUsedBy[] {
...
async function enrichModuleReportWithLspNow(
  ...
  const byName = new Map<string, { usedBy?: ModuleSymbolUsedBy[]; hasImpl?: boolean }>();
