 ...
 */
export function getAstGrepRuleSources(projectRoot = process.cwd()): AstGrepRuleSource[] {
...
export function shippedRuleDirsInPrecedenceOrder(projectRoot = process.cwd()): string[] {
...
function materializeMergedRuleDir(sources: RuleSourceSnapshot[], mergedDir: string): void {
  ...
  for (const [sourceIndex, source] of sources.entries()) {
    ...
    for (const [fileIndex, file] of source.files.entries()) {
      ...
      if (survivingDocuments.length === file.documents.length) {
        ...
        fs.writeFileSync(
          destination,
          `${survivingDocuments.map((document) => document.raw.trim()).join("\n---\n")}\n`,
        );
      }
    }
 ...
 */
export function resolveBaselineSgconfig(projectRoot = process.cwd()): string | undefined {
