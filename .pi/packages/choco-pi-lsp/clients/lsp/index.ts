...
const NAV_CLIENT_WAIT_TIMEOUT_MS = Math.max(
  0,
  Number.parseInt(process.env.CHOCO_PI_LSP_LSP_NAV_CLIENT_WAIT_MS ?? "1500", 10) || 1500,
);
const TOUCH_DEBOUNCE_MS = Math.max(
  0,
  Number.parseInt(process.env.CHOCO_PI_LSP_LSP_TOUCH_DEBOUNCE_MS ?? "1500", 10) || 1500,
);
...
const RENAME_NOTIFY_TIMEOUT_MS = Math.max(
  50,
  Number.parseInt(process.env.CHOCO_PI_LSP_LSP_RENAME_NOTIFY_TIMEOUT_MS ?? "1500", 10) || 1500,
);
...
export function getTypeScriptIdleEvictMs(): number {
  ...
  return Number.isSafeInteger(parsed) && parsed > 0 ? parsed : DEFAULT_TS_IDLE_EVICT_MS;
}
...
function warmupTimeoutMs(): number {
  const raw = Number.parseInt(process.env.CHOCO_PI_LSP_LSP_WARMUP_TIMEOUT_MS ?? "", 10);
...
function warmupRetryBackoffMs(): number {
  const raw = Number.parseInt(process.env.CHOCO_PI_LSP_LSP_WARMUP_RETRY_BACKOFF_MS ?? "", 10);
...
const DIAGNOSTICS_SEMANTIC_SETTLE_THRESHOLD_MS = Math.max(
  0,
  Number.parseInt(process.env.CHOCO_PI_LSP_LSP_DIAGNOSTICS_SEMANTIC_THRESHOLD_MS ?? "250", 10) ||
    250,
);
const DIAGNOSTICS_SEMANTIC_SETTLE_WAIT_MS = Math.max(
  0,
  Number.parseInt(process.env.CHOCO_PI_LSP_LSP_DIAGNOSTICS_SEMANTIC_SETTLE_MS ?? "400", 10) || 400,
);
...
const EARLY_UNBLOCK_GRACE_MS = Math.max(
  0,
  Number.parseInt(process.env.CHOCO_PI_LSP_LSP_EARLY_UNBLOCK_GRACE_MS ?? "400", 10) || 400,
);
...
const WORKSPACE_SWEEP_EXCLUDED_SERVER_IDS: ReadonlySet<string> = new Set(["opengrep"]);
...
function getMaxWorkspaceDiagnosticFiles(): number {
  const override = Number.parseInt(process.env.CHOCO_PI_LSP_LSP_WORKSPACE_MAX_FILES ?? "", 10);
...
export class LSPService {
  ...
  private readonly lastDiagnosticsHealth = new Map<string, LSPDiagnosticsHealth>();
  /**
   ...
   */
  private readonly typeScriptIdleTimers = new Map<string, ReturnType<typeof setTimeout>>();
  ...
  private async resolveServerRoot(
    ...
    for (const key of new Set([...this.state.clients.keys(), ...this.state.inFlight.keys()])) {
      if (!key.startsWith(prefix)) continue;
      const ancestorKey = key.slice(prefix.length);
      const pathApi = isWindowsPath(ancestorKey) || isWindowsPath(rootKey) ? path.win32 : path;
  ...
  private async clientKeyFor(entry: SpawnedServer, filePath: string): Promise<string | undefined> {
  ...
  private async brokenSkippedAuxiliaryServerIds(
    ...
    const enabledAuxiliaries =
      clientScope === "with-auxiliary" ? new Set(options.auxiliaryServerIds ?? []) : undefined;
  ...
  async getClientForFile(
    ...
    try {
      waitResult = await Promise.race<SpawnedServer | undefined | typeof timeoutSentinel>([
        withBudget(),
   ...
   */
  async getWarmClientForFile(filePath: string): Promise<SpawnedServer | undefined> {
    if (this.checkDestroyed()) return undefined;
    const servers = getServersForFileWithConfig(filePath);
    for (const server of servers) {
      const root = await this.resolveServerRoot(server, filePath);
   ...
   */
  async isServerAliveForFile(serverId: string, filePath: string): Promise<boolean> {
        ...
        if (this.recordRuntimeExitWindow(key, exitedAt, uptimeMs)) {
          ...
          this.state.broken.set(
            ...
            Math.max(
              ...
              Date.now() +
                (isOptionalServer ? OPTIONAL_LSP_RETRY_COOLDOWN_MS : BROKEN_MAX_COOLDOWN_MS),
            ),
          );
          if (isOptionalServer) {
  ...
  > {
    if (this.checkDestroyed()) {
      ...
      logLatency({
        ...
        metadata: {
          ...
          serverCountReady: 0,
          clientScope:
            options.clientScope ?? (destroyedDiagnosticsMode === "full" ? "all" : "primary"),
    ...
    if (useAllClients) {
      ...
      spawned = entry ? [entry] : [];
      serverCountAttempted =
        spawned.length > 0 ? 1 : getServersForFileWithConfig(filePath).length > 0 ? 1 : 0;
    }
    if (spawned.length === 0) {
      logLatency({
        type: "phase",
    ...
    try {
      ...
      const readPathVersion = (client: (typeof spawned)[number]["client"]): number | undefined =>
        client.getDiagnosticsVersionForPath?.(filePath);
      ...
      // equals a hash.
      const bindingMatchesTouchContent = (binding: StoredDiagnosticBinding | undefined): boolean =>
        binding?.contentHash === touchContentHash;
      ...
      const primaryNotifyWriteTimedOutServerIds = notifyWriteTimedOutServerIds.filter((id) =>
      ...
      if (diagnosticsMode !== "none") {
        ...
        if (
          ...
          getStrategy(spawned[0].client.serverId, spawned[0].client.getLaunchVariant?.())
            .silentOnClean === true
        ) {
          try {
        ...
        const perServerWaits = spawned.map((entry) => {
          ...
          const pullOnly =
            classifyServerWaitTier(
              entry.client.serverId,
              pressureSnapshots.find((snapshot) => snapshot.serverId === entry.client.serverId),
            ) === "pull-capable";
        ...
        if (tsserverSyncConfirmed !== undefined) {
          ...
          const answeredForThisTouch = (entry: (typeof spawned)[number]): boolean => {
            const baseline = diagnosticBaselines.get(entry.client);
            const currentPathVersion = readPathVersion(entry.client);
            if (
              baseline !== undefined &&
          ...
          diagnosticsTimedOut =
            !hasWaitedPrimary || diagnosticsUnansweredPrimaryServerIds.length > 0;
              ...
              if (allSilent) {
                ...
                const liveness = await Promise.all(
                  outstanding.map((entry) =>
                    (entry.client.pingLiveness?.() ?? Promise.resolve(true)).catch(() => false),
                  ),
                );
      ...
      if (
        ...
        try {
          ...
          if (classifyCascadeWaitTier(this, filePath, snapshots) === "tier3-silent") {
            ...
            const alive = await (spawned[0].client.pingLiveness?.() ?? Promise.resolve(true)).catch(
              () => false,
            );
            if (alive) {
              diagnosticsTimedOut = false;
  ...
  async getDiagnostics(
    ...
    const perServer = await raceToCompletion(
      ...
      (results) =>
        results.some(
          (r) => r.diagnosticCount > 0 || getStrategy(r.serverId, r.launchVariant).seedFirstPush,
        ),
      {
        timeoutMs: Math.max(
          ...spawned.map(
            (entry) =>
              getStrategy(entry.info.id, entry.client.getLaunchVariant?.()).aggregateWaitMs,
          ),
        ),
        graceMs,
   ...
   */
  async references(filePath: string, line: number, character: number, includeDeclaration = true) {
    const spawned = await this.getClientForFile(filePath, NAV_CLIENT_WAIT_TIMEOUT_MS);
    if (!spawned) return [];
   ...
   * If filePath is provided, probes that server; otherwise uses first active client.
   */
  async getCapabilitySnapshots(filePath?: string): Promise<LSPCapabilitySnapshot[]> {
    if (this.checkDestroyed()) return [];
   ...
   */
  async rename(filePath: string, line: number, character: number, newName: string) {
    const spawned = await this.getClientForFile(filePath, NAV_CLIENT_WAIT_TIMEOUT_MS);
    if (!spawned) return null;
  ...
  async renameFile(
    ...
    const relOld =
      path.relative(cwd, oldFilePath).replace(/\\/g, "/") || path.basename(oldFilePath);
    const relNew =
      path.relative(cwd, newFilePath).replace(/\\/g, "/") || path.basename(newFilePath);
          ...
          () =>
            opened?.oldUri
              ? client.didRenameFiles(
                  ...
                  newFilePath,
                  opened.oldUri,
                  destinationUriPreservingSpelling(opened.oldUri, oldFilePath, newFilePath),
                )
        ...
        results: [
          {
            ...
            appliedOperationTotal:
              applied.appliedOperationTotal + renameApplied.appliedOperationTotal,
            ...
            operationCounts: {
              textEdits:
                applied.operationCounts.textEdits + renameApplied.operationCounts.textEdits,
  ...
  async ensureWarmForSweep(
    ...
    const keys = await Promise.all(
      servers.map((server) => this.demonstratedReadyKeyFor(server, representativeFile)),
    );
  ...
  private async runWorkspaceDiagnosticsSwept(
    ...
    const workspaceSweepScopeKey = buildScopeKey("all", [...WORKSPACE_SWEEP_EXCLUDED_SERVER_IDS]);
    ...
    const preOpenGroupFiles = async (groupFiles: readonly string[]): Promise<void> => {
    ...
    const processFile = async (filePath: string): Promise<void> => {
      try {
        const content =
          contentCache.get(filePath) ?? (await nodeFs.promises.readFile(filePath, "utf-8"));
    ...
    const unconfirmedByReason: Partial<Record<LSPWorkspaceUnconfirmedReason, number>> = {};
  ...
  > {
    ...
    const bindingsByPath = new Map<string, (StoredDiagnosticBinding | undefined)[]>();
    const now = Date.now();
    for (const [_key, client] of this.state.clients) {
      ...
      client.pruneDiagnostics(
        (filePath, ts) => !existingPaths.has(filePath) || now - ts > CASCADE_DIAGNOSTICS_TTL_MS,
      );
  ...
  getBrokenStatus(): Array<{
    ...
    const keys = new Set([...this.state.broken.keys(), ...this.permanentlyBroken]);
    return [...keys].map((key) => {
      const separator = key.indexOf(":");
      return {
 ...
 * binary.
 */
export async function isAuxiliaryLspAlive(serverId: string, filePath: string): Promise<boolean> {
 ...
 */
export async function notifyExternalFileChange(filePath: string, type: number): Promise<void> {
