 ...
 */
function extractSpawnMarker(args: readonly string[] | undefined): string | undefined {
...
export interface LSPClientInfo {
  ...
  typeDefinition(filePath: string, line: number, character: number): Promise<LSPLocation[]>;
  /** Go to the declaration of the symbol at a position */
  declaration(filePath: string, line: number, character: number): Promise<LSPLocation[]>;
  /** Find all references */
  references(
    filePath: string,
    line: number,
...
const INITIALIZE_TIMEOUT_MS = positiveIntFromEnv("CHOCO_PI_LSP_LSP_INIT_TIMEOUT_MS", 15_000); // 15s — npx downloads are handled by ensureTool, not here
...
const EXIT_NOTIFY_TIMEOUT_MS = positiveIntFromEnv("CHOCO_PI_LSP_LSP_EXIT_NOTIFY_TIMEOUT_MS", 1000);
...
export interface LSPClientState {
  ...
  readonly documentContentHashes: Map<string, { version: number; hash: string; text?: string }>;
 ...
 */
function mergeDiagnosticLists(...lists: (LSPDiagnostic[] | undefined)[]): LSPDiagnostic[] {
...
function pullSourceKey(normalizedPath: string, identifier: string | undefined): string {
...
function pullSourceKeysForPath(state: LSPClientState, normalizedPath: string): string[] {
...
function pullGenerationFor(state: LSPClientState, normalizedPath: string): number {
  return state.pullGenerations?.get(normalizedPath) ?? 0;
}

function bumpPullGeneration(state: LSPClientState, normalizedPath: string): void {
...
function claimPullRequestSequence(state: LSPClientState, sourceKey: string): number {
...
function isNewestPullRequest(state: LSPClientState, sourceKey: string, sequence: number): boolean {
 ...
 */
export function pullDiagnosticSources(state: LSPClientState): (string | undefined)[] {
...
export function bumpDiagnosticsVersion(state: LSPClientState, normalizedPath: string): void {
...
export function diagnosticsVersionForPath(state: LSPClientState, normalizedPath: string): number {
...
export function setupIncomingHandlers(
  ...
  state.connection.onNotification(
    ...
    (params: { uri: string; diagnostics?: LSPDiagnostic[]; version?: number }) => {
      ...
      if (strategy.seedFirstPush && !state.pushDiagnostics.has(normalizedPath)) {
  ...
  const runRefreshRepullPool = async (): Promise<void> => {
    do {
      ...
      const toRepull =
        state.workspaceDiagnosticsSupport.mode === "pull" ? [...state.openDocuments] : [];
  ...
  state.connection.onRequest("workspace/diagnostic/refresh", async () => {
    ...
    if (state.workspaceDiagnosticsSupport.mode === "pull" && state.openDocuments.size > 0) {
...
async function clientRequestPullDiagnostics(
  ...
  const settled = await raceToCompletion(
    ...
    (results) => results.some((r) => r.status === "found" && r.primaryCount > 0),
    {
      ...
      timeoutMs:
        Math.max(1, Math.min(PULL_REQUEST_TIMEOUT_MS, budgetMs)) + PULL_RACE_BACKSTOP_MARGIN_MS,
    },
  );
...
> {
  ...
  const previousResultIds = Array.from(state.workspacePullResultCache.values()).map((entry) => ({
    uri: entry.uri,
...
export async function clientWaitForDiagnostics(
  ...
  const hasFreshDiagnostics = (): boolean =>
    minVersion === undefined || diagnosticsVersionForPath(state, normalizedPath) > minVersion;
  ...
  if (state.workspaceDiagnosticsSupport.mode === "pull") {
    ...
    const strategy = getStrategy(state.serverId, state.launchVariant);
    const retryBudgetMs =
      strategy.pullRetryBudgetMs > 0 ? Math.min(timeoutMs, strategy.pullRetryBudgetMs) : 0;
...
export async function handleNotifyOpen(
  ...
  if (state.openDocuments.has(normalizedPath) || state.pendingOpens.has(normalizedPath)) {
    ...
    if (getStrategy(state.serverId, state.launchVariant).reopenOnResync) {
      ...
      if (!isClientAlive(state)) return;
      const reopenSent = await safeSendNotification(state.connection, "textDocument/didOpen", {
  ...
  const openSent = await safeSendNotification(state.connection, "textDocument/didOpen", {
  ...
  void probeTsserverProjectIdentity({
    ...
    probedFiles: state.projectIdentityProbedFiles ?? (state.projectIdentityProbedFiles = new Set()),
    commandChannel: {
...
export async function closeDocument(state: LSPClientState, filePath: string): Promise<void> {
  if (!isClientAlive(state)) return;
  const normalizedPath = normalizeMapKey(filePath);
...
export async function navRequest<T>(
  ...
  const requestVersion =
    normalizedPath !== undefined ? state.documentVersions.get(normalizedPath) : undefined;
...
export async function createLSPClient(options: {
  ...
  const recentStderr = (lines = 10): string => stderrRing.slice(-lines).join("").trim();
  ...
  const streamErrorHandler = (_label: string) => (err: Error & { code?: string }) => {
    if (
  ...
  return {
    ...
    getTrackedDiagnosticPaths() {
      return [
        ...new Set([...state.pushDiagnostics.keys(), ...state.documentPullDiagnostics.keys()]),
      ].map((filePath) =>
    ...
    async prepareCallHierarchy(filePath, line, character) {
      const result = await navRequest<LSPCallHierarchyItem | LSPCallHierarchyItem[]>(
        state,
