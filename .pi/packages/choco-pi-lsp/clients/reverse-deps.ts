 ...
 */
export function rankFilesByReverseDependencyCentrality(index: ReverseDependencyIndex): string[] {
  const files = new Set([...Object.keys(index.imports), ...Object.keys(index.importedBy)]);
...
function sortedUnique(values: Iterable<string>): string[] {
  return [...new Set([...values].map((value) => normalizeMapKey(value)))].sort((a, b) =>
    a.localeCompare(b),
  );
}

function addEdge(index: ReverseDependencyIndex, fromFile: string, toFile: string) {
...
function normalizeIndex(index: ReverseDependencyIndex): ReverseDependencyIndex {
  const fileKeys = new Set([...Object.keys(index.imports), ...Object.keys(index.importedBy)]);
...
function filePathForNode(graph: ReviewGraph, nodeId: string): string | undefined {
  ...
  for (const change of changes) {
    ...
    for (const target of sortedUnique(change.priorTargets)) {
      importedBy[target] = sortedUnique(
        (importedBy[target] ?? []).filter((importer) => normalizeMapKey(importer) !== file),
      );
    }
...
export function getReverseDepsFromIndex(index: ReverseDependencyIndex, filePath: string): string[] {
...
export function getAffectedFilesFromIndex(
  ...
  const queue: Array<{ filePath: string; depth: number }> = [{ filePath: start, depth: 0 }];
...
function snapshotFileFor(filePath: string, imports: string[]): ProjectSnapshotFile {
  try {
...
export function writeReverseDependencyIndexToSnapshot(args: {
  ...
  try {
    ...
    for (const [filePath, imports] of Object.entries(args.index.imports)) {
      const normalized = normalizeMapKey(filePath);
      files[normalized] = {
        ...(files[normalized] ?? snapshotFileFor(normalized, sortedUnique(imports))),
        path: normalized,
