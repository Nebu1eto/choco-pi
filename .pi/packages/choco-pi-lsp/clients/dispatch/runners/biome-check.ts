  ...
  try {
    ...
    return {
      diagnostics: diagnostics.map((d) => ({
        ...
        autoFixAvailable:
          (d.tags?.includes("fixable") ?? false) && (autofix?.safePipelineAutofix ?? false),
        fixKind:
          d.tags?.includes("fixable") && autofix?.fixKind !== "none" ? autofix?.fixKind : undefined,
      })),
    };
  } catch (err) {
    return {
...
const biomeCheckJsonRunner: RunnerDefinition = {
  ...
  async run(ctx: DispatchContext): Promise<RunnerResult> {
    ...
    const checkResult = await safeSpawnAsync(
      cmd,
      ["lint", "--reporter=json", "--no-errors-on-unmatched", ...configArg, ctx.filePath],
      { timeout: 30000, cwd },
    );
