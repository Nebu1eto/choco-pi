...
import { hasMypyConfig } from "../../tool-policy.ts";
...
export function parseMypyOutput(raw: string, fallbackPath: string, cwd: string): Diagnostic[] {
  ...
  for (const match of raw.matchAll(linePattern)) {
    ...
    const filePath =
      file && file.trim() ? (path.isAbsolute(file) ? file : path.resolve(cwd, file)) : fallbackPath;
    diagnostics.push({
