...
import { getLinterPolicyForCwd, hasPhpstanConfig } from "../../tool-policy.ts";
import { PRIORITY } from "../priorities.ts";
import type { Diagnostic, DispatchContext, RunnerDefinition, RunnerResult } from "../types.ts";
import { createAvailabilityChecker, resolveVendorToolCommand } from "./utils/runner-helpers.ts";
...
export function parsePhpstanJson(raw: string, fallbackPath: string, cwd: string): Diagnostic[] {
  try {
