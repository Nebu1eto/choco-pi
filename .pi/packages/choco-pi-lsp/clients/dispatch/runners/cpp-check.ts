...
type CompilerSpec = { command: string; args: string[]; flavor: "gcc" | "msvc" } | undefined;
...
const CPP_HEADER_EXTENSIONS = new Set([".hh", ".hpp", ".hxx", ".inl", ".ipp", ".tpp", ".txx"]);
...
function getGccLikeCandidates(absPath: string): Array<{ key: CompilerKey; args: string[] }> {
  const ext = path.extname(absPath).toLowerCase();
  const cMode =
    C_SOURCE_EXTENSIONS.has(ext) || (C_HEADER_EXTENSIONS.has(ext) && !headerLooksLikeCpp(absPath));
...
async function resolveCompiler(absPath: string, cwd: string): Promise<CompilerSpec> {
