...
function isShellWrapper(value: string): boolean {
  return new Set(["sh", "bash", "dash", "zsh", "ash", "cmd", "pwsh", "powershell"]).has(
...
function containsCommitOrPush(tokens: string[], depth: number): boolean {
  ...
  if (gitIndex >= 0) {
    ...
    while (i < gitTokens.length && gitTokens[i].startsWith("-")) {
      ...
      if (
        ["-C", "-c"].some((prefix) => option.startsWith(prefix) && option.length > prefix.length)
      ) {
        i += 1;
        continue;
      }
      if (
        ["--config-env", "--git-dir", "--work-tree", "--exec-path", "--namespace"].some((prefix) =>
  ...
  if (!knownCommandStringWrapper) {
    ...
    const unknownSwitchIndex = commandTokens
      .slice(1)
      .findIndex((token) => (token.startsWith("-") || token.startsWith("/")) && token.length > 1);
...
export function writeGitGuardRecord(
  ...
  const fileContentHashes = Object.fromEntries(
    currentProvenance.files.map((file) => [guardPathKey(file.path, cwd), file.sha256]),
  );
...
export function mergeGitGuardTestFailure(
  ...
  const testFailureFiles = [...(existing?.testFailureFiles ?? []), ...failedFiles].filter(
    (file, index, all) =>
      all.findIndex((candidate) => guardPathKey(candidate, cwd) === guardPathKey(file, cwd)) ===
      index,
  );
  writeGitGuardRecord(cacheManager, runtime, cwd, {
...
export function clearGitGuardTestFailure(
  ...
  const passedKeys = new Set(
    (passedFiles.length > 0 ? passedFiles : (existing.testFailureFiles ?? [])).map((file) =>
      guardPathKey(file, cwd),
    ),
  );
...
export function evaluateGitGuard(
  ...
  for (const file of record.affectedFiles) {
    ...
    if (recordedSeq === undefined || recordedSeq !== (runtime.getFileSeq?.(resolved) ?? 0)) {
