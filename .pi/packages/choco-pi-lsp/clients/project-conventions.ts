...
function detectReact(
  ...
  if (hasDep(pkg, "react-dom")) signals.push("package.json:dependencies.react-dom");
  if (hasDep(pkg, "@types/react")) signals.push("package.json:devDependencies.@types/react");
  if (signals.length === 0) return undefined;
  const confidence: FrameworkConfidence = hasDep(pkg, "react-dom") ? "high" : "medium";
...
function detectNext(
  ...
  for (const candidate of ["next.config.js", "next.config.mjs", "next.config.ts"]) {
...
function detectVite(
  ...
  if (signals.length === 0) return undefined;
  const confidence: FrameworkConfidence =
    hasDep(pkg, "vite") && signals.some((s) => s.startsWith("vite.config.")) ? "high" : "medium";
...
function detectVitest(
  ...
  if (signals.length === 0) return undefined;
  const confidence: FrameworkConfidence = hasDep(pkg, "vitest") ? "high" : "medium";
  return { id: "vitest", confidence, signals };
}

const AGENT_DOC_CANDIDATES = ["AGENTS.md", "CLAUDE.md", ".cursorrules"] as const;
