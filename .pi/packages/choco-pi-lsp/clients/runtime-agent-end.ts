...
export async function handleAgentEnd({
  ...
  const { claimed, staleClaimed, deferredToOwner, droppedOrphans } = runtime.claimDeferredMutations(
    currentSessionId,
    Date.now(),
    staleAfterMs,
  ...
  for (let autofixIndex = 0; autofixIndex < autofixRecords.length; autofixIndex++) {
    ...
    if (!biomeClient || !ruffClient) {
      ...
      requeue(
        [{ ...record, kinds: new Set(["autofix"]), toolNames: new Set(record.toolNames) }],
        "clients-unavailable",
      );
      continue;
    }
  ...
  if (formatRecords.length > 0) {
    ...
    await Promise.all(
      Array.from({ length: Math.min(DEFERRED_FORMAT_CONCURRENCY, formatRecords.length) }, () =>
        worker(),
      ),
    );
    if (ambientSignal?.aborted) {
    ...
    for (const entry of work) {
      ...
      if (entry.missing) {
        ...
        if (
          !summary.skipped.some((item) => item.filePath === filePath && item.reason === "missing")
        ) {
          summary.skipped.push({ filePath, reason: "missing" });
        }
        continue;
      ...
      if (entry.error) {
        ...
        requeue(
          [{ ...record, kinds: new Set(["format"]), toolNames: new Set(record.toolNames) }],
          "format-failed",
        );
        continue;
      }
  ...
  if (inspectActionableReport) {
    ...
    if (!actionReport?.data) {
      ...
      if (inspection === "missing") {
        ...
        dbg("agent_end actionable_warnings_autofix: cache expired, skipping fixes");
      } else {
        dbg(
        ...
        (total, file) =>
          ...
          file.warnings.reduce(
            (count, warning) =>
              ...
              (warning.suppressed
                ? 0
                : warning.actions.filter((action) => action.autoFixEligible).length),
            0,
          ),
        0,
      );
        ...
        publishAutofixStart({
          ...
          paths: eligibleReport.files
            .filter((file) =>
              file.warnings.some(
                (warning) =>
                  !warning.suppressed && warning.actions.some((action) => action.autoFixEligible),
              ),
            )
