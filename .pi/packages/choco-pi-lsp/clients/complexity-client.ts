...
const GO: LangNodes = {
  functionLike: new Set(["function_declaration", "method_declaration", "func_literal"]),
  nesting: new Set([
    "if_statement",
    "for_statement",
...
function isLogicalOp(node: TsNode, nodes: LangNodes): boolean {
  ...
  if (nodes.logicalBinaryOps.size > 0 && node.type === "binary_expression") {
    return (node.children ?? []).some((c: TsNode) => c && nodes.logicalBinaryOps.has(c.type));
  }
  return false;
}
...
function subtreeMaxNesting(node: TsNode, currentDepth: number, nodes: LangNodes): number {
 ...
 */
function calculateMaintainabilityIndex(cyclomatic: number, loc: number, comments: number): number {
  if (loc === 0) return 100;
...
function calculateMaxParams(functions: FunctionMetrics[]): number {
  ...
  return Math.min(
    ...
    Math.max(
      2,
      Math.round(functions.reduce((a, f) => a + f.length, 0) / Math.max(1, functions.length) / 5),
    ),
  );
}
  ...
  private computeMetrics(
    ...
    return {
      ...
      maintainabilityIndex:
        Math.round(
          calculateMaintainabilityIndex(funcStats.avgCyclomatic, codeLines, commentLines) * 10,
        ) / 10,
