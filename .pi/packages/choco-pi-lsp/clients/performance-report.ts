...
function boundedPositiveInteger(value: number, maximum: number, fallback = maximum): number {
...
function orderedEntries(buffer: BoundedPhaseEntries): LatencyEntry[] {
  return buffer.total > buffer.values.length
    ? [...buffer.values.slice(buffer.nextIndex), ...buffer.values.slice(0, buffer.nextIndex)]
    : buffer.values;
}
...
export function summarizePhaseLatency(
  ...
  const summaries: RawPhaseLatencySummary[] = Array.from(durationsByPhase, ([phase, durations]) => {
  ...
  const rankBy = (
    ...
    [...summaries]
      .sort((a, b) => b[primary] - a[primary] || b[secondary] - a[secondary] || tieBreak(a, b))
      .slice(0, limit)
      .map(toPublic);
...
function renderScope(lines: string[], label: string, scope: PhaseLatencyScope): void {
  const range =
...
export function renderLatencyPerformanceReport(report: LatencyPerformanceReport): string {
  const lines = ["⏱️ PI-LENS PERFORMANCE", `Top ${report.topN} sustained and tail latency phases.`];
