...
export function _setBeforeWarningStateLockForTests(hook: (() => void) | null): void {
...
function updateWarningState(cwd: string, warnings: ActionableWarningRecord[]): void {
  const statePath = path.join(getProjectDataDir(cwd), "cache", "actionable-warning-state.json");
...
function mergeWarnings(records: ActionableWarningRecord[]): ActionableWarningRecord[] {
  ...
  return [...byId.values()].sort(
    (a, b) => a.displayPath.localeCompare(b.displayPath) || (a.line ?? 0) - (b.line ?? 0),
  );
}
  ...
  if (args.includeLspCodeActions) {
    for (const file of args.files) {
      ...
      if (cached !== undefined) {
        ...
        try {
          if (currentContent) await lspService.openFile(filePath, currentContent);
  ...
  for (const file of report.files) {
    for (const warning of file.warnings) {
      entries.push({
        ...
        autoFixEligibleActionCount: warning.actions.filter((action) => action.autoFixEligible)
          .length,
