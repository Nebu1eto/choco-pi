...
export function _telemetryErrorForTest(raw: string | undefined): string | undefined {
  if (!raw) return undefined;
  return raw.replace(/\0/g, "\\0").slice(0, 2_000);
}

export function _telemetryClassificationErrorForTest(raw: string | undefined): string | undefined {
 ...
 */
function getPatternHint(pattern: string, lang: string, selector?: string): string | null {
  ...
  return {
    ...
    async execute(
      ...
      function logOutcome(
        ...
        try {
          ...
          logAstGrepToolEvent({
            ...
            outcome,
            errorKind: outcome === "error" ? classifyAstGrepError(classificationError) : undefined,
            errorRaw,
      ...
      function abortError() {
        ...
        return {
          content: [{ type: "text" as const, text: "Error: operation aborted" }],
          isError: true,
          details: {},
        };
      }
      ...
      try {
        ...
        if (rawRuleError) {
          ...
          return {
            content: [{ type: "text" as const, text: `Error: ${rawRuleError}` }],
            isError: true,
            details: {},
          };
        }
        ...
        if (pattern.trim() && patternError) {
          ...
          return {
            content: [{ type: "text" as const, text: `Error: ${patternError}` }],
            isError: true,
            details: {},
          };
        }
        ...
        if (!lang.trim()) {
          ...
          return {
            content: [{ type: "text" as const, text: "Error: lang is required" }],
            isError: true,
            details: {},
          };
        }
        ...
        if (result.error) {
          ...
          return {
            content: [{ type: "text" as const, text: errorTextWithHint(result.error) }],
            isError: true,
            details: {},
          };
        }
        ...
        const output = groupByFile
          ...
          : astGrepClient.formatMatches(page, false, false, PAGE_SIZE);
        const hint =
          page.length === 0 && !result.error ? getPatternHint(pattern, lang, selector) : undefined;
        const paginationNote =
          hasMore && page.length > 0
