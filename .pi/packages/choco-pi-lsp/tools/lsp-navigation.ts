...
function operationSupportStatus(
  ...
  if (operation === "documentSymbol" || operation === "findSymbol") return support.documentSymbol;
...
function emptyReasonForOperation(operation: LspNavigationOperation): string {
  ...
  if (operation === "workspaceSymbol") return "no-matching-symbols-or-server-index-unavailable";
...
function findNthMatch(lineText: string, regex: RegExp, occurrence: number): RegExpExecArray | null {
  let seen = 0;
  regex.lastIndex = 0;
...
function tokenAtPosition(content: string, line1: number, char1: number): string | undefined {
...
function collectSearchReadsForOperation(
  ...
  if (
    ["definition", "typeDefinition", "declaration", "references", "implementation"].includes(
      operation,
    )
  ) {
...
function formatCapabilities(snapshots: CapabilitySnapshot[], filePath?: string): string {
...
export function createLspNavigationTool(
  ...
  mutationDeps?: Pick<LspMutationContext, "runtime" | "cacheManager" | "readGuard" | "dbg">,
) {
  return {
    ...
    async execute(
        ...
        return finalize(
          {
            content: [
              {
                type: "text" as const,
                text: "lsp_navigation requires LSP to be enabled. LSP usage is currently disabled (/lsp off, --no-lsp, or lsp.enabled=false) — ask the user to run /lsp on to re-enable it.",
              },
            ],
            isError: true,
          },
      ...
      if (requestedApply && ["rename", "rename_file", "executeCommand"].includes(operation)) {
        ...
        mutationContext = {
          ...
          source: "lsp-edit",
          ...mutationDeps,
          readGuard: getFlag("no-read-guard", cwd) ? undefined : mutationDeps?.readGuard,
        };
      }
      ...
      const resolvedCharacter =
        needsPosition && filePath
          ? resolveSymbolColumn(
              nodeFs.existsSync(filePath) ? nodeFs.readFileSync(filePath, "utf-8") : "",
              line ?? 1,
              character,
              symbol,
            )
        ...
        switch (operation) {
          ...
          case "findSymbol": {
            ...
            return findSymbolMatches(symbols, query, {
              ...
              kinds: new Set(
                (kinds ?? []).map((kind) => kind.trim().toLowerCase()).filter(Boolean),
              ),
            });
          }
      ...
      return finalize(
        {
          ...
          details: {
            ...
            columnResolution:
              columnResolution?.strategy === "explicit" ? undefined : columnResolution,
            resultCount,
          },
        },
        {
