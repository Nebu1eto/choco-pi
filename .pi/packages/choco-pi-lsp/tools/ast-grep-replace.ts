    ...
    async execute(
      ...
      function logOutcome(
        ...
        try {
          logAstGrepToolEvent({
            ...
            applied: applyFlag,
            outcome,
            errorKind: outcome === "error" ? classifyAstGrepError(details.errorRaw) : undefined,
      ...
      if (hasStructuralIntent({ insideKind, hasKind, hasDescendantKind, follows, precedes })) {
        ...
        if (ruleResult.error) {
          ...
          return {
            content: [{ type: "text" as const, text: errorTextWithHint(ruleResult.error) }],
            isError: true,
            details: {},
          };
        }
        ...
        return {
          ...
          details: { matchCount: ruleResult.matches.length, applied: applyFlag },
        };
      }

      const result = await astGrepClient.replace(pattern, rewrite, lang, searchPaths, applyFlag, {
        strictness,
      });
