...
export function createModuleReportTool(getProjectRoot: () => string) {
  return {
    ...
    renderResult: compactRenderResult<{
      ...
      const parts = [`${details?.symbols ?? 0} symbols`, `${details?.exports ?? 0} exports`];
    ...
    async execute(
      ...
      const text =
        params.view === "compact" ? renderCompactModuleReport(report) : JSON.stringify(report);
      return {
...
export function createReadSymbolTool(getProjectRoot: () => string, recordSymbolRead: ReadRecorder) {
  return {
...
export function createReadEnclosingTool(
  ...
  return {
    ...
    renderResult: compactRenderResult<{
      ...
      const range =
        details?.startLine && details?.endLine ? `:${details.startLine}-${details.endLine}` : "";
