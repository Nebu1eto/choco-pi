...
async function fetchCodexUsageWithHeaders(
  ...
  if (
    includeDetailedResetCredits &&
    (!snapshot.resetCredits || snapshot.resetCredits.availableCount > 0)
  ) {
    try {
...
export async function fetchCodexWeeklyUsageLeft(
  ctx: ExtensionContext,
...
export async function consumeCodexRateLimitResetCredit(
  ...
  if (!isCanonicalCodexSubscriptionModel(model)) {
    throw new Error(
      "Codex reset credits are only available for canonical OpenAI Codex subscription models.",
    );
  }
  const headers = await buildCodexUsageHeaders(ctx, model);
