...
export async function createCodexDiagnosticsRuntime(options: {
  ...
  const latestRequests = new Map<
    CodexDiagnosticsLane,
    Extract<CodexDiagnosticsEvent, { type: "request" }>
  >();
  ...
  return {
    record(event) {
      ...
      if (event.type === "request") {
        ...
        showCurrent(
          [laneLabel(event.lane), requestTransportLabel(event)].filter(Boolean).join(" • "),
        );
        return;
      }
      if (event.type === "usage") {
        const totalInput =
          event.inputTokens + event.cachedInputTokens + event.cacheWriteInputTokens;
        ...
        showCurrent(
          [prefix, totalInput > 0 ? "HIT" : "cache unavailable", transport]
            .filter(Boolean)
            .join(" • "),
        );
        return;
