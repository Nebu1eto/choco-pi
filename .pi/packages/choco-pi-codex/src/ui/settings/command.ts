...
export function registerCodexCommand(
  ...
  async function openSettings(ctx: ExtensionContext, tab: SettingsTab): Promise<void> {
    ...
    if (configScope === "folder") {
      ...
      if (!materialized.ok) {
        ctx.ui.notify(
          `Could not materialize folder Codex settings: ${materialized.error}`,
          "error",
        );
        return;
      }
  ...
  pi.registerCommand("codex", {
    ...
    getArgumentCompletions: (prefix) =>
      CODEX_COMMAND_COMPLETIONS.filter((item) => item.startsWith(prefix.trim().toLowerCase())).map(
