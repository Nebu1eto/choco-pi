...
export interface UsageTabOptions {
  ...
  onConsumeResetCredit?:
    | ((redeemRequestId: string) => Promise<CodexRateLimitResetConsumeResult>)
    | undefined;
}
...
export function createUsageTab(
...
function formatResetCreditLines(
  ...
  const hint =
    count && count > 0
      ? theme.fg(
          "dim",
          resetLockedUntilRefresh ? "  R to refresh before another reset" : "  Ctrl+R to use one",
        )
      : "";
  const lines = [
