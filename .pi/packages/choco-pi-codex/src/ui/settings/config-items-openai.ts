...
export function buildOpenAISettings(config: CodexConversionConfig, theme: Theme): ConfigSetting[] {
  return [
    ...
    projectToggle("cacheKeepalive", "Experimental cache keepalive", config.openai.cacheKeepalive),
    setting(
      {
        id: "verbosity",
        label: "Verbosity",
      ...
      (value, current) => ({
        ...
        openai: {
          ...current.openai,
          verbosity:
            normalizeCodexVerbosity(value) ?? DEFAULT_CODEX_CONVERSION_CONFIG.openai.verbosity,
        },
      }),
    ),
    setting({
    ...
    setting(
      {
        ...
        currentValue: config.openai.harnessIdentifierHeader ? "pi-codex-conversion <3" : "off",
