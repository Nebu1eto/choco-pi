...
// This fork owns its own config file. Obsolete upstream keys (voice.*,
// ui.backgroundShell*) are still parsed and silently ignored.
export const CODEX_CONVERSION_CONFIG_BASENAME = "choco-pi-codex.json";
...
function mergeConfigDocument(existing: JsonObject, owned: JsonObject): JsonObject {
  ...
  for (const [key, value] of Object.entries(owned)) {
    ...
    merged[key] =
      isRecord(previous) && isRecord(value) ? mergeConfigDocument(previous, value) : value;
  }
  return merged;
}
...
export function readCodexConversionConfig(
...
function applyProcessOverrides(
...
export function readEffectiveCodexConversionConfig(
  options: EffectiveCodexConversionConfigOptions,
): CodexConversionConfig {
...
export function setProjectCodexCacheKeepalive(
  ...
  try {
    ...
    const message = error instanceof Error ? error.message : String(error);
    console.warn(
      `[pi-codex-conversion] Failed to write project cache keepalive ${path}: ${message}`,
    );
