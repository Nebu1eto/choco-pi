...
const PI_DEFAULT_INTRO =
  "You are an expert coding assistant operating inside pi, a coding agent harness. You help users by reading files, executing commands, editing code, and writing new files.";
const PI_CUSTOM_TOOLS_NOTE =
...
const STATIC_CODEX_GUIDELINES_BY_KEY = new Map([
  ...ALL_STATIC_CODEX_GUIDELINES.map(
    (guideline) => [withoutCosmeticTerminalPeriod(guideline), guideline] as const,
  ),
...
function injectShell(prompt: string, shell?: string): string {
  ...
  const zshGuidance =
    shellName === "zsh" || shellName === "zsh.exe" ? "; status is read-only, capture $? as rc" : "";
...
export function promptSkillsFromStructuredSkills(
  skills: readonly StructuredPromptSkill[] | undefined,
): PromptSkill[] {
...
export function resolvePromptSkills(
  ...
  return structuredSkills === undefined
    ? [...fallbackSkills]
...
function stripPiToolScaffold(prompt: string, options: PiSystemPromptOptions): string {
  ...
  const toolsHeader =
    start === -1 ? -1 : prompt.indexOf("\nAvailable tools:\n", start + PI_DEFAULT_INTRO.length);
  const noteStart = toolsHeader === -1 ? -1 : prompt.indexOf(PI_CUSTOM_TOOLS_NOTE, toolsHeader + 1);
...
function stripPiDocumentation(prompt: string): string {
  ...
  const examplesPath = prompt
    .match(/^- Examples: (.+) \(extensions, custom tools, SDK\)$/m)?.[1]
    ?.trim();
...
function replaceHeavyGuidelines(prompt: string, guidelines: string[]): string {
  ...
  const kept = match[2]!
    ...
    .filter(
      (line) =>
        !PI_DEFAULT_GUIDELINES.has(withoutCosmeticTerminalPeriod(line.trim().replace(/^-\s*/, ""))),
    );
