...
export interface CodexExtensionRuntime {
  ...
  codexSystemPrompt(
    basePrompt: string,
    ctx: CodexContext,
    skills?: AdapterState["promptSkills"],
...
): Pick<OpenAICodexStreamOptions, "reasoning"> | Record<never, never> {
      ...
      try {
        ...
        const transportSettlement = prewarmOpenAICodexWebSocket(
          ...
          },
          {
            ...
            useResponsesLite: (currentModel) =>
              resolveCodexRuntimePlan({ model: currentModel }, config, executionMode).transport ===
              "responses-lite",
