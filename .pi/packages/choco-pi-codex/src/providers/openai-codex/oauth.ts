...
export function clampOpenAICodexModelWindows(models: Model<Api>[]): Model<Api>[] {
  return models.map((model) =>
    /^gpt-5\.6-(?:luna|terra|sol)$/i.test(model.id) &&
...
function startLocalOAuthServer(state: string): Promise<{
  ...
  return new Promise((resolve) => {
    server
      ...
      .on("error", () =>
        resolve({ close: () => {}, cancelWait: () => {}, waitForCode: async () => null }),
      );
  });
}
...
export const openaiCodexNativeOAuthProvider: NonNullable<ProviderConfig["oauth"]> & {
  ...
  modifyModels(models) {
    return clampOpenAICodexModelWindows(models);
  },
};
