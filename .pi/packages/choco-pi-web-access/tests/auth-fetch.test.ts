...
test("authFetch profiles resolve named, single true, and host policy", () => {
  ...
  try {
    writeFileSync(
      ...
      JSON.stringify({
        authFetch: { social: ["x.com"], work: { hosts: ["docs.company.com"], cache: "off" } },
      }) + "\n",
    );
...
test("authFetch true selects the only profile", () => {
  ...
  try {
    writeFileSync(
      join(root, "web-search.json"),
      JSON.stringify({ authFetch: { work: ["example.com"] } }) + "\n",
    );
    const output = runModule(
      root,
      `
...
test("authenticated fetch sends only browser-scoped cookies with raw values", (t) => {
  ...
  try {
    ...
    assert.ok(
      output.calls[0].cookie.indexOf("sid=private=a=b/%") <
        output.calls[0].cookie.indexOf("sid=root"),
    );
...
test("authenticated fetch failure does not fall through to hosted providers", () => {
  ...
  try {
    ...
    assert.match(
      output.result.error,
      /could not read browser cookies|No detected Chromium profile/,
    );
  } finally {
    rmSync(root, { recursive: true, force: true });
  }
