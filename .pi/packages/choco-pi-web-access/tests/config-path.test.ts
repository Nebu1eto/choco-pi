...
function run(script, env) {
	return spawnSync(process.execPath, ["--input-type=module"], { input: script, encoding: "utf8", env: { ...process.env, ...env } });
}
...
test("retained providers read credentials from the shared config path", async () => {
	...
	await writeFile(join(root, "web-search.json"), JSON.stringify({ exaApiKey: "exa-config", kagiApiKey: "kagi-config" }));
	const result = run(`
		let exaKey;
