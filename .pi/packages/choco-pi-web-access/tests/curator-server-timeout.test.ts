...
test("curator assigns one selectable result index per provider", async () => {
	...
	try {
		...
		assert.deepEqual(searchBody.entries.map(entry => entry.provider), ["exa", "kagi"]);
...
test("curator state replay keeps all-provider entries that share one slot", async () => {
	...
	try {
		...
		handle.pushResult(1, { answer: "kagi answer", results: [], provider: "kagi", query: "query", slotIndex: 0 });

		const response = await fetch(new URL("/state?session=test-token", handle.url));
		assert.equal(response.status, 200);
		const body = await response.json();
		assert.deepEqual(body.events.map((event) => event.data.provider), ["exa", "kagi"]);
