...
test("many individually small images have no per-payload exemption", { timeout: 20_000 }, () => {
  ...
  const content = Array.from(
    { length: count },
    (_, index) =>
      `Paragraph ${index} ![small ${index}](data:image/png;base64,${encoded}) end ${index}.`,
  ).join("\n");
...
test("readable prose and Markdown alt text remain unchanged around replacements", () => {
  const input =
    'Before ![architecture diagram](data:image/png;base64,SGk=) between <img src="data:text/plain,hello%20world" alt="inline"> after.';
...
test("malformed enclosed data URIs are wholly removed without payload suffix leakage", () => {
  ...
  for (const fixture of cases) {
    ...
    if (fixture.encodedBytes !== undefined)
      assert.equal(result.omissions[0].encodedBytes, fixture.encodedBytes);
    if (fixture.decodedBytes !== undefined)
      assert.equal(result.omissions[0].decodedBytes, fixture.decodedBytes);
  }
  assert.equal(
...
test(
  "malformed comma-less candidates scale linearly and are explicitly marked",
  { timeout: 15_000 },
  () => {
    ...
    assert.ok(
      largeMs <= smallMs * 4 + 500,
      `nonlinear malformed scan: ${smallMs}ms -> ${largeMs}ms`,
    );
  },
);
