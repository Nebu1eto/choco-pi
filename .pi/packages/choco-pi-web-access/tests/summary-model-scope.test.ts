...
test("summary model scope matches nested provider model ids and thinking suffixes", () => {
  ...
  assert.equal(
    modelMatchesEnabledPatterns({ provider: "openrouter", id: "anthropic/claude-sonnet-4" }, [
      "openrouter/*:low",
    ]),
    true,
  );
  assert.equal(
    modelMatchesEnabledPatterns({ provider: "openrouter", id: "ai21/jamba-large-1.7" }, [
      "openrouter/nvidia/*",
    ]),
    false,
  );
  ...
  const result = await generateSummaryDraft(
    ...
    () => {
      ...
      return Promise.resolve({
        stopReason: "stop",
        content: [{ type: "text", text: "Routed summary" }],
      });
    },
    1000,
  );
...
test("summaryModel thinking suffix strips before lookup and reaches completion options", async () => {
  ...
  const result = await generateSummaryDraft(
    ...
    (_model, _request, requestOptions) => {
      ...
      return Promise.resolve({
        stopReason: "stop",
        content: [{ type: "text", text: "Low-thinking summary" }],
      });
    },
    1000,
  );
  ...
  assert.ok(
    lookups.some((lookup) => lookup.provider === "anthropic" && lookup.id === "claude-haiku-4-5"),
  );
...
test("summaryModel keeps non-thinking colons in model ids", async () => {
  ...
  const result = await generateSummaryDraft(
    ...
    (_model, _request, requestOptions) => {
      ...
      return Promise.resolve({
        stopReason: "stop",
        content: [{ type: "text", text: "Free model summary" }],
      });
    },
    1000,
  );
  ...
  assert.ok(
    lookups.some(
      (lookup) =>
        lookup.provider === "openrouter" && lookup.id === "nvidia/nemotron-3-super-120b-a12b:free",
    ),
  );
...
test("summaryModel thinking suffix omits effort for non-reasoning models", async () => {
  ...
  await generateSummaryDraft(
    ...
    (_model, _request, requestOptions) => {
      ...
      return Promise.resolve({
        stopReason: "stop",
        content: [{ type: "text", text: "No-thinking summary" }],
      });
    },
    1000,
  );
...
test("routed model resolution follows available-model ordering", () => {
  ...
  assert.equal(findModelWithProviderRouting(registry, "anthropic", "claude-haiku-4-5"), firstRoute);
});
...
test("enabledModels loading uses trusted project settings over global settings", async () => {
  ...
  await writeFile(
    ...
    JSON.stringify({ enabledModels: ["global/model"] }),
  );
  ...
  try {
    assert.deepEqual(loadEnabledModelPatterns({ cwd: projectDir, isProjectTrusted: () => true }), [
      "project/model",
    ]);
    assert.deepEqual(loadEnabledModelPatterns({ cwd: projectDir, isProjectTrusted: () => false }), [
      "global/model",
    ]);
  } finally {
...
test("summary generation deadline config defaults, validates, caps, and reaches both workflows", async () => {
  ...
  assert.match(
    indexSrc,
    /generateSummaryDraft\(\s*selectedResults,\s*summaryContext,\s*signal,\s*modelOverride,\s*feedback,\s*undefined,\s*getSummaryGenerationDeadlineMs\(\),\s*\)/,
  );
...
test("summary generation no longer uses catalog fallback or first available model", () => {
  ...
  assert.match(
    summarySrc,
    /findModelWithProviderRouting\(ctx\.modelRegistry, spec\.provider, spec\.id\)/,
  );
