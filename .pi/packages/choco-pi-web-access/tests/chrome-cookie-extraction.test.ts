...
function createFixture(home, profile, rows = [], options = {}) {
  ...
  const dbPath =
    targetPlatform === "win32"
      ? join(base, profile, "Network", "Cookies")
      : join(base, profile, "Cookies");
...
`,
  ...
  if (options.windowsKey) {
    writeFileSync(
      ...
      JSON.stringify({
        os_crypt: {
          encrypted_key: Buffer.concat([Buffer.from("DPAPI"), Buffer.from("protected")]).toString(
            "base64",
          ),
        },
      }),
...
function runCookies(
  home,
  env,
  options = "{ hosts: ['example.com'], requiredCookies: ['session', 'session_ts'], cookieNames: ['session', 'session_ts'] }",
  platformOverride,
) {
  ...
  createFixture(
    ...
    [
      [
        ...
        "",
        ".example.com",
        `hex:${encryptWindowsCookie("one", key, "v10", ".example.com")}`,
        1,
      ],
      [
        "session_ts",
        "",
        ".example.com",
        `hex:${encryptWindowsCookie("two", key, "v10", ".example.com")}`,
        2,
      ],
    ],
...
test("configured profile names cannot escape the browser profile root", (t) => {
  ...
  const bin = mkdtempSync(join(tmpdir(), "pi-cookie-bin-"));
  const env = makeEnvironment(home, bin);
  const result = runCookies(
    home,
    env,
    "{ hosts: ['example.com'], profile: '../Profile 2', requiredCookies: ['session'], cookieNames: ['session'] }",
  );
  assert.equal(result.result, null);
...
test("explicit profile symlinks cannot escape the browser profile root", (t) => {
  ...
  const result = runCookies(
    home,
    env,
    "{ hosts: ['example.com'], profile: 'link', requiredCookies: ['session', 'session_ts'], cookieNames: ['session', 'session_ts'] }",
  );
  assert.equal(result.result, null);
