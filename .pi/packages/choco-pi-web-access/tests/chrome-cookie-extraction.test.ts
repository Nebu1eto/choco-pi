...
function writePasswordCommand(bin, countPath, targetPlatform = process.platform, argsPath) {
	...
	const environment = { COUNT_FILE: countPath };
	if (argsPath) environment.ARGS_FILE = argsPath;
	return environment;
}
...
function runCookies(home, env, options = "{ hosts: ['example.com'], requiredCookies: ['session', 'session_ts'], cookieNames: ['session', 'session_ts'] }", platformOverride) {
	...
	const child = spawnSync(process.execPath, ["--experimental-strip-types", "--input-type=module"], {
		encoding: "utf8",
		env,
		input: `${override}const m = await import(${JSON.stringify(moduleUrl)}); const r = await m.getBrowserCookiesForHosts(${options}); console.log(JSON.stringify({ result: r, diagnostic: m.getLastBrowserCookieDiagnostic() }));`,
	});
...
test("cookie extraction remains opt-in even when a browser database exists", (t) => {
	...
	createFixture(home, "Default", [["session", "one", ".example.com", null, 1]]);
...
test("auto-discovery finds a non-default Chromium profile", (t) => {
	...
	createFixture(home, "Profile 2", [
		["session", "one", ".example.com", null, 1],
		["session_ts", "two", ".example.com", null, 2],
	]);
	const env = makeEnvironment(home, bin);
	Object.assign(env, writePasswordCommand(bin, join(home, "password-count")));
	const result = runCookies(home, env);
	assert.deepEqual(result.result.cookies, { "session_ts": "two", "session": "one" });
	rmSync(home, { recursive: true, force: true });
	rmSync(bin, { recursive: true, force: true });
});
...
test("Windows Chrome profiles decrypt v10 cookies with DPAPI keys", (t) => {
	...
	createFixture(home, "Default", [
		["session", "", ".example.com", `hex:${encryptWindowsCookie("one", key, "v10", ".example.com")}`, 1],
		["session_ts", "", ".example.com", `hex:${encryptWindowsCookie("two", key, "v10", ".example.com")}`, 2],
...
test("Windows Edge uses the USERPROFILE AppData fallback and reports unsupported v20 cookies", (t) => {
	...
	createFixture(home, "Default", [
		["session", "", ".example.com", `hex:${encryptWindowsCookie("one", key, "v20")}`, 1],
		["session_ts", "", ".example.com", `hex:${encryptWindowsCookie("two", key, "v20")}`, 2],
...
test("required-cookie preflight avoids password invocation for unrelated profiles", (t) => {
	...
	const bin = mkdtempSync(join(tmpdir(), "pi-cookie-bin-"));
	createFixture(home, "Profile 1", [["NID", "unrelated", ".example.com", null, 1]]);
...
test("browser encryption password is cached within a process", (t) => {
	...
	const child = spawnSync(process.execPath, ["--experimental-strip-types", "--input-type=module"], {
		encoding: "utf8",
		env,
		input: `const { getBrowserCookiesForHosts } = await import(${JSON.stringify(moduleUrl)}); const options = { hosts: ['example.com'], profile: 'Profile 2', requiredCookies: ['session', 'session_ts'], cookieNames: ['session', 'session_ts'] }; await getBrowserCookiesForHosts(options); await getBrowserCookiesForHosts(options);`,
	});
	assert.equal(child.status, 0, child.stderr);
...
test("failed password lookups are retried instead of cached", (t) => {
	...
	const child = spawnSync(process.execPath, ["--experimental-strip-types", "--input-type=module"], {
		encoding: "utf8",
		env,
		input: `const { getBrowserCookiesForHosts } = await import(${JSON.stringify(moduleUrl)}); const options = { hosts: ['example.com'], profile: 'Profile 2', requiredCookies: ['session', 'session_ts'], cookieNames: ['session', 'session_ts'] }; const first = await getBrowserCookiesForHosts(options); const second = await getBrowserCookiesForHosts(options); console.log(JSON.stringify({ first, second }));`,
	});
	assert.equal(child.status, 0, child.stderr);
	const result = JSON.parse(child.stdout);
	assert.equal(result.first, null);
	assert.deepEqual(result.second.cookies, { "session_ts": "two", "session": "one" });
...
test("configured profile names cannot escape the browser profile root", (t) => {
	...
	const bin = mkdtempSync(join(tmpdir(), "pi-cookie-bin-"));
	const env = makeEnvironment(home, bin);
	const result = runCookies(home, env, "{ hosts: ['example.com'], profile: '../Profile 2', requiredCookies: ['session'], cookieNames: ['session'] }");
	assert.equal(result.result, null);
...
test("explicit profile symlinks cannot escape the browser profile root", (t) => {
	...
	const env = makeEnvironment(home, bin);
	Object.assign(env, writePasswordCommand(bin, join(home, "password-count")));
	const result = runCookies(home, env, "{ hosts: ['example.com'], profile: 'link', requiredCookies: ['session', 'session_ts'], cookieNames: ['session', 'session_ts'] }");
	assert.equal(result.result, null);
...
test("unavailable SQLite backends produce actionable sanitized diagnostics", (t) => {
	...
	const bin = mkdtempSync(join(tmpdir(), "pi-cookie-bin-"));
	createFixture(home, "Profile 2", [["session", "one", ".example.com", null, 1]]);
