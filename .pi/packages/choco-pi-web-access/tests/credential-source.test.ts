...
test("literal and legacy environment credentials preserve existing precedence", async () => {
  assert.equal(
    await resolveCredential(options("literal-value", "environment-value")),
    "environment-value",
  );
  assert.equal(await resolveCredential(options("literal-value", undefined)), "literal-value");
  assert.equal(
    await resolveCredential(options(undefined, "environment-value")),
    "environment-value",
  );
...
test("command failures are categorized and redact command output", async () => {
  ...
  for (const [failure, category] of [
    [
      Object.assign(new Error(`spawn failed ${secret}`), { code: "ENOENT", stderr: secret }),
      "command-failed",
    ],
    [
      Object.assign(new Error(`timed out ${secret}`), { killed: true, stderr: secret }),
      "command-timeout",
    ],
    [
      Object.assign(new Error(`too large ${secret}`), {
...
test("escaped source prefixes remain literal and override legacy environment values", async () => {
  assert.equal(
    await resolveCredential(options("$$OPENAI_API_KEY", "legacy-value")),
    "$OPENAI_API_KEY",
  );
  assert.equal(
    await resolveCredential(options("$!literal-command", "legacy-value")),
    "!literal-command",
  );
