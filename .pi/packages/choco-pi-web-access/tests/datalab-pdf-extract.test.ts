...
const { extractPDFViaDatalab, getDatalabApiBase } = await import("../datalab-pdf-extract.ts");
...
test("Datalab conversion uploads the PDF, converts with a datalab reference, and polls to completion", async () => {
  ...
  assert.equal(byUrl.filter((url) => url.endsWith("/files/7/confirm")).length, 1);
  ...
  assert.equal(new Headers(upload.init.headers).get("x-api-key"), "synthetic-datalab-key");
  assert.equal(new Headers(upload.init.headers).get("content-type"), "application/json");
  ...
  assert.equal(new Headers(put.init.headers).get("content-type"), "application/pdf");

  const convert = calls.find((call) => call.url.endsWith("/convert"));
  assert.equal(convert.init.method, "POST");
  assert.equal(new Headers(convert.init.headers).get("x-api-key"), "synthetic-datalab-key");
...
test("Datalab conversion honors a custom API base", async () => {
  ...
  assert.ok(seen.some((url) => url.startsWith("https://datalab.example.test/api/v1/files/upload")));
  assert.ok(seen.some((url) => url.startsWith("https://datalab.example.test/api/v1/convert")));
...
function route(url) {
  ...
  return new Response(JSON.stringify({ detail: "unexpected fetch: " + urlString }), {
    status: 418,
    headers: { "content-type": "application/json" },
  });
}
