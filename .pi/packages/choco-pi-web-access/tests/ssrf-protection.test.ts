...
test("validateRemoteUrl permits public HTTP and HTTPS targets", async () => {
  assert.equal(
    (await validateRemoteUrl("https://example.com/path", { lookup: publicLookup })).hostname,
    "example.com",
  );
...
test("domain policy allows exact and subdomain matches, and is off by default", async () => {
  assert.equal(
    (await validateRemoteUrl("https://example.com/", { lookup: publicLookup })).hostname,
    "example.com",
  );
  assert.equal(
    (
      await validateRemoteUrl("https://www.example.com/", {
        lookup: publicLookup,
...
test("allowRanges accepts a bare host (no prefix) and treats it as /32", async () => {
  assert.equal(
    (await validateRemoteUrl("http://198.18.1.2/", { allowRanges: ["198.18.1.2"] })).hostname,
    "198.18.1.2",
  );
});
...
test("invalid allowRanges entries throw a descriptive error", async () => {
  for (const bad of ["not-an-ip", "198.18.0.0/33", "198.18.0.0/-1", "999.0.0.0/8", "fd00::/129"]) {
    await assert.rejects(
      validateRemoteUrl("http://198.18.0.5/", { allowRanges: [bad] }),
      new RegExp(
        `Invalid CIDR notation in ssrf.allowRanges: "${bad.replace(/[.*+?^${}()|[\\]\\\\]/g, "\\\$&")}"`,
      ),
    );
  }
  await assert.rejects(
