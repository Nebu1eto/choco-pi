...
function getSearchConfig(): SearchConfig {
	...
	if (Object.hasOwn(raw, "searchRouting")) cachedSearchConfig.searchRouting = normalizeSearchRouting(raw.searchRouting);
	return cachedSearchConfig;
}
...
function normalizeResolvedProviderList<Value>(value: Value, label: string): ResolvedSearchProvider[] {
...
export function normalizeSearchProviderSelection<Value>(value: Value, label = "provider"): SearchProviderSelection {
...
function shouldTryOpenAIInAuto(options: SearchOptions): boolean {
	...
	if (Check(NumberValueSchema, options.numResults) && Number.isFinite(options.numResults) && Math.floor(options.numResults) !== 5) return false;
	return true;
}
...
function classifyProviderError(provider: ResolvedSearchProvider, cause: unknown): SearchProviderError {
	...
	if (cause instanceof CredentialResolutionError || /(?:api )?key (?:not found|missing)|credential resolution/.test(lower)) kind = "credential";
	...
	else if (status !== undefined && (status === 408 || status === 425 || status >= 500)) kind = "transient";
	...
	else if (/invalid json|no parseable response|no parseable results|invalid response|returned empty response/.test(lower)) kind = "invalid-response";
	else if (/temporar|service unavailable|server error/.test(lower)) kind = "transient";
	else if (cause instanceof TypeError || /fetch failed|network|econnreset|econnrefused|enotfound|etimedout|timed out|socket/.test(lower)) kind = "network";
	else if (/invalid or missing|invalid config|failed to parse|must be an? |configuration/.test(lower)) kind = "config";
	return new SearchProviderError(provider, kind, message, status, cause);
}

async function searchWithResolvedProvider(provider: ResolvedSearchProvider, query: string, options: FullSearchOptions): Promise<ProviderSearchResponse> {
	if (provider === "openai") return { ...(await searchWithOpenAI(query, options, options.extensionContext)), provider };
...
async function searchWithProviders(query: string, options: FullSearchOptions, selectedProviders?: ResolvedSearchProvider[]): Promise<AttributedSearchResponse> {
	...
	if (providers.length === 0) throw new Error("No configured search provider available for provider \"all\".");

	const settled = await Promise.allSettled(providers.map((provider) => searchWithResolvedProvider(provider, query, options)));
	...
	if (failures.length > 0) answerSections.push(`## Provider errors\n\n${failures.map(({ provider, error }) => `- **${providerLabel(provider)}:** ${error}`).join("\n")}`);
...
async function searchWithConfiguredRouting(query: string, options: FullSearchOptions, routing: SearchRoutingConfig): Promise<AttributedSearchResponse> {
	...
	for (const provider of routing.providers) {
		...
		try {
			...
			if (!isFallbackKind(classified.kind) || !routing.fallbackOn.includes(classified.kind)) throw classified;
		}
	}
...
export async function search(query: string, options: FullSearchOptions = {}): Promise<AttributedSearchResponse> {
	...
	if (Array.isArray(provider)) return searchWithProviders(query, options, normalizeResolvedProviderList(provider, "provider"));
	...
	if (!config.searchProviderConfigured && config.searchRouting) return searchWithConfiguredRouting(query, options, config.searchRouting);
