...
interface SanitizeInlineDataUriResult {
  text: string;
...
function sanitizeTextInternal(text: string, sourcePath: string, state: SanitizationState): string {
  ...
  while (scanFrom < text.length) {
    ...
    if (missingComma) {
      ...
      const decoded =
        encoding === "base64" ? decodeBase64Payload(payload) : decodePercentEncoded(payload);
      if ("error" in decoded) {
    ...
    if (decodeError) omission.decodeError = decodeError;
