  ...
  if (!model)
    throw new Error(
      override
        ...
        : "No current model available for page answering",
    );
...
export async function answerFromPage(
  ...
  const response =
    registryComplete !== undefined
      ? await registryComplete.call(ctx.modelRegistry, model, completionContext, {
          signal,
          maxTokens: OUTPUT_TOKENS,
        })
      : await complete(model, completionContext, {
          apiKey: auth.apiKey,
