...
function textFromContentPart<Part>(part: Part): string {
	return isObject(part) && "text" in part && isString(part.text) ? part.text : "";
}

function responseText(content: Awaited<ReturnType<typeof complete>>["content"]): string {
...
export async function answerFromPage(
	...
	const response = registryComplete !== undefined
		? await registryComplete.call(ctx.modelRegistry, model, completionContext, { signal, maxTokens: OUTPUT_TOKENS })
		: await complete(model, completionContext, { apiKey: auth.apiKey, headers: auth.headers, signal, maxTokens: OUTPUT_TOKENS });
