...
export function buildSearchErrorPlan(details: SearchErrorDetails | undefined | null): SearchErrorPlan | null {
	...
	const queryCount = details.queryCount !== undefined && details.queryCount > 0
		? details.queryCount
		: queries.length;
	...
	if (details.cancelled === true || queries.length > 0) {
		...
		if (details.lastHeartbeatAgeMs !== undefined && details.lastHeartbeatAgeMs !== null && Number.isFinite(details.lastHeartbeatAgeMs)) {
