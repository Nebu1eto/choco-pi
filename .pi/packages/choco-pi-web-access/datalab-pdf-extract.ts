...
import { hasCredentialSource, resolveCredential, redactCredential } from "./credential-source.ts";
...
function primitiveString(value: JsonValue | undefined): string | null {
...
function loadConfig(): DatalabConfig {
  ...
  try {
    ...
    cachedConfig = {};
    if (parsed.datalabApiKey !== undefined) cachedConfig.datalabApiKey = parsed.datalabApiKey;
    return cachedConfig;
  } catch (err) {
...
function normalizeFileId(value: JsonValue | undefined): string | null {
...
export function getDatalabProcessingLocation(): DatalabProcessingLocation {
  const value =
    normalizeString(process.env.DATALAB_PROCESSING_LOCATION) ?? DEFAULT_PROCESSING_LOCATION;
...
export function normalizeDatalabMode<T>(value: T): DatalabMode {
  const stringValue =
    Object.prototype.toString.call(value) === "[object String]" && Object(value) !== value
      ? String(value)
      : null;
  const raw = stringValue?.trim() || null;
  if (!raw) return DEFAULT_DATALAB_MODE;
  const normalized = raw.toLowerCase();
  if (normalized === "fast" || normalized === "balanced" || normalized === "accurate")
    return normalized;
  throw new Error(
...
interface DatalabUploadRequest extends JsonObject {
  file_id?: JsonValue;
  upload_url?: JsonValue;
  reference?: JsonValue;
}

interface DatalabConfirmResponse extends JsonObject {
  file_id?: JsonValue;
  reference?: JsonValue;
}

interface DatalabConversionState extends JsonObject {
  status?: JsonValue;
  request_check_url?: JsonValue;
  markdown?: JsonValue;
  page_count?: JsonValue;
  parse_quality_score?: JsonValue;
  error?: JsonValue;
}
...
export async function extractPDFViaDatalab(
  ...
  const uploadUrl = primitiveString(upload.upload_url);
  if (!uploadUrl) {
...
function toResult(state: DatalabConversionState): DatalabPDFExtractResult {
  ...
  const result: DatalabPDFExtractResult = {
    markdown,
    pages: numericField(state.page_count) ?? countPageMarkers(markdown),
  };
  if (isNumber(quality) && Number.isFinite(quality)) result.parseQualityScore = quality;
  return result;
}

function numericField(value: JsonValue | undefined): number | null {
  return isNumber(value) && Number.isFinite(value) && value > 0 ? Math.floor(value) : null;
}
...
function normalizeCheckUrl(value: JsonValue | undefined): string {
  const stringValue = primitiveString(value);
  ...
  let checkUrl: URL;
  try {
    checkUrl = new URL(stringValue.trim(), apiBase);
  } catch {
...
async function readJsonResponse(response: Response, apiKey: string): Promise<JsonObject> {
...
function parseJsonRecord(text: string): JsonObject {
