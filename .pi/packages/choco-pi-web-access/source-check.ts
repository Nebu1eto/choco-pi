...
function isSourceQuality<Value>(value: Value): value is Value & SourceQuality {
	return value === "official_docs" || value === "vendor_docs" || value === "repo_issue" ||
		value === "blog" || value === "forum" || value === "news" || value === "unknown";
}

function isClaimStatus<Value>(value: Value): value is Value & ClaimStatus {
	return value === "supported" || value === "contradicted" || value === "unclear" || value === "missing-evidence";
}
...
function isResearchSource<Value>(value: Value): value is Value & ResearchSource {
	return isObject(value) &&
		...
		"quality" in value && isSourceQuality(value.quality) &&
...
function isExtractionSpan<Value>(value: Value): value is Value & { start: number; end: number } {
	return isObject(value) &&
		"start" in value && isNumber(value.start) &&
		"end" in value && isNumber(value.end);
}

function isResearchPassage<Value>(value: Value): value is Value & ResearchPassage {
	return isObject(value) &&
		"passage_id" in value && isString(value.passage_id) &&
		"source_url" in value && isString(value.source_url) &&
		"source_rank" in value && isNumber(value.source_rank) &&
		"text" in value && isString(value.text) &&
...
function isClaimAssessment<Value>(value: Value): value is Value & ClaimAssessment {
	return isObject(value) &&
		"claim" in value && isString(value.claim) &&
		"status" in value && isClaimStatus(value.status) &&
		"supporting_passages" in value && isStringArray(value.supporting_passages) &&
		"contradicting_passages" in value && isStringArray(value.contradicting_passages) &&
		"rationale" in value && isString(value.rationale) &&
		"confidence" in value && isNumber(value.confidence);
}

function isResearchFilters<Value>(value: Value): value is Value & NonNullable<ResearchArtifact["filters"]> {
	return isObject(value) &&
...
function isResearchArtifact<Value>(value: Value): value is Value & ResearchArtifact {
	return isObject(value) &&
		...
		"type" in value && value.type === "research" &&
		"timestamp" in value && isNumber(value.timestamp) &&
		"query" in value && isString(value.query) &&
		"sources" in value && Array.isArray(value.sources) && value.sources.every(isResearchSource) &&
		"passages" in value && Array.isArray(value.passages) && value.passages.every(isResearchPassage) &&
		(!("claims" in value) || (Array.isArray(value.claims) && value.claims.every(isClaimAssessment))) &&
