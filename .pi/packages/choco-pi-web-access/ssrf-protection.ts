...
function parseDomainEntries(value: JsonValue | undefined, field: "allow" | "deny"): string[] {
	...
	return value.map((entry, index) => {
		if (!isString(entry)) {
			throw new Error(`fetchContent.domainPolicy.${field} in ${WEB_SEARCH_CONFIG_PATH} must contain only hostnames; entry ${index + 1} is ${jsonType(entry)}`);
		}
...
export function loadSsrfConfig(): SsrfConfig {
	...
	const allowRanges = allowRangesValue.map((entry, index) => {
		if (!isString(entry)) {
			throw new Error(`ssrf.allowRanges in ${WEB_SEARCH_CONFIG_PATH} must contain only CIDR strings; entry ${index + 1} is ${jsonType(entry)}`);
		}
		return entry.trim();
	}).filter(Boolean);
