...
let cachedConfigRoot: { signature: string; value: JsonObject | null } | null = null;
...
function loadConfigRoot(): JsonObject | null {
...
function parseDomainEntries(value: JsonValue | undefined, field: "allow" | "deny"): string[] {
  if (value === undefined || value === null) return [];
  if (!Array.isArray(value)) {
    throw new Error(
      `fetchContent.domainPolicy.${field} in ${WEB_SEARCH_CONFIG_PATH} must be an array of hostnames`,
    );
  }
  return value.map((entry, index) => {
    if (!isString(entry)) {
      throw new Error(
        `fetchContent.domainPolicy.${field} in ${WEB_SEARCH_CONFIG_PATH} must contain only hostnames; entry ${index + 1} is ${jsonType(entry)}`,
      );
    }
...
export function loadSsrfConfig(): SsrfConfig {
  ...
  if (!isJsonObject(ssrf)) {
    throw new Error(`ssrf in ${WEB_SEARCH_CONFIG_PATH} must be an object`);
  }
  const config = ssrf;
  if (
    config.allowRanges !== undefined &&
    config.allowRanges !== null &&
    !Array.isArray(config.allowRanges)
  ) {
    throw new Error(
      `ssrf.allowRanges in ${WEB_SEARCH_CONFIG_PATH} must be an array of CIDR strings`,
    );
  }
  if (config.trustEnvProxy !== undefined && !isBoolean(config.trustEnvProxy)) {
    throw new Error(`ssrf.trustEnvProxy in ${WEB_SEARCH_CONFIG_PATH} must be a boolean`);
  }
  const allowRangesValue: JsonValue[] = Array.isArray(config.allowRanges) ? config.allowRanges : [];
  const allowRanges = allowRangesValue
    .map((entry, index) => {
      if (!isString(entry)) {
        throw new Error(
          `ssrf.allowRanges in ${WEB_SEARCH_CONFIG_PATH} must contain only CIDR strings; entry ${index + 1} is ${jsonType(entry)}`,
        );
      }
      return entry.trim();
    })
...
function parseAllowRanges(input: JsonValue | undefined): ParsedCidr[] {
  ...
  for (const entry of input) {
    if (!isString(entry)) {
      throw new Error(`ssrf.allowRanges entries must be strings, got ${jsonType(entry)}`);
    }
