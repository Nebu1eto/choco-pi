...
function resolveConfiguredSearchProviders<Value>(value: Value): readonly string[] {
	...
	if (!Array.isArray(value) || value.some((entry) => !Check(StringValueSchema, entry) || entry.trim().length === 0)) {
...
function buildInstructions(options: SearchOptions): string {
	...
	if (options.recencyFilter) {
		...
		switch (options.recencyFilter) {
			case "day": label = "past 24 hours"; break;
			case "week": label = "past week"; break;
			case "month": label = "past month"; break;
			case "year": label = "past year"; break;
		}
		lines.push(`Prefer sources from the ${label}.`);
	}

	if (Check(NumberValueSchema, options.numResults) && Number.isFinite(options.numResults) && options.numResults > 0) {
...
async function parseOpenAIResponse(response: Response): Promise<OpenAIJsonObject> {
	...
	for (const line of text.split("\n")) {
		...
		try {
			...
			if ((parsed.type === "response.done" || parsed.type === "response.completed") && isJsonObject(parsed.response)) {
...
function extractSnippetAround(text: string, start: OpenAIJsonValue | undefined, end: OpenAIJsonValue | undefined): string {
...
function addResult(results: SearchResult[], seen: Set<string>, url: OpenAIJsonValue | undefined, title: OpenAIJsonValue | undefined, snippet = ""): void {
...
function extractSearchResults(output: OpenAIJsonValue[], numResults: number | undefined): SearchResult[] {
