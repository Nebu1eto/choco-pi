...
interface SqliteRow { [column: string]: DatabaseValue | undefined }
...
export async function getBrowserCookiesForHosts(
	...
	const rawProfile = Object.prototype.toString.call(options.profile) === "[object String]" && Object(options.profile) !== options.profile
		...
		for (const profile of profiles) {
			...
			try {
				...
				for (const row of rowsResult.rows) {
					...
					if (!value && isStringValue(row.encrypted_value_hex) && /^[0-9a-f]*$/i.test(row.encrypted_value_hex)) {
...
async function readWindowsEncryptionKey(config: BrowserConfig, home: string): Promise<Buffer | null> {
	try {
		const localState = parseJson(readFileSync(join(browserBasePath(home, config), "Local State"), "utf8"));
...
async function readCookieColumns(dbPath: string): Promise<{ status: "success"; columns: Set<string> } | { status: "failure"; failure: SqliteFailure }> {
	...
	return { status: "success", columns: new Set(result.rows.map(row => isStringValue(row.name) ? row.name : "")) };
}
