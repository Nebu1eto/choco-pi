...
type DatabaseValue = JsonValue | bigint | Uint8Array;
interface SqliteRow {
  [column: string]: DatabaseValue | undefined;
}
...
function isStringValue(value: DatabaseValue | undefined): value is string {
  return Object.prototype.toString.call(value) === "[object String]" && Object(value) !== value;
}

function isNumberValue(value: DatabaseValue | undefined): value is number {
  return Object.prototype.toString.call(value) === "[object Number]" && Object(value) !== value;
}

function parseJson(text: string): JsonValue {
  // SAFETY: JSON.parse accepts only JSON syntax, whose runtime values are exactly JsonValue.
  return JSON.parse(text) as JsonValue;
}

function parseJsonRows(value: JsonValue): SqliteRow[] | null {
...
type EmitWarningArgument = string | Function | EmitWarningOptions | undefined;
type EmitWarningForwarder = (warning: string | Error, ...args: EmitWarningArgument[]) => void;
...
export async function getBrowserCookiesForHosts(options: {
  ...
  requestUrl?: URL;
}): Promise<BrowserCookieResult | null> {
  lastCookieDiagnostic = null;
  if (!isBrowserCookieAccessAllowed()) {
    lastCookieDiagnostic =
      "Browser cookie access is disabled; enable allowBrowserCookies to use browser cookies.";
    return null;
  }
  ...
  const rawProfile =
    Object.prototype.toString.call(options.profile) === "[object String]" &&
    Object(options.profile) !== options.profile
      ? String(options.profile).trim()
      : "";
  const requestedProfile = normalizeProfileName(options.profile);
  if (rawProfile && !requestedProfile) {
    lastCookieDiagnostic =
      "Configured Chromium profile must be a profile directory name, not a path.";
    return null;
  }
  const requiredCookies = normalizeCookieNames(options.requiredCookies);
  const cookieNames = normalizeCookieNames(
    options.cookieNames ? [...options.cookieNames] : undefined,
  );
  ...
  for (const config of configs) {
    ...
    for (const profile of profiles) {
      ...
      try {
        ...
        const rowsResult = await queryCookieRows(
          tempDb,
          hosts,
          cookieNames ?? null,
          Boolean(options.requestUrl),
        );
        ...
        for (const row of rowsResult.rows) {
          const name = isStringValue(row.name) ? row.name : "";
          if (!name) continue;
          let value = isStringValue(row.value) && row.value.length > 0 ? row.value : null;
          if (
            !value &&
            isStringValue(row.encrypted_value_hex) &&
          ...
          if (!value) continue;
          const path = isStringValue(row.path) && row.path.startsWith("/") ? row.path : "/";
        ...
        const result: BrowserCookieResult = {
          cookies,
          warnings: [...warningSet],
        };
        if (options.requestUrl) result.cookieHeader = buildCookieHeader(entries);
        return result;
      } finally {
  ...
  if (sawBackendFailure === "unavailable") {
    ...
    lastCookieDiagnostic =
      "Configured Chromium profile must resolve inside the browser profile root.";
    ...
    lastCookieDiagnostic = "Required browser cookies were not available or could not be decrypted.";
  }
  return null;
}
...
function normalizeCookieNames(names: string[] | undefined): string[] | undefined {
  ...
  const normalized = names
    .flatMap((name) =>
      Object.prototype.toString.call(name) === "[object String]" && Object(name) !== name
        ? [String(name).trim()]
        : [],
    )
    .filter(Boolean);
...
function compareProfileNames(a: string, b: string): number {
  ...
  const [bp, bi] = key(b);
  return (
    ap - bp || ai - bi || a.localeCompare(b, undefined, { sensitivity: "base", numeric: true })
  );
}
...
function decryptCookieValue(encrypted: Uint8Array, key: Buffer, stripHash: boolean): string | null {
  ...
  try {
    ...
    const unpadded = removePkcs7Padding(
...
function readBrowserPassword(
  config: BrowserConfig,
  currentPlatform: typeof process.platform,
): Promise<string | null> {
...
async function readWindowsEncryptionKey(
  config: BrowserConfig,
  home: string,
): Promise<Buffer | null> {
  try {
    const localState = parseJson(
      readFileSync(join(browserBasePath(home, config), "Local State"), "utf8"),
    );
    if (!isJsonObject(localState)) return null;
    const osCrypt = localState.os_crypt;
    if (!isJsonObject(osCrypt)) return null;
    const encodedKey = osCrypt.encrypted_key;
    if (!isStringValue(encodedKey)) return null;
...
function unprotectWindowsData(protectedData: Buffer): Promise<Buffer | null> {
  return new Promise((resolve) => {
    const script =
      "$data=[Convert]::FromBase64String($args[0]);$clear=[Security.Cryptography.ProtectedData]::Unprotect($data,$null,[Security.Cryptography.DataProtectionScope]::CurrentUser);[Console]::Write([Convert]::ToBase64String($clear))";
    execFile(
      "powershell.exe",
      ["-NoProfile", "-NonInteractive", "-Command", script, protectedData.toString("base64")],
  ...
  return new Promise((resolve) => {
    execFile(
      ...
      (err, stdout) => {
        ...
        resolve(
          password ? { password, cacheable: true } : { password: "peanuts", cacheable: false },
        );
      },
    );
  });
...
async function importSqlite(): Promise<typeof import("node:sqlite") | null> {
  ...
  // SAFETY: EmitWarningForwarder lists every positional argument accepted by Node's emitWarning overloads.
  const forwardWarning = orig as EmitWarningForwarder;
  const emitWarning = (warning: string | Error, ...args: EmitWarningArgument[]): void => {
    const msg = warning instanceof Error ? warning.message : warning;
    if (msg.includes("SQLite is an experimental feature")) return;
    forwardWarning(warning, ...args);
  };
  // SAFETY: emitWarning accepts every documented emitWarning overload and preserves its arguments unchanged.
  ...
  if (sqlite) {
    try {
      ...
      try {
        ...
        // SAFETY: node:sqlite returns row objects whose values are SQLite scalar values.
        return { status: "success", rows: rows as SqliteRow[] };
      } finally {
        db.close();
      }
    } catch {
  ...
  return new Promise((resolve) => {
    execFile(
      ...
      (err, stdout) => {
        ...
        try {
          const rows = parseJsonRows(parseJson(stdout || "[]"));
          resolve(rows ? { status: "success", rows } : { status: "failure", failure: "query" });
        } catch {
...
async function readMetaVersion(
  ...
  if (isNumberValue(value)) return { value: Math.floor(value) };
  if (isStringValue(value)) return { value: parseInt(value, 10) || 0 };
  return { value: 0 };
}
...
async function hasCookieNames(
  ...
  const present = new Set(result.rows.map((row) => (isStringValue(row.name) ? row.name : "")));
...
async function queryCookieRows(
  ...
  const expiryFilter =
    filterExpired && columns.columns.has("expires_utc")
      ? ` AND (expires_utc = 0 OR expires_utc > ${chromeExpiryNowMicros()})`
      : "";
  const partitionFilter = filterExpired ? unpartitionedCookieFilter(columns.columns) : "";
  return runSqliteQuery(
    dbPath,
    `SELECT name, value, host_key, ${pathExpr}, ${expiresExpr}, hex(encrypted_value) AS encrypted_value_hex FROM cookies WHERE ${buildCookieWhere(hosts, names ?? undefined)}${expiryFilter}${partitionFilter} ORDER BY length(path) DESC, expires_utc ASC`,
  );
}
...
async function readCookieColumns(
  ...
  { status: "success"; columns: Set<string> } | { status: "failure"; failure: SqliteFailure }
> {
  const result = await runSqliteQuery(dbPath, "PRAGMA table_info(cookies)");
  if (result.status === "failure") return result;
  return {
    status: "success",
    columns: new Set(result.rows.map((row) => (isStringValue(row.name) ? row.name : ""))),
  };
}
