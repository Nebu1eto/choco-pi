...
function metadataForUrls(urls: ExtractedContent[]): StoredFetchUrlMetadata[] {
	return urls.map((url) => {
		const metadata: StoredFetchUrlMetadata = {
		...
		if (url.mimeType) metadata.mimeType = truncateMetadataText(url.mimeType);
		if (url.status !== undefined) metadata.status = url.status;
		return metadata;
	});
}
...
function isFetchCacheRef<Value>(value: Value): value is Value & FetchCacheRef {
	return isObject(value) &&
		"version" in value && value.version === FETCH_CACHE_VERSION &&
		"key" in value && isString(value.key) && CACHE_KEY_PATTERN.test(value.key) &&
		"storedAt" in value && isNumber(value.storedAt);
}

function isStoredFetchUrlMetadata<Value>(value: Value): value is Value & StoredFetchUrlMetadata {
	return isObject(value) &&
		"url" in value && isString(value.url) &&
		"title" in value && isString(value.title) &&
		"error" in value && (value.error === null || isString(value.error)) &&
		"contentLength" in value && isNumber(value.contentLength) && value.contentLength >= 0 &&
		(!("mimeType" in value) || isString(value.mimeType)) &&
		(!("status" in value) || isNumber(value.status));
}

function isThumbnail<Value>(value: Value): value is Value & NonNullable<ExtractedContent["thumbnail"]> {
	return isObject(value) &&
		"data" in value && isString(value.data) &&
		"mimeType" in value && isString(value.mimeType);
}

function isInlineFetchedUrl<Value>(value: Value): value is Value & ExtractedContent {
	return isObject(value) &&
		"url" in value && isString(value.url) &&
		"title" in value && isString(value.title) &&
		"content" in value && isString(value.content) &&
		"error" in value && (value.error === null || isString(value.error)) &&
		(!("thumbnail" in value) || isThumbnail(value.thumbnail)) &&
		(!("mimeType" in value) || isString(value.mimeType)) &&
		(!("status" in value) || isNumber(value.status));
}
...
function errorCode(cause: unknown): string | undefined {
	return cause instanceof Error && "code" in cause && isString(cause.code)
		? cause.code
		: undefined;
}
...
function safeFetchCacheDir(create: boolean): string | null {
	...
	try {
		...
		if (!create && errorCode(err) === "ENOENT") return null;
		throw err;
	}
...
function openRegularFile(path: string): OpenedRegularFile {
...
function unlinkCacheFile(dir: string, file: CacheFile): CacheUnlinkResult {
	try {
		...
		try {
			...
			return errorCode(err) === "ENOENT" ? "missing" : "error";
		}
...
function cacheWriteError(cause: unknown): string {
...
function createFetchSessionData(data: StoredSearchData & { urls: ExtractedContent[] }, ref: FetchCacheRef | null, cacheError?: string): StoredSearchData {
	const sessionData: StoredSearchData = {
		id: data.id,
		type: "fetch",
		timestamp: data.timestamp,
		urlMetadata: metadataForUrls(data.urls),
	};
	if (ref) sessionData.fetchCache = ref;
	if (cacheError) sessionData.fetchCacheError = truncateMetadataText(cacheError);
	return sessionData;
}
...
function unavailableFetchData(data: StoredSearchData, reason: string): StoredSearchData {
	return {
		...
		urls: fetchUrlMetadata(data).map((meta) => {
			const url: ExtractedContent = {
				url: meta.url,
				title: meta.title,
				content: "",
				error: reason,
			};
			if (meta.mimeType) url.mimeType = meta.mimeType;
			if (meta.status !== undefined) url.status = meta.status;
			return url;
		}),
	};
}
...
function isSearchResult<Value>(value: Value): value is Value & SearchResult {
	return isObject(value) &&
		"title" in value && isString(value.title) &&
		"url" in value && isString(value.url) &&
...
function isQueryResultData<Value>(value: Value): value is Value & QueryResultData {
	return isObject(value) &&
		"query" in value && isString(value.query) &&
		"answer" in value && isString(value.answer) &&
		"results" in value && Array.isArray(value.results) && value.results.every(isSearchResult) &&
		"error" in value && (value.error === null || isString(value.error)) &&
		(!("provider" in value) || isString(value.provider));
}

function isValidStoredData<Value>(data: Value): data is Value & StoredSearchData {
	if (!isObject(data) || !("id" in data) || !isString(data.id) || !data.id) return false;
	if (!("type" in data) || (data.type !== "search" && data.type !== "fetch" && data.type !== "research")) return false;
	if (!("timestamp" in data) || !isNumber(data.timestamp)) return false;
	if (data.type === "search") {
		return "queries" in data && Array.isArray(data.queries) && data.queries.every(isQueryResultData);
	}
	if (data.type === "fetch") {
		if ("urls" in data && Array.isArray(data.urls)) return data.urls.every(isInlineFetchedUrl);
		if (!("urlMetadata" in data) || !Array.isArray(data.urlMetadata) || !data.urlMetadata.every(isStoredFetchUrlMetadata)) return false;
		if (!("fetchCache" in data)) return !("fetchCacheError" in data) || isString(data.fetchCacheError);
		return isFetchCacheRef(data.fetchCache);
	}
	return "artifact" in data && isObject(data.artifact);
}
