...
function isFetchCacheRef<Value>(value: Value): value is Value & FetchCacheRef {
  return (
    isObject(value) &&
    "version" in value &&
    value.version === FETCH_CACHE_VERSION &&
    "key" in value &&
    isString(value.key) &&
    CACHE_KEY_PATTERN.test(value.key) &&
    "storedAt" in value &&
    isNumber(value.storedAt)
  );
}
...
function isThumbnail<Value>(
  value: Value,
): value is Value & NonNullable<ExtractedContent["thumbnail"]> {
  return (
    isObject(value) &&
    "data" in value &&
    isString(value.data) &&
...
function pruneFetchCache(
  ...
  while (usage.entries > limits.maxEntries || usage.bytes > limits.maxBytes) {
    const index = files.findIndex(
      (file) => file.name !== preferredKey && !attempted.has(file.name),
    );
    if (index < 0) break;
...
function writeFetchCache(data: StoredSearchData & { urls: ExtractedContent[] }): FetchCacheRef {
  ...
  if (size > limits.maxBytes)
    throw new Error(`Fetched content cache entry exceeds ${limits.maxBytes} bytes`);
...
function readCachedFetchData(data: StoredSearchData, now = Date.now()): StoredSearchData {
  ...
  if (!data.fetchCache) {
    return unavailableFetchData(
      data,
      data.fetchCacheError ?? "Cached fetched content is unavailable",
    );
  }
...
export function pruneExpiredFetchCache(
  now = Date.now(),
  requestedLimits?: Partial<FetchCacheLimits>,
): void {
...
function isValidStoredData<Value>(data: Value): data is Value & StoredSearchData {
  ...
  if (
    !("type" in data) ||
    (data.type !== "search" && data.type !== "fetch" && data.type !== "research")
  )
    return false;
  if (!("timestamp" in data) || !isNumber(data.timestamp)) return false;
  if (data.type === "search") {
    return (
      "queries" in data && Array.isArray(data.queries) && data.queries.every(isQueryResultData)
    );
  }
  if (data.type === "fetch") {
    ...
    if (
      !("urlMetadata" in data) ||
      !Array.isArray(data.urlMetadata) ||
      !data.urlMetadata.every(isStoredFetchUrlMetadata)
    )
      return false;
    if (!("fetchCache" in data))
      return !("fetchCacheError" in data) || isString(data.fetchCacheError);
