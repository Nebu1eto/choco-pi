...
type ExaMcpArguments = ExaMcpBasicArguments | ExaSearchArguments;
...
function loadConfig(): WebSearchConfig {
	...
	try {
		// SAFETY: WebSearchConfig intentionally leaves its two config fields unparsed for their owning credential and URL boundary parsers.
...
function exaApiHeaders(apiKey: string) {
	return {
		"x-api-key": apiKey,
...
function mapDomainFilter(domainFilter: string[] | undefined): ExaDomainFilter {
	...
	const excludeDomains = domainFilter
		...
		.filter(Boolean);
	const mapped: ExaDomainFilter = {};
	if (includeDomains.length) mapped.includeDomains = includeDomains;
	if (excludeDomains.length) mapped.excludeDomains = excludeDomains;
	return mapped;
}

function exaSearchArgs(query: string, options: ExaSearchOptions): ExaSearchArguments {
...
function normalizeHighlights(value: ExaSearchResponse["results"] extends Array<infer Result> | undefined ? Result extends { highlights?: infer Highlights } ? Highlights : never : never): string[] {
	if (!Array.isArray(value)) return [];
	return value.filter((item): item is string => Check(StringValueSchema, item) && item.trim().length > 0);
}
...
function buildAnswerFromSearchResults(results: ExaSearchResponse["results"]): string {
	...
	for (let i = 0; i < results.length; i++) {
		...
		const content = highlights.length > 0
			? highlights.join(" ")
			: Check(StringValueSchema, item.text) ? item.text.trim().slice(0, 1000) : "";
		if (!content) continue;
...
function mapInlineContent(results: ExaSearchResponse["results"]): ExtractedContent[] {
	...
	return results
		.filter((r): r is NonNullable<ExaSearchResponse["results"]>[number] & { url: string; text: string } =>
			!!r?.url && Check(StringValueSchema, r.text) && r.text.length > 0)
		.map(r => ({
			url: r.url,
			title: r.title || "",
			content: r.text,
...
export async function callExaMcp(
	...
	for (const line of dataLines) {
		...
		if (!payload) continue;
		try {
			// SAFETY: Exa MCP JSON-RPC responses own the optional result/error envelope consumed below.
	...
	const text = parsed.result?.content
		?.find(item => item.type === "text" && Check(StringValueSchema, item.text) && item.text.trim().length > 0)
		?.text;
...
function parseJsonMcpResults(text: string): ExaSearchResponse["results"] | null {
	try {
		// SAFETY: The advanced Exa MCP tool returns the documented Exa search response envelope.
...
async function searchWithFilteredExaMcp(
	query: string,
	options: ExaSearchOptions,
	basicArgs: ExaMcpBasicArguments,
): Promise<SearchResponse | null> {
	try {
