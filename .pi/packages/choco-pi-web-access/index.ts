...
function resolveToolNames(config: WebSearchConfig): ToolNames {
  ...
  for (const key of registeredKeys) {
    ...
    if (previous)
      throw new Error(
        `toolNames.${key} duplicates toolNames.${previous} in ${WEB_SEARCH_CONFIG_PATH}`,
      );
    seen.set(name, key);
  }
  return names;
...
function resolveCuratorSearchProvider<Value>(
  requested: Value,
...
interface PendingCurate {
  ...
  curatorUrl?: string;
  onUpdate:
    | ((update: { content: Array<{ type: string; text: string }>; details?: DetailObject }) => void)
    | undefined;
...
function storeFetchResult(
  pi: { appendEntry<Value>(type: string, data: Value): void },
  responseId: string,
...
export default function (pi: ExtensionAPI) {
  ...
  async function loadSummaryModelChoices(summaryContext: SummaryGenerationContext): Promise<{
    ...
    const config = loadConfig();
    const configuredSummaryModel = Check(StringValueSchema, config.summaryModel)
  ...
  function resolveSummaryForSubmit(
    ...
    const submittedSummary = Check(StringValueSchema, payload.summary)
  ...
  function buildSearchReturn(opts: SearchReturnOptions): AgentToolResult<DetailObject> {
    ...
    const approvedSummary = Check(StringValueSchema, opts.approvedSummary)
      ? opts.approvedSummary.trim()
      : "";
        ...
        if (shouldCurate) {
          ...
          for (let qi = 0; qi < queryList.length; qi++) {
            ...
            try {
              ...
              for (let entryIndex = 0; entryIndex < entries.length; entryIndex++) {
                ...
                if (curator) {
                  if (entry.error) {
                    ...
                    curator.pushResult(resultIndex, {
                      ...entry,
                      query: queryList[qi],
                      slotIndex: qi,
                    });
              ...
              const curator = activeCurators.get(callId);
              if (curator) {
                curator.pushError(qi, message, failedProvider, {
                  query: queryList[qi],
                  slotIndex: qi,
                });
              }
      ...
      renderResult(result, { expanded, isPartial }, theme) {
        ...
        if (isPartial) {
          if (details?.phase === "curator-fallback") {
            ...
            const timeout = Check(NumberValueSchema, details?.timeoutSeconds)
              ? details.timeoutSeconds
              : undefined;
            const shortcut = Check(StringValueSchema, details?.shortcut)
              ? details.shortcut
              : curateKey;
            lines.push(
              theme.fg(
  ...
  if (sourceCheckEnabled)
    pi.registerTool({
      ...
      parameters: Type.Object({
        ...
        provider: Type.Optional(
          searchProviderSchema(
            "Search provider or non-empty list of providers to search simultaneously; all searches every available provider",
          ),
        ),
      }),
      async execute(_callId, params, signal, _onUpdate, ctx) {
        ...
        const requestedQueries = Array.isArray(params.queries)
          ? params.queries
              .filter((query): query is string => Check(StringValueSchema, query))
              .map((query) => query.trim())
              .filter(Boolean)
          : [];
        const queries = (requestedQueries.length > 0 ? requestedQueries : [claim]).slice(0, 8);
        const numResults =
          Check(NumberValueSchema, params.numResults) && Number.isFinite(params.numResults)
            ? Math.min(20, Math.max(1, Math.floor(params.numResults)))
            : 5;
        const domainFilter = Array.isArray(params.domainFilter)
          ? params.domainFilter.filter((domain): domain is string =>
              Check(StringValueSchema, domain),
            )
          : undefined;
        const recencyFilter = normalizeRecencyFilter(params.recencyFilter);
  ...
  if (fetchContentEnabled)
    pi.registerTool({
      ...
      async execute(
        ...
        onUpdate,
        ctx,
      ): Promise<AgentToolResult<DetailObject>> {
        ...
        if (
          mode === "raw" &&
          (options.forceClone === true || options.prompt || options.answerModel)
        ) {
          return {
            content: [
              {
                type: "text",
                text: "Error: mode raw cannot be combined with forceClone, prompt, or answerModel.",
              },
            ],
      ...
      renderResult(result, { expanded, isPartial }, theme) {
        ...
        if (details?.error) {
          const extras: string[] = [];
          if (
            Check(NumberValueSchema, details.urlCount) ||
            Check(NumberValueSchema, details.successful)
          ) {
      ...
      parameters: Type.Object({
        ...
        findText: Type.Optional(
          Type.Union(
            ...
            {
              description:
                "Text or texts to find in the selected stored content. Cannot be combined with offset or limit.",
            },
          ),
        ),
        ...
        if (
          ...
          return {
            content: [
              {
                type: "text",
                text: `findText cannot be combined with offset or limit. Received offset=${offset}, limit=${limit}; omit offset and limit when using findText.`,
              },
            ],
          ...
          if (params.findText !== undefined) {
            try {
              const found = findContent(
                ...
                normalizeFindMode(params.findMode),
              );
