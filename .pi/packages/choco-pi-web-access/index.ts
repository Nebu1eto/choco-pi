...
import { getWebSearchConfigDir, getWebSearchConfigPath, resolveCuratorNetworkConfig } from "./utils.ts";
import {
	clearResults,
	deleteResult,
	generateId,
...
function isAbortError(cause: unknown): boolean {
	return (cause instanceof Error ? cause.message : String(cause)).toLowerCase().includes("abort");
}
...
type DetailScalar = boolean | number | string | null | undefined;
type DetailValue = DetailScalar | DetailObject | DetailValue[];
interface DetailObject {
	[key: string]: DetailValue;
}

interface WebSearchConfig extends ConfigObject {
	openaiApiKey?: ConfigValue;
	exaApiKey?: ConfigValue;
	...
	curatorRemote?: ConfigValue;
	summaryModel?: string;
	summaryGenerationDeadlineMs?: ConfigValue;
	maxInlineContentChars?: ConfigValue;
	webSearch?: {
		enabled?: boolean;
	};
...
function isDetailObject<Value>(value: Value): value is Value & DetailObject {
	return value !== null && Object(value) === value && !Array.isArray(value);
}

function parseConfigRoot(raw: string): ConfigObject {
...
function isToolEnabled(config: WebSearchConfig, key: keyof ToolNames): boolean {
	...
	if (Check(BooleanValueSchema, override)) return override;
...
function resolveToolNames(config: WebSearchConfig): ToolNames {
	if (config.toolNames !== undefined && !isConfigObject(config.toolNames)) {
		throw new Error(`toolNames in ${WEB_SEARCH_CONFIG_PATH} must be an object`);
	}
	const names = { ...DEFAULT_TOOL_NAMES };
	// SAFETY: DEFAULT_TOOL_NAMES is a closed ToolNames object, so Object.keys returns only ToolNames keys.
	for (const key of Object.keys(DEFAULT_TOOL_NAMES) as Array<keyof ToolNames>) {
		const value = config.toolNames?.[key];
		if (value === undefined) continue;
		if (!Check(StringValueSchema, value)) throw new Error(`toolNames.${key} in ${WEB_SEARCH_CONFIG_PATH} must be a string`);
...
function normalizeProviderInput<Value>(value: Value, label = "provider"): SearchProviderSelection | undefined {
	if (value === undefined) return undefined;
	return normalizeSearchProviderSelection(value, label);
}

function resolveRequestedProvider<Value>(requested: Value): SearchProviderSelection {
...
function resolveCuratorSearchProvider<Value>(requested: Value, current: SearchProviderSelection): SearchProviderSelection {
...
function normalizeRecencyFilter<Value>(value: Value): RecencyFilter | undefined {
	if (!Check(StringValueSchema, value)) return undefined;
	switch (value) {
		case "day": return "day";
...
function normalizeCuratorTimeoutSeconds<Value>(value: Value): number | undefined {
	if (!Check(NumberValueSchema, value) || !Number.isFinite(value)) return undefined;
	const normalized = Math.floor(value);
	if (normalized < 1) return undefined;
	return Math.min(normalized, MAX_CURATOR_TIMEOUT_SECONDS);
}

function resolveWorkflow<Value>(input: Value, hasUI: boolean): WebSearchWorkflow {
	const normalized = Check(StringValueSchema, input) ? input.trim().toLowerCase() : "";
...
function normalizeQueryList(queryList: unknown[]): string[] {
	...
	for (const query of queryList) {
		if (!Check(StringValueSchema, query)) continue;
...
export function getSummaryGenerationDeadlineMs(): number {
	...
	if (!Check(NumberValueSchema, value) || !Number.isFinite(value) || !Number.isInteger(value) || value <= 0) {
...
function shouldPreferOpenAI(options: Pick<PendingCurate, "numResults" | "recencyFilter"> | undefined, preferOpenAICodexDefault: boolean): boolean {
	...
	if (Check(NumberValueSchema, options?.numResults) && Number.isFinite(options.numResults) && Math.floor(options.numResults) !== 5) {
		return false;
	}
...
function resolveProvider(
	...
	if (provider === "all" && !available.all) return firstAvailableProvider(available, preferOpenAI, "exa");
	if (provider !== "all" && !available[provider]) return firstAvailableProvider(available, preferOpenAI, provider);
	return provider;
}
...
interface PendingCurate {
	...
	timeoutSeconds: number;
	curatorUrl?: string;
	onUpdate: ((update: { content: Array<{ type: string; text: string }>; details?: DetailObject }) => void) | undefined;
	signal: AbortSignal | undefined;
	abortSearches: () => void;
	finish: (value: AgentToolResult<DetailObject>) => void;
...
function storeFetchResult(pi: { appendEntry<Value>(type: string, data: Value): void }, responseId: string, data: StoredSearchData & { type: "fetch"; urls: ExtractedContent[] }, authProfile?: AuthFetchProfile): boolean {
...
function initialContentSlice(content: string, maxChars: number): InitialContentSlice {
...
function normalizeFindMode(value: string | undefined): FindMode {
	return value === "exact" || value === "case-insensitive" || value === "fuzzy"
		? value
		: "case-insensitive";
}
...
function formatInputValue<Value>(value: Value): string {
	if (Check(StringValueSchema, value)) return JSON.stringify(value);
	if (Check(NumberValueSchema, value)) return Number.isNaN(value) ? "NaN" : String(value);
	try {
...
interface GlimpseWindow {
	...
	on(event: "message", handler: (data: DetailValue) => void): void;
	on(event: "ready", handler: (info: { screen?: { visibleHeight?: number } }) => void): void;
	close(): void;
	_write(obj: DetailObject): void;
}

interface GlimpseOptions extends DetailObject {
	width: number;
	height: number;
	title: string;
}

let glimpseOpen: ((html: string, opts: GlimpseOptions) => GlimpseWindow) | null | undefined;
...
function openInGlimpse(
	open: (html: string, opts: GlimpseOptions) => GlimpseWindow,
	url: string,
	title: string,
): GlimpseWindow {
...
<body style="margin:0; background:#1a1a2e;">
  <script>window.location.replace(${JSON.stringify(url)});</script>
	...
	win.on("ready", (info) => {
		...
		if (Check(NumberValueSchema, visibleHeight) && visibleHeight > 0) {
			maxHeight = Math.floor(visibleHeight * 0.85);
		}
	});
	win.on("message", (data) => {
		if (!isDetailObject(data)) return;
		const msg = data;
		if (msg.type !== "resize" || !Check(NumberValueSchema, msg.height)) return;
...
export default function (pi: ExtensionAPI) {
	...
	async function loadSummaryModelChoices(
		...
		const config = loadConfig();
		const configuredSummaryModel = Check(StringValueSchema, config.summaryModel) ? config.summaryModel.trim() : "";
	...
	interface ResolvedSubmittedSummary {
		approvedSummary: string;
		summaryMeta: SummaryMeta;
	}
	...
	function resolveSummaryForSubmit(
		...
		resultsByIndex: Map<number, QueryResultData>,
	): ResolvedSubmittedSummary {
		const submittedSummary = Check(StringValueSchema, payload.summary) ? payload.summary.trim() : "";
	...
	function buildSearchReturn(opts: SearchReturnOptions): AgentToolResult<DetailObject> {
		const sc = opts.results.filter(r => !r.error).length;
		const tr = opts.results.reduce((sum, r) => sum + r.results.length, 0);

		const approvedSummary = Check(StringValueSchema, opts.approvedSummary) ? opts.approvedSummary.trim() : "";
		...
		if (opts.curated) {
			...
			details.curatedFrom = opts.curatedFrom;
			details.curatedQueries = opts.results.map(r => ({
				query: r.query,
	...
	if (webSearchEnabled) pi.registerTool({
		...
		parameters: Type.Object({
			...
			provider: Type.Optional(searchProviderSchema("Search provider or non-empty list of providers to search simultaneously; use all to search every available provider, omit this field to use the configured provider, or use auto when none is configured")),
		...
		async execute(callId, params, signal, onUpdate, ctx) {
			...
			if (shouldCurate) {
				...
				let resolvePromise: (value: AgentToolResult<DetailObject>) => void = () => {};
				const promise = new Promise<AgentToolResult<DetailObject>>((resolve) => {
				...
				// SAFETY: Pi's update callback accepts the text-only result shape used by PendingCurate; details remain an optional property bag.
				...
				const finish = (value: AgentToolResult<DetailObject>) => {
					if (cancelled) return;
					cancelled = true;
					pc.abortSearches();
		...
		renderCall(args, theme) {
			// SAFETY: This renderer receives arguments validated by the registered web-search parameter schema.
		...
		renderResult(result, { expanded, isPartial }, theme) {
			...
			// SAFETY: Search results and partial updates are created by this tool with the fields declared below.
			...
			if (isPartial) {
				if (details?.phase === "curator-fallback") {
					...
					const timeout = Check(NumberValueSchema, details?.timeoutSeconds) ? details.timeoutSeconds : undefined;
					const shortcut = Check(StringValueSchema, details?.shortcut) ? details.shortcut : curateKey;
			...
			if (details?.error) {
				...
				// SAFETY: The error branch established the required error field; remaining fields are optional diagnostics.
	...
	if (sourceCheckEnabled) pi.registerTool({
		...
		parameters: Type.Object({
			...
			domainFilter: Type.Optional(Type.Array(Type.String(), { description: "Limit to domains; prefix with - to exclude." })),
			provider: Type.Optional(searchProviderSchema("Search provider or non-empty list of providers to search simultaneously; all searches every available provider")),
		}),
		async execute(_callId, params, signal, _onUpdate, ctx) {
			const claim = Check(StringValueSchema, params.claim) ? params.claim.trim() : "";
			if (!claim) {
			...
			const requestedQueries = Array.isArray(params.queries)
				? params.queries.filter((query): query is string => Check(StringValueSchema, query)).map((query) => query.trim()).filter(Boolean)
				: [];
			const queries = (requestedQueries.length > 0 ? requestedQueries : [claim]).slice(0, 8);
			const numResults = Check(NumberValueSchema, params.numResults) && Number.isFinite(params.numResults)
				? Math.min(20, Math.max(1, Math.floor(params.numResults)))
				: 5;
			const domainFilter = Array.isArray(params.domainFilter)
				? params.domainFilter.filter((domain): domain is string => Check(StringValueSchema, domain))
				: undefined;
			const recencyFilter = normalizeRecencyFilter(params.recencyFilter);
	...
	if (fetchContentEnabled) pi.registerTool({
		...
		async execute(_toolCallId, params, signal, onUpdate, ctx): Promise<AgentToolResult<DetailObject>> {
			...
			let fetchOptions: ExtractOptions;
			if (mode === "answer") {
				const { prompt: _prompt, ...rest } = extractionOptions;
				fetchOptions = rest;
			} else {
				fetchOptions = extractionOptions;
			}
			if (authFetchProfile) fetchOptions.authFetchProfile = authFetchProfile;
			...
			const presentedResults = mode === "answer"
				? await Promise.all(fetchResults.map(async result => {
					...
					try {
						const answerOptions: Parameters<typeof answerFromPage>[0] = {
							question: options.prompt!,
							pageText: result.content,
							sourceUrl: result.url,
						};
						if (options.answerModel) answerOptions.model = options.answerModel;
						const answer = await answerFromPage(answerOptions, ctx, signal);
			...
			if (urlList.length === 1) {
				...
				if (result.error) {
					const errorDetails: DetailObject = {
						urls: urlList,
						urlCount: 1,
						successful: 0,
						error: result.error,
					...
					if (storedContent) errorDetails.responseId = responseId;
					return {
				...
				const successDetails: DetailObject = {
					urls: urlList,
					urlCount: 1,
					successful: 1,
				...
				if (storedContent) successDetails.responseId = responseId;
				return { content, details: successDetails };
			}
			...
			const details: DetailObject = { urls: urlList, urlCount: urlList.length, successful, totalChars };
			if (storedContent) details.responseId = responseId;
			return {
		...
		renderResult(result, { expanded, isPartial }, theme) {
			// SAFETY: Fetch results and partial updates are created by this tool with the fields declared below.
			const details = result.details as {
				urls?: string[];
				urlCount?: number;
				successful?: number;
			...
			if (details?.error) {
				const extras: string[] = [];
				if (Check(NumberValueSchema, details.urlCount) || Check(NumberValueSchema, details.successful)) {
	...
	if (getSearchContentEnabled) {
		...
		async execute(_toolCallId, params): Promise<AgentToolResult<DetailObject>> {
			...
			if (data.type === "search" && data.queries) {
				...
				if (params.findText !== undefined) {
					try {
						const found = findContent(fullResults, normalizeFindQueries(params.findText), normalizeFindMode(params.findMode));
			...
			if (data.type === "fetch" && data.urls) {
				...
				if (params.findText !== undefined) {
					try {
						const found = findContent(urlData.content, normalizeFindQueries(params.findText), normalizeFindMode(params.findMode));
						const { text, ...findDetails } = found;
						return {
		...
		renderCall(args, theme) {
			// SAFETY: This renderer receives arguments validated by the registered get-search-content parameter schema.
		...
		renderResult(result, { expanded }, theme) {
			// SAFETY: Get-search-content results are created by this tool with the fields declared below.
			const details = result.details as {
				error?: string;
				query?: string;
				url?: string;
			...
			let statusLine: string;
			if (Check(NumberValueSchema, details?.matchCount)) {
