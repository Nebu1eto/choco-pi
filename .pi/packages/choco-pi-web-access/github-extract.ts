...
function normalizeEnabled(value: ConfigValue | undefined, fallback: boolean): boolean {
	return value === true ? true : value === false ? false : fallback;
}

function normalizePositiveNumber(value: ConfigValue | undefined, fallback: number): number {
	if (!isNumber(value) || !Number.isFinite(value)) return fallback;
...
function normalizeClonePath(value: ConfigValue | undefined, fallback: string): string {
...
function loadGitHubConfig(): GitHubCloneConfig {
	...
	const gc = isConfigObject(raw.githubClone) ? raw.githubClone : {};
	cachedConfig = {
...
export function parseGitHubUrl(url: string): GitHubUrlInfo | null {
	...
	if (ref.length === 0 || ref.length > 1024 || hasAsciiControlCharacter(ref)) return null;
