...
export interface CuratorServerHandle {
  ...
  pushResult: (
    queryIndex: number,
    data: CuratorSearchEntry & { query?: string; slotIndex?: number },
  ) => void;
  pushError: (
    queryIndex: number,
    error: string,
...
function sendJson<Payload>(res: ServerResponse, status: number, payload: Payload): void {
...
function normalizeSelectedIndices(
  ...
  for (const item of value) {
    if (!isNumber(item) || !Number.isInteger(item) || item < 0) {
      return { ok: false, error: "Invalid selection" };
    }
...
function normalizeSummaryMeta(value: JsonValue | undefined): SummaryMeta | null {
  if (!isJsonObject(value)) return null;
  const meta = value;

  const model = meta.model === null ? null : isString(meta.model) ? meta.model : undefined;
  if (model === undefined) return null;

  const durationMs = meta.durationMs;
  if (!isNumber(durationMs) || durationMs < 0) return null;

  const tokenEstimate = meta.tokenEstimate;
  if (!isNumber(tokenEstimate) || tokenEstimate < 0) return null;

  const fallbackUsed = meta.fallbackUsed;
  if (fallbackUsed !== true && fallbackUsed !== false) return null;

  const fallbackReason = isString(meta.fallbackReason) ? meta.fallbackReason : undefined;
  if (meta.fallbackReason !== undefined && fallbackReason === undefined) return null;

  const phase =
    meta.phase === "summary-model" || meta.phase === "deterministic-fallback"
      ? meta.phase
      : undefined;
  ...
  const edited = meta.edited === true || meta.edited === false ? meta.edited : undefined;
...
export function startCuratorServer(
  ...
  function validateToken(body: JsonValue, res: ServerResponse): body is JsonObject {
  ...
  function writeSSE<Data>(res: ServerResponse, event: string, data: Data): boolean {
  ...
  function sendSSE<Data>(event: string, data: Data): void {
  ...
  const server = http.createServer(async (req, res) => {
    try {
      ...
      if (method === "POST" && url.pathname === "/heartbeat") {
        ...
        touchHeartbeat();
        if (isNumber(body.timeoutSec) && body.timeoutSec > 0) {
          clientTimeoutSeconds = Math.min(600, Math.floor(body.timeoutSec));
        }
        if (isNumber(body.idleMs) && body.idleMs >= 0) {
      ...
      if (method === "POST" && url.pathname === "/search") {
        ...
        try {
          ...
          const entry: IndexedCuratorSearchEntry = {
            ...
            results: [],
            error: message,
            provider: isString(provider) && provider.length > 0 ? provider : defaultProvider,
          };
      ...
      if (method === "POST" && url.pathname === "/summarize") {
        ...
        const feedback =
          isString(bodyFeedback) && bodyFeedback.trim().length > 0
            ? bodyFeedback.trim()
            : undefined;
      ...
      if (method === "POST" && url.pathname === "/submit") {
        ...
        sendJson(res, 200, { ok: true });
        const submitPayload: Parameters<CuratorServerCallbacks["onSubmit"]>[0] = {
          selectedQueryIndices: parsed.indices,
          rawResults,
        };
        if (summary !== undefined) submitPayload.summary = summary;
        if (summaryMeta !== undefined) submitPayload.summaryMeta = summaryMeta;
        setImmediate(() => callbacks.onSubmit(submitPayload));
        return;
      }
