...
async function parseBodyOrSend(req: IncomingMessage, res: ServerResponse): Promise<JsonValue | null> {
	try {
	...
	const server = http.createServer(async (req, res) => {
		try {
			...
			if (method === "POST" && url.pathname === "/summarize") {
				...
				const feedback = isString(bodyFeedback) && bodyFeedback.trim().length > 0
					? bodyFeedback.trim()
					: undefined;
