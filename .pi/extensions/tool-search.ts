...
type McpMetadataCache = {
	...
	servers?: Record<
		string,
		{ tools?: CachedMcpTool[]; resources?: CachedMcpResource[] }
	>;
};
...
function schemaText(value: unknown, key = ""): string {
	...
	if (Array.isArray(value))
		return value.map((item) => schemaText(item, key)).join(" ");
...
function termFrequency(tokens: string[], term: string): number {
	return tokens.reduce(
		(count, token) =>
			count + (token === term ? 1 : token.startsWith(term) ? 0.5 : 0),
		0,
	);
}
...
function buildPiDocuments(tools: ToolInfo[]): SearchDocument[] {
	return tools.map((tool) =>
		makeDocument(
			{ kind: "pi", tool },
			`${tool.sourceInfo.source} ${tool.sourceInfo.path}`,
		),
	);
}
	...
	try {
		...
		for (const [server, entry] of Object.entries(cache.servers)) {
			...
			for (const tool of entry.tools ?? []) {
				if (
					!tool.name ||
					(tool.uiVisibility && !tool.uiVisibility.includes("model"))
				)
					continue;
...
function rankTools(documents: SearchDocument[], query: string): SearchTarget[] {
	...
	return documents
		.map((document) => {
			...
			const describedTokens = [
				...document.nameTokens,
				...document.descriptionTokens,
			];
			const coverage =
				terms.filter((term) => matchesTerm(describedTokens, term)).length /
				terms.length;
		...
		.sort(
			(left, right) =>
				...
				targetName(left.target).localeCompare(targetName(right.target)),
		)
...
export default function toolSearch(pi: ExtensionAPI): void {
	...
	const applyLeanSurface = (attempt = 0): void => {
		...
		searchableNames = new Set(
			[...allNames].filter(
				(name) => allowedNames.has(name) && !ALWAYS_ACTIVE.has(name),
			),
		);
		searchableDocuments = [
			...buildPiDocuments(
				allTools.filter((tool) => searchableNames.has(tool.name)),
			),
			...loadMcpDocuments(enabledMcpServers),
		];
		const active = pi.getActiveTools();
		const leanSurface = active.filter(
			(name) => ALWAYS_ACTIVE.has(name) || loadedNames.has(name),
		);
	...
	pi.registerTool({
		...
		promptSnippet:
			"Search for deferred tools when the active tools cannot perform the task",
		parameters: Type.Object({
			...
			limit: Type.Optional(
				Type.Integer({ minimum: 1, maximum: 5, default: DEFAULT_LIMIT }),
			),
		}),
		async execute(_toolCallId, params) {
			...
			const matches = rankTools(searchableDocuments, query).slice(
				0,
				params.limit ?? DEFAULT_LIMIT,
			);
			const added = matches
				.filter(
					(target): target is Extract<SearchTarget, { kind: "pi" }> =>
						target.kind === "pi",
				)
	...
	pi.events.on("pi-mcp-adapter/status/v1", (payload) => {
		...
		enabledMcpServers = new Set(
			(snapshot.servers ?? [])
				.filter(
					(server) => server.disabled !== true && typeof server.name === "string",
				)
