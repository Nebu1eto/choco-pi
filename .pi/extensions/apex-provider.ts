...
function normalizeInput(value: unknown, fallback: ModelInput): ModelInput {
	...
	const input = [...new Set(value.filter((item): item is "text" | "image" => item === "text" || item === "image"))];
...
export function normalizeApexModels(
	...
	for (const entry of modelEntries(payload)) {
		...
		const apiReasoning = typeof model.reasoning === "boolean"
			? model.reasoning
		...
		const overrideName = typeof override.name === "string" && override.name.trim()
			? override.name.trim()
			: undefined;
		...
		models.push({
			...
			cost: ZERO_COST,
			contextWindow: positiveInteger(override.contextWindow) ?? firstPositiveInteger(
				model.contextWindow,
				model.context_window,
				model.context_length,
			) ?? defaults.contextWindow,
			maxTokens: positiveInteger(override.maxTokens) ?? firstPositiveInteger(
				model.maxTokens,
				model.max_tokens,
...
function resolveDefaults(config: ApexProviderConfig): ApexModelDefaults {
	return {
		contextWindow: positiveInteger(config.defaults?.contextWindow) ?? FALLBACK_DEFAULTS.contextWindow,
		maxTokens: positiveInteger(config.defaults?.maxTokens) ?? FALLBACK_DEFAULTS.maxTokens,
		reasoning: typeof config.defaults?.reasoning === "boolean" ? config.defaults.reasoning : FALLBACK_DEFAULTS.reasoning,
...
export default async function apexProvider(pi: ExtensionAPI): Promise<void> {
	...
	const overrides = config.overrides && typeof config.overrides === "object" && !Array.isArray(config.overrides)
		? config.overrides
		: {};
	...
	pi.registerProvider(PROVIDER_ID, {
		...
		refreshModels: async (context) => {
			...
			const persist = (models: ProviderModelConfig[]) => context.publish({
				persist: {
			...
			try {
				const models = await discoverApexModels(baseUrl, apiKey, defaults, overrides, context.signal);
	...
	pi.registerCommand("apex-refresh", {
		...
		handler: async (_args, context) => {
			...
			const count = context.modelRegistry.getAll().filter((model) => model.provider === PROVIDER_ID).length;
