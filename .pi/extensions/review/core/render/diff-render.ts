...
function changedTokenSpans(oldText: string, newText: string): [Span[], Span[]] {
	...
	for (let oldIndex = oldTokens.length - 1; oldIndex >= 0; oldIndex -= 1) {
		for (let newIndex = newTokens.length - 1; newIndex >= 0; newIndex -= 1) {
			common[oldIndex][newIndex] = oldTokens[oldIndex].value === newTokens[newIndex].value
	...
	return [
		mergeChangedRanges(oldTokens, oldChanged),
		mergeChangedRanges(newTokens, newChanged),
	];
}
...
function renderSplitHunk(
	...
	return splitRows(lines).map(([leftIndex, rightIndex]) => {
		...
		return [
			...
			splitSide(right, rightIndex, right?.newLine, newWidth, rightWidth, highlighted, changes, styler),
		].join("");
	});
}
	...
	for (const hunk of file.hunks) {
		if (options.fold(hunk.id)) {
			...
			lines.push(truncateToWidth(
				styler.fg("dim", `… ${hunk.header} (${reason})`),
				width,
				"…",
			));
			continue;
