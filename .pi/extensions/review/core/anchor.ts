...
export function buildCommentAnchor(
	...
	const firstPosition = sideIndexes.findIndex((index) => lineNumber(hunk, side, index) === startLine);
	let lastPosition = -1;
	...
	if (firstPosition < 0 || lastPosition < firstPosition) {
		throw new Error(`Comment lines ${startLine}-${line} are not present on the ${side} side of the hunk.`);
	}

	const from = Math.max(0, firstPosition - contextLines);
	const to = Math.min(sideIndexes.length - 1, lastPosition + contextLines);
	const snippet = sideIndexes.slice(from, to + 1)
...
export function relocateAnchor(
	...
	const exact = findMatches(chunks, anchorLines)
		.filter((match) => hashSnippet(match.snippet) === anchor.snippetHash);
