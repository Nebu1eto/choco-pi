...
function isTarget(value: unknown): value is ReviewTarget {
	...
	switch (value.kind) {
		...
		case "session-turn":
			return typeof value.sessionId === "string" && Number.isSafeInteger(value.turnIndex) && Number(value.turnIndex) >= 0;
		case "branch":
			return typeof value.base === "string" && (value.target === undefined || typeof value.target === "string");
		case "pr":
...
function parseRecord(value: unknown, path: string): ReviewRecord | undefined {
	...
	if (value.version !== 1) {
		if ("version" in value) throw new Error(`Unsupported review record version in ${path}: ${String(value.version)}`);
		return undefined;
	}
	...
	if (
		...
		|| (value.verdict !== undefined && value.verdict !== "comment" && value.verdict !== "approve" && value.verdict !== "request-changes")
...
export function unreviewedHunks(record: ReviewRecord, diff: DiffModel): UnreviewedHunk[] {
	...
	return diff.files.flatMap((file) => file.hunks
		.filter((hunk) => !reviewed.has(hunk.id))
		.map((hunk) => ({ path: file.path, hunk })));
}

/** Return a new record with the supplied stable hunk ids marked reviewed. */
export function markHunksReviewed(record: ReviewRecord, hunkIds: Iterable<string>, headSha: string): ReviewRecord {
	const reviewed = new Set(record.cursor.reviewedHunkIds);
