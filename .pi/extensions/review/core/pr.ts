...
const GH_FIELDS = ["number", "title", "baseRefName", "headRefOid", "author", "url", "updatedAt"] as const;
const GH_HELP = "GitHub CLI is unavailable or not authenticated. Install `gh`, run `gh auth login`, and retry.";
...
function parsePullRequest(value: unknown, context: string): PullRequestMetadata {
	if (!isObject(value)
		...
		|| (value.updatedAt !== undefined && value.updatedAt !== null && typeof value.updatedAt !== "string")) {
...
export async function getPullRequestMetadata(
	...
	const value = await runGhJson(cwd, ["pr", "view", String(number), "--json", GH_FIELDS.join(",")], runner);
...
export async function listPullRequests(
	...
	const value = await runGhJson(cwd, [
...
function canonicalRemoteUrl(value: string): string {
	...
	if (scp && !/^[A-Za-z]:[\\/]/.test(trimmed)) return `${scp[1].toLowerCase()}/${scp[2].toLowerCase()}`;
...
async function resolvePullRequestRemote(cwd: string, runner: ExecRunner): Promise<string> {
	const repository = parseRepository(await runGhJson(cwd, [
		"repo", "view", "--json", "nameWithOwner,url,sshUrl",
	], runner));
	...
	const matches = remotes.filter((remote) => remote.urls.some((url) => identities.has(canonicalRemoteUrl(url))));
	if (matches.length === 0) {
		throw new Error(`No Git remote matches GitHub repository ${repository.nameWithOwner}. Add a fetch remote for that repository and retry.`);
	}
...
async function readOwner(path: string): Promise<WorktreeOwner | undefined> {
	try {
		...
		if (!isObject(value)
			...
			|| !Number.isSafeInteger(value.pullRequest)) return undefined;
...
async function removeOwnedWorktree(
	...
	if (!owner
		...
		|| (token !== undefined && owner.token !== token)) return false;
	try {
...
export async function resolvePullRequestTarget(
	...
	if (headSha !== metadata.headSha) {
		throw new Error(`Pull request #${target.number} changed while it was being prepared. Retry to review the latest head.`);
	}
	...
	try {
		...
		if (checkedOutSha !== headSha) throw new Error("Detached pull request worktree resolved to the wrong commit.");
	} catch (error) {
		await removeOwnedWorktree(root, checkout.path, {
			repositoryRoot: root,
			pullRequest: target.number,
		}, runner, checkout.token);
		throw error;
	}
	return {
		metadata,
...
export async function disposePullRequestWorktree(
	...
	const removed = await removeOwnedWorktree(review.repositoryRoot, review.worktreePath, {
		repositoryRoot: review.repositoryRoot,
		pullRequest: review.metadata.number,
	}, runner, review.worktreeToken);
