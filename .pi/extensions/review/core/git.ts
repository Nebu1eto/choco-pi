...
export async function gitCommonDirectory(
	...
	return (await runGit(cwd, ["rev-parse", "--path-format=absolute", "--git-common-dir"], runner)).trim();
}
...
export async function readRawDiffToWorkingTree(
	...
	const untrackedOutput = await runGit(cwd, [
		"ls-files",
		"--others",
		"--exclude-standard",
		"-z",
	...
	const untrackedPatches = await Promise.all(untrackedPaths.map((path) =>
		runGitAllowing(cwd, [
			"diff",
			"--patch",
			"--no-color",
