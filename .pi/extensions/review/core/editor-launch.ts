...
export type SpawnedEditor = {
	...
	once(event: "close", listener: (code: number | null, signal: NodeJS.Signals | null) => void): SpawnedEditor;
	unref(): void;
};
...
export function resolveEditorCommand(config: EditorConfig, location: EditorLocation): ResolvedEditorCommand {
	if (config.command.length === 0) throw new Error("Editor command must not be empty.");
	if (!Number.isInteger(location.line) || location.line < 1) throw new Error("Editor line must be a positive integer.");
	const column = location.column ?? 1;
	if (!Number.isInteger(column) || column < 1) throw new Error("Editor column must be a positive integer.");
