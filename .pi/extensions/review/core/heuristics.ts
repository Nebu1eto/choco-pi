...
function firstFileLines(file: DiffFile, limit = 20): string[] {
  ...
  lines.sort(
    (left, right) =>
      (left.newLine ?? left.oldLine ?? Number.MAX_SAFE_INTEGER) -
      (right.newLine ?? right.oldLine ?? Number.MAX_SAFE_INTEGER),
  );
...
function isGenerated(file: DiffFile): boolean {
  return firstFileLines(file).some((line) =>
    /(?:code generated .* do not edit|@generated\b|automatically generated|generated file.*do not edit)/i.test(
      line,
    ),
  );
}
...
function isFormatOnlyHunk(path: string, hunk: DiffHunk): boolean {
  ...
  const removed = changedLines
    ...
    .map((line) => whitespaceNormalized(line.text));
...
function builtInCollapseReason(file: DiffFile): string | undefined {
  ...
  if (
    /^(?:package-lock\.json|npm-shrinkwrap\.json|yarn\.lock|pnpm-lock\.yaml|bun\.lockb?|cargo\.lock|composer\.lock|gemfile\.lock|poetry\.lock|pdm\.lock|uv\.lock|go\.sum|pubspec\.lock|mix\.lock)$/.test(
      name,
    )
  ) {
  ...
  if (hasSegment(path, new Set(["vendor", "vendored", "third_party", "node_modules"])))
    return "Vendored dependency path";
  if (hasSegment(path, new Set(["dist", "build", "out", "target", "coverage"])))
...
function riskSignals(file: DiffFile): RiskSignal[] {
  ...
  if (
    /\b(?:auth|authentication|authorization|authorisation|oauth|sso|login|session|permission|permissions|rbac|acl)\b/.test(
      haystack,
    )
  ) {
    signals.push({
      score: 45,
      reason: "Authentication, authorization, or permission logic changed",
    });
  }
  if (
    /\b(?:crypto|cryptography|cipher|encrypt|decrypt|hashing|hmac|jwt|secret|secrets|credential|credentials|private[_ -]?key|api[_ -]?key)\b/.test(
      haystack,
    )
  ) {
  ...
  if (
    ...
    /\b(?:alter table|create table|drop table|schema migration)\b/.test(text)
  ) {
    signals.push({ score: 40, reason: "Database migration or schema changed" });
  }
  if (
    /\b(?:payment|payments|billing|checkout|invoice|stripe|refund|subscription)\b/.test(haystack)
  ) {
    signals.push({ score: 40, reason: "Payment or billing path changed" });
  }
  if (
    isTestSource(path) &&
    (file.kind === "deleted" || (file.deletions > 0 && file.additions === 0))
  ) {
    signals.push({
      score: 40,
      reason: file.kind === "deleted" ? "Test file was deleted" : "Test file may have been emptied",
    });
  }
  if (
    /^(?:package\.json|deno\.jsonc?|pyproject\.toml|requirements(?:-[^.]+)?\.txt|pipfile|cargo\.toml|go\.mod|composer\.json|gemfile|pubspec\.yaml|mix\.exs)$/.test(
      name,
    )
  ) {
  ...
  if (
    ...
    /^(?:dockerfile(?:\..+)?|compose\.ya?ml|docker-compose\.ya?ml|\.env(?:\..+)?|\.gitlab-ci\.ya?ml)$/.test(
      name,
    ) ||
  ...
  if (file.deletions >= 250 && file.deletions >= file.additions * 2) {
    signals.push({
      score: 30,
      reason: `Unusually large deletion (${file.deletions} lines removed)`,
    });
  }
  return signals;
}
