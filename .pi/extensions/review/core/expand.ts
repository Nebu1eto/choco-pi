...
import type { DiffFile, DiffHunk, DiffLine, DiffModel, ExecRunner } from "./types.ts";
...
function availableBelow(
  ...
  if (next) {
    const gap = Math.max(0, Math.min(next.oldStart - firstOldLine, next.newStart - firstNewLine));
    return Math.max(0, gap - (expandedContext?.get(next.id)?.above.length ?? 0));
  }
  const newFileLines =
    oldFileLines +
    file.hunks.reduce((total, candidate) => total + candidate.newLines - candidate.oldLines, 0);
  return Math.max(0, Math.min(oldFileLines - firstOldLine + 1, newFileLines - firstNewLine + 1));
}
...
export async function expandHunkContext(
  ...
  const available =
    options.edge === "above"
      ...
      : availableBelow(options.file, options.hunk, index, source.length, options.expandedContext);
  ...
  const below = Array.from({ length: target }, (_unused, offset) =>
    contextLine(
...
export function collapseHunkContext(expansion: HunkExpansion, edge: ExpansionEdge): HunkExpansion {
  if (edge === "above") {
