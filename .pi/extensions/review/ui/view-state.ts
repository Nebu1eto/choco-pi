...
export function reviewLocations(
  state: Pick<ReviewViewState, "model" | "fileOrder">,
): ReviewLocation[] {
...
function cursorForLocation(
  state: Pick<
    ReviewViewState,
    "model" | "fileOrder" | "fileFolds" | "hunkFolds" | "expandedContext"
  >,
...
export function createReviewViewState(options: CreateReviewViewStateOptions): ReviewViewState {
  ...
  const fileFolds = new Set(
    assessments.files.filter((file) => file.collapsed).map((file) => file.path),
  );
  const hunkFolds = new Set(
    assessments.hunks.filter((hunk) => hunk.collapsed).map((hunk) => hunk.hunkId),
  );
...
function navigableRows(state: ReviewViewState): NavigableReviewRow[] {
  return state.fileOrder.flatMap((path) => {
    ...
    return file.hunks.flatMap((hunk) =>
      state.hunkFolds.has(hunk.id)
        ? [{ path, hunkId: hunk.id }]
        : hunkWithExpandedContext(hunk, state.expandedContext.get(hunk.id)).lines.map(
            (_line, lineIndex) => ({ path, hunkId: hunk.id, lineIndex }),
          ),
    );
  });
}
...
export function moveLine(state: ReviewViewState, delta: number): ReviewViewState {
  ...
  if (index < 0) {
    ...
    if (delta > 0) {
      ...
      index = rows.findLastIndex(
        (row) => locationIndex(locations, row.path, row.hunkId) <= hunkIndex,
      );
      if (index < 0) return state;
      index += delta + 1;
    }
  ...
  if (index < 0) {
    if (delta > 0) {
      index = locations.findIndex(
        (candidate) => state.fileOrder.indexOf(candidate.path) > state.fileIndex,
      );
      if (index < 0) index = locations.length - 1;
    } else {
      index = locations.findLastIndex(
        (candidate) => state.fileOrder.indexOf(candidate.path) < state.fileIndex,
      );
      if (index < 0) index = 0;
    }
...
export function toggleFileFold(
  state: ReviewViewState,
  path = currentFile(state)?.path,
): ReviewViewState {
  ...
  return currentDisplayHunk(next)?.lines.length
    ? { ...next, lineIndex: state.lineIndex ?? 0 }
    : next;
}

export function toggleHunkFold(
  state: ReviewViewState,
  hunkId = currentHunk(state)?.id,
): ReviewViewState {
  ...
  return currentDisplayHunk(state)?.lines.length
    ? { ...next, lineIndex: state.lineIndex ?? 0 }
    : next;
}
...
export function fileFoldReason(state: ReviewViewState, path: string): string | undefined {
  ...
  return (
    state.assessments.files.find((file) => file.path === path)?.collapseReason ??
    "Folded by reviewer"
  );
}
...
export function hunkFoldReason(state: ReviewViewState, hunkId: string): string | undefined {
  ...
  if (state.record.cursor.reviewedHunkIds.includes(hunkId))
    return "Already reviewed in this review";
  return (
    state.assessments.hunks.find((hunk) => hunk.hunkId === hunkId)?.reason ?? "Folded by reviewer"
  );
}
...
export function moveSearchMatch(state: ReviewViewState, delta: number): ReviewViewState {
  ...
  if (!search || search.matches.length === 0 || !Number.isInteger(delta) || delta === 0)
    return state;
...
export function commitCommentDraft(
  state: ReviewViewState,
  input: CommitCommentInput,
): ReviewViewState {
  ...
  if (!file || !originalHunk)
    throw new Error("The comment draft no longer points to a review hunk.");
...
export function markCurrentHunkReviewed(
  state: ReviewViewState,
