...
export function composeSessionReviewInstruction(record: ReviewRecord, markdown: string): string | undefined {
...
function fileListLines(state: ReviewViewState, width: number, theme: Theme): string[] {
	...
	const start = Math.max(0, Math.min(
		state.fileIndex - Math.floor(count / 2),
		state.fileOrder.length - count,
	));
	const lines: string[] = [];
	if (start > 0) lines.push(theme.fg("dim", `  ↑ ${start} higher-risk file${start === 1 ? "" : "s"}`));
	for (let index = start; index < start + count; index += 1) {
		...
		if (folded) {
			lines.push(truncateToWidth(
				theme.fg("dim", `    Hidden: ${fileFoldReason(state, path)}`),
				width,
				"…",
			));
		}
	...
	if (remaining > 0) lines.push(theme.fg("dim", `  ↓ ${remaining} lower-risk file${remaining === 1 ? "" : "s"}`));
	return lines;
}
...
function commentRowForHunk(
	...
	const lineIndex = hunk.lines.findIndex((line) => (comment.side === "LEFT"
...
function commentBlock(comment: ReviewComment, width: number, styler: DiffStyler): string[] {
	...
	return wrapPlainText(comment.body, bodyWidth).map((line, index) => (index === 0
...
function diffBody(
	...
	const diffLines = file.hunks.reduce((total, hunk) => (
	...
	for (const originalHunk of file.hunks) {
		...
		if (hunk.id === selected) {
			selectedRow = state.lineIndex === undefined
				? 0
			...
			const selectedRows = new Set(selectedLineIndexes(state)
				.map((lineIndex) => 1 + renderedRowForLine(hunk, lineIndex, state.mode, width)));
...
function statusLine(state: ReviewViewState, theme: Theme): string {
	...
	if (search) parts.push(search.matches.length === 0
...
export function formatRelativeTime(updatedAt: string | null | undefined, now: string): string | undefined {
...
function reviewHeaderLine(options: {
	...
	let pieces: HeaderPiece[] = [
		...
		...(options.scroll ? [{ kind: "scroll" as const, text: options.theme.fg("dim", options.scroll) }] : []),
		...(pullRequest ? [{ kind: "base" as const, text: `base ${pullRequest.baseRefName}` }] : []),
		...(pullRequest?.author ? [{ kind: "author" as const, text: `@${pullRequest.author.login}` }] : []),
...
function chatBodyLines(
	...
	for (const message of chat.messages) {
		...
		if (message.role === "user") {
			...
			content.push(...wrapped.map((line, index) => (
...
export async function openReviewView(options: OpenReviewViewOptions): Promise<ReviewViewResult | undefined> {
	...
	try {
		return await options.host.custom<ReviewViewResult | undefined>((tui, theme, _keybindings, done) => {
			...
			const chatCommands: SlashCommand[] = [
				...
				},
				{
					...
					getArgumentCompletions: (argumentPrefix) => {
						const needle = argumentPrefix.trim().toLowerCase();
						const matches = REVIEW_CHAT_THINKING_LEVELS.filter((level) => level.startsWith(needle));
						return matches.length === 0
							? null
			...
			const toolViews = new Map<string, { component: ToolExecutionComponent; result?: unknown; done?: boolean }>();
		...
		function submitComment(body: string): void {
			...
			commentInput.focused = false;
			changed(commitCommentDraft(withBody, {
				id: createCommentId(),
				timestamp: now(),
			}));
		}
		...
		function openChat(): void {
			try {
				if (!chat) {
					...
					void chat.prepare(options.reviewRoot)
		...
		async function handleChatCommand(kind: "model" | "effort" | "reload", argument: string): Promise<void> {
			if (!chat) return;
			try {
				if (kind === "reload") {
					await chat.reload();
					options.host.notify("Chat session resources reloaded: extensions, skills, prompts, and context files.", "info");
					return;
				}
				if (kind === "model") {
					if (!argument) {
						options.host.notify(`Chat model: ${chat.status.model ?? "Pi default"} · effort ${chat.status.thinkingLevel ?? "default"}`, "info");
						return;
					}
				...
				if (!argument) {
					options.host.notify(`Chat effort: ${chat.status.thinkingLevel ?? "default"} · levels: off, minimal, low, medium, high, xhigh, max`, "info");
					return;
				}
		...
		function submitChat(question: string): void {
			...
			if (command) {
				...
				const kind = command[1] === "model" ? "model" : command[1] === "effort" ? "effort" : "reload";
		...
		async function expandContext(): Promise<void> {
			...
			if (!file || !hunk || !currentCommentPosition(state)) {
				options.host.notify("Select an expanded hunk line before expanding context.", "warning");
				return;
			}
			const expansion = hunkExpansion(state, hunk.id) ?? { above: [], below: [] };
			const expandEdge = (edge: "above" | "below") => expandHunkContext({
				cwd: options.reviewRoot,
				model: state.model,
				file,
				hunk,
			...
			changed(setHunkExpansion(state, hunk.id, {
		...
		function collapseContext(): void {
			...
			if (!hunk || !currentCommentPosition(state)) {
				options.host.notify("Select an expanded hunk line before collapsing context.", "warning");
				return;
			}
			const expansion = hunkExpansion(state, hunk.id) ?? { above: [], below: [] };
			...
			changed(setHunkExpansion(state, hunk.id, collapseHunkContext(
				collapseHunkContext(expansion, "above"),
				"below",
			)));
		}
		...
		async function editCurrentHunk(): Promise<void> {
			...
			return {
				render: (width: number) => {
					...
					const chatWidth = splitChat ? Math.max(40, Math.floor(contentWidth * 0.4)) : contentWidth;
					...
					if (state.commentDraft) {
						...
						hintLineTwo = draftPosition.startLine === undefined
						...
						hintLineOne = "Ask agent · Enter ask · Shift+Enter newline · Ctrl+O tool output · Shift+Tab review · Esc close chat";
						hintLineTwo = "Context: selected file, side, line, focused row, and current hunk only · /model /effort /reload act on the chat";
					} else {
						inputLines = [theme.fg("dim", "Input inactive · c writes a review comment · a asks the agent")];
					...
					if (chatOpen && chat) {
						...
						if (splitChat) {
							visibleBody = visibleDiff.map((line, index) => (
					...
					const scroll = diffVisible && (scrollOffset > 0 || hiddenBelow > 0)
						? `↑${scrollOffset} ↓${hiddenBelow}`
						: undefined;
					const reviewFocus = focus === "review" ? "Review [FOCUSED]" : "Review";
					const chatFocus = !chatOpen ? "" : focus === "chat" ? " · Chat [FOCUSED]" : " · Chat open";
				...
				handleInput: (data: string) => {
					...
					if (state.commentDraft) {
						if (matchesKey(data, "escape") && !commentInput.isShowingAutocomplete()) stopComment();
						else {
					...
					if (focus === "chat") {
						...
						else if (matchesKey(data, "escape") && !chatInput.isShowingAutocomplete()) closeChat();
					...
					else if (data === "E") void guarded(() => launchEditorProject(
						options.config.editor,
						options.reviewRoot,
						tui,
						options.spawnEditor,
					...
					else if (matchesKey(data, "pageUp")) changed(moveLine(state, -Math.max(1, lastBodyHeight - 2)));
					else if (matchesKey(data, "pageDown")) changed(moveLine(state, Math.max(1, lastBodyHeight - 2)));
					else if (data === "t") {
						options.host.notify("Type information from review is planned for a later phase.", "info");
					}
				},
			};
		}, {
