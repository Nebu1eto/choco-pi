---
name: task-inline
description: Execute a new repository-changing implementation directly in the main choco-pi agent when parallel implementation would not materially help. Use as the authoritative default modifying workflow, not for read-only, diagnostic, operational, or follow-up work.
---
...
2. Read the full path-scoped instruction chain. Load another skill only for a distinct domain operation required by the task, never to replace this workflow.
...
Unless the user explicitly excluded a commit, load and follow the harness `commit` skill. Never create the commit directly or substitute another commit workflow. The checkpoint creates an immutable review and runtime target but does not finish the task.
