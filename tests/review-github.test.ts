...
test("a draft submission omits event so GitHub leaves the review pending", async () => {
	...
	const { runner, calls } = stubRunner(() => ({ stdout: JSON.stringify({ id: 9, state: "PENDING" }) }));
...
test("a moved head relocates comments and demotes the unmappable ones into the body", () => {
	...
	assert.deepEqual(moved.payload.comments.map((entry) => entry.path), [
		"src/app.ts",
		"src/app.ts",
		"src/app.ts",
	]);
	...
	assert.deepEqual(moved.demotedComments.map((entry) => ({
		id: entry.commentId,
		path: entry.path,
		line: entry.line,
		reason: entry.reason,
...
test("the plan description tells a human exactly what is about to happen", () => {
	...
	const description = describeSubmissionPlan(planReviewSubmission({
		record: withUtil,
		pullRequest: PR,
		event: "APPROVE",
		currentHeadSha: "head-2",
...
test("preparing a submission verifies the pull request head through gh", async () => {
	const { runner, calls } = stubRunner(() => ({
		stdout: JSON.stringify({ headSha: "head-2", baseSha: "base-1", headRef: "topic", state: "open" }),
	}));

	const prepared = await prepareReviewSubmission(
		{ record: record(), pullRequest: PR, event: "COMMENT" },
		{ runner, loadHeadDiff: async (headSha) => {
...
test("a missing gh binary reports how to install and authenticate it", async () => {
	const { runner } = stubRunner(() =>
		Promise.reject(Object.assign(new Error("spawn gh ENOENT"), {
			code: "ENOENT",
			syscall: "spawn gh",
		})));
...
test("a validation failure names the refused field, code, and reason", async () => {
	...
	await assert.rejects(
		...
		(error: unknown) => {
			...
			assert.match(error.message, /PullRequestReviewComment\.line \(custom\): line must be part of the diff/);
...
test("provider text that echoes a comment body is redacted before it is reported", async () => {
	const { runner } = stubRunner(() => ({
		...
		stdout: JSON.stringify({
			message: "Validation Failed",
			errors: [{ resource: "PullRequestReviewComment", code: "custom", field: "body", message: `is invalid: ${BODY_ADDED}` }],
		}),
		stderr: "gh: Validation Failed (HTTP 422)\n",
	}));
...
test("preparing a submission rejects an unrecognized event and submits nothing", async () => {
	const { runner, calls } = stubRunner(() => ({
		stdout: JSON.stringify({ headSha: "head-1", baseSha: "base-1", headRef: "topic", state: "open" }),
	}));
