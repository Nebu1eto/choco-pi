...
function appendixText(result: { content: { type: string; text?: string }[] } | undefined): string | undefined {
	if (!result) return undefined;
	const block = result.content.findLast((item) => item.type === "text" && item.text?.includes("<subdirectory_agents_context>"));
	return block?.text;
}
...
test("dedups: an already-injected AGENTS.md is not injected again for a later touch in the same chain", async () => {
	...
	try {
		...
		assert.ok(!secondText!.includes("A guidance"), "A guidance was already injected and must not repeat");
	} finally {
		fs.rmSync(root, { recursive: true, force: true });
	}
});
...
test("caps total injected appendix size, dropping the root-most files first", async () => {
	...
	try {
		...
		assert.ok(text!.length <= MAX_TOTAL_APPENDIX_CHARS + 2000, "appendix should respect the total size cap (plus wrapper markup)");
		// Root-most (l1) must be dropped before the leaf-most (l3) file when over budget.
		assert.ok(!text!.includes("L1:xxx"), "expected the root-most oversized file to be dropped first");
