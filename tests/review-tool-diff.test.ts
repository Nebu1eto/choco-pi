...
test("leaves the renderers it does not own untouched", () => {
  ...
  assert.equal(
    decoratedWrite.renderResult,
    write.tool.renderResult,
    "write result rendering stays stock",
  );
...
test("renders apply_patch hunks with blank gutters when the file cannot be located", () => {
  ...
  const files = buildApplyPatchDiffFiles(
    envelope,
    mkdtempSync(join(tmpdir(), "choco-pi-tool-diff-")),
  );
...
test("renders apply_patch through the review diff renderer", () => {
  ...
  try {
    ...
    const lines = renderCall(
      decorated,
      args,
      context({ args, cwd: dir, toolCallId: "apply-1" }),
    ).render(60);
...
test("reuses the first resolved apply_patch model after the file has changed", () => {
  ...
  try {
    ...
    writeFileSync(file, "one\nTWO\nthree\n", "utf8");
    const after = renderCall(decorated, args, context({ ...ctx, lastComponent: undefined })).render(
      60,
    );
    assert.deepEqual(
      after,
      before,
      "post-execution re-renders must not re-resolve against patched content",
    );
  } finally {
    rmSync(dir, { recursive: true, force: true });
  }
...
test("falls back to the stock renderer for unusable arguments", () => {
  ...
  assert.deepEqual(renderCall(decoratedWrite, badArgs, context({ args: badArgs })).render(60), [
    "stock-call:write",
  ]);
  ...
  assert.deepEqual(
    renderCall(
      decoratedApplyPatch,
      garbage,
      context({ args: garbage, toolCallId: "bad-1" }),
    ).render(60),
...
test("falls back to the stock edit result renderer for an unusable result", () => {
  ...
  assert.deepEqual(
    renderResult(
      decorated,
      { content: [], details: { diff: "d", patch: "garbage" } },
      context({ args }),
    ).render(60),
    ["stock-result:edit"],
  );
  assert.deepEqual(
    renderResult(
      decorated,
      { content: [], details: { diff: "d", patch: "x" } },
...
test("falls back when the decorated renderer itself throws", () => {
  ...
  assert.deepEqual(renderCall(decorated, hostile, context({ args: hostile })).render(60), [
    "stock-call:write",
  ]);
  assert.equal(calls.call.length, 1);
});
...
test("decorating Pi's real edit tool changes rendering and nothing else", async () => {
  ...
  try {
    ...
    const args = {
      path: "sample.ts",
      edits: [{ oldText: "const b = 2;", newText: "const b = 22;" }],
    };
