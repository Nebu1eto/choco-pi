...
test("a configured editor command wins over Zed and VISUAL/EDITOR", async (t) => {
  const box = await sandbox(t);
  await box.writeProject(
    JSON.stringify({ editor: { command: ["code", "-w", "{path}:{line}"], mode: "gui" } }),
  );
...
test("VISUAL and EDITOR resolve to a terminal editor when Zed is absent", async () => {
  assert.deepEqual(
    await resolveEditor(undefined, { VISUAL: "hx", EDITOR: "nano" }, async () => false),
    {
      command: ["hx", "{path}"],
      mode: "terminal",
    },
...
test("a blank VISUAL falls through to EDITOR when Zed is absent", async () => {
  assert.deepEqual(
    await resolveEditor(undefined, { VISUAL: "   ", EDITOR: "vi" }, async () => false),
    {
      command: ["vi", "{path}"],
      mode: "terminal",
    },
