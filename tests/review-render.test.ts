...
const testStyler: DiffStyler = {
	...
	getFgAnsi: (color) => color === "toolDiffAdded"
		? "\u001b[38;2;100;220;100m"
...
test(`split view falls back to unified below ${MIN_SPLIT_WIDTH} columns`, () => {
	assert.equal(effectiveDiffMode(options({ mode: "split", width: MIN_SPLIT_WIDTH - 1 })), "unified");
...
test("line backgrounds resume after syntax highlighting resets", () => {
	const highlight: HighlightFn = (code) => code.split("\n").map((line) => [
		"\u001b[38;2;120;160;220m",
		line.slice(0, 5),
		"\u001b[0m",
		line.slice(5, 10),
