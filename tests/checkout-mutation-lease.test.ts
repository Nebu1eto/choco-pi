...
async function writeLiveSession(liveDir: string, session: { sessionId: string; pid: number; updatedAt?: string }): Promise<void> {
	await mkdir(liveDir, { recursive: true });
	await writeFile(path.join(liveDir, `${session.sessionId}.json`), JSON.stringify({
		version: 1,
...
async function runLease(action: string, options: { checkout: string; liveDir: string; leaseDir: string; owner?: string }): Promise<LeaseResult> {
