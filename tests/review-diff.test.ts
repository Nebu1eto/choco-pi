...
test("real git patches parse text, path, binary, and metadata edge cases", async (t) => {
	...
	await put(root, "modified.txt", Array.from({ length: 30 }, (_, index) => `line ${index + 1}`).join("\n") + "\n");
	...
	assert.deepEqual(
		{ additions: byPath.get("modified.txt")?.additions, deletions: byPath.get("modified.txt")?.deletions },
		{ additions: 2, deletions: 2 },
	);
	assert.ok(byPath.get("crlf.txt")?.hunks.flatMap((hunk) => hunk.lines).every((line) => !line.text.endsWith("\r")));
	assert.deepEqual(
		byPath.get("no-newline.txt")?.hunks.flatMap((hunk) => hunk.lines).map((line) => line.text),
		["old", "new"],
	);
...
test("branch helpers resolve merge bases, include working changes, and list names", async (t) => {
	...
	assert.equal(
		parseGitDiff(workingPatch, baseSha, headSha).files.find((file) => file.path === "untracked addition.txt")?.kind,
		"added",
	);
});

test("the default exec runner resolves non-zero exits and rejects spawn failures", async () => {
	const exited = await defaultExecRunner(process.execPath, ["-e", "process.stderr.write('no'); process.exit(7)"]);
	assert.deepEqual(exited, { stdout: "", stderr: "no", code: 7 });
	const signaled = await defaultExecRunner(process.execPath, ["-e", "process.kill(process.pid, 'SIGTERM')"]);
