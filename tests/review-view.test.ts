...
test("pull request relative time uses the injected clock across compact ranges", () => {
	const relative = (elapsedMs: number) => formatRelativeTime(
		new Date(Date.parse(NOW) - elapsedMs).toISOString(),
		NOW,
	);
...
test("pull request header protects its title while right metadata drops by priority", async () => {
	...
	const headerAt = (width: number) => stripTerminalSequences(mounted.component.render(width)[0] ?? "");
...
test("every selected row renders highlighted, not only the ends", async () => {
	...
	let highlighted = mounted.component.render(100)
	...
	mounted.component.handleInput?.(SHIFT_UP);
	assert.equal(
		mounted.component.render(100).map(stripTerminalSequences).filter((line) => line.startsWith("«")).length,
		1,
	);
	await mounted.close();
});
		...
		comments: [
			{
				...
				anchor: { hunkHash: LOW_HUNK_2.id, snippetHash: "0".repeat(64), snippet: "ordinary change" },
				createdAt: NOW,
				updatedAt: NOW,
			},
			{
...
test("a range comment stores startLine and submits start_line and start_side; one line submits neither", async () => {
	...
	assert.deepEqual(
		...
		[["RIGHT", 13, 14], ["RIGHT", undefined, 13]],
	);
...
test("in-view comment editor wraps, accepts newlines, submits, and cancels without a host input", async () => {
	...
	const custom: ReviewViewHost["custom"] = async (factory, customOptions) => {
		overlaySettings = typeof customOptions?.overlayOptions === "function"
...
test("model and effort sit under the input like the main footer, and /model and /effort switch them", async () => {
	...
	assert.ok(mounted.notifications.some((note) => note.includes("Chat model set to claude-sonnet-5")));
	...
	mounted.component.handleInput?.("\r");
	await settleInput();
	assert.ok(mounted.notifications.some((note) => note.includes("/frobnicate is not available in the review chat")));
	assert.deepEqual(fakeChat.ranCommands, ["/review-agent HEAD"], "unknown commands never reach the session");
	assert.ok(
...
test("a collapsed tool preview truncates long lines to the pane instead of flooding it", async () => {
	...
	assert.equal(rendered.filter((line) => line.includes("xxxx")).length, 0, "no preview rows at all");
	...
	assert.ok(rendered.filter((line) => line.includes("xxxx")).length > 1, "expanded wraps the full line");
...
test("chat splits at 120 columns and replaces the diff below that width", async () => {
	...
	let rendered = mounted.component.render(REVIEW_CHAT_SPLIT_MIN_WIDTH)
		.map(stripTerminalSequences).join("\n");
	assert.match(rendered, /Agent chat \[FOCUSED\]/);
	assert.match(rendered, /@@ -5,2 \+5,2 @@ high-1/);

	rendered = mounted.component.render(REVIEW_CHAT_SPLIT_MIN_WIDTH - 1)
		.map(stripTerminalSequences).join("\n");
	assert.match(rendered, /Agent chat \[FOCUSED\]/);
...
test("the framed comment input fits the reserved rows at every width", {
	...
	for (const width of [40, 80, 120, 200]) {
		...
		if (width >= 80) {
			assert.match(lines.at(-2)!, /Comment · Enter submit · Shift\+Enter newline · Tab path · Esc cancel/);
...
test("framed comment and chat inputs keep typing, Enter, Shift+Enter, and Esc", {
	skip: SKIP_WITHOUT_ZENTUI,
}, async () => {
	const fakeChat = new FakeReviewChat();
	const mounted = await mountReview({
	...
	assert.match(afterSubmit, /● 5 framed comment/, "the comment reappears inline, not in the input");
...
test("the comment input completes the review root's paths, inside the frame and without it", {
	skip: SKIP_WITHOUT_ZENTUI,
}, async () => {
	const reviewRoot = temporaryReviewRoot();
	const framed = await mountReview({ terminalRows: 24, reviewRoot, zentuiLoader: realZentuiLoader });
...
test("each input recalls its own history and never the other's", async () => {
	...
	let rendered = read();
	assert.match(rendered, /Chat \[FOCUSED\]/);
	assert.doesNotMatch(inputRows(rendered), new RegExp(COMMENT), "comments stay out of the question box");
	mounted.component.handleInput?.(QUESTION);
	mounted.component.handleInput?.("\r");
	assert.equal(fakeChat.asks.at(-1)?.question, QUESTION);

	mounted.component.handleInput?.(UP);
	assert.match(inputRows(read()), new RegExp(QUESTION), "the question box recalls the last question");
	mounted.component.handleInput?.("\u001b");
...
function expansionReviewFixture({
	...
	const expandableHunk: DiffHunk = {
		...
		lines: [
			...
			{ kind: "context", oldLine: oldStart + 1, newLine: oldStart + 2, text: `base line ${oldStart + 1}` },
		],
	};
	const model: DiffModel = {
		baseSha: "expand-base",
...
test("e opens at the cursor's side-specific line, including revealed context", async () => {
	...
	assert.deepEqual(launches, [
		["/repo/src/expand.ts:12"],
		["/repo/src/expand.ts:13"],
	]);
	await mounted.close();
});

test("editor command substitutes embedded path, line, column, and directory tokens", () => {
	assert.deepEqual(resolveEditorCommand(
...
test("terminal editor restores the TUI when spawning throws", async () => {
	const events: string[] = [];
	await assert.rejects(() => launchEditor(
