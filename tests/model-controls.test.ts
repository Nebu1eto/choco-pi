...
test("OpenAI Codex editor metadata shows only fast with a two-column right margin", () => {
	...
	assert.match(stripTerminalSequences(decorated[2] ?? ""), /^ gpt-5\.6-sol  OpenAI  high +fast {2}$/);
...
test("wrapped editor updates fast mode and preserves Zentui ownership", () => {
	...
	enabled = true;
	assert.match(stripTerminalSequences(editor.render(80)[0] ?? ""), /^ gpt-5\.6-sol  OpenAI  high +fast {2}$/);
...
test("editor installation waits until Zentui owns the factory", () => {
	...
	const plainFactory = (() => ({ render: () => [], invalidate: () => {}, handleInput: () => {}, getText: () => "", setText: () => {} })) as unknown as EditorFactory;
	const zentuiFactory = Object.assign(plainFactory.bind(undefined) as unknown as EditorFactory, { [zentuiKey]: true });
