...
test("real git context uses correct old and new numbering without changing hunk identity or anchors", async (t) => {
	...
	assert.deepEqual(aboveResult.expansion.above.map((line) => ({
		text: line.text,
	...
	assert.deepEqual(belowResult.expansion.below.map((line, offset) => ({
		text: line.text,
		oldLine: line.oldLine,
		newLine: line.newLine,
...
test("expansion is capped per edge and added, deleted, binary, and unread revisions fail safely", async (t) => {
	...
	assert.equal((await fullyExpand(root, model, longFile, longHunk, "above")).above.length, EXPANSION_LIMIT);
	...
	await assert.rejects(() => expandHunkContext({
		cwd: root,
