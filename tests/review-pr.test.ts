...
function stubbedGitHubRunner(remote: string, headSha: string, calls: Command[]): ExecRunner {
...
test("gh metadata and completion listing parse every relied-on field, including nullable authors", async () => {
  ...
  assert.deepEqual(
    ...
    [
      [
        "gh",
        ["pr", "view", "17", "--json", "number,title,baseRefName,headRefOid,author,url,updatedAt"],
      ],
      [
        "gh",
        [
          ...
          "100",
          "--json",
          "number,title,baseRefName,headRefOid,author,url,updatedAt",
        ],
      ],
    ],
  );
...
test("PR resolution uses a non-origin remote, private refs, a merge base, and a detached owned worktree", async (t) => {
  ...
  const review = await resolvePullRequestTarget(
    fixture.root,
    { kind: "pr", number: 7 },
    {
      runner,
  ...
  assert.equal(
    await git(review.worktreePath, "symbolic-ref", "-q", "HEAD").catch(() => "detached"),
    "detached",
  );
  ...
  assert.equal(
    await git(fixture.root, "for-each-ref", "--format=%(refname)", "refs/heads"),
    branchesBefore,
  );
  ...
  assert.ok(
    gitArgs.some(
      (args) =>
        args.slice(-4).join(" ") === "fetch --force code-review pull/7/head:refs/choco-pi/pr/7",
    ),
  );
  assert.ok(
    gitArgs.some(
      (args) =>
        args.slice(-4).join(" ") === "fetch --force code-review main:refs/choco-pi/pr-base/7",
    ),
  );
  assert.ok(
    gitArgs.some(
      (args) =>
        ...
        args.includes(fixture.headSha) &&
        args.includes("refs/choco-pi/pr-base/7"),
    ),
  );
...
test("owned leftovers and stale worktree entries recover without touching an unowned collision", async (t) => {
  ...
  const first = await resolvePullRequestTarget(
    fixture.root,
    { kind: "pr", number: 7 },
    {
      runner,
  ...
  const second = await resolvePullRequestTarget(
    fixture.root,
    { kind: "pr", number: 7 },
    {
      runner,
