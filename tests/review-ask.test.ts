...
test("model and effort set before the session exists shape its creation, and status tracks them", async () => {
	...
	await assert.rejects(
		() => chat.setThinkingLevel("ultra"),
		/Unknown thinking level "ultra"/,
	);
	await chat.ask("Who calls this?", context());
...
test("session commands run through the session's prompt path and stream into the transcript", async () => {
	...
	const emitFromCommand = (text: string) => {
		...
		fake.emit({ type: "message_end", message: assistantMessage(`Ran ${text}.`, { stopReason: "stop" }) });
	};
	const chat = createReviewChat({
		cwd: "/repo",
		createSession: async () => ({
	...
	assert.deepEqual(
		chat.messages.map((message) => (message.role === "tool" ? "tool" : `${message.role}:${message.text}`)),
...
test("a turn that ends on a tool call without a final answer is named, not silent", async () => {
	const fake = fakeSession((emit) => {
		...
		emit({ type: "message_end", message: assistantMessage("Let me check the callers.", { stopReason: "toolUse" }) });
		emit({ type: "tool_execution_start", toolCallId: "call-1", toolName: "grep", args: { pattern: "resolveTarget" } });
		emit({
...
test("the prompt sends the location — file, hunk header, focused line — and no diff body", () => {
	...
	const prompt = buildReviewChatPrompt("What calls this?", context({
		side: "LEFT",
		line: 3,
		focusedLineText,
	}));
...
test("tool executions enter the transcript between assistant messages, in event order", async () => {
	const fake = fakeSession((emit) => {
		...
		emit({ type: "message_end", message: assistantMessage("Let me check the caller.", { stopReason: "stop" }) });
		emit({
		...
		emit({ type: "message_end", message: assistantMessage("The caller handles it.", { stopReason: "stop" }) });
	});
	const chat = createReviewChat({ cwd: "/repo", createSession: async () => fake.session });
	...
	assert.deepEqual(tool.result, { content: [{ type: "text", text: "const parsed = parse(trimmed);" }] });
	chat.dispose();
});
...
test("a rejected request becomes a transcript message rather than a throw", async () => {
	...
	assert.equal(
		...
		`${REVIEW_CHAT_FAILURE_PREFIX}provider is unreachable`,
	);
...
test("a cancelled request becomes a transcript message, keeping partial text", async () => {
	...
	assert.equal(
		textOf(chat.messages[2]),
		`${REVIEW_CHAT_FAILURE_PREFIX}the request was cancelled.`,
	);
...
test("a silent turn reports that no answer arrived instead of leaving the pane empty", async () => {
	...
	assert.equal(textOf(chat.messages[1]), `${REVIEW_CHAT_FAILURE_PREFIX}the model returned no answer.`);
