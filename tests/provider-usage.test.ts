...
test("labels Claude plans from the live profile, with Team seats before the rate-limit tier", () => {
	const profile = (organization: Record<string, unknown>, account: Record<string, unknown> = {}) => ({
		account,
		organization,
	});
	...
	assert.equal(claudePlanLabel(profile({ rate_limit_tier: "default_claude_max_20x" })), "Max (20x)");
	assert.equal(
		claudePlanLabel(profile({ organization_type: "claude_team", rate_limit_tier: "default_claude_max_5x", seat_tier: "team_tier_1" })),
		"Team",
	);
	assert.equal(
		claudePlanLabel(profile({ organization_type: "claude_team", rate_limit_tier: "default_claude_max_20x", seat_tier: "team_premium" })),
		"Team Premium",
	);
...
test("shows the Claude plan next to the usage windows", () => {
	const usage = normalizeClaudeUsage(
		{ five_hour: { utilization: 12, resets_at: "2999-01-01T00:00:00Z" }, extra_usage: { is_enabled: true, utilization: 4 } },
...
test("labels ChatGPT Codex plans and keeps unknown wire values readable", () => {
	...
	const usage = normalizeCodexUsage({ plan_type: "prolite", credits: { balance: "0" }, rate_limit: {} });
