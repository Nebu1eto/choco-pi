...
Each session keeps one ref under `refs/choco-pi/checkpoints/`, chaining its checkpoints so a single ref keeps them reachable, and turns that changed nothing reuse the previous checkpoint commit. Refs untouched for 14 days are pruned at session start so `git gc` can reclaim the objects; set `CHOCO_PI_CHECKPOINT_RETENTION_DAYS` to change that window, or `0` to reclaim every idle checkpoint at once.
